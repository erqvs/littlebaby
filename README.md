...
```text
/home/littlebaby/.littlebaby/littlebaby.json
```
...
| 变量 | 说明 |
| --- | --- |
| `HOME` | 服务用户家目录，生产中通常是 `/home/littlebaby` |
| `LITTLEBABY_STATE_DIR` | 状态目录，默认 `$HOME/.littlebaby` |
| `LITTLEBABY_CONFIG_PATH` | 显式指定配置文件路径 |
| `NODE_ENV` | 运行环境 |

配置文件中会包含飞书、ZAI、MCP 或关联服务的密钥。不要把密钥写入 README、PROJECT.md、提交信息、日志或临时排查文档。
...
```bash
cd /var/www/projects/littlebaby
...
3. 通过飞书 WebSocket 接收事件。
4. 消息进入去重、权限、群聊触发判断。
5. 需要回复时，构造系统提示词和近期会话历史。
6. 调用 ZAI chat completions。
7. 如果模型请求工具，服务连接 MCP server 并转发 tool call。
8. 生成最终回复后，通过飞书消息接口发送；长文本按配置分段。
9. 保存会话历史和消息去重状态。
