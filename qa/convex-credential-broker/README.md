...
- `LITTLEBABY_QA_CONVEX_SECRET_MAINTAINER`
- `LITTLEBABY_QA_CONVEX_SECRET_CI`

Client URL policy:

- `LITTLEBABY_QA_CONVEX_SITE_URL` must use `https://` in normal use.
- Local development may use loopback `http://` only when `LITTLEBABY_QA_ALLOW_INSECURE_HTTP=1`.
