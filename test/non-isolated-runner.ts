...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.LITTLEBABY_TEST_HOME;
}
...
function restoreSharedTestHomeAfterEnvUnstub(testHomeRaw: string | undefined): void {
  ...
  process.env.LITTLEBABY_TEST_HOME = testHome;
...
export default class LittleBabyNonIsolatedRunner extends TestRunner {
  override onCollectStart(file: { filepath: string }) {
    ...
    const orderLogPath = process.env.LITTLEBABY_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {
