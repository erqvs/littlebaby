...
export function loadIncludePatternsFromEnv(
  ...
  return loadPatternListFromEnv("LITTLEBABY_VITEST_INCLUDE_FILE", env);
}

export function loadExtraExcludePatternsFromEnv(
  env: Record<string, string | undefined> = process.env,
): string[] {
  return loadPatternListFromEnv("LITTLEBABY_VITEST_EXTRA_EXCLUDE_FILE", env) ?? [];
}
...
export function createUnitVitestConfigWithOptions(
  ...
  return defineProject({
    ...
    test: {
      ...
      setupFiles: [
        ...new Set(
          [...(sharedTest.setupFiles ?? []), "test/setup-littlebaby-runtime.ts"].map(
            resolveRepoRootPath,
          ),
        ),
      ],
