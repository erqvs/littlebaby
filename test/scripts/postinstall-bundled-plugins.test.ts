...
async function createExtensionsDir() {
  const root = await createTempDirAsync("littlebaby-postinstall-");
...
describe("bundled plugin postinstall", () => {
  ...
  it("prunes source-checkout bundled plugin node_modules", async () => {
    const packageRoot = await createTempDirAsync("littlebaby-source-checkout-");
  ...
  it("keeps source-checkout prune non-fatal", async () => {
    const packageRoot = await createTempDirAsync("littlebaby-source-checkout-prune-error-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("honors disable env before source-checkout pruning", async () => {
    const packageRoot = await createTempDirAsync("littlebaby-source-checkout-disabled-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
    ...
    runBundledPluginPostinstall({
      env: { LITTLEBABY_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1" },
      packageRoot,
      log: { log: vi.fn(), warn: vi.fn() },
    });
  ...
  it("prunes stale dist files from packaged installs", async () => {
    const packageRoot = await createTempDirAsync("littlebaby-packaged-install-");
  ...
  it("restores only postinstall-generated QA compat sidecars after pruning old installs", async () => {
    const packageRoot = await createTempDirAsync("littlebaby-packaged-install-qa-compat-");
  ...
  it("creates only empty QA compat sidecars for fresh installs", async () => {
    const packageRoot = await createTempDirAsync("littlebaby-packaged-install-no-qa-compat-");
    ...
    await expect(
      fs.stat(path.join(packageRoot, "dist", "extensions", "qa-channel", "littlebaby.plugin.json")),
    ).rejects.toMatchObject({ code: "ENOENT" });
    await expect(
      fs.stat(path.join(packageRoot, "dist", "extensions", "qa-lab", "package.json")),
    ).rejects.toMatchObject({ code: "ENOENT" });
    await expect(
      fs.stat(path.join(packageRoot, "dist", "extensions", "qa-lab", "littlebaby.plugin.json")),
    ).rejects.toMatchObject({ code: "ENOENT" });
  });

  it("keeps packaged postinstall non-fatal when the dist inventory is missing", async () => {
    const packageRoot = await createTempDirAsync("littlebaby-packaged-install-missing-inventory-");
    const staleFile = path.join(packageRoot, "dist", "channel-CJUAgRQR.js");
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is invalid", async () => {
    const packageRoot = await createTempDirAsync("littlebaby-packaged-install-invalid-inventory-");
    const currentFile = path.join(packageRoot, "dist", "channel-BOa4MfoC.js");
  ...
  it("runs nested local installs with sanitized env when the sentinel package is missing", async () => {
    ...
    runBundledPluginPostinstall({
      env: {
        LITTLEBABY_EAGER_BUNDLED_PLUGIN_DEPS: "1",
        npm_config_global: "true",
        npm_config_location: "global",
        npm_config_prefix: "/opt/homebrew",
        HOME: "/tmp/home",
  ...
  it("reinstalls bundled runtime deps when optional native children are missing", async () => {
    ...
    runBundledPluginPostinstall({
      env: { HOME: "/tmp/home", LITTLEBABY_EAGER_BUNDLED_PLUGIN_DEPS: "1" },
      extensionsDir,
      packageRoot,
      arch: "arm64",
