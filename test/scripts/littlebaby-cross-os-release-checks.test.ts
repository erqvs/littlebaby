...
} from "../../scripts/littlebaby-cross-os-release-checks.ts";

describe("scripts/littlebaby-cross-os-release-checks", () => {
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'littlebaby.cmd')");
    expect(script).toContain("$cmd = Get-Command littlebaby -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        LITTLEBABY_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        LITTLEBABY_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        LITTLEBABY_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("littlebaby-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("uses the published installer URLs for native installer lanes", () => {
    expect(resolvePublishedInstallerUrl("darwin")).toBe("https://littlebaby.ai/install.sh");
    expect(resolvePublishedInstallerUrl("linux")).toBe("https://littlebaby.ai/install.sh");
    expect(resolvePublishedInstallerUrl("win32")).toBe("https://littlebaby.ai/install.ps1");
  });
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/littlebaby", "darwin"),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("littlebaby@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("littlebaby@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "littlebaby-cross-os-installed-version-"));
    try {
      const packageRoot =
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "littlebaby-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
