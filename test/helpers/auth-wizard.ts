...
export function requireLittleBabyAgentDir(): string {
  const agentDir = process.env.LITTLEBABY_AGENT_DIR;
  if (!agentDir) {
    throw new Error("LITTLEBABY_AGENT_DIR not set");
  }
  return agentDir;
}
