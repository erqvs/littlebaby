...
export function unwrapRemoteConfigSnapshot(raw: unknown): LittleBabyConfig {
  const rawObj = asRecord(raw);
  const resolved = asRecord(rawObj.resolved);
  if (Object.keys(resolved).length > 0) {
    return resolved as LittleBabyConfig;
  }

  const wrapped = asRecord(rawObj.config);
  if (Object.keys(wrapped).length > 0) {
    return wrapped as LittleBabyConfig;
  }
