  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.LITTLEBABY_HOME;
        delete env.LITTLEBABY_STATE_DIR;
        delete env.LITTLEBABY_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "littlebaby.mjs");
