...
} from "../scripts/littlebaby-npm-postpublish-verify.ts";
...
describe("buildPublishedInstallScenarios", () => {
  it("uses a single fresh scenario for plain stable releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["littlebaby@2026.3.23"],
  ...
  it("adds a stable-to-correction upgrade scenario for correction releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23-2")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["littlebaby@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
      {
        name: "upgrade-from-base-stable",
        installSpecs: ["littlebaby@2026.3.23", "littlebaby@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
    ]);
  });
...
describe("buildPublishedInstallCommandArgs", () => {
  it("runs lifecycle scripts for published install verification", () => {
    const args = buildPublishedInstallCommandArgs("/tmp/littlebaby-prefix", "littlebaby@2026.4.10");

    expect(args).toEqual([
      "install",
      "-g",
      "--prefix",
      "/tmp/littlebaby-prefix",
      "littlebaby@2026.4.10",
      "--no-fund",
      "--no-audit",
    ]);
...
describe("collectInstalledPackageErrors", () => {
  it("flags version mismatches and missing runtime sidecars", () => {
    const errors = collectInstalledPackageErrors({
      ...
      packageRoot: "/tmp/empty-littlebaby",
    });
...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "littlebaby-postpublish-context-engine-"));
  }
...
describe("normalizeInstalledBinaryVersion", () => {
  it("accepts decorated CLI version output", () => {
    expect(normalizeInstalledBinaryVersion("LittleBaby 2026.4.8 (9ece252)")).toBe("2026.4.8");
    expect(normalizeInstalledBinaryVersion("LittleBaby 2026.4.8-beta.1 (9ece252)")).toBe(
      "2026.4.8-beta.1",
    );
  });
});
...
describe("resolveInstalledBinaryPath", () => {
  it("uses the Unix global bin path on non-Windows platforms", () => {
    expect(resolveInstalledBinaryPath("/tmp/littlebaby-prefix", "darwin")).toBe(
      "/tmp/littlebaby-prefix/bin/littlebaby",
    );
  });

  it("uses the Windows npm shim path on win32", () => {
    expect(resolveInstalledBinaryPath("C:/littlebaby-prefix", "win32")).toBe(
      "C:/littlebaby-prefix/littlebaby.cmd",
    );
  });
});
...
describe("collectInstalledMirroredRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "littlebaby-postpublish-installed-"));
  }
  ...
  it("skips manifest-only sidecar directories without package.json", () => {
    ...
    try {
      ...
      writePackageFile(packageRoot, "dist/extensions/device-pair/littlebaby.plugin.json", {
        id: "device-pair",
      });
