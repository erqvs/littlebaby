...
describe("resolveVitestIsolation", () => {
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ LITTLEBABY_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ LITTLEBABY_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ LITTLEBABY_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
  ...
  it("loads scoped include overrides from LITTLEBABY_VITEST_INCLUDE_FILE", () => {
    ...
    try {
      ...
      const config = createScopedVitestConfig(["src/utils/**/*.test.ts"], {
        dir: "src",
        env: {
          LITTLEBABY_VITEST_INCLUDE_FILE: includeFile,
        },
      });
