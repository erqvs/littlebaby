...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("littlebaby-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@littlebaby/whatsapp",
      version: "2026.3.23",
      description: "LittleBaby WhatsApp channel plugin",
      littlebaby: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("littlebaby-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@littlebaby/whatsapp",
      littlebaby: {
        channel: {
