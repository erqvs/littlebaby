...
function createTempHome(): string {
  return makeTempDir(tempDirs, "littlebaby-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-littlebaby.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.littlebaby/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.littlebaby/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    process.env.LITTLEBABY_LIVE_TEST_QUIET = "1";
    process.env.LITTLEBABY_CONFIG_PATH = "~/custom-littlebaby.json5";
    process.env.LITTLEBABY_TEST_HOME = priorIsolatedHome;
    process.env.LITTLEBABY_STATE_DIR = path.join(priorIsolatedHome, ".littlebaby");
    ...
    expect(process.env.LITTLEBABY_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".littlebaby", "littlebaby.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".littlebaby", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".littlebaby", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.LITTLEBABY_LIVE_TEST = "1";
    process.env.LITTLEBABY_LIVE_USE_REAL_HOME = "1";
    process.env.LITTLEBABY_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.LITTLEBABY_LIVE_TEST;
    delete process.env.LITTLEBABY_LIVE_GATEWAY;
    delete process.env.LITTLEBABY_LIVE_USE_REAL_HOME;
    delete process.env.LITTLEBABY_LIVE_TEST_QUIET;
