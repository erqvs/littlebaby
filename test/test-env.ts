...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.LITTLEBABY_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "LITTLEBABY_TEST_FAST", value: process.env.LITTLEBABY_TEST_FAST },
    {
      key: "LITTLEBABY_STRICT_FAST_REPLY_CONFIG",
      value: process.env.LITTLEBABY_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "LITTLEBABY_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.LITTLEBABY_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "LITTLEBABY_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.LITTLEBABY_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "LITTLEBABY_STATE_DIR", value: process.env.LITTLEBABY_STATE_DIR },
    { key: "LITTLEBABY_CONFIG_PATH", value: process.env.LITTLEBABY_CONFIG_PATH },
    { key: "LITTLEBABY_GATEWAY_PORT", value: process.env.LITTLEBABY_GATEWAY_PORT },
    { key: "LITTLEBABY_BRIDGE_ENABLED", value: process.env.LITTLEBABY_BRIDGE_ENABLED },
    { key: "LITTLEBABY_BRIDGE_HOST", value: process.env.LITTLEBABY_BRIDGE_HOST },
    { key: "LITTLEBABY_BRIDGE_PORT", value: process.env.LITTLEBABY_BRIDGE_PORT },
    { key: "LITTLEBABY_CANVAS_HOST_PORT", value: process.env.LITTLEBABY_CANVAS_HOST_PORT },
    { key: "LITTLEBABY_TEST_HOME", value: process.env.LITTLEBABY_TEST_HOME },
    { key: "LITTLEBABY_AGENT_DIR", value: process.env.LITTLEBABY_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  process.env.LITTLEBABY_TEST_HOME = tempHome;
  process.env.LITTLEBABY_TEST_FAST = "1";
  process.env.LITTLEBABY_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.LITTLEBABY_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.LITTLEBABY_BRIDGE_ENABLED;
  delete process.env.LITTLEBABY_BRIDGE_HOST;
  delete process.env.LITTLEBABY_BRIDGE_PORT;
  delete process.env.LITTLEBABY_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.LITTLEBABY_LIVE_TEST_NORMALIZE_CONFIG)) {
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.LITTLEBABY_LIVE_TEST === "1" ||
    process.env.LITTLEBABY_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.LITTLEBABY_LIVE_USE_REAL_HOME);
