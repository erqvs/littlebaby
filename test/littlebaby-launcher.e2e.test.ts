...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = makeTempDir(fixtureRoots, "littlebaby-launcher-");
  await fs.copyFile(
    path.resolve(process.cwd(), "littlebaby.mjs"),
    path.join(fixtureRoot, "littlebaby.mjs"),
  );
...
describe("littlebaby launcher", () => {
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      ...
      'import "missing-littlebaby-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "littlebaby.mjs"), "--help"], {
      cwd: fixtureRoot,
      encoding: "utf8",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("missing-littlebaby-launcher-dep");
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:littlebaby/littlebaby#<ref>");
  });
});
