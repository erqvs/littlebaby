...
In the active OpenClaw config file (`$LITTLEBABY_CONFIG_PATH`, default `~/.littlebaby/openclaw.json`):
...
1. Check server bind: `CONFIG_PATH="${LITTLEBABY_CONFIG_PATH:-${LITTLEBABY_STATE_DIR:-$HOME/.littlebaby}/openclaw.json}"; cat "$CONFIG_PATH" | jq '.gateway.bind'`
