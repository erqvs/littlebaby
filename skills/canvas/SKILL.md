...
Display HTML content on connected LittleBaby nodes (Mac app, iOS, Android).
...
```
http://<tailscale-hostname>:18793/__littlebaby__/canvas/<file>.html
```
...
In the active LittleBaby config file (`$LITTLEBABY_CONFIG_PATH`, default `~/.littlebaby/littlebaby.json`):
...
- **loopback**: `http://127.0.0.1:18793/__littlebaby__/canvas/<file>.html`
- **lan/tailnet/auto**: `http://<hostname>:18793/__littlebaby__/canvas/<file>.html`
...
```
canvas action:present node:mac-63599bc4-b54d-4392-9048-b97abd58343a target:http://peters-mac-studio-1.sheep-coho.ts.net:18793/__littlebaby__/canvas/snake.html
```
...
1. Check server bind: `CONFIG_PATH="${LITTLEBABY_CONFIG_PATH:-${LITTLEBABY_STATE_DIR:-$HOME/.littlebaby}/littlebaby.json}"; cat "$CONFIG_PATH" | jq '.gateway.bind'`
2. Check what port canvas is on: `lsof -i :18793`
3. Test URL directly: `curl http://<hostname>:18793/__littlebaby__/canvas/<file>.html`
...
Node is offline. Use `littlebaby nodes list` to find online nodes.
