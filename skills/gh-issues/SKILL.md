...
If still empty, check `/data/.clawdbot/littlebaby.json`:

```
cat /data/.clawdbot/littlebaby.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
CONFIG_PATH="${LITTLEBABY_CONFIG_PATH:-${LITTLEBABY_STATE_DIR:-$HOME/.littlebaby}/littlebaby.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/littlebaby.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/littlebaby.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")
