...
```
CONFIG_PATH="${LITTLEBABY_CONFIG_PATH:-${LITTLEBABY_STATE_DIR:-$HOME/.littlebaby}/openclaw.json}"
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the OpenClaw dashboard or in the active OpenClaw config path (`$LITTLEBABY_CONFIG_PATH`, default `~/.littlebaby/openclaw.json`) under `skills.entries.gh-issues`."
...
export CONFIG_PATH="${LITTLEBABY_CONFIG_PATH:-${LITTLEBABY_STATE_DIR:-$HOME/.littlebaby}/openclaw.json}"
