...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by LittleBaby. Pass it as a Bearer token in all API calls:
...
```
CONFIG_PATH="${LITTLEBABY_CONFIG_PATH:-${LITTLEBABY_STATE_DIR:-$HOME/.littlebaby}/littlebaby.json}"
cat "$CONFIG_PATH" | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```

If still empty, check `/data/.littlebaby/littlebaby.json`:

```
cat /data/.littlebaby/littlebaby.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the LittleBaby dashboard or in the active LittleBaby config path (`$LITTLEBABY_CONFIG_PATH`, default `~/.littlebaby/littlebaby.json`) under `skills.entries.gh-issues`."
...
7. **Check claim-based in-progress tracking:**
   ...
   ```
   CLAIMS_FILE="/data/.littlebaby/gh-issues-claims.json"
   if [ ! -f "$CLAIMS_FILE" ]; then
     mkdir -p /data/.littlebaby
     echo '{}' > "$CLAIMS_FILE"
   fi
   ```
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  ```
  CURSOR_FILE="/data/.littlebaby/gh-issues-cursor-{SOURCE_REPO_SLUG}.json"
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., littlebaby-littlebaby)
  ```
...
CONFIG_PATH="${LITTLEBABY_CONFIG_PATH:-${LITTLEBABY_STATE_DIR:-$HOME/.littlebaby}/littlebaby.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.littlebaby/littlebaby.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.littlebaby/littlebaby.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${LITTLEBABY_CONFIG_PATH:-${LITTLEBABY_STATE_DIR:-$HOME/.littlebaby}/littlebaby.json}"
