---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for LittleBaby deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, LittleBaby cron scheduling for periodic checks, or version status checks on a machine running LittleBaby (laptop, workstation, Pi, VPS).
---

# LittleBaby Host Hardening

## Overview

Assess and harden the host running LittleBaby, then align it to a user-defined risk tolerance without breaking access. Use LittleBaby security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim LittleBaby changes the host firewall, SSH, or OS updates; it does not.
...
5. LittleBaby gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if LittleBaby can access sensitive data.
...
### 2) Run LittleBaby security audits (read-only)

As part of the default read-only checks, run `littlebaby security audit --deep`. Only offer alternatives if the user requests them:

1. `littlebaby security audit` (faster, non-probing)
2. `littlebaby security audit --json` (structured output)

Offer to apply LittleBaby safe defaults (numbered):

1. `littlebaby security audit --fix`

Be explicit that `--fix` only tightens LittleBaby defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check LittleBaby version/update status (read-only)

As part of the default read-only checks, run `littlebaby update status`.
...
- Credential hygiene notes (location of LittleBaby creds, prefer disk encryption)
...
- LittleBaby security audit (re-run)
...
After LittleBaby install or first hardening pass, run at least one baseline audit and version check:

- `littlebaby security audit`
- `littlebaby security audit --deep`
- `littlebaby update status`

Ongoing monitoring is recommended. Use the LittleBaby cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `littlebaby cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `littlebaby update status`
...
Before creating, `littlebaby cron list` and match on exact `name`. If found, `littlebaby cron edit <id> ...`.
If not found, `littlebaby cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `littlebaby update status` (preferred for source checkouts and channels)
2. `npm view littlebaby version` (published npm version)

## LittleBaby command accuracy

Use only supported commands and flags:

- `littlebaby security audit [--deep] [--fix] [--json]`
- `littlebaby status` / `littlebaby status --deep`
- `littlebaby health --json`
- `littlebaby update status`
- `littlebaby cron add|list|runs|run`

Do not invent CLI flags or imply LittleBaby enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by LittleBaby compaction:
