---
name: node-connect
description: Diagnose LittleBaby node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify LittleBaby is advertising that route, then fix pairing/auth.
...
- whether `littlebaby devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `littlebaby qr --json`. It uses the same setup-code payload Android scans.

```bash
littlebaby config get gateway.mode
littlebaby config get gateway.bind
littlebaby config get gateway.tailscale.mode
littlebaby config get gateway.remote.url
littlebaby config get gateway.auth.mode
littlebaby config get gateway.auth.allowTailscale
littlebaby config get plugins.entries.device-pair.config.publicUrl
littlebaby qr --json
littlebaby devices list
littlebaby nodes status
```

If this LittleBaby instance is pointed at a remote gateway, also run:

```bash
littlebaby qr --remote --json
```
...
`littlebaby qr --json` success means:
...
If `littlebaby qr --json` says `Gateway is only bound to loopback`:
...
- `littlebaby devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run littlebaby qr again, rescan, then approve the pending device pairing.`
