...
declare const __LITTLEBABY_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "littlebaby";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["LITTLEBABY_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["LITTLEBABY_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["LITTLEBABY_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.LITTLEBABY_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current LittleBaby version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __LITTLEBABY_VERSION__ === "string" ? __LITTLEBABY_VERSION__ : undefined,
  bundledVersion: process.env.LITTLEBABY_BUNDLED_VERSION,
});
