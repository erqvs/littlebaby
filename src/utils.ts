...
export function resolveConfigDir(
  ...
  const configPath = env.LITTLEBABY_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".littlebaby");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.LITTLEBABY_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$LITTLEBABY_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via LITTLEBABY_STATE_DIR.
