...
import { z } from "zod";
import { createLittleBabyChannelMcpServer, LittleBabyChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createLittleBabyChannelMcpServer({
...
describe("littlebaby channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new LittleBabyChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
