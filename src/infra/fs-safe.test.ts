...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("littlebaby-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("littlebaby-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("littlebaby-fs-safe-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("littlebaby-fs-safe-root-");
    const sourceDir = await tempDirs.make("littlebaby-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("littlebaby-fs-safe-root-");
    const outside = await tempDirs.make("littlebaby-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    process.env.LITTLEBABY_HOME = fakeHome;
    try {
    ...
    const root = await tempDirs.make("littlebaby-tilde-test-");
    process.env.HOME = root;
    process.env.LITTLEBABY_HOME = root;
    try {
    ...
    const outsideRoot = await tempDirs.make("littlebaby-tilde-outside-");
    await expect(
      openFileWithinRoot({
        rootDir: outsideRoot,
        relativePath: "~/escape.txt",
