...
describe("package-json helpers", () => {
  it("reads package version and trims package name", async () => {
    await withTempDir({ prefix: "littlebaby-package-json-" }, async (root) => {
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ version: " 1.2.3 ", name: "  @littlebaby/demo  " }),
        "utf8",
      );
  ...
  it.each([
    ...
    },
    {
      ...
      writePackageJson: async (root: string) => {
        await fs.writeFile(
          path.join(root, "package.json"),
          JSON.stringify({ version: "   ", name: "@littlebaby/demo" }),
          "utf8",
        );
      },
      expectedVersion: null,
