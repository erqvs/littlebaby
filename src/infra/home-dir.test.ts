...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for LITTLEBABY_HOME",
      env: { LITTLEBABY_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when LITTLEBABY_HOME is tilde-only and no fallback home exists",
      env: { LITTLEBABY_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores LITTLEBABY_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          LITTLEBABY_HOME: "/srv/openclaw-home",
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of LITTLEBABY_HOME", () => {
    expect(
