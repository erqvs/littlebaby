...
describe("isMainModule", () => {
  ...
  it("returns true for configured wrapper-to-entry pairs", () => {
    expect(
      isMainModule({
        ...
        argv: ["node", "/repo/littlebaby.mjs"],
        cwd: "/repo",
        env: {},
        wrapperEntryPairs: [{ wrapperBasename: "littlebaby.mjs", entryBasename: "entry.js" }],
      }),
    ).toBe(true);
  });
  ...
  it("returns false when this module is only imported under PM2", () => {
    expect(
      isMainModule({
        currentFile: "/repo/node_modules/littlebaby/dist/index.js",
