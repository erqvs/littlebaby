...
import { resolveLittleBabyPackageRoot } from "./littlebaby-root.js";
...
function resolveAutoUpdatePolicy(cfg: LittleBabyConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: LittleBabyConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const root = await resolveLittleBabyPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("littlebaby update")}`,
      );
