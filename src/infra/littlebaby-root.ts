...
import { littleBabyRootFs, littleBabyRootFsSync } from "./littlebaby-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["littlebaby"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  try {
    return parsePackageName(await littleBabyRootFs.readFile(path.join(dir, "package.json"), "utf-8"));
  } catch {
    return null;
  }
}
...
function readPackageNameSync(dir: string): string | null {
  try {
    return parsePackageName(
      littleBabyRootFsSync.readFileSync(path.join(dir, "package.json"), "utf-8"),
    );
  } catch {
    return null;
  }
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = littleBabyRootFsSync.realpathSync(normalized);
...
export async function resolveLittleBabyPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveLittleBabyPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
