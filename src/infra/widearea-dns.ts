...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.LITTLEBABY_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*littlebaby-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "littlebaby";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "littlebaby");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "littlebaby-gw");
  ...
  records.push(`_littlebaby-gw._tcp IN PTR ${instanceLabel}._littlebaby-gw._tcp`);
  records.push(`${instanceLabel}._littlebaby-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._littlebaby-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; littlebaby-content-hash: ${contentHash}\n${contentBody}`;
}
