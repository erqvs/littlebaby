...
import { POSIX_LITTLEBABY_TMP_DIR, resolvePreferredLittleBabyTmpDir } from "./tmp-littlebaby-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredLittleBabyTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `littlebaby-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredLittleBabyTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_LITTLEBABY_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredLittleBabyTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredLittleBabyTmpDir", () => {
  it("prefers /tmp/littlebaby when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_LITTLEBABY_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/littlebaby when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_LITTLEBABY_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it.each([
    {
      name: "falls back to os.tmpdir()/littlebaby when /tmp/littlebaby is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/littlebaby when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/littlebaby exists but is not writable",
    ...
    {
      name: "falls back when /tmp/littlebaby is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/littlebaby is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/littlebaby is group/other writable",
  ...
  it("repairs existing /tmp/littlebaby permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_LITTLEBABY_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_LITTLEBABY_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_LITTLEBABY_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/littlebaby after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_LITTLEBABY_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_LITTLEBABY_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_LITTLEBABY_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_LITTLEBABY_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback LittleBaby temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "littlebaby");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredLittleBabyTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_LITTLEBABY_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback LittleBaby temp dir/);
  });
});
