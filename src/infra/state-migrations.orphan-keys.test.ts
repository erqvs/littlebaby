...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".littlebaby");
...
function sharedMainOpsConfig(sharedStorePath: string): LittleBabyConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: LittleBabyConfig = OPS_WORK_CONFIG) {
  return migrateOrphanedSessionKeys({
    cfg,
    env: { LITTLEBABY_STATE_DIR: stateDir },
  });
}
...
describe("migrateOrphanedSessionKeys", () => {
  ...
  it("is idempotent — running twice produces same result", async () => {
    await withStateFixture(async ({ stateDir }) => {
      ...
      const env = { LITTLEBABY_STATE_DIR: stateDir };
