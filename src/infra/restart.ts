...
export function triggerLittleBabyRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.LITTLEBABY_SYSTEMD_UNIT,
      process.env.LITTLEBABY_PROFILE,
    );
  ...
  const label =
    process.env.LITTLEBABY_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.LITTLEBABY_PROFILE);
