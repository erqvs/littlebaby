  ...
  it("includes the cross-platform supervisor hint env vars", () => {
    expect(SUPERVISOR_HINT_ENV_VARS).toEqual(
      expect.arrayContaining([
        "LAUNCH_JOB_LABEL",
        "INVOCATION_ID",
        "LITTLEBABY_WINDOWS_TASK_NAME",
        "LITTLEBABY_SERVICE_MARKER",
        "LITTLEBABY_SERVICE_KIND",
      ]),
    );
  });
});
...
describe("detectRespawnSupervisor", () => {
  it("detects launchd and systemd only from non-blank platform-specific hints", () => {
    expect(detectRespawnSupervisor({ LAUNCH_JOB_LABEL: " ai.littlebaby.gateway " }, "darwin")).toBe(
      "launchd",
    );
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ LITTLEBABY_WINDOWS_TASK_NAME: "LittleBaby Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
      detectRespawnSupervisor(
        {
          LITTLEBABY_SERVICE_MARKER: "littlebaby",
          LITTLEBABY_SERVICE_KIND: "gateway",
        },
        "win32",
      ),
    ).toBe("schtasks");
    expect(
      detectRespawnSupervisor(
        {
          LITTLEBABY_SERVICE_MARKER: "littlebaby",
          LITTLEBABY_SERVICE_KIND: "worker",
        },
        "win32",
      ),
    ).toBeNull();
  ...
  it("ignores service markers on non-Windows platforms and unknown platforms", () => {
    ...
    expect(
      detectRespawnSupervisor({ LAUNCH_JOB_LABEL: "ai.littlebaby.gateway" }, "freebsd"),
    ).toBeNull();
  });
});
