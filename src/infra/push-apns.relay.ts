...
const GATEWAY_DEVICE_ID_HEADER = "x-littlebaby-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-littlebaby-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-littlebaby-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.LITTLEBABY_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "LITTLEBABY_APNS_RELAY_BASE_URL"
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.LITTLEBABY_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require LITTLEBABY_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.LITTLEBABY_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
