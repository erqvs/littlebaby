...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): LittleBabyConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): LittleBabyConfig {
  return {
