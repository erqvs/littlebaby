...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__littlebaby_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "littlebaby-root");
...
function setPackageRoot(root: string, name = "littlebaby") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./littlebaby-root.js").resolveLittleBabyPackageRootSync,
  asyncResolver: typeof import("./littlebaby-root.js").resolveLittleBabyPackageRoot,
  opts: Parameters<typeof import("./littlebaby-root.js").resolveLittleBabyPackageRootSync>[0],
...
vi.mock("./littlebaby-root.fs.runtime.js", () => ({
  littleBabyRootFsSync: mockFsModule(),
  littleBabyRootFs: mockFsPromisesModule(),
}));

describe("resolveLittleBabyPackageRoot", () => {
  let resolveLittleBabyPackageRoot: typeof import("./littlebaby-root.js").resolveLittleBabyPackageRoot;
  let resolveLittleBabyPackageRootSync: typeof import("./littlebaby-root.js").resolveLittleBabyPackageRootSync;

  beforeAll(async () => {
    ({ resolveLittleBabyPackageRoot, resolveLittleBabyPackageRootSync } =
      await import("./littlebaby-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "littlebaby");
        const pkgRoot = path.join(project, "node_modules", "littlebaby");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "littlebaby");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "littlebaby.mjs")));
    ...
    {
      name: "falls through from a non-littlebaby moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-littlebaby");
    ...
    {
      name: "returns null for non-littlebaby package roots",
      setup: () => {
        const pkgRoot = fx("not-littlebaby");
        setPackageRoot(pkgRoot, "not-littlebaby");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "littlebaby.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "littlebaby");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
