...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.LITTLEBABY_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for LittleBaby. You can keep running the web gateway without it: \`${formatCliCommand("littlebaby gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
