...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open LittleBaby to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "littlebaby.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.LITTLEBABY_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.LITTLEBABY_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set LITTLEBABY_APNS_TEAM_ID and LITTLEBABY_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.LITTLEBABY_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.LITTLEBABY_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.LITTLEBABY_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set LITTLEBABY_APNS_PRIVATE_KEY_P8 or LITTLEBABY_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading LITTLEBABY_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
