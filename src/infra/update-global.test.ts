...
describe("update global helpers", () => {
  ...
  it("resolves global roots and package roots from runner output", async () => {
    ...
    await expect(resolveGlobalPackageRoot("npm", runCommand, 1000)).resolves.toBe(
      path.join("/tmp/npm-root", "littlebaby"),
    );
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "littlebaby", tag: "main" })).toBe(
      LITTLEBABY_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
  ...
  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "openclaw-update-global-" }, async (base) => {
      ...
      const pkgRoot = path.join(pnpmRoot, "littlebaby");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "littlebaby"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "littlebaby"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "littlebaby"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "littlebaby"), { recursive: true, force: true });
