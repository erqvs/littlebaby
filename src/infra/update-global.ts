...
const PRIMARY_PACKAGE_NAME = "littlebaby";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const LITTLEBABY_MAIN_PACKAGE_SPEC = "github:littlebaby/littlebaby#main";
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "littlebaby.plugin.json")))
      ) {
        expectedFiles.add(relativePath);
      }
    }),
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "LittleBaby", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.LITTLEBABY_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.LITTLEBABY_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return LITTLEBABY_MAIN_PACKAGE_SPEC;
  }
