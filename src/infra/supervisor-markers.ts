const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "LITTLEBABY_LAUNCHD_LABEL"],
  systemd: ["LITTLEBABY_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["LITTLEBABY_WINDOWS_TASK_NAME"],
} as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.LITTLEBABY_SERVICE_MARKER?.trim();
    const serviceKind = env.LITTLEBABY_SERVICE_KIND?.trim();
