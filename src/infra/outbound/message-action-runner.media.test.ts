...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred LittleBaby tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideLittleBaby = path.join(os.tmpdir(), "outside-littlebaby", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
            params: {
              channel: "slack",
              target: "#C12345678",
              media: hostTmpOutsideLittleBaby,
              message: "",
            },
            sandboxRoot: sandboxDir,
            dryRun: true,
