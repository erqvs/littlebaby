...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: LittleBabyConfig | null = null;
  ...
  get(key: string, cfg: LittleBabyConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: LittleBabyConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: LittleBabyConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: LittleBabyConfig): void {
