...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.LITTLEBABY_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "LittleBaby";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(LittleBaby\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.LITTLEBABY_MDNS_HOSTNAME?.trim() || "littlebaby";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
