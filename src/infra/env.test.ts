...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", () => {
    ...
    withEnv(
      {
        ...
        LITTLEBABY_TEST_ENV: "  line one\nline two  ",
      },
      () => {
        logAcceptedEnvOption({
          key: "LITTLEBABY_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith(
      "env: LITTLEBABY_TEST_ENV=<redacted> (test option)",
    );
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        LITTLEBABY_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "LITTLEBABY_BLANK_ENV",
