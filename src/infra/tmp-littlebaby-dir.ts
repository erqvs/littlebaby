...
export const POSIX_LITTLEBABY_TMP_DIR = "/tmp/littlebaby";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredLittleBabyTmpDirOptions = {
...
export function resolvePreferredLittleBabyTmpDir(
  options: ResolvePreferredLittleBabyTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "littlebaby" : `littlebaby-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[littlebaby] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback LittleBaby temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback LittleBaby temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_LITTLEBABY_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_LITTLEBABY_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_LITTLEBABY_TMP_DIR)) {
      return POSIX_LITTLEBABY_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_LITTLEBABY_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_LITTLEBABY_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_LITTLEBABY_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_LITTLEBABY_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_LITTLEBABY_TMP_DIR;
