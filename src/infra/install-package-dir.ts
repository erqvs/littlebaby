...
const STAGED_NPM_PROJECT_CONFIG_PREFIX = ".littlebaby-install-hidden-npmrc-";
...
export async function installPackageDir(params: {
  ...
  try {
    ...
    stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".littlebaby-install-stage-"));
  ...
  if (params.mode === "update" && (await fileExists(canonicalTargetDir))) {
    const backupRoot = path.join(installBaseRealPath, ".littlebaby-install-backups");
