...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "littlebaby-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".littlebaby");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".littlebaby", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      LITTLEBABY_STATE_DIR: path.join(home, ".littlebaby"),
      ...env,
    };
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    ...
    await fs.writeFile(
      path.join(stateDir, "littlebaby.json"),
