...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.LITTLEBABY_GATEWAY_PORT).toBe("18789");
    expect(env.LITTLEBABY_GATEWAY_TOKEN).toBeUndefined();
    expect(env.LITTLEBABY_SERVICE_MARKER).toBe("littlebaby");
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", LITTLEBABY_PROFILE: "work" },
      port: 18789,
    });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through LITTLEBABY_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", LITTLEBABY_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.LITTLEBABY_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits LITTLEBABY_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
