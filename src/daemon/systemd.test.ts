...
describe("isSystemdServiceEnabled", () => {
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "littlebaby-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "littlebaby-gateway.service"]);
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled littlebaby-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled littlebaby-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when LITTLEBABY_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/littlebaby-gateway.service",
    },
    {
      name: "uses profile-specific service name when LITTLEBABY_PROFILE is set to a custom value",
      env: { HOME: "/home/test", LITTLEBABY_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/littlebaby-gateway-jbphoenix.service",
    },
    {
      name: "prefers LITTLEBABY_SYSTEMD_UNIT over LITTLEBABY_PROFILE",
      env: {
        HOME: "/home/test",
        LITTLEBABY_PROFILE: "jbphoenix",
        LITTLEBABY_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles LITTLEBABY_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        LITTLEBABY_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from LITTLEBABY_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        LITTLEBABY_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads LITTLEBABY_GATEWAY_TOKEN from EnvironmentFile", async () => {
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.LITTLEBABY_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.LITTLEBABY_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.LITTLEBABY_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.littlebaby/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue === "/home/test/.littlebaby/first.env") {
        return "LITTLEBABY_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.littlebaby/second env.env") {
        return 'LITTLEBABY_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      LITTLEBABY_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "LITTLEBABY_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "LITTLEBABY_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "LITTLEBABY_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      LITTLEBABY_GATEWAY_TOKEN: "override-token",
      LITTLEBABY_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue === "/home/test/.littlebaby/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'LITTLEBABY_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "LITTLEBABY_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      LITTLEBABY_GATEWAY_TOKEN: "quoted token",
      LITTLEBABY_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      LITTLEBABY_GATEWAY_TOKEN: "file",
      LITTLEBABY_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("stageSystemdService", () => {
  ...
  it("writes dotenv-backed values to a separate env file and keeps inline env minimal", async () => {
    ...
    const env = {
      ...
      LITTLEBABY_SYSTEMD_UNIT: "littlebaby-gateway-stage-test",
    };
    ...
    await fs.writeFile(
      ...
      ["LITTLEBABY_GATEWAY_TOKEN=dotenv-token", "LLM_API_KEY=dotenv-key"].join("\n"),
      "utf8",
    );
    ...
    try {
      ...
      expect(unit).not.toContain("Environment=LITTLEBABY_GATEWAY_TOKEN=dotenv-token");
      expect(unit).not.toContain("Environment=LLM_API_KEY=dotenv-key");
      expect(envFile).toBe("LITTLEBABY_GATEWAY_TOKEN=dotenv-token\nLLM_API_KEY=dotenv-key\n");
  ...
  it("keeps inline overrides out of the generated env file", async () => {
    ...
    await fs.writeFile(
      path.join(stateDir, ".env"),
      ["LITTLEBABY_GATEWAY_TOKEN=stale-token", "LLM_API_KEY=dotenv-key"].join("\n"),
      "utf8",
    );
    ...
    try {
      ...
      expect(unit).toContain(`EnvironmentFile=-${envFilePath}`);
      expect(unit).toContain("Environment=LITTLEBABY_GATEWAY_TOKEN=fresh-token");
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "littlebaby-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ LITTLEBABY_PROFILE: "work" });
  });
