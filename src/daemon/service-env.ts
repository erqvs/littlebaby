...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.LITTLEBABY_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    LITTLEBABY_PROFILE: profile,
    LITTLEBABY_GATEWAY_PORT: String(port),
    LITTLEBABY_LAUNCHD_LABEL: resolvedLaunchdLabel,
    LITTLEBABY_SYSTEMD_UNIT: systemdUnit,
    LITTLEBABY_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    LITTLEBABY_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    LITTLEBABY_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    LITTLEBABY_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.LITTLEBABY_GATEWAY_TOKEN);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    LITTLEBABY_GATEWAY_TOKEN: gatewayToken,
    LITTLEBABY_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    LITTLEBABY_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    LITTLEBABY_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    LITTLEBABY_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    LITTLEBABY_LOG_PREFIX: "node",
    LITTLEBABY_SERVICE_MARKER: NODE_SERVICE_MARKER,
    LITTLEBABY_SERVICE_KIND: NODE_SERVICE_KIND,
    LITTLEBABY_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    LITTLEBABY_STATE_DIR: sharedEnv.stateDir,
    LITTLEBABY_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.LITTLEBABY_STATE_DIR;
  const configPath = env.LITTLEBABY_CONFIG_PATH;
