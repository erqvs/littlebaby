...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds LITTLEBABY_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `littlebaby gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service LITTLEBABY_GATEWAY_TOKEN does not match gateway.auth.token in littlebaby.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.LITTLEBABY_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.LITTLEBABY_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `littlebaby gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
