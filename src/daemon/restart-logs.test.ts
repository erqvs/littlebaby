...
describe("restart log conventions", () => {
  it("resolves profile-aware gateway logs and restart attempts together", () => {
    ...
    expect(resolveGatewayLogPaths(env)).toEqual({
      logDir: "/Users/test/.littlebaby-work/logs",
      stdoutPath: "/Users/test/.littlebaby-work/logs/gateway.log",
      stderrPath: "/Users/test/.littlebaby-work/logs/gateway.err.log",
    });
    expect(resolveGatewayRestartLogPath(env)).toBe(
      `/Users/test/.littlebaby-work/logs/${GATEWAY_RESTART_LOG_FILENAME}`,
    );
  });
  ...
  it("renders best-effort POSIX log setup with escaped paths", () => {
    ...
    expect(setup).toContain(
      "if mkdir -p '/Users/test'\\''s/.littlebaby/logs' 2>/dev/null && : >>'/Users/test'\\''s/.littlebaby/logs/gateway-restart.log' 2>/dev/null; then",
    );
    expect(setup).toContain("exec >>'/Users/test'\\''s/.littlebaby/logs/gateway-restart.log' 2>&1");
  });
  ...
  it("renders CMD log setup with quoted paths", () => {
    ...
    expect(setup.quotedLogPath).toBe('"C:\\Users\\Test User/.littlebaby/logs/gateway-restart.log"');
    expect(setup.lines).toContain(
      'if not exist "C:\\Users\\Test User/.littlebaby/logs" mkdir "C:\\Users\\Test User/.littlebaby/logs" >nul 2>&1',
    );
  });
});
