...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        LITTLEBABY_BASE_ENV: "base",
        LITTLEBABY_TO_REMOVE: undefined,
      },
      env: {
        LITTLEBABY_TEST_ENV: "ok",
      },
    });

    expect(resolved.LITTLEBABY_BASE_ENV).toBe("base");
    expect(resolved.LITTLEBABY_TEST_ENV).toBe("ok");
    expect(resolved.LITTLEBABY_TO_REMOVE).toBeUndefined();
    expect(resolved.LITTLEBABY_CLI).toBe(LITTLEBABY_CLI_ENV_VALUE);
  });
