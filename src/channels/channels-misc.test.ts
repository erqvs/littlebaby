...
describe("WA_WEB_AUTH_DIR", () => {
  ...
  it("resolves lazily and caches across the legacy and channels/web entrypoints", async () => {
    const resolveWebChannelAuthDir = vi.fn(() => "/tmp/littlebaby-whatsapp-auth");
    ...
    expect(readLazyString(channelWeb.WA_WEB_AUTH_DIR)).toBe("/tmp/littlebaby-whatsapp-auth");
    expect(readLazyString(webEntry.WA_WEB_AUTH_DIR)).toBe("/tmp/littlebaby-whatsapp-auth");
