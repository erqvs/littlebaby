...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): LittleBabyConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as LittleBabyConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: LittleBabyConfig) => string[];
  input: LittleBabyConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: LittleBabyConfig, expected: string) {
