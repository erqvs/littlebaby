...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as LittleBabyConfig).toSorted()).toEqual([
      "buttons",
      "cards",
      "interactive",
    ]);
    expect(channelSupportsMessageCapability({} as LittleBabyConfig, "interactive")).toBe(true);
    expect(channelSupportsMessageCapability({} as LittleBabyConfig, "buttons")).toBe(true);
    expect(channelSupportsMessageCapability({} as LittleBabyConfig, "cards")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as LittleBabyConfig, channel: "demo-buttons" },
        "interactive",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as LittleBabyConfig, channel: "demo-cards" },
        "interactive",
      ),
    ).toBe(false);
    expect(
    ...
    expect(channelSupportsMessageCapabilityForChannel({ cfg: {} as LittleBabyConfig }, "cards")).toBe(
      false,
    );
  });
  ...
  it("uses unified message tool discovery for actions, capabilities, and schema", () => {
    ...
    expect(listChannelMessageActions({} as LittleBabyConfig)).toEqual(["send", "broadcast", "react"]);
    expect(listChannelMessageCapabilities({} as LittleBabyConfig)).toEqual(["interactive"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as LittleBabyConfig,
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as LittleBabyConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as LittleBabyConfig)).toEqual([]);
