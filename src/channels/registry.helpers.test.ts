...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://littlebaby.ai");
  });
  ...
  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    ...
    startupRegistry.channels = [
      {
        pluginId: "littlebaby-weixin",
        plugin: { id: "littlebaby-weixin", meta: { aliases: ["weixin"] } },
        source: "test",
      },
    ] as never;
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["littlebaby-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("littlebaby-weixin");
  });
