...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withTempHome(async (home) => {
      const prefsPath = path.join(home, ".littlebaby", "settings", "tts.json");
  ...
  it("derives the default prefs path from LITTLEBABY_CONFIG_PATH when set", async () => {
    await withTempHome(
      async (home) => {
        const stateDir = path.join(home, ".littlebaby-dev");
        ...
        vi.stubEnv("LITTLEBABY_CONFIG_PATH", path.join(stateDir, "littlebaby.json"));
        try {
          expect(
            resolveStatusTtsSnapshot({
              cfg: {
      ...
      { env: { LITTLEBABY_STATE_DIR: undefined } },
    );
  });
});
