...
type LoadLittleBabyPlugins = typeof import("./loader.js").loadLittleBabyPlugins;
...
const loadLittleBabyPluginsMock = vi.fn<LoadLittleBabyPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/littlebaby.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadLittleBabyPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: LittleBabyConfig = {
    plugins: {},
  };
  const autoEnabledConfig: LittleBabyConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadLittleBabyPlugins: (...args: Parameters<LoadLittleBabyPlugins>) =>
        loadLittleBabyPluginsMock(...args),
  ...
  beforeEach(() => {
    ...
    loadLittleBabyPluginsMock.mockReturnValue(registry);
