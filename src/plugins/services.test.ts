...
import type { LittleBabyPluginService, LittleBabyPluginServiceContext } from "./types.js";
...
function createRegistry(services: LittleBabyPluginService[]) {
...
function expectServiceContext(
  ctx: LittleBabyPluginServiceContext,
...
function expectServiceLogger(ctx: LittleBabyPluginServiceContext) {
...
function expectServiceContexts(
  contexts: LittleBabyPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
async function startTrackingServices(params: {
  services: LittleBabyPluginService[];
  config?: Parameters<typeof startPluginServices>[0]["config"];
  workspaceDir?: string;
}) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: LittleBabyPluginServiceContext[] = [];
