...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadLittleBabyPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
...
describe("resolvePluginTools optional tools", () => {
  ...
  beforeEach(() => {
    loadLittleBabyPluginsMock.mockClear();
    resolveRuntimePluginRegistryMock.mockReset();
    resolveRuntimePluginRegistryMock.mockImplementation((params) =>
      loadLittleBabyPluginsMock(params),
    );
  ...
  it.each([
    {
      ...
      expectedLoaderCall: {
        env: { LITTLEBABY_HOME: "/srv/littlebaby-home" },
      },
    },
    {
