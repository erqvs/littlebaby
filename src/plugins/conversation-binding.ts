...
const APPROVALS_PATH = "~/.littlebaby/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "littlebaby-app-server:thread:",
  "littlebaby-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("littlebaby.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("littlebaby.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to LittleBaby instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
