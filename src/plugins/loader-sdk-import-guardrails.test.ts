...
const ALLOWED_PLUGIN_SDK_FIXTURE_IMPORTS = new Set([
  ...
  'src/plugins/loader.test.ts:configSchema: (require("littlebaby/plugin-sdk").emptyPluginConfigSchema)(),',
  'src/plugins/loader.test.ts:const { onDiagnosticEvent } = require("littlebaby/plugin-sdk");',
  // Intentional jiti alias regression test.
  'src/plugins/loader.git-path-regression.test.ts:`import { resolveOutboundSendDep } from "littlebaby/plugin-sdk/infra-runtime";',
  'src/plugins/loader.git-path-regression.test.ts:          "littlebaby/plugin-sdk/infra-runtime": ${JSON.stringify(copiedChannelRuntimeShim)},',
]);
...
function findLoaderFixtureSdkImports(): string[] {
  ...
  for (const file of LOADER_FIXTURE_TEST_FILES) {
    ...
    for (const line of source.split("\n")) {
      if (
        line.includes('require("littlebaby/plugin-sdk') ||
        (line.includes("import ") && line.includes('"littlebaby/plugin-sdk'))
      ) {
