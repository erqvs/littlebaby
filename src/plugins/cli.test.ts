...
const mocks = vi.hoisted(() => ({
  ...
  loadLittleBabyPluginCliRegistry: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadLittleBabyPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadLittleBabyPluginCliRegistry(...args),
  loadLittleBabyPlugins: (...args: unknown[]) => mocks.loadLittleBabyPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: LittleBabyConfig;
  autoEnabledConfig: LittleBabyConfig;
  ...
  expect(mocks.loadLittleBabyPlugins).toHaveBeenCalledWith(
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadLittleBabyPluginCliRegistry.mockReset();
    mocks.loadLittleBabyPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadLittleBabyPlugins.mockReset();
    mocks.loadLittleBabyPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as LittleBabyConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as LittleBabyConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as LittleBabyConfig, env);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadLittleBabyPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadLittleBabyPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadLittleBabyPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("lazy-registers descriptor-backed plugin commands on first invocation", async () => {
    ...
    await registerPluginCliCommands(program, {} as LittleBabyConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as LittleBabyConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as LittleBabyConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
  ...
  it("skips plugin CLI registration from validated config when the snapshot is invalid", async () => {
    ...
    expect(mocks.loadLittleBabyPlugins).not.toHaveBeenCalled();
  });
});
