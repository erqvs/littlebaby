...
function readBundledPluginRecords(): BundledPluginRecord[] {
  return fs
    ...
    .flatMap((dirName) => {
      ...
      const manifestPath = path.join(rootDir, "littlebaby.plugin.json");
      if (!fs.existsSync(packagePath) || !fs.existsSync(manifestPath)) {
        return [];
      }

      const manifest = readJsonFile<PluginManifestShape>(manifestPath);
      const pkg = readJsonFile<LittleBabyPackageShape>(packagePath);
      ...
      return [
        {
          ...
          packageName,
          manifestId,
          installNpmSpec: normalizeText(pkg.littlebaby?.install?.npmSpec),
          channelId: normalizeText(pkg.littlebaby?.channel?.id),
        },
      ];
    });
}
...
function resolveAllowedPackageNamesForId(pluginId: string): string[] {
  return ALLOWED_PACKAGE_SUFFIXES.map((suffix) => `@littlebaby/${pluginId}${suffix}`);
}
  ...
  it.each([
    {
      ...
      message: `Bundled extension package names must stay anchored to the manifest id via @littlebaby/<id> or an approved suffix (${ALLOWED_PACKAGE_SUFFIXES.join(", ")}). Update the plugin naming docs and this invariant before adding a new naming form.`,
    ...
    },
    {
      ...
      message:
        "Bundled extension directory names should match littlebaby.plugin.json:id. If a legacy exception is unavoidable, add it to DIR_ID_EXCEPTIONS with a comment.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    {
      name: "keeps bundled littlebaby.install.npmSpec aligned with the package name",
      message:
        "Bundled littlebaby.install.npmSpec values must match the package name so install/update paths stay deterministic.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    },
    {
      ...
      message:
        "Bundled littlebaby.channel.id values must match littlebaby.plugin.json:id for the owning plugin.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
