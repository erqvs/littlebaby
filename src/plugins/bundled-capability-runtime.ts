...
import type { LittleBabyPluginDefinition, LittleBabyPluginModule } from "./types.js";
...
export function buildVitestCapabilityShimAliasMap(): Record<string, string> {
  return Object.fromEntries(
    CAPABILITY_VITEST_SHIM_ALIASES.flatMap(({ subpath, target }) => {
      ...
      return [
        [`littlebaby/plugin-sdk/${subpath}`, targetPath],
        [`@littlebaby/plugin-sdk/${subpath}`, targetPath],
      ];
    }),
  );
...
function applyVitestCapabilityAliasOverrides(params: {
  ...
  const {
    "littlebaby/plugin-sdk": _ignoredLegacyRootAlias,
    "@littlebaby/plugin-sdk": _ignoredScopedRootAlias,
    ...scopedAliasMap
  } = params.aliasMap;
  return {
    ...scopedAliasMap,
...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (resolved && typeof resolved === "object") {
    const definition = resolved as LittleBabyPluginDefinition;
    return {
      definition,
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getJiti(safeSource)(safeSource) as LittleBabyPluginModule;
    } catch (error) {
