...
const fixtureRoot = makeTrackedTempDir("littlebaby-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "littlebaby.plugin.json");
...
async function loadPlugins(pluginPaths: string[], warnings?: string[]) {
  const { loadLittleBabyPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("littlebaby plugins list");
  });
});
