...
type PluginConfig = NonNullable<LittleBabyConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: LittleBabyConfig["channels"];
}): LittleBabyConfig {
...
function createSinglePluginWithEmptySlotsConfig(): LittleBabyConfig {
...
function createSingleNpmInstallConfig(installPath: string): LittleBabyConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "littlebaby-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-littlebaby-plugin-install/my-plugin",
      },
      extensionsDir,
    });
