...
import type {
  ...
  LittleBabyPluginApi as CoreLittleBabyPluginApi,
  PluginRuntime as CorePluginRuntime,
} from "littlebaby/plugin-sdk/core";
import * as providerEntrySdk from "littlebaby/plugin-sdk/provider-entry";
...
import type {
  ...
  LittleBabyPluginApi as SharedLittleBabyPluginApi,
...
import type { LittleBabyPluginApi } from "../types.js";
...
const BROWSER_HELPER_EXPORT_PARITY_CONTRACTS: readonly BrowserHelperExportParityContract[] = [
  ...
  },
  {
    ...
    expectedExports: [
      ...
      "DEFAULT_LITTLEBABY_BROWSER_ENABLED",
...
describe("plugin-sdk subpath exports", () => {
  ...
  it("keeps the deprecated channel-runtime shim unused in repo imports", () => {
    const matches = findRepoFilesContaining({
      ...
      pattern: /littlebaby\/plugin-sdk\/channel-runtime(?=["'])/u,
      exclude: [
  ...
  it("keeps shared plugin-sdk types aligned", () => {
    ...
    expectTypeOf<CoreLittleBabyPluginApi>().toMatchTypeOf<LittleBabyPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<PluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<ChannelMessageActionContext>();
    expectTypeOf<CoreLittleBabyPluginApi>().toMatchTypeOf<SharedLittleBabyPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<SharedPluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<SharedChannelMessageActionContext>();
  });

  it("keeps runtime entry subpaths importable", async () => {
    const coreSdk = await importResolvedPluginSdkSubpath("littlebaby/plugin-sdk/core");
    ...
    const globalSingletonSdk = await importResolvedPluginSdkSubpath(
      "littlebaby/plugin-sdk/global-singleton",
    );
    const textRuntimeSdk = await importResolvedPluginSdkSubpath("littlebaby/plugin-sdk/text-runtime");
    const pluginEntrySdk = await importResolvedPluginSdkSubpath("littlebaby/plugin-sdk/plugin-entry");
    ...
    for (const id of representativeRuntimeSmokeSubpaths) {
      representativeModules.push(await importResolvedPluginSdkSubpath(`littlebaby/plugin-sdk/${id}`));
    }
