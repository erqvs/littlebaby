...
const FORBIDDEN_CONTRACT_MODULE_SPECIFIER_PATTERNS = [
  /^vitest$/u,
  /^littlebaby\/plugin-sdk\/testing$/u,
  /(^|\/)test-api(?:\.[cm]?[jt]s)?$/u,
  /(^|\/)__tests__(\/|$)/u,
  /(^|\/)test-support(\/|$)/u,
  /(^|\/)[^/]*\.test(?:[-.][^/]*)?(?:\.[cm]?[jt]s)?$/u,
...
function analyzeSourceModule(params: { filePath: string; source: string }): {
  ...
  for (const match of source.matchAll(STATIC_FROM_IMPORT_RE)) {
    ...
    if (specifier === "littlebaby/plugin-sdk/core" && importsDefinePluginEntry(importClause)) {
    ...
    for (const plugin of listBundledPluginRoots()) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          littlebaby?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.littlebaby?.extensions) ? pkg.littlebaby.extensions : [];
        if (
          extensions.some(
  ...
  it("follows relative import edges while scanning guarded contract graphs", () => {
    expect(
      analyzeSourceModule({
        ...
        import { y } from "littlebaby/plugin-sdk/testing";
      `,
  ...
  it("detects aliased definePluginEntry imports from core", () => {
    expect(
      analyzeSourceModule({
        ...
        source: `
          import { definePluginEntry as dpe } from "littlebaby/plugin-sdk/core";
          import { somethingElse } from "littlebaby/plugin-sdk/core";
        `,
      }).importsDefinePluginEntryFromCore,
    ).toBe(true);
  });
