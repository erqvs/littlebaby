...
const discoverLittleBabyPluginsMock = vi.fn();
const loadPluginManifestMock = vi.fn();

vi.mock("./discovery.js", () => ({
  discoverLittleBabyPlugins: (...args: unknown[]) => discoverLittleBabyPluginsMock(...args),
}));
...
function setBundledDiscoveryCandidates(candidates: unknown[]) {
  discoverLittleBabyPluginsMock.mockReturnValue({
    candidates,
    diagnostics: [],
  });
}
  ...
  loadPluginManifestMock.mockImplementation((rootDir: string) =>
    rootDir in manifestIds
      ...
      : {
          ...
          manifestPath: `${rootDir}/littlebaby.plugin.json`,
        },
  );
}
...
function createResolvedBundledSource(params: {
  ...
  return {
    ...
    npmSpec: params.npmSpec ?? `@littlebaby/${params.pluginId}`,
  };
}
...
describe("bundled plugin sources", () => {
  beforeEach(() => {
    discoverLittleBabyPluginsMock.mockReset();
  ...
  it("resolves bundled sources keyed by plugin id", () => {
    setBundledDiscoveryCandidates([
      ...
      createBundledCandidate({
        ...
        packageName: "@littlebaby/msteams",
      }),
    ]);
    setBundledManifestIdsByRoot({
      [appBundledPluginRoot("feishu")]: "feishu",
  ...
  it.each([
    [
      ...
      { kind: "npmSpec", value: "@littlebaby/feishu" } as const,
      { pluginId: "feishu", localPath: appBundledPluginRoot("feishu") },
    ],
    [
      "returns undefined for missing npm spec",
      { kind: "npmSpec", value: "@littlebaby/not-found" } as const,
      undefined,
    ],
    [
  ...
  it("forwards an explicit env to bundled discovery helpers", () => {
    ...
    const env = { HOME: "/tmp/littlebaby-home" } as NodeJS.ProcessEnv;
    ...
    expect(discoverLittleBabyPluginsMock).toHaveBeenNthCalledWith(1, {
      workspaceDir: "/workspace",
      env,
    });
    expect(discoverLittleBabyPluginsMock).toHaveBeenNthCalledWith(2, {
      workspaceDir: "/workspace",
      env,
    });
  });
  ...
  it("reuses a pre-resolved bundled map for repeated lookups", () => {
    ...
    expect(
      findBundledPluginSourceInMap({
        bundled,
        lookup: { kind: "npmSpec", value: "@littlebaby/feishu" },
      })?.pluginId,
    ).toBe("feishu");
  });
});
