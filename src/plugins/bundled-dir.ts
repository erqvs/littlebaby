...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "littlebaby-empty-bundled-plugins");

function bundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const raw = normalizeOptionalLowercaseString(env.LITTLEBABY_DISABLE_BUNDLED_PLUGINS);
...
function isSourceCheckoutRoot(_packageRoot: string): boolean {
  return false;
}
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "littlebaby.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.LITTLEBABY_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveLittleBabyPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveLittleBabyPackageRootSync({ argv1: process.argv[1] }),
      resolveLittleBabyPackageRootSync({ cwd: process.cwd() }),
      resolveLittleBabyPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
