...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing littlebaby.extensions; update the plugin package to include littlebaby.extensions (for example ["./dist/index.js"]). See https://docs.littlebaby.ai/help/troubleshooting#plugin-install-fails-with-missing-littlebaby-extensions';
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_LITTLEBABY_EXTENSIONS: "missing_littlebaby_extensions",
  EMPTY_LITTLEBABY_EXTENSIONS: "empty_littlebaby_extensions",
...
function ensureLittleBabyExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_LITTLEBABY_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json littlebaby.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_LITTLEBABY_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "littlebaby security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureLittleBabyExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureLittleBabyExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from littlebaby.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-littlebaby"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json littlebaby.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires LittleBaby >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set LITTLEBABY_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires LittleBaby >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade LittleBaby and retry.`,
...
export async function installPluginFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "littlebaby-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "littlebaby-npm-pack-",
    spec,
    timeoutMs,
