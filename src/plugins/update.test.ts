...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-littlebaby",
    targetDir: params?.targetDir ?? "/tmp/opik-littlebaby",
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@littlebaby/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["littlebaby-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/littlebaby-codex-app-server",
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-littlebaby",
        spec: "@opik/opik-littlebaby",
        integrity: "sha512-old",
        installPath: "/tmp/opik-littlebaby",
      }),
      pluginIds: ["opik-littlebaby"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-littlebaby",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-littlebaby",
        spec: "@opik/opik-littlebaby@0.2.5",
        integrity: "sha512-old",
        installPath: "/tmp/opik-littlebaby",
      }),
      pluginIds: ["opik-littlebaby"],
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "littlebaby-codex-app-server",
        spec: "littlebaby-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/littlebaby-codex-app-server",
      }),
      ...
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "littlebaby-codex-app-server",
        targetDir: "/tmp/littlebaby-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "littlebaby-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @littlebaby/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@littlebaby/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @littlebaby/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        ...
        resolvedSpec: "littlebaby-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "littlebaby-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "littlebaby-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "littlebaby-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
      ...
      expectedSpec: "littlebaby-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "littlebaby-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "littlebaby-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@littlebaby/voice-call",
      targetDir: "/tmp/littlebaby-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@littlebaby/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@littlebaby/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@littlebaby/voice-call");
    expect(result.config.plugins?.entries?.["@littlebaby/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@littlebaby/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@littlebaby/voice-call",
      installPath: "/tmp/littlebaby-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { LITTLEBABY_HOME: "/srv/littlebaby-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/littlebaby-home";
    mockBundledSources(
      createBundledSource({
