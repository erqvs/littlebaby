...
function writeBundledPlugin(params: {
  ...
  process.env.LITTLEBABY_BUNDLED_PLUGINS_DIR = bundledDir;
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".littlebaby", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ LITTLEBABY_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.LITTLEBABY_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
      ...
      JSON.stringify(
        {
          ...
          littlebaby: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.LITTLEBABY_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadLittleBabyPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadLittleBabyPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadLittleBabyPlugins>[0], "cache" | "config">,
) {
  return loadLittleBabyPlugins({
    cache: false,
    ...options,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadLittleBabyPlugins>,
  pluginId: string,
) {
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadLittleBabyPlugins>;
  loadSecond: () => ReturnType<typeof loadLittleBabyPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadLittleBabyPlugins>;
  loadVariant: () => ReturnType<typeof loadLittleBabyPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const littlebabyHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(littlebabyHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...process.env,
    LITTLEBABY_HOME: littlebabyHome,
    HOME: ignoredHome,
...
describe("loadLittleBabyPlugins", () => {
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadLittleBabyPlugins>) => {
  ...
  it.each([
    {
      ...
      run: () => {
        process.env.LITTLEBABY_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
        const plugin = writePlugin({
...
module.exports = { id: "manifest-only-plugin", register() { throw new Error("manifest-only snapshot should not register"); } };`,
  ...
  register() {},
        ...
        fs.writeFileSync(
          path.join(memoryPlugin.dir, "littlebaby.plugin.json"),
          JSON.stringify(
            {
              id: "memory-demo",
              kind: "memory",
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const importMarker = "__littlebaby_loader_import_throw_marker";
...
module.exports = { id: "throws-after-import", register() {} };`,
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__littlebaby_loader_reentry_error";
        const reenterFnMarker = "__littlebaby_loader_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          reenterFnMarker,
          (options: Parameters<typeof loadLittleBabyPlugins>[0]) => loadLittleBabyPlugins(options),
        );
        const pluginDir = makeTempDir();
        ...
        } satisfies Parameters<typeof loadLittleBabyPlugins>[0];
        writePlugin({
  ...
  register() {
    try {
      ...
      throw error;
        ...
        const registry = loadLittleBabyPlugins(nestedOptions);
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__littlebaby_runtime_registry_reentry_marker";
        const resolverMarker = "__littlebaby_runtime_registry_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          resolverMarker,
  ...
  register() {
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadLittleBabyPlugins(options);
        const scoped = loadLittleBabyPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadLittleBabyPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadLittleBabyPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("replaces prior plugin hook registrations on activating reloads", async () => {
    ...
    loadLittleBabyPlugins(loadOptions);
    loadLittleBabyPlugins(loadOptions);
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadLittleBabyPlugins(options);
    ...
    const second = loadLittleBabyPlugins(options);
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadLittleBabyPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadLittleBabyPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
        ...
        return {
          ...
          loadFirst: () =>
            loadLittleBabyPlugins({
              ...
              env: {
                ...process.env,
                LITTLEBABY_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadLittleBabyPlugins({
              ...
              env: {
                ...process.env,
                LITTLEBABY_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadLittleBabyPlugins({
              ...
              env: {
                ...
                LITTLEBABY_STATE_DIR: stateDir,
                LITTLEBABY_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadLittleBabyPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(littlebabyHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadLittleBabyPlugins(options),
          loadVariant: () =>
            loadLittleBabyPlugins({
              ...options,
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadLittleBabyPlugins({
      env: {
        ...
        HOME: homeDir,
        LITTLEBABY_HOME: undefined,
        LITTLEBABY_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers LITTLEBABY_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const littlebabyHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "littlebaby-home-demo",
      dir: path.join(littlebabyHome, "plugins", "littlebaby-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "littlebaby-home-demo", register() {} };`,
    });
    ...
    const registry = loadLittleBabyPlugins({
      ...
      config: {
        plugins: {
          allow: ["littlebaby-home-demo"],
          entries: {
            "littlebaby-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/littlebaby-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "littlebaby-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadLittleBabyPlugins>) =>
  ...
  api.registerContextEngine("shared-context-engine-loader-test", () => ({}));
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadLittleBabyPlugins>) => {
          expect(
  ...
  it("loads bundled channel entries through nested default export wrappers", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@littlebaby/nested-default-channel",
          littlebaby: {
            extensions: ["./index.cjs"],
          },
        },
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "littlebaby.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "littlebaby.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const broadSetupRegistry = loadLittleBabyPlugins({
      cache: false,
      config,
    ...
    const scopedSetupRegistry = loadLittleBabyPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@littlebaby/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@littlebaby/setup-only-bundled-contract-test",
        fullBlurb: "full entry should not run in setup-only mode",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@littlebaby/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@littlebaby/setup-runtime-bundled-contract-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@littlebaby/setup-runtime-bundled-contract-secrets-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@littlebaby/setup-runtime-bundled-contract-runtime-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@littlebaby/setup-runtime-bundled-runtime-merge-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@littlebaby/setup-runtime-not-preferred-test",
  ...
  it("applies the bundled runtime setter before loading the merged setup-runtime plugin", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@littlebaby/setup-runtime-order-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("records setup runtime setter failures without aborting the full load pass", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@littlebaby/setup-runtime-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledSetupEntryContract: true,
  ...
  it("rejects mismatched bundled runtime entry ids before applying setup-runtime setters", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@littlebaby/setup-runtime-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("rejects mismatched bundled setup export ids before loading setup-runtime entry code", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@littlebaby/setup-export-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("isolates loadSetupPlugin errors as per-plugin diagnostics instead of crashing registry load", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@littlebaby/setup-entry-throws-test",
          littlebaby: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
  ...
  it("keeps healthy sibling channel plugins loadable when a setup entry throws", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@littlebaby/setup-entry-throws-sibling-test",
          littlebaby: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
    ...
    fs.writeFileSync(
      path.join(brokenDir, "littlebaby.plugin.json"),
      JSON.stringify(
        {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "littlebaby.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "littlebaby.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryCoreDir, "littlebaby.plugin.json"),
            JSON.stringify(
          ...
          fs.writeFileSync(
            path.join(memoryLanceDir, "littlebaby.plugin.json"),
            JSON.stringify(
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ LITTLEBABY_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  if (!Object.getOwnPropertyDescriptor(runtime, "channel")) {
    ...
    const registry = withEnv({ LITTLEBABY_STATE_DIR: stateDir }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("littlebaby/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ LITTLEBABY_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadLittleBabyPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__littlebabyLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("littlebaby/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("littlebaby/plugin-sdk");
  ...
  it("suppresses trust warning logs for non-activating snapshot loads", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ LITTLEBABY_STATE_DIR: stateDir }, () => {
      const globalDir = path.join(stateDir, "extensions", "rogue");
      mkdirSafe(globalDir);
      writePlugin({
        id: "rogue",
