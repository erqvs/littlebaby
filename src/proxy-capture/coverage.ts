...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[littlebaby proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[littlebaby proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`littlebaby proxy coverage\` for details.`,
  );
}
