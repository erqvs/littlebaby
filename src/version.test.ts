...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir({ prefix: "littlebaby-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "littlebaby", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir({ prefix: "littlebaby-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "littlebaby", version: "2.3.4" });
  ...
  it("ignores non-littlebaby package and blank build-info versions", async () => {
    await withTempDir({ prefix: "littlebaby-version-" }, async (root) => {
  ...
  it("prefers LITTLEBABY_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        LITTLEBABY_VERSION: "9.9.9",
        LITTLEBABY_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale LITTLEBABY_VERSION for compatibility checks", () => {
    const previous = process.env.LITTLEBABY_VERSION;
    const previousService = process.env.LITTLEBABY_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.LITTLEBABY_VERSION = "2026.3.25";
      process.env.LITTLEBABY_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.LITTLEBABY_VERSION = previous;
      process.env.LITTLEBABY_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ...
        LITTLEBABY_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        LITTLEBABY_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        LITTLEBABY_VERSION: "2026.3.99",
        LITTLEBABY_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        LITTLEBABY_VERSION: "   ",
        LITTLEBABY_SERVICE_VERSION: "  2.0.0  ",
