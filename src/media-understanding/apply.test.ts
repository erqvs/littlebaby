...
const TEMP_MEDIA_PREFIX = "littlebaby-media-";
...
function createGroqAudioConfig(): LittleBabyConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): LittleBabyConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): LittleBabyConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: LittleBabyConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredLittleBabyTmpDir();
