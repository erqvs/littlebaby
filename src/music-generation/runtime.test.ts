...
describe("music-generation runtime", () => {
  ...
  it("lists runtime music-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeMusicGenerationProviders({ config: {} as LittleBabyConfig })).toEqual(
      providers,
    );
    expect(mocks.listMusicGenerationProviders).toHaveBeenCalledWith({} as LittleBabyConfig);
  });
