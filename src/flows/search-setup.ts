...
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<LittleBabyConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: LittleBabyConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: LittleBabyConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: LittleBabyConfig, provider: SearchProvider): SecretRef {
  const entry =
...
export function applySearchKey(
  ...
  const nextBase: LittleBabyConfig = {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: LittleBabyConfig, result: LittleBabyConfig): LittleBabyConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as LittleBabyConfig["plugins"],
  };
}
...
async function finalizeSearchProviderSetup(params: {
  originalConfig: LittleBabyConfig;
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with littlebaby configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "LittleBaby will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.littlebaby.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — LittleBaby will store a reference instead of the API key.",
