...
export type PromptDefaultModelResult = { model?: string; config?: LittleBabyConfig };
...
function resolveConfiguredModelRaw(cfg: LittleBabyConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: LittleBabyConfig): string[] {
...
export function applyModelAllowlist(cfg: LittleBabyConfig, models: string[]): LittleBabyConfig {
