...
This directory contains hooks that ship with LittleBaby. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.littlebaby/workspace`)
...
homepage: https://docs.littlebaby.ai/automation/hooks#my-hook
metadata:
  { "littlebaby": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.littlebaby/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.littlebaby/littlebaby.json`:
...
2. Restart gateway: `pkill -9 -f 'littlebaby.*gateway' && pnpm littlebaby gateway`
3. Enable the hook: `littlebaby hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.littlebaby.ai/automation/hooks
