...
homepage: https://docs.littlebaby.ai/automation/hooks#command-logger
metadata:
  {
    "littlebaby":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.littlebaby/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.littlebaby/logs/commands.log ~/.littlebaby/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/littlebaby`:
   ```
   /home/username/.littlebaby/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
