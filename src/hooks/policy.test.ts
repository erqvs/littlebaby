...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "littlebaby-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "littlebaby-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "littlebaby-bundled");
      const plugin = makeHookEntry("shared", "littlebaby-plugin");
      const managed = makeHookEntry("shared", "littlebaby-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("littlebaby-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "littlebaby-managed");
      const workspace = makeHookEntry("shared", "littlebaby-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "littlebaby-workspace");
      const second = makeHookEntry("shared", "littlebaby-workspace");
      second.hook.handlerPath = "/tmp/littlebaby-workspace/shared/handler-2.js";
