...
const fixtureRoot = path.join(process.cwd(), ".tmp", `littlebaby-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@littlebaby/test-hooks",
      version: "0.0.1",
      littlebaby: { hooks: params.hooks },
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "littlebaby.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "littlebaby.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  it("does not expose dangerous force unsafe install through npm-spec archive params", async () => {
    ...
    try {
      const result = await installHooksFromNpmSpec({
        spec: "@littlebaby/test-hooks@0.0.1",
      });
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@littlebaby/test-hooks@0.0.1",
              name: "@littlebaby/test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.npmResolution?.resolvedSpec).toBe("@littlebaby/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@littlebaby/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@littlebaby/test-hooks@0.0.2-beta.1",
      name: "@littlebaby/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@littlebaby/test-hooks@beta"');
    }
  });
});
