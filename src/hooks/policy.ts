import type { LittleBabyConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "littlebaby-bundled": {
    ...
    canOverride: ["littlebaby-bundled"],
    canBeOverriddenBy: ["littlebaby-managed", "littlebaby-plugin"],
  },
  "littlebaby-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["littlebaby-bundled", "littlebaby-plugin"],
    canBeOverriddenBy: ["littlebaby-managed"],
  },
  "littlebaby-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["littlebaby-bundled", "littlebaby-managed", "littlebaby-plugin"],
    canBeOverriddenBy: ["littlebaby-managed"],
  },
  "littlebaby-workspace": {
    ...
    canOverride: ["littlebaby-workspace"],
    canBeOverriddenBy: ["littlebaby-workspace"],
  },
};
...
export function resolveHookConfig(
  config: LittleBabyConfig | undefined,
  hookKey: string,
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "littlebaby-plugin") {
    return { enabled: true };
  }
