...
import { createLittleBabyReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createLittleBabyCodingTools read behavior", () => {
  it("applies sandbox path guards to canonical path", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "littlebaby-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createLittleBabyReadTool(
      baseRead as unknown as Parameters<typeof createLittleBabyReadTool>[0],
    );
