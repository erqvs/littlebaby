...
const TEST_ENV_VAR = "LITTLEBABY_MODELS_CONFIG_TEST_ENV";
...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/littlebaby-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): LittleBabyConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: LittleBabyConfig) {
