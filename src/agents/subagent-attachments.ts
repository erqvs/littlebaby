...
function resolveAttachmentLimits(config: LittleBabyConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".littlebaby", "attachments");
  const relDir = path.posix.join(".littlebaby", "attachments", attachmentId);
