...
import "./test-helpers/fast-littlebaby-tools.js";
import { createLittleBabyCodingTools } from "./pi-tools.js";
...
describe("createLittleBabyCodingTools", () => {
  const testConfig: LittleBabyConfig = {};

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createLittleBabyCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createLittleBabyCodingTools({
      config: testConfig,
    ...
    const codexTools = createLittleBabyCodingTools({
      config: testConfig,
    ...
    const disabledConfig: LittleBabyConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createLittleBabyCodingTools({
    ...
    const anthropicTools = createLittleBabyCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: LittleBabyConfig = {
      tools: {
        exec: {
    ...
    const allowed = createLittleBabyCodingTools({
    ...
    const denied = createLittleBabyCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createLittleBabyCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createLittleBabyCodingTools({ config: testConfig });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createLittleBabyCodingTools({ messageProvider: "discord" });
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createLittleBabyCodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-depth-policy-"));
    try {
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createLittleBabyCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createLittleBabyCodingTools({
      modelProvider: "xai",
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createLittleBabyCodingTools();
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-read-"));
    try {
      ...
      const contents = "Hello from littlebaby read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "littlebaby-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "littlebaby-workspace"),
    ...
    const tools = createLittleBabyCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-canonical-"));
    try {
      const tools = createLittleBabyCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-legacy-alias-"));
    try {
      const tools = createLittleBabyCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-structured-write-"));
    try {
      const tools = createLittleBabyCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-structured-edit-"));
    try {
