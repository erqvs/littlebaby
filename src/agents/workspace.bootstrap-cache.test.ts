...
describe("workspace bootstrap file caching", () => {
  ...
  beforeEach(async () => {
    workspaceDir = await makeTempWorkspace("littlebaby-bootstrap-cache-test-");
  });
  ...
  it("caches files independently by path", async () => {
    ...
    const workspace1 = await makeTempWorkspace("littlebaby-cache-test1-");
    const workspace2 = await makeTempWorkspace("littlebaby-cache-test2-");
