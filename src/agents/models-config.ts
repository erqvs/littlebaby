...
import { planLittleBabyModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: LittleBabyConfig): {
  config: LittleBabyConfig;
  sourceConfigForSecrets: LittleBabyConfig;
} {
...
export async function ensureLittleBabyModelsJson(
  config?: LittleBabyConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveLittleBabyAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planLittleBabyModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
