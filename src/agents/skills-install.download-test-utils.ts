...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("littlebaby-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-skills-install-"));
  try {
