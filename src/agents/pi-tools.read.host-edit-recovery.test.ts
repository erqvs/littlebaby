...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when LITTLEBABY_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const littlebabyHome = path.join(tmpDir, "littlebaby-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(littlebabyHome, { recursive: true });
    ...
    process.env.LITTLEBABY_HOME = littlebabyHome;
    ...
    try {
      ...
      await expect(fs.access(path.join(littlebabyHome, "demo.txt"))).rejects.toBeDefined();
    } finally {
