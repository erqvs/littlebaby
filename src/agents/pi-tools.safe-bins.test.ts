...
let createLittleBabyCodingTools: typeof import("./pi-tools.js").createLittleBabyCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      LITTLEBABY_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "littlebaby-test-no-bundled-extensions"),
    },
    async () => {
      ({ createLittleBabyCodingTools } = await import("./pi-tools.js"));
    },
  );
});
...
vi.mock("./littlebaby-tools.js", () => ({
  createLittleBabyTools: () => [],
}));
...
async function withSafeBinsExecTool(
  ...
  try {
    await withEnvAsync(
      {
        LITTLEBABY_SHELL_ENV_TIMEOUT_MS: "1",
        SHELL: "/bin/sh",
      },
      async () => {
        await run(ctx);
...
describe("createLittleBabyCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "littlebaby-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "littlebaby-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "littlebaby-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "littlebaby-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "littlebaby-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "littlebaby-safe-bins-grep-",
        safeBins: ["grep"],
