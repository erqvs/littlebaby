...
let configOverride: LittleBabyConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: LittleBabyConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/littlebaby/littlebaby/issues/31298
    previousFastTestEnv = process.env.LITTLEBABY_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.LITTLEBABY_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // LITTLEBABY_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("LittleBaby runtime context (internal):");
