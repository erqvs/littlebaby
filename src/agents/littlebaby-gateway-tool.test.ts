...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-test-"));

    try {
      await withEnvAsync(
        { LITTLEBABY_STATE_DIR: stateDir, LITTLEBABY_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: littlebaby --profile isolated doctor --non-interactive",
          );
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/littlebaby-bin"],
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/littlebaby" } } }',
      }),
    ).rejects.toThrow(
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/littlebaby-bin"] } } }',
      }),
    ).rejects.toThrow(
