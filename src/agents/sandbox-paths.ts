...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const littleBabyTmpDir = path.resolve(resolvePreferredLittleBabyTmpDir());
  if (!isPathInside(littleBabyTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: littleBabyTmpDir });
  return resolved;
}
