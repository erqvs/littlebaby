...
describe("runEmbeddedPiAgent incomplete-turn safety", () => {
  ...
  it("does not strict-agentic retry casual Discord status chatter", async () => {
    ...
    const result = await runEmbeddedPiAgent({
      ...overflowBaseRunParams,
      prompt:
        "made a bunch of improvements to the student's source code (littlebaby) this weekend, along with a few other maintainers. hopefully he will be more proactive now",
