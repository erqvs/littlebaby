...
vi.mock("../../docs-path.js", () => ({
  resolveLittleBabyDocsPath: async () => undefined,
}));
...
vi.mock("../../pi-tools.js", () => ({
  createLittleBabyCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) => [
    {
      name: "sessions_spawn",
      execute: async (
        _callId: string,
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("littlebaby.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "littlebaby.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-ctx-engine-agent-"));
