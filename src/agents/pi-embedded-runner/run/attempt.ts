...
import { resolveLittleBabyDocsPath } from "../../docs-path.js";
...
import { createLittleBabyCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveLittleBabyAgentDir();
    const toolsRaw = params.disableTools
      ? []
      : (() => {
          const allTools = createLittleBabyCodingTools({
            agentId: sessionAgentId,
    ...
    const docsPath = await resolveLittleBabyDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/littlebaby/littlebaby/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("littlebaby:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/littlebaby/littlebaby/issues/8643
