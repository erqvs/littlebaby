...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("littlebaby-ws-", async (workspaceDir) => {
      await withTempDir("littlebaby-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createLittleBabyCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "littlebaby" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("littlebaby-ws-", async (workspaceDir) => {
      await withTempDir("littlebaby-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("littlebaby-ws-", async (workspaceDir) => {
      const cfg: LittleBabyConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createLittleBabyCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-littlebaby.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("littlebaby-sandbox-", async (sandboxDir) => {
      await withTempDir("littlebaby-workspace-", async (workspaceDir) => {
        ...
        const tools = createLittleBabyCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
