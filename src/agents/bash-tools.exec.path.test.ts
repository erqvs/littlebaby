...
vi.mock("../process/supervisor/index.js", () => ({
  getProcessSupervisor: () => ({
    spawn: async (input: {
      ...
      const env = input.env ?? {};
      if (command.includes("LITTLEBABY_SHELL")) {
        input.onStdout?.(env.LITTLEBABY_SHELL ?? "");
...
describe("exec PATH login shell merge", () => {
  ...
  it("sets LITTLEBABY_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-littlebaby-shell", {
      command: 'printf "%s" "${LITTLEBABY_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "littlebaby-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/littlebaby-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      const result = await tool.execute("call1", {
      ...
      expect(output).not.toContain("/tmp/littlebaby-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a littlebaby /approve abc123 deny",
