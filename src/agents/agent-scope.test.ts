...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: LittleBabyConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/littlebaby" }],
      },
    };
        ...
        list: [
          {
            id: "main",
            name: "Main Agent",
            workspace: "~/littlebaby",
            agentDir: "~/.littlebaby/agents/main",
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: LittleBabyConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: LittleBabyConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: LittleBabyConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: LittleBabyConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: LittleBabyConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: LittleBabyConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: LittleBabyConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: LittleBabyConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/littlebaby-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/littlebaby");
  });

  it("uses LITTLEBABY_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "littlebaby-home");
    vi.stubEnv("LITTLEBABY_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as LittleBabyConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".littlebaby", "workspace"));
  });

  it("uses LITTLEBABY_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "littlebaby-home");
    vi.stubEnv("LITTLEBABY_HOME", home);
    // Clear state dir so it falls back to LITTLEBABY_HOME
    vi.stubEnv("LITTLEBABY_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as LittleBabyConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".littlebaby", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/littlebaby-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/littlebaby-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "littlebaby-agent-scope-"));
