...
type AgentEntry = NonNullable<NonNullable<LittleBabyConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: LittleBabyConfig): AgentEntry[] {
...
export function listAgentIds(cfg: LittleBabyConfig): string[] {
...
export function resolveDefaultAgentId(cfg: LittleBabyConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: LittleBabyConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentWorkspaceDir(cfg: LittleBabyConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
