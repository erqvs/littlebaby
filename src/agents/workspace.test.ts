...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses LITTLEBABY_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      LITTLEBABY_HOME: "/srv/littlebaby-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/littlebaby-home"), ".littlebaby", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".littlebaby", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("littlebaby-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("littlebaby-workspace-");
    await fs.mkdir(path.join(tempDir, ".littlebaby"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-workspace-hardlink-"));
    try {
