...
describe("owner-only tool gating", () => {
  it("removes owner-only tools for unauthorized senders", () => {
    const tools = createLittleBabyCodingTools({ senderIsOwner: false });
  ...
  it("keeps owner-only tools for authorized senders", () => {
    const tools = createLittleBabyCodingTools({ senderIsOwner: true });
    const toolNames = tools.map((tool) => tool.name);
  ...
  it("restricts node-originated runs to the node-safe tool subset", () => {
    const tools = createLittleBabyCodingTools({ messageProvider: "node", senderIsOwner: false });
    const toolNames = tools.map((tool) => tool.name);
