...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.LITTLEBABY_PROFILE?.trim();
  if (profile && normalizeOptionalLowercaseString(profile) !== "default") {
    return path.join(home, ".littlebaby", `workspace-${profile}`);
  }
  return path.join(home, ".littlebaby", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".littlebaby";
