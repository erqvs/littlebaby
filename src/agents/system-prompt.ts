...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; LittleBaby handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "Community: https://discord.com/invite/littlebaby",
    "Find new skills: https://littlebabyhub.ai",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running LittleBaby process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List LittleBaby agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List LittleBaby agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside LittleBaby.";
  }

  const lines = [
    "You are a personal assistant running inside LittleBaby.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control LittleBaby's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    ...safetySection,
    "## LittleBaby CLI Quick Reference",
    "LittleBaby is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- littlebaby gateway status",
    "- littlebaby gateway start",
    "- littlebaby gateway stop",
    "- littlebaby gateway restart",
    "If unsure, ask the user to run `littlebaby help` (or `littlebaby gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## LittleBaby Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, LittleBaby pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
