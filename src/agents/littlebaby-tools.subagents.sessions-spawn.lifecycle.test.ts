...
} from "./littlebaby-tools.subagents.sessions-spawn.test-harness.js";
import {
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.LITTLEBABY_TEST_FAST;
...
describe("littlebaby-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnAnnounceFlowOverride();
    resetSessionsSpawnHookRunnerOverride();
    resetSessionsSpawnConfigOverride();
  ...
  afterAll(() => {
    ...
    process.env.LITTLEBABY_TEST_FAST = fastModeEnv.previous;
  });
