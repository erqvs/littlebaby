...
async function withOutsideHardlinkInLittleBabyTmp<T>(
  params: {
    littleBabyTmpDir: string;
  ...
  const hardlinkPath = path.join(params.littleBabyTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.littleBabyTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.littleBabyTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.littleBabyTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const littleBabyTmpDir = resolvePreferredLittleBabyTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred LittleBaby tmp root",
      media: path.join(littleBabyTmpDir, "image.png"),
      expected: path.join(littleBabyTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred LittleBaby tmp root",
      media: pathToFileURL(path.join(littleBabyTmpDir, "photo.png")).href,
      expected: path.join(littleBabyTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred LittleBaby tmp root",
      media: path.join(littleBabyTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(littleBabyTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(littleBabyTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside littlebaby tmp root",
      media: path.join(os.tmpdir(), "outside-littlebaby", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked LittleBaby tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(littleBabyTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(littleBabyTmpDir, { recursive: true });
      const symlinkPath = path.join(littleBabyTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked LittleBaby tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInLittleBabyTmp(
      {
        littleBabyTmpDir,
  ...
  it("rejects symlinked LittleBaby tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInLittleBabyTmp(
