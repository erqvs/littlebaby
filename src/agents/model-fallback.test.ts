...
function makeFallbacksOnlyCfg(): LittleBabyConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): LittleBabyConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "littlebaby-auth-suite-mock");
...
// https://github.com/littlebaby/littlebaby/issues/23440
...
describe("runWithModelFallback", () => {
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("littlebaby-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
