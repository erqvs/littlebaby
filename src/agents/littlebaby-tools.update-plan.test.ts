...
import { isUpdatePlanToolEnabledForLittleBabyTools } from "./littlebaby-tools.registration.js";
import { createUpdatePlanTool } from "./tools/update-plan-tool.js";

type UpdatePlanGatingParams = Parameters<typeof isUpdatePlanToolEnabledForLittleBabyTools>[0];

function expectUpdatePlanEnabled(params: UpdatePlanGatingParams, expected: boolean): void {
  expect(isUpdatePlanToolEnabledForLittleBabyTools(params)).toBe(expected);
}
...
describe("littlebaby-tools update_plan gating", () => {
  it("keeps update_plan disabled by default", () => {
    expectUpdatePlanEnabled({ config: {} as LittleBabyConfig }, false);
  });
