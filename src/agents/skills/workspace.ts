...
function resolveSkillsLimits(config?: LittleBabyConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "littlebaby-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "littlebaby-bundled") {
    return {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "littlebaby-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "littlebaby-managed",
  });
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`littlebaby skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`littlebaby skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
