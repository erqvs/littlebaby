...
function isLikelyLittleBabyCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "littlebaby" ||
    entryBasename === "littlebaby.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real LittleBaby CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureLittleBabyModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyLittleBabyCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): LittleBabyConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureLittleBabyModelsJson(cfg);
    } catch {
