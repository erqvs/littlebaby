...
import { markLittleBabyExecEnv } from "../../infra/littlebaby-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "littlebaby.sandbox=1");
  args.push("--label", `littlebaby.sessionKey=${params.scopeKey}`);
  args.push("--label", `littlebaby.createdAtMs=${createdAtMs}`);
  args.push("--label", `littlebaby.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `littlebaby.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markLittleBabyExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "littlebaby.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`littlebaby sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`littlebaby sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("littlebaby sandbox recreate --all");
}
