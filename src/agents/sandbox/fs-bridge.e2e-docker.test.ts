...
describe("sandbox fs bridge docker e2e", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-fsbridge-e2e-"));
      const workspaceDir = path.join(stateDir, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });

      const suffix = `${process.pid}-${Date.now()}`;
      const containerName = `littlebaby-fsbridge-${suffix}`.slice(0, 63);
      ...
      try {
        ...
        const sandbox = createSandboxTestContext({
          ...
          dockerOverrides: {
            ...
            containerPrefix: "littlebaby-fsbridge-",
            user: "",
          },
        });
