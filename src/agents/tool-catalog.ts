...
type CoreToolDefinition = {
  ...
  includeInLittleBabyGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const littlebabyTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInLittleBabyGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:littlebaby": littlebabyTools,
