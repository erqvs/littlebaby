...
function resolveAnthropicOpusThinking(cfg: LittleBabyConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: LittleBabyConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function createProviderWithModelsConfig(provider: string, models: Array<Record<string, unknown>>) {
  ...
  } as Partial<LittleBabyConfig>;
}

function resolveConfiguredRefForTest(cfg: Partial<LittleBabyConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as LittleBabyConfig,
...
describe("model-selection", () => {
  ...
  describe("isCliProvider", () => {
    ...
    it("returns true for setup-registered cli backends", () => {
      expect(isCliProvider("claude-cli", {} as LittleBabyConfig)).toBe(true);
    });

    it("returns false for provider ids", () => {
      expect(isCliProvider("example-cli", {} as LittleBabyConfig)).toBe(false);
    });
  });
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<LittleBabyConfig> = {
        agents: {
          defaults: {
            models: {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("sanitizes control characters in providerless-model warnings", () => {
      const warnLogs = createWarnLogCapture("littlebaby-model-selection-test");
      try {
        const cfg: Partial<LittleBabyConfig> = {
          agents: {
            defaults: {
