...
const GOOGLE_LIVE_SINGLE_KEY = "LITTLEBABY_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "LITTLEBABY_LIVE_ANTHROPIC_KEY",
    listVar: "LITTLEBABY_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "LITTLEBABY_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `LITTLEBABY_LIVE_${base}_KEY`;
