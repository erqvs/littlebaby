...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: LittleBabyConfig) {
    return createLittleBabyCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<LittleBabyConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<LittleBabyConfig["agents"]>["list"]>[number]["tools"];
  }): LittleBabyConfig {
    return {
      tools: params.tools,
      agents: {
