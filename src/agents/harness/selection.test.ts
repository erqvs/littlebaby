...
const originalRuntime = process.env.LITTLEBABY_AGENT_RUNTIME;
const originalHarnessFallback = process.env.LITTLEBABY_AGENT_HARNESS_FALLBACK;

afterEach(() => {
  clearAgentHarnesses();
  piRunAttempt.mockClear();
  if (originalRuntime == null) {
    delete process.env.LITTLEBABY_AGENT_RUNTIME;
  } else {
    process.env.LITTLEBABY_AGENT_RUNTIME = originalRuntime;
  }
  if (originalHarnessFallback == null) {
    delete process.env.LITTLEBABY_AGENT_HARNESS_FALLBACK;
  } else {
    process.env.LITTLEBABY_AGENT_HARNESS_FALLBACK = originalHarnessFallback;
  }
});
...
describe("runAgentHarnessAttemptWithFallback", () => {
  ...
  it("falls back to the PI harness in auto mode when the selected plugin harness fails", async () => {
    process.env.LITTLEBABY_AGENT_RUNTIME = "auto";
