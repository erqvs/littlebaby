...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          LITTLEBABY_MCP_TOKEN: mcpLoopbackRuntime.token,
          LITTLEBABY_MCP_AGENT_ID: sessionAgentId ?? "",
          LITTLEBABY_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          LITTLEBABY_MCP_SESSION_KEY: params.sessionKey ?? "",
          LITTLEBABY_MCP_MESSAGE_CHANNEL: params.messageProvider ?? "",
          LITTLEBABY_MCP_SENDER_IS_OWNER: params.senderIsOwner === true ? "true" : "false",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
