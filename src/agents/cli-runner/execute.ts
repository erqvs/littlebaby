...
const CLI_BACKEND_PRESERVE_ENV = "LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.LITTLEBABY_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.LITTLEBABY_MCP_SESSION_KEY || "<empty>"}`,
    `agentId=${childEnv.LITTLEBABY_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.LITTLEBABY_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.LITTLEBABY_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.LITTLEBABY_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (
            env.LITTLEBABY_MCP_TOKEN ||
            env.LITTLEBABY_MCP_SESSION_KEY ||
            env.LITTLEBABY_MCP_SENDER_IS_OWNER
          ) {
