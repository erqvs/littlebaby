...
describe("prepareCliBundleMcpConfig", () => {
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: { plugins: { enabled: false } },
      env: {
        LITTLEBABY_MCP_TOKEN: "loopback-token-123",
        LITTLEBABY_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
        LITTLEBABY_MCP_SENDER_IS_OWNER: "false",
      },
    });
  ...
  it("writes Gemini system settings for bundle MCP servers", async () => {
    ...
    expect(prepared.env?.LITTLEBABY_MCP_TOKEN).toBe("loopback-token-123");
