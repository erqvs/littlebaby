...
const LITTLEBABY_CLAUDE_PLUGIN_NAME = "littlebaby-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredLittleBabyTmpDir(), "littlebaby-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, LITTLEBABY_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: LITTLEBABY_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped LittleBaby skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
