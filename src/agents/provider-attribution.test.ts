...
describe("provider attribution", () => {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { LITTLEBABY_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { LITTLEBABY_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ LITTLEBABY_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
