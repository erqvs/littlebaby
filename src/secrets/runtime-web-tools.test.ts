...
function setConfiguredFetchProviderKey(configTarget: LittleBabyConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: LittleBabyConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: LittleBabyConfig, provider: ProviderUnderTest): unknown {
