...
export const LITTLEBABY_NODE_OPTIONS_READY = "LITTLEBABY_NODE_OPTIONS_READY";
export const LITTLEBABY_NODE_EXTRA_CA_CERTS_READY = "LITTLEBABY_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (shouldSkipRespawnForArgv(argv) || isTruthyEnvValue(env.LITTLEBABY_NO_RESPAWN)) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[LITTLEBABY_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[LITTLEBABY_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !isTruthyEnvValue(env[LITTLEBABY_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[LITTLEBABY_NODE_OPTIONS_READY] = "1";
