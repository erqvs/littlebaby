...
describe("plugin activation boundary", () => {
  ...
  let browserHelpersPromise:
    | Promise<{
        ...
        DEFAULT_LITTLEBABY_BROWSER_COLOR: typeof import("./plugin-sdk/browser-config.js").DEFAULT_LITTLEBABY_BROWSER_COLOR;
        DEFAULT_LITTLEBABY_BROWSER_PROFILE_NAME: typeof import("./plugin-sdk/browser-config.js").DEFAULT_LITTLEBABY_BROWSER_PROFILE_NAME;
  ...
  function importBrowserHelpers() {
    browserHelpersPromise ??= Promise.all([
      ...
      DEFAULT_LITTLEBABY_BROWSER_COLOR: config.DEFAULT_LITTLEBABY_BROWSER_COLOR,
      DEFAULT_LITTLEBABY_BROWSER_PROFILE_NAME: config.DEFAULT_LITTLEBABY_BROWSER_PROFILE_NAME,
  ...
  it("keeps generic boundaries cold and loads only narrow browser helper surfaces on use", async () => {
    ...
    expect(browser.DEFAULT_LITTLEBABY_BROWSER_COLOR).toBe("#FF4500");
    expect(browser.DEFAULT_LITTLEBABY_BROWSER_PROFILE_NAME).toBe("littlebaby");
    ...
    expect(browser.resolveProfile(resolved, "littlebaby")).toEqual(
