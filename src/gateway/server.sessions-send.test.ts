...
const { createLittleBabyTools } = await import("../agents/littlebaby-tools.js");
...
type SessionSendTool = ReturnType<typeof createLittleBabyTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createLittleBabyTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["LITTLEBABY_GATEWAY_PORT", "LITTLEBABY_GATEWAY_TOKEN"]);
  ...
  process.env.LITTLEBABY_GATEWAY_PORT = String(gatewayPort);
  process.env.LITTLEBABY_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("LITTLEBABY_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createLittleBabyTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
