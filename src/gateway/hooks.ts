...
export function resolveHooksConfig(cfg: LittleBabyConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: LittleBabyConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-littlebaby-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-littlebaby-idempotency-key"]) ||
