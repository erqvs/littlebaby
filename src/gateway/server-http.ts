...
async function resolvePluginGatewayAuthBypassPaths(
  configSnapshot: LittleBabyConfig,
): Promise<Set<string>> {
...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-LittleBaby-Token header (query parameters are not allowed).",
      );
      return true;
    }
...
export function createGatewayHttpServer(opts: {
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      // Plugin routes run after core built-in gateway routes so core endpoints
      // keep precedence on overlapping paths.
      requestStages.push(
        ...
        wss.handleUpgrade(req, socket, head, (ws) => {
          (
            ws as unknown as import("ws").WebSocket & {
              __littlebabyPreauthBudgetClaimed?: boolean;
              __littlebabyPreauthBudgetKey?: string;
            }
          ).__littlebabyPreauthBudgetKey = preauthBudgetKey;
          ...
          const budgetClaimed = Boolean(
            ...
            ).__littlebabyPreauthBudgetClaimed,
          );
          if (budgetClaimed) {
            budgetTransferred = true;
