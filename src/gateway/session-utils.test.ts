...
function createSingleAgentAvatarConfig(workspace: string): LittleBabyConfig {
  return {
...
describe("gateway session utils", () => {
  ...
  test("listAgentsForGateway keeps explicit agents.list scope over disk-only agents (scope boundary)", async () => {
    await withStateDirEnv("littlebaby-agent-list-scope-", async ({ stateDir }) => {
...
describe("resolveSessionModelIdentityRef", () => {
  const resolveLegacyIdentityRef = (cfg: LittleBabyConfig, modelProvider?: string) =>
  ...
  test("infers provider from configured provider catalogs when allowlist is absent", () => {
    ...
    } as unknown as LittleBabyConfig["models"];
