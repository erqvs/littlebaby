...
const CODEX_HARNESS_LIVE = isTruthyEnvValue(process.env.LITTLEBABY_LIVE_CODEX_HARNESS);
const CODEX_HARNESS_DEBUG = isTruthyEnvValue(process.env.LITTLEBABY_LIVE_CODEX_HARNESS_DEBUG);
const CODEX_HARNESS_IMAGE_PROBE = isTruthyEnvValue(
  process.env.LITTLEBABY_LIVE_CODEX_HARNESS_IMAGE_PROBE,
);
const CODEX_HARNESS_MCP_PROBE = isTruthyEnvValue(process.env.LITTLEBABY_LIVE_CODEX_HARNESS_MCP_PROBE);
const CODEX_HARNESS_AUTH_MODE =
  process.env.LITTLEBABY_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "api-key" : "codex-auth";
...
function snapshotEnv(): EnvSnapshot {
  return {
    agentRuntime: process.env.LITTLEBABY_AGENT_RUNTIME,
    configPath: process.env.LITTLEBABY_CONFIG_PATH,
    gatewayToken: process.env.LITTLEBABY_GATEWAY_TOKEN,
...
function restoreEnv(snapshot: EnvSnapshot): void {
  restoreEnvVar("LITTLEBABY_AGENT_RUNTIME", snapshot.agentRuntime);
  restoreEnvVar("LITTLEBABY_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("LITTLEBABY_GATEWAY_TOKEN", snapshot.gatewayToken);
...
describeLive("gateway live (Codex harness)", () => {
  it(
    ...
    async () => {
      const modelKey = process.env.LITTLEBABY_LIVE_CODEX_HARNESS_MODEL ?? DEFAULT_CODEX_MODEL;
      ...
      process.env.LITTLEBABY_AGENT_RUNTIME = "codex";
      process.env.LITTLEBABY_AGENT_HARNESS_FALLBACK = "none";
