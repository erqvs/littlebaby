...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.LITTLEBABY_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: LittleBabyConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: LittleBabyConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: LittleBabyConfig, requestedAgentId: string) => {
