...
function shouldLogCliCronProbe(): boolean {
  return (
    isTruthyEnvValue(process.env.LITTLEBABY_LIVE_CLI_BACKEND_DEBUG) ||
    isTruthyEnvValue(process.env.LITTLEBABY_CLI_BACKEND_LOG_OUTPUT)
  );
}
...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.LITTLEBABY_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "LITTLEBABY_LIVE_CLI_BACKEND_ARGS",
      process.env.LITTLEBABY_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `LITTLEBABY_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "LITTLEBABY_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.LITTLEBABY_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.LITTLEBABY_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.LITTLEBABY_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.LITTLEBABY_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.LITTLEBABY_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.LITTLEBABY_TEST_MINIMAL_GATEWAY,
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("LITTLEBABY_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("LITTLEBABY_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("LITTLEBABY_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("LITTLEBABY_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("LITTLEBABY_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("LITTLEBABY_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("LITTLEBABY_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("LITTLEBABY_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("LITTLEBABY_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("LITTLEBABY_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("LITTLEBABY_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
...
export async function verifyCliBackendImageProbe(params: {
  ...
  const imageProbe = await params.client.request(
    "agent",
    {
      ...
      message:
        "Best match for the image: laptop, mouse, cat, horse. " +
