...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("littlebaby doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with LITTLEBABY_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with LITTLEBABY_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.LITTLEBABY_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://littlebaby-gateway.ai:18789",
      onConnectError,
    });
