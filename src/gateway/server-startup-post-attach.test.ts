...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/littlebaby-state",
    resolveConfigPath: vi.fn(() => "/tmp/littlebaby-state/littlebaby.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/littlebaby-state"),
  };
});
...
function createPostAttachParams(overrides: Partial<PostAttachParams> = {}): PostAttachParams {
  return {
    ...
    defaultWorkspaceDir: "/tmp/littlebaby-workspace",
    deps: {} as never,
