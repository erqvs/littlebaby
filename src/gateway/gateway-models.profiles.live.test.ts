...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.LITTLEBABY_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.LITTLEBABY_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.LITTLEBABY_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.LITTLEBABY_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.LITTLEBABY_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.LITTLEBABY_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["LITTLEBABY_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.LITTLEBABY_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.LITTLEBABY_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.LITTLEBABY_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.LITTLEBABY_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.LITTLEBABY_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.LITTLEBABY_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.LITTLEBABY_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.LITTLEBABY_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("LITTLEBABY_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("LITTLEBABY_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("LITTLEBABY_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.LITTLEBABY_LIVE_GATEWAY_MODELS;
    delete process.env.LITTLEBABY_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.LITTLEBABY_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.LITTLEBABY_LIVE_GATEWAY_MODELS = "openai/gpt-5.4,anthropic/claude-opus-4-6";
    delete process.env.LITTLEBABY_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.LITTLEBABY_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.LITTLEBABY_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.LITTLEBABY_CONFIG_PATH,
    token: process.env.LITTLEBABY_GATEWAY_TOKEN,
    skipChannels: process.env.LITTLEBABY_SKIP_CHANNELS,
    skipGmail: process.env.LITTLEBABY_SKIP_GMAIL_WATCHER,
    skipCron: process.env.LITTLEBABY_SKIP_CRON,
    skipCanvas: process.env.LITTLEBABY_SKIP_CANVAS_HOST,
    disableBonjour: process.env.LITTLEBABY_DISABLE_BONJOUR,
    logLevel: process.env.LITTLEBABY_LOG_LEVEL,
    agentDir: process.env.LITTLEBABY_AGENT_DIR,
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.LITTLEBABY_DISABLE_BONJOUR = "1";
    process.env.LITTLEBABY_LOG_LEVEL = "silent";
  }
  ...
  process.env.LITTLEBABY_AGENT_DIR = tempAgentDir;
  ...
  try {
    ...
    process.env.LITTLEBABY_CONFIG_PATH = previous.configPath;
    process.env.LITTLEBABY_GATEWAY_TOKEN = previous.token;
    process.env.LITTLEBABY_SKIP_CHANNELS = previous.skipChannels;
    process.env.LITTLEBABY_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.LITTLEBABY_SKIP_CRON = previous.skipCron;
    process.env.LITTLEBABY_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.LITTLEBABY_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.LITTLEBABY_LOG_LEVEL = previous.logLevel;
    process.env.LITTLEBABY_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.LITTLEBABY_STATE_DIR = previous.stateDir;
  }
}
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via LITTLEBABY_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
