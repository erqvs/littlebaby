...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.LITTLEBABY_CONFIG_PATH) {
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["LITTLEBABY_GATEWAY_TOKEN"]);
  const prev = process.env.LITTLEBABY_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.LITTLEBABY_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.LITTLEBABY_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
