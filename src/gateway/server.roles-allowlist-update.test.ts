...
const connectNodeClient = async (params: {
  ...
  const token = process.env.LITTLEBABY_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("LITTLEBABY_GATEWAY_TOKEN is required for node test clients");
  }
...
function getGatewayTestConfigPath(): string {
  ...
  if (!configPath) {
    throw new Error("LITTLEBABY_CONFIG_PATH is required in the gateway test environment");
  }
  return configPath;
}
