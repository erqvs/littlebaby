...
let lastCreateLittleBabyToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/littlebaby-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateLittleBabyToolsContext?.agentTo,
          agentThreadId: lastCreateLittleBabyToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createLittleBabyTools: (ctx: Record<string, unknown>) => {
      lastCreateLittleBabyToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateLittleBabyToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-littlebaby-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-littlebaby-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateLittleBabyToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateLittleBabyToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateLittleBabyToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateLittleBabyToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateLittleBabyToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateLittleBabyToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-littlebaby-message-to": "channel:24514",
        "x-littlebaby-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
