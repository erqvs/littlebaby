...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__littlebaby_unknown_client_ip__";

export function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.LITTLEBABY_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (env.VITEST && env.LITTLEBABY_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP);
  if (!configured) {
