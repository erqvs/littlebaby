...
export function formatConfigRecoveryNotice(params: {
  ...
  const configName = path.basename(params.configPath) || "littlebaby.json";
  return [
    `Config recovery warning: LittleBaby restored ${configName} from the last-known-good backup during ${params.phase} (${params.reason}).`,
