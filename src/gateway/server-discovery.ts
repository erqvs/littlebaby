...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/littlebaby/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (LittleBaby)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.LITTLEBABY_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "littlebaby");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.LITTLEBABY_TAILNET_DNS?.trim();
