...
import { attachLittleBabyTranscriptMeta, readSessionMessages } from "./session-utils.js";
...
export type SessionHistoryMessage = Record<string, unknown> & {
  __littlebaby?: SessionHistoryTranscriptMeta;
};
...
export function resolveMessageSeq(message: SessionHistoryMessage | undefined): number | undefined {
  const seq = message?.__littlebaby?.seq;
...
export class SessionHistorySseState {
  ...
  appendInlineMessage(update: {
    ...
    const nextMessage = attachLittleBabyTranscriptMeta(update.message, {
