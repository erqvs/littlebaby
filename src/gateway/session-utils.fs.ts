...
export function attachLittleBabyTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__littlebaby && typeof record.__littlebaby === "object" && !Array.isArray(record.__littlebaby)
      ? (record.__littlebaby as Record<string, unknown>)
      : {};
  return {
    ...record,
    __littlebaby: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachLittleBabyTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "littlebaby" && model === "delivery-mirror";
