...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.LITTLEBABY_SKIP_CHANNELS;
    prevSkipGmail = process.env.LITTLEBABY_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.LITTLEBABY_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.LITTLEBABY_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.LITTLEBABY_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.LITTLEBABY_SKIP_GMAIL_WATCHER;
    } else {
      process.env.LITTLEBABY_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.LITTLEBABY_SKIP_PROVIDERS;
    } else {
      process.env.LITTLEBABY_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    const configPath = process.env.LITTLEBABY_CONFIG_PATH;
    if (!configPath) {
      throw new Error("LITTLEBABY_CONFIG_PATH is not set");
    }
  ...
  async function withNonMinimalGatewayServer(
    ...
    return await withEnvAsync({ LITTLEBABY_TEST_MINIMAL_GATEWAY: undefined }, async () =>
...
`,
    ...
    const previousGatewayTokenEnv = process.env.LITTLEBABY_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.LITTLEBABY_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
