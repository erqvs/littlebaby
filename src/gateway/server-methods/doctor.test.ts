...
const loadConfig = vi.hoisted(() => vi.fn(() => ({}) as LittleBabyConfig));
const resolveDefaultAgentId = vi.hoisted(() => vi.fn(() => "main"));
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg: LittleBabyConfig, _agentId: string) => "/tmp/littlebaby"),
);
const resolveMemorySearchConfig = vi.hoisted(() =>
  vi.fn<(_cfg: LittleBabyConfig, _agentId: string) => { enabled: boolean } | null>(() => ({
    enabled: true,
  })),
);
...
describe("doctor.memory.status", () => {
  beforeEach(() => {
    ...
    resolveAgentWorkspaceDir.mockReset().mockReturnValue("/tmp/littlebaby");
  ...
  it("includes dreaming counts and managed cron status when workspace data is available", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((cfg: LittleBabyConfig, agentId: string) => {
      if (agentId === "alpha") {
    ...
    const cronList = vi.fn(async () => [
      {
        ...
        payload: {
          kind: "systemEvent",
          text: "__littlebaby_memory_core_short_term_promotion_dream__",
        },
  ...
  it("reads dreaming config from the selected memory slot plugin", async () => {
    loadConfig.mockReturnValue({
      plugins: {
        slots: {
          memory: "memos-local-littlebaby-plugin",
        },
        entries: {
          "memos-local-littlebaby-plugin": {
            config: {
              dreaming: {
                enabled: true,
                frequency: "0 */4 * * *",
  ...
  it("merges workspace store errors when multiple workspace stores are unreadable", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg: LittleBabyConfig, agentId: string) =>
...
describe("doctor.memory dream actions", () => {
  it("clears grounded-only staged short-term entries without touching the diary", async () => {
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/littlebaby");
    removeGroundedShortTermCandidates.mockResolvedValue({
      removed: 3,
      storePath: "/tmp/littlebaby/memory/.dreams/short-term-recall.json",
    });
    const respond = vi.fn();
  ...
  it("repairs contaminated dreaming artifacts for control-ui callers", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValue({
      changed: true,
      archiveDir: "/tmp/littlebaby/.littlebaby-repair/dreaming/2026-04-11T22-00-00-000Z",
  ...
  it("dedupes exact dream diary duplicates for control-ui callers", async () => {
    ...
    dedupeDreamDiaryEntries.mockResolvedValue({
      dreamsPath: "/tmp/littlebaby/DREAMS.md",
      removed: 2,
      kept: 7,
    });
    const respond = vi.fn();
