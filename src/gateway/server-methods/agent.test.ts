...
describe("gateway agent handler", () => {
  afterEach(() => {
    if (ORIGINAL_STATE_DIR === undefined) {
      ...
      process.env.LITTLEBABY_STATE_DIR = ORIGINAL_STATE_DIR;
    }
    resetDetachedTaskLifecycleRuntimeForTests();
  ...
  it("does not create task rows for inter-session completion wakes", async () => {
    ...
    await invokeAgent(
      {
        message: [
          "[Mon 2026-04-06 02:42 GMT+1] <<<BEGIN_LITTLEBABY_INTERNAL_CONTEXT>>>",
  ...
  it("tracks async gateway agent runs in the shared task registry", async () => {
    await withTempDir({ prefix: "littlebaby-gateway-agent-task-" }, async (root) => {
  ...
  it("dispatches async gateway agent task creation through the detached task runtime seam", async () => {
    await withTempDir({ prefix: "littlebaby-gateway-agent-seam-" }, async (root) => {
      process.env.LITTLEBABY_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("prepends runtime-loaded startup memory to bare /new agent runs", async () => {
    await withTempDir({ prefix: "littlebaby-gateway-reset-startup-" }, async (workspaceDir) => {
  ...
  it("uses shared bootstrap reset wording for bare /new when workspace bootstrap is pending", async () => {
    await withTempDir({ prefix: "littlebaby-gateway-reset-bootstrap-" }, async (workspaceDir) => {
  ...
  it("resolves bare /new bootstrap state from the effective spawned workspace", async () => {
    await withTempDir(
      { prefix: "littlebaby-gateway-reset-default-" },
      async (defaultWorkspaceDir) => {
        await withTempDir(
          { prefix: "littlebaby-gateway-reset-spawned-" },
  ...
  it("suppresses full bootstrap wording for bare /new on subagent sessions", async () => {
    await withTempDir({ prefix: "littlebaby-gateway-reset-subagent-" }, async (workspaceDir) => {
      await fs.writeFile(`${workspaceDir}/BOOTSTRAP.md`, "bootstrap ritual", "utf-8");
      mocks.loadConfigReturn = {
        agents: {
          defaults: {
  ...
  it("uses request model override when resolving bare /new bootstrap file access", async () => {
    await withTempDir(
      { prefix: "littlebaby-gateway-reset-model-override-" },
      async (workspaceDir) => {
        await fs.writeFile(`${workspaceDir}/BOOTSTRAP.md`, "bootstrap ritual", "utf-8");
        mocks.loadConfigReturn = {
          agents: {
