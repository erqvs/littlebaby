  ...
  try {
    ...
    if (registration.transport === "relay") {
      ...
      result = await sendApnsAlert({
        registration,
        nodeId,
        title: "LittleBaby needs a quick reopen",
        body: "Tap to reopen LittleBaby and restore the node connection.",
        relayConfig: relay.relayConfig,
      });
    } else {
      const auth = await resolveDirectNodePushConfig();
...
export const nodeHandlers: GatewayRequestHandlers = {
  ...
  "node.canvas.capability.refresh": async ({ params, respond }) => {
    if (!validateNodeListParams(params)) {
      respondInvalidParams({
        respond,
    ...
    respond(
      false,
      undefined,
      errorShape(ErrorCodes.UNAVAILABLE, "node canvas capability is unavailable"),
    );
  },
