...
vi.mock("../config/config.js", () => ({
  applyConfigOverrides: vi.fn((config: LittleBabyConfig) => config),
  isNixMode: false,
...
const configPath = "/tmp/littlebaby-startup-recovery.json";
const validConfig = {
  gateway: {
    mode: "local",
  },
...
describe("gateway startup config recovery", () => {
  ...
  it("keeps startup validation loud when last-known-good recovery is unavailable", async () => {
    ...
    await expect(
      ...
      `Invalid config at ${configPath}.\ngateway.mode: Expected 'local' or 'remote'\nRun "littlebaby doctor --fix" to repair, then retry.`,
    );
