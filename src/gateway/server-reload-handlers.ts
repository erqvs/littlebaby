...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: LittleBabyConfig) => ChannelHealthMonitor | null;
};
...
> & {
  ...
  initialConfig: LittleBabyConfig;
...
export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: LittleBabyConfig) => {
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (LITTLEBABY_SKIP_GMAIL_WATCHER=1)"),
      });
    }

    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.LITTLEBABY_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.LITTLEBABY_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (LITTLEBABY_SKIP_CHANNELS=1 or LITTLEBABY_SKIP_PROVIDERS=1)",
        );
      } else {
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: LittleBabyConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
