...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.littlebabyCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.littlebabyCanvasA2UIAction &&
        typeof window.littlebabyCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.littlebabySendUserAction === "function";
  ...
  window.addEventListener("littlebaby:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android LittleBaby node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.littlebabySendUserAction === "function"
        ? window.littlebabySendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.LITTLEBABY_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.LITTLEBABY_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>LittleBaby Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
