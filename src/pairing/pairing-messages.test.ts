...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["LITTLEBABY_CONTAINER_HINT", "LITTLEBABY_PROFILE"]);
    delete process.env.LITTLEBABY_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:littlebaby|littlebaby) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
