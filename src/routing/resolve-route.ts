...
function listAgents(cfg: LittleBabyConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: LittleBabyConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<LittleBabyConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: LittleBabyConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: LittleBabyConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: LittleBabyConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<LittleBabyConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  LittleBabyConfig,
  {
    bindingsRef: LittleBabyConfig["bindings"];
    agentsRef: LittleBabyConfig["agents"];
    sessionRef: LittleBabyConfig["session"];
...
function resolveRouteCacheForConfig(cfg: LittleBabyConfig): Map<string, ResolvedAgentRoute> {
