...
beforeAll(() => {
  sharedFixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "littlebaby-run-plan-fixtures-"));
...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `littlebaby-${params.binName}-bin-`,
    run: params.run,
  });
}
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "littlebaby-node-import-inline-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "littlebaby-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "littlebaby-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "littlebaby-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "littlebaby-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "littlebaby-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "littlebaby-inline-shell-node-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "littlebaby-pnpm-dlx-unknown-flag-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "littlebaby-pnpm-dlx-unknown-prefix-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "littlebaby-pnpm-dlx-unknown-prefix-value-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "littlebaby-pnpm-dlx-global-double-dash-",
    command: ["pnpm", "--", "dlx", "--future-flag", "tsx", "./run.ts"],
    setup: (tmp) => {
      fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE")\n');
    },
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32").each(cases)("$name", (testCase) => {
    const tmp = createFixtureDir("littlebaby-approval-hardening-");
    const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "littlebaby-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("keeps fail-closed behavior for relative native-binary shell payloads", () => {
    ...
    const tmp = createFixtureDir("littlebaby-shell-relative-binary-binding-");
    const binaryPath = resolveNativeBinaryFixturePath();
  ...
  it("keeps fail-closed behavior for writable absolute native-binary shell payloads", () => {
    ...
    const tmp = createFixtureDir("littlebaby-shell-absolute-binary-binding-");
    const binaryPath = resolveNativeBinaryFixturePath();
  ...
  it("keeps fail-closed behavior for owner-controlled read-only absolute binaries", () => {
    ...
    const tmp = createFixtureDir("littlebaby-shell-owned-readonly-binding-");
  ...
  it("keeps fail-closed behavior for symlinked binaries with writable targets", () => {
    ...
    const tmp = createFixtureDir("littlebaby-shell-symlink-binary-binding-");
  ...
  it("keeps fail-closed behavior for mutable or ambiguous shell payload files", () => {
    for (const testCase of [
      {
        tmpPrefix: "littlebaby-shell-script-binding-",
        fileName: "run.sh",
        body: "#!/bin/sh\necho SAFE\n",
      },
      {
        tmpPrefix: "littlebaby-shell-empty-binding-",
        fileName: "empty",
        body: "",
      },
      {
        tmpPrefix: "littlebaby-shell-mz-text-binding-",
        fileName: "mz-script",
        body: "MZ not really a PE file\n",
      },
      {
        tmpPrefix: "littlebaby-shell-nul-header-binding-",
        fileName: "nul-script",
  ...
  it("keeps fail-closed behavior when the shell payload probe stops seeing a file", () => {
    ...
    const tmp = createFixtureDir("littlebaby-shell-race-binding-");
  ...
  it("detects rewritten script operands for pnpm dlx approval plans", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "littlebaby-pnpm-dlx-approval-",
            fixture: {
  ...
  it("does not bind pnpm dlx shell-mode commands to a mutable file operand", () => {
    withFakeRuntimeBins({
      binNames: ["pnpm", "tsx"],
      run: () => {
        const tmp = createFixtureDir("littlebaby-pnpm-dlx-shell-mode-");
        fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE");\n');
        expect(
          resolveMutableFileOperandSnapshotSync({
  ...
  it("allows pnpm dlx package binaries that do not bind mutable local files", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        const cases = [
          {
            prefix: "littlebaby-pnpm-dlx-package-bin-",
            command: ["pnpm", "dlx", "cowsay", "hello"],
          },
          {
            prefix: "littlebaby-pnpm-dlx-package-runtime-token-",
            command: ["pnpm", "dlx", "cowsay", "node"],
          },
          {
            prefix: "littlebaby-pnpm-dlx-package-runtime-token-multi-",
            command: ["pnpm", "dlx", "cowsay", "node", "hello"],
          },
          {
            prefix: "littlebaby-pnpm-dlx-package-file-",
          ...
          {
            prefix: "littlebaby-pnpm-dlx-package-data-tail-",
  ...
  it("treats -- as the end of pnpm dlx option parsing", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "littlebaby-pnpm-dlx-double-dash-",
            fixture: {
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = createFixtureDir("littlebaby-shell-option-value-");
    const scriptPath = path.join(tmp, "run.sh");
    fs.writeFileSync(scriptPath, "#!/bin/sh\necho SAFE\n");
