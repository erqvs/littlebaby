...
function createRemoteGatewayTokenRefConfig(tokenId: string): LittleBabyConfig {
  return {
    secrets: {
      providers: {
        default: { source: "env" },
...
describe("resolveNodeHostGatewayCredentials", () => {
  it("does not inherit gateway.remote token in local mode", async () => {
    ...
    await expectNoGatewayCredentials(config, {
      ...
      LITTLEBABY_GATEWAY_PASSWORD: undefined,
    });
  });
  ...
  it("prefers LITTLEBABY_GATEWAY_TOKEN over configured refs", async () => {
    const config = createRemoteGatewayTokenRefConfig("REMOTE_GATEWAY_TOKEN");
