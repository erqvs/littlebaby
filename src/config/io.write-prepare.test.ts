...
describe("config io write prepare", () => {
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("littlebaby config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('littlebaby config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/littlebaby.json",
        expectedConfigPath: "/tmp/littlebaby.json",
  ...
  it("keeps plugin AJV defaults out of the persisted candidate", () => {
    ...
    const nextConfig: LittleBabyConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("does not preserve include-only $schema into the root persisted candidate", () => {
    const sourceConfig = {
      $schema: "https://littlebaby.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
