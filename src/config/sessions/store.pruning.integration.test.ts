...
// Keep integration tests deterministic: never read a real littlebaby.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "littlebaby-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.LITTLEBABY_SESSION_CACHE_TTL_MS;
    process.env.LITTLEBABY_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.LITTLEBABY_SESSION_CACHE_TTL_MS;
    } else {
      process.env.LITTLEBABY_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
