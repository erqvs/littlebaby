...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): LittleBabyConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: LittleBabyConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        LITTLEBABY_STATE_DIR: envStateDir,
      };
      const cfg: LittleBabyConfig = {};
