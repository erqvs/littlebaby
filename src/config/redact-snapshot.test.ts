...
import type { ConfigFileSnapshot } from "./types.littlebaby.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/littlebaby/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/littlebaby/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/littlebaby/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/littlebaby/nickserv-password.txt");
