...
import type { LittleBabyConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.littlebaby.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: LittleBabyConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: LittleBabyConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
...
): { ok: true; config: LittleBabyConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = LittleBabySchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as LittleBabyConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: LittleBabyConfig | null | undefined;
  ...
  const ensureCompatConfig = (): LittleBabyConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native LittleBaby config schema.
