...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("littlebaby-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "littlebaby-sandbox-browser-global" },
      agentBrowser: { network: "littlebaby-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("littlebaby-sandbox-browser-agent");
  });
