...
function collectConfigEnvVarsByTarget(cfg?: LittleBabyConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: LittleBabyConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: LittleBabyConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: LittleBabyConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.LITTLEBABY_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
