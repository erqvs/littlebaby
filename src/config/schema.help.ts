...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by LittleBaby to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when LittleBaby writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for LittleBaby. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
    "Legacy guided setup state fields retained only for observability across upgrades.",
  "wizard.lastRunAt":
    "ISO timestamp for when guided setup most recently completed on this host.",
  "wizard.lastRunVersion":
    "LittleBaby version recorded at the time of the most recent guided setup run on this config.",
  "wizard.lastRunCommit":
    "Source commit identifier recorded for the last guided setup run in development builds.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping LittleBaby runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "agents.list[].runtime":
    "Optional runtime descriptor for this agent. Use embedded for default LittleBaby execution or acp for external ACP harness defaults.",
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default LittleBaby runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this LittleBaby agent (for example codex, claude, cursor, gemini, littlebaby).",
  ...
  "discovery.wideArea.domain":
    "Optional unicast DNS-SD domain for wide-area discovery, such as littlebaby.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
  "discovery.mdns":
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: littlebaby-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `littlebaby security audit`.",
  ...
  "nodeHost.browserProxy.allowProfiles":
    "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, LittleBaby enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
  media:
  ...
  "bindings[].match.channel":
    "Channel/provider identifier this binding applies to, such as `feishu` or a plugin channel ID. Use the configured channel key exactly so binding evaluation works reliably.",
  ...
  "bindings[].match.guildId":
    "Optional server/workspace ID constraint for binding evaluation in multi-workspace deployments. Use this when the same peer identifiers can appear across different workspaces.",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $LITTLEBABY_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "approvals.exec.targets[].channel":
    "Channel/provider ID used for forwarded approval delivery, such as feishu or a plugin channel id. Use valid channel IDs only so approvals do not silently fail due to unknown routes.",
  ...
  "approvals.plugin.targets[].channel":
    "Channel/provider ID used for forwarded plugin approval delivery, such as feishu or a plugin channel id.",
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether LittleBaby injects `options.num_ctx` for local OpenAI-compatible providers. Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "agents.defaults.experimental.localModelLean":
    "Experimental local-model prompt trim. When enabled, LittleBaby drops heavyweight default tools like browser, cron, and message for weaker or smaller local-model backends.",
  ...
  "agents.defaults.memorySearch.provider":
    'Selects the embedding backend used to build/query memory vectors: "openai", "gemini", "voyage", "mistral", "bedrock", "lmstudio", or "local". Keep your most reliable provider here and configure fallback for resilience.',
  ...
  "agents.defaults.memorySearch.fallback":
    'Backup provider used when primary embeddings fail: "openai", "gemini", "voyage", "mistral", "bedrock", "lmstudio", "local", or "none". Set a real fallback for production reliability; use "none" only if you prefer explicit failures.',
  "agents.defaults.memorySearch.store.path":
    "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.littlebaby/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
  "agents.defaults.memorySearch.store.vector.enabled":
    "Enables vector similarity queries in memory search (default: true). Keep this enabled for normal semantic recall; disable only for debugging or fallback-only operation.",
  "agents.defaults.memorySearch.store.vector.extensionPath":
    "Overrides the auto-discovered vector extension library path (`.dylib`, `.so`, or `.dll`). Use this when your runtime cannot find the extension automatically or you pin a known-good build.",
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses LittleBaby memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
  "memory.citations":
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `littlebaby plugins update` to locate install sources).",
  ...
  "agents.defaults.embeddedPi":
    "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in LittleBaby sessions.",
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage LittleBaby MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  "commands.ownerAllowFrom":
    "Explicit owner allowlist for owner-only tools/commands. Use channel-native IDs (optionally prefixed like \"feishu:ou_xxx\"). '*' is ignored.",
  ...
  mcp: "Global MCP server definitions managed by LittleBaby. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. LittleBaby stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  session:
  ...
  "session.identityLinks":
    "Maps canonical identities to provider-prefixed peer IDs so equivalent users resolve to one DM thread (example: feishu:ou_xxx). Use this when the same human appears across multiple channels or accounts.",
  ...
  "session.parentForkMaxTokens":
    "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, LittleBaby starts a fresh thread session instead of forking; set 0 to disable this protection.",
  "session.mainKey":
  ...
  "session.sendPolicy.rules[].match.channel":
    "Matches rule application to a specific channel/provider id (for example feishu). Use this when one channel should permit or deny delivery independently of others.",
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in LittleBaby. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
  ...
  "hooks.mappings[].channel":
    'Delivery channel override for mapping outputs (for example "last" or "feishu"). Keep channel overrides explicit to avoid accidental cross-channel sends.',
  "hooks.mappings[].to":
  ...
  "messages.queue.byChannel":
    "Per-channel queue mode overrides keyed by provider id (for example feishu). Use this when one channel’s traffic pattern needs different queue behavior than global defaults.",
