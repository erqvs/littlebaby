...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    const schema = LittleBabySchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "LittleBabyConfig";
    const hints = mapSensitivePaths(LittleBabySchema, "", {});
...
describe("collectMatchingSchemaPaths", () => {
  it("finds base-config URL fields that may embed secrets", () => {
    const paths = collectMatchingSchemaPaths(LittleBabySchema, "", isSensitiveUrlConfigPath);
