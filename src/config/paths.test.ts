...
describe("oauth paths", () => {
  it("prefers LITTLEBABY_OAUTH_DIR over LITTLEBABY_STATE_DIR", () => {
    const env = {
      LITTLEBABY_OAUTH_DIR: "/custom/oauth",
      LITTLEBABY_STATE_DIR: "/custom/state",
  ...
  it("derives oauth path from LITTLEBABY_STATE_DIR when unset", () => {
    const env = {
      LITTLEBABY_STATE_DIR: "/custom/state",
    } as NodeJS.ProcessEnv;
...
describe("gateway port resolution", () => {
  it("prefers numeric env values over config", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19002 } }, envWith({ LITTLEBABY_GATEWAY_PORT: "19001" })),
    ).toBe(19001);
  });
  ...
  it("accepts Compose-style IPv4 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ LITTLEBABY_GATEWAY_PORT: "127.0.0.1:18789" }),
      ),
    ).toBe(18789);
  });
  ...
  it("accepts Compose-style IPv6 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ LITTLEBABY_GATEWAY_PORT: "[::1]:28789" }),
      ),
    ).toBe(28789);
  });
  ...
  it("falls back to config when the Compose-style suffix is invalid", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19003 } },
        envWith({ LITTLEBABY_GATEWAY_PORT: "127.0.0.1:not-a-port" }),
      ),
    ).toBe(19003);
  });
  ...
  it("falls back when malformed IPv6 inputs do not provide an explicit port", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19003 } }, envWith({ LITTLEBABY_GATEWAY_PORT: "::1" })),
    ).toBe(19003);
    expect(resolveGatewayPort({}, envWith({ LITTLEBABY_GATEWAY_PORT: "2001:db8::1" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });

  it("falls back to the default port when env is invalid and config is unset", () => {
    expect(resolveGatewayPort({}, envWith({ LITTLEBABY_GATEWAY_PORT: "127.0.0.1:not-a-port" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });
});
...
describe("state + config path candidates", () => {
  ...
  it("uses LITTLEBABY_STATE_DIR when set", () => {
    const env = {
      LITTLEBABY_STATE_DIR: "/new/state",
    } as NodeJS.ProcessEnv;
