...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.littlebaby) || !isRecord(entry.littlebaby.channel)) {
      continue;
    }
    const channel = entry.littlebaby.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: LittleBabyConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: LittleBabyConfig, pluginId: string) => boolean;
