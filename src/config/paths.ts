...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.LITTLEBABY_NIX_MODE === "1";
}

export const isNixMode = resolveIsNixMode();

// Support the remaining legacy pre-rebrand state dir.
const LEGACY_STATE_DIRNAMES = [".littlebaby"] as const;
const NEW_STATE_DIRNAME = ".littlebaby";
const CONFIG_FILENAME = "littlebaby.json";
const LEGACY_CONFIG_FILENAMES = ["littlebaby.json"] as const;

function resolveDefaultHomeDir(): string {
  return resolveRequiredHomeDir(process.env, os.homedir);
}

/** Build a homedir thunk that respects LITTLEBABY_HOME for the given env. */
 ...
 * Can be overridden via LITTLEBABY_STATE_DIR.
 * Default: ~/.littlebaby
 */
export function resolveStateDir(
  ...
  const override = env.LITTLEBABY_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, effectiveHomedir);
  }
  const newDir = newStateDir(effectiveHomedir);
  if (env.LITTLEBABY_TEST_FAST === "1") {
    return newDir;
  }
...
export function resolveCanonicalConfigPath(
  ...
  const override = env.LITTLEBABY_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.LITTLEBABY_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.LITTLEBABY_CONFIG_PATH?.trim();
  if (explicit) {
 ...
 *
 * Precedence:
 * - `LITTLEBABY_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.LITTLEBABY_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
