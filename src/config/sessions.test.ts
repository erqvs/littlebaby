...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ LITTLEBABY_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-littlebaby",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-littlebaby#general");
  });
  ...
  it("derives session transcripts dir from LITTLEBABY_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { LITTLEBABY_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when LITTLEBABY_STATE_DIR differs from stored paths", () => {
