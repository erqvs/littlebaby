...
export function parseLittleBabyVersion(raw: string | null | undefined): LittleBabyVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeLittleBabyVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseLittleBabyVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameLittleBabyStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseLittleBabyVersion(a);
  const parsedB = parseLittleBabyVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareLittleBabyVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseLittleBabyVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseLittleBabyVersion(current);
  const parsedTouched = parseLittleBabyVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameLittleBabyStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareLittleBabyVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: LittleBabyVersion): number {
