...
const ETC_LITTLEBABY_DIR = path.join(ROOT_DIR, "etc", "littlebaby");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "littlebaby.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcLittleBabyPath(...parts: string[]) {
  return path.join(ETC_LITTLEBABY_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcLittleBabyPath("agents.json");
    ...
    expectResolveIncludeError(
      () =>
        resolve(
          ...
          configPath("sub", "littlebaby.json"),
        ),
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      await withTempDir({ prefix: "littlebaby-includes-symlink-" }, async (tempRoot) => {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "littlebaby.json"),
        );
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      await withTempDir({ prefix: "littlebaby-includes-hardlink-" }, async (tempRoot) => {
    ...
    it("rejects oversized include files", async () => {
      await withTempDir({ prefix: "littlebaby-includes-big-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "littlebaby.json")),
