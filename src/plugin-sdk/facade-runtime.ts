...
const LITTLEBABY_SOURCE_EXTENSIONS_ROOT = path.resolve(LITTLEBABY_PACKAGE_ROOT, "extensions");
...
function resolveFacadeBoundaryRoot(modulePath: string, bundledPluginsDir: string | undefined) {
  if (!bundledPluginsDir) {
    return LITTLEBABY_PACKAGE_ROOT;
  }
...
function resolveFacadeModuleLocationUncached(params: {
  ...
  if (preferSource) {
    const modulePath =
      resolveBundledPluginSourcePublicSurfacePath({
        ...params,
        sourceRoot: bundledPluginsDir ?? path.resolve(LITTLEBABY_PACKAGE_ROOT, "extensions"),
      }) ??
      resolveBundledPluginPublicSurfacePath({
        rootDir: LITTLEBABY_PACKAGE_ROOT,
        env: params.env,
...
function buildFacadeActivationCheckParams(
  ...
  return {
    ...params,
    location,
    sourceExtensionsRoot: LITTLEBABY_SOURCE_EXTENSIONS_ROOT,
    resolutionKey: createFacadeResolutionKey(params),
  };
}
