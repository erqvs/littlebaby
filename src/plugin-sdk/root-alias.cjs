...
const pluginSdkPackageNames = ["littlebaby/plugin-sdk", "@littlebaby/plugin-sdk"];
...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.LITTLEBABY_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function onDiagnosticEvent(listener) {
  ...
  if (!diagnosticEvents || typeof diagnosticEvents.onDiagnosticEvent !== "function") {
    throw new Error("littlebaby/plugin-sdk root alias could not resolve onDiagnosticEvent");
  }
...
function listPluginSdkExportedSubpaths() {
  ...
  const cacheKey = `${packageRoot}::privateQa=${process.env.LITTLEBABY_ENABLE_PRIVATE_QA_CLI === "1" ? "1" : "0"}`;
...
function listPrivateLocalOnlyPluginSdkSubpaths() {
  if (process.env.LITTLEBABY_ENABLE_PRIVATE_QA_CLI !== "1") {
    return [];
  }
  try {
...
function getJiti(tryNative) {
  ...
  const jitiLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled LittleBaby core graph.
...
function loadMonolithicSdk() {
  ...
  monolithicSdk = getJiti(false)(path.join(getPackageRoot(), "src", "plugin-sdk", "index.ts"));
  return monolithicSdk;
}
