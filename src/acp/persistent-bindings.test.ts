...
type ConfiguredBinding = NonNullable<LittleBabyConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<LittleBabyConfig>,
): LittleBabyConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: LittleBabyConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/littlebaby" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
