...
const createTempDir = () => tempDirs.make("littlebaby-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets LITTLEBABY_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "littlebaby",
    });

    expect(env.LITTLEBABY_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("littlebaby");
  });

  it("overrides pre-existing LITTLEBABY_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      LITTLEBABY_SHELL: "wrong",
    });
    expect(env.LITTLEBABY_SHELL).toBe("acp-client");
  });
  ...
  it("preserves LITTLEBABY_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        LITTLEBABY_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["LITTLEBABY_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default LittleBaby bridge", () => {
    ...
    expect(env.LITTLEBABY_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.LITTLEBABY_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "littlebaby",
        serverArgs: ["acp"],
        defaultServerCommand: "littlebaby",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("LITTLEBABY_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "littlebaby", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "littlebaby", "dist", "entry.js");
    const shimPath = path.join(dir, "littlebaby.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\littlebaby\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/littlebaby-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/littlebaby-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/littlebaby-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
