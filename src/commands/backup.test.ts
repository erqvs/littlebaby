...
describe("backup commands", () => {
  ...
  async function mockWorkspaceBackupPlan(stateDir: string, workspaceDir: string, nowMs: number) {
    vi.spyOn(backupShared, "resolveBackupPlanFromDisk").mockResolvedValue(
      await resolveBackupPlanFromPaths({
        stateDir,
        configPath: path.join(stateDir, "littlebaby.json"),
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("littlebaby-backup-test-");
  });
  ...
  async function withInvalidWorkspaceBackupConfig<T>(fn: (runtime: RuntimeEnv) => Promise<T>) {
    const stateDir = path.join(tempHome.home, ".littlebaby");
    const configPath = path.join(tempHome.home, "custom-config.json");
    process.env.LITTLEBABY_CONFIG_PATH = configPath;
    await fs.writeFile(path.join(stateDir, "littlebaby.json"), JSON.stringify({}), "utf8");
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".littlebaby");
    const workspaceDir = path.join(stateDir, "workspace");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-workspace-link-"));
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    const stateDir = path.join(tempHome.home, ".littlebaby");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-backups-"));
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".littlebaby");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-backup-link-"));
  ...
  it("falls back to the home directory when cwd is inside a backed-up source tree", async () => {
    ...
    if (process.platform !== "win32") {
      const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-backup-cwd-link-"));
