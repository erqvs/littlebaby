...
export function resolveProviderAuthLoginCommand(params: {
  ...
  return formatCliCommand(`littlebaby models auth login --provider ${provider.id}`);
}
...
export function buildProviderAuthRecoveryHint(params: {
  ...
  if (params.includeConfigure !== false) {
    parts.push(`\`${formatCliCommand("littlebaby configure")}\``);
  }
  if (params.includeEnvVar) {
    parts.push("set an API key env var");
  }
  if (parts.length === 0) {
    return `Run \`${formatCliCommand("littlebaby configure")}\`.`;
  }
  if (parts.length === 1) {
    return `${parts[0]}.`;
  }
