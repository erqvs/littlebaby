...
const ensureLittleBabyModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveLittleBabyAgentDir = vi.fn().mockReturnValue("/tmp/littlebaby-agent");
...
beforeEach(() => {
  ...
  ensureLittleBabyModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureLittleBabyModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the source snapshot config when provided", async () => {
    ...
    expect(ensureLittleBabyModelsJson).toHaveBeenCalled();
    expect(ensureLittleBabyModelsJson.mock.calls[0]?.[0]).toEqual(sourceConfig);
  });
