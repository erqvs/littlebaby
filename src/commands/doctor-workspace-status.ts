...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("littlebaby tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("littlebaby tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: LittleBabyConfig) {
