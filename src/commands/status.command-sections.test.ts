...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:littlebaby security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:littlebaby security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:littlebaby status --all",
      "  Need to debug live? cmd:littlebaby logs --follow",
      "  Fix reachability first: cmd:littlebaby gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:littlebaby devices approve req-123)",
      "muted(Fallback: cmd:littlebaby devices approve --latest)",
      "muted(Inspect: cmd:littlebaby devices list)",
    ]);
  });
