...
const SEARCH_TOOL = "https://docs.littlebaby.ai/mcp.SearchLittleBaby";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.littlebaby.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('littlebaby docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.littlebaby.ai/");
      runtime.log(`Search: ${formatCliCommand('littlebaby docs "your query"')}`);
    }
    return;
  }
