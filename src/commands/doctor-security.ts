...
import type { LittleBabyConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: LittleBabyConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: LittleBabyConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "LittleBaby default (full)";
  const defaultRequestedAskSource = "LittleBaby default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("littlebaby approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: LittleBabyConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: LittleBabyConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("littlebaby security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.littlebaby/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("littlebaby approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("littlebaby configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("littlebaby config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("littlebaby doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("littlebaby config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('littlebaby config set session.dmScope "per-channel-peer"') +
