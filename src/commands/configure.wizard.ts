...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.LITTLEBABY_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.LITTLEBABY_GATEWAY_PASSWORD ?? configuredPassword;
...
export async function runConfigureWizard(
  ...
  try {
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.LITTLEBABY_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.LITTLEBABY_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
