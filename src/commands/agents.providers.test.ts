...
describe("buildProviderStatusIndex", () => {
  ...
  it("prefers inspectAccount for read-only status surfaces", async () => {
    ...
    const map = await buildProviderStatusIndex({} as LittleBabyConfig);
  ...
  it("records accounts that throw during read-only resolution as not configured", async () => {
    ...
    await expect(buildProviderStatusIndex({} as LittleBabyConfig)).resolves.toEqual(
      new Map([
        [
          "telegram:default",
          {
  ...
  it("rethrows unexpected read-only account resolution errors", async () => {
    ...
    await expect(buildProviderStatusIndex({} as LittleBabyConfig)).rejects.toThrow("plugin crash");
  });
});
