...
function createTelegramCfg(botToken: string, enabled?: boolean): LittleBabyConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function createMSTeamsCatalogEntry(): ChannelPluginCatalogEntry {
  return {
    id: "external-chat",
    pluginId: "@littlebaby/external-chat-plugin",
    meta: {
      id: "external-chat",
      label: "External Chat",
    ...
    install: {
      npmSpec: "@littlebaby/external-chat",
    },
  };
}
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveConfigured: ({ cfg }: { cfg: LittleBabyConfig }) =>
            ...
            credentials: [
              {
                ...
                inspect: ({ cfg }: { cfg: LittleBabyConfig }) => ({
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveSelectionHint: async ({ cfg }: { cfg: LittleBabyConfig }) =>
            ...
            textInputs: [
              {
                ...
                required: true,
                applySet: ({ cfg, value }: { cfg: LittleBabyConfig; value: string }) =>
                  ({
                    ...cfg,
                    channels: {
                      ...cfg.channels,
...
function patchTelegramAdapter(overrides: ChannelSetupWizardAdapterPatch) {
  ...
  const patch = {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: LittleBabyConfig }) => ({
        channel: "telegram",
...
async function expectQuickstartPickerSkipsWithoutRuntime() {
  ...
  await expect(
    runSetupChannels({} as LittleBabyConfig, prompter, {
      quickstartDefaults: true,
    }),
  ).resolves.toEqual({} as LittleBabyConfig);
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as LittleBabyConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("../commands/channel-setup/plugin-install.js", async () => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: LittleBabyConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    await runSetupChannels({} as LittleBabyConfig, prompter, {
      quickstartDefaults: true,
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@littlebaby/external-chat-plugin": { enabled: true },
          },
        },
      } as LittleBabyConfig,
      prompter,
  ...
  it("treats installed external plugin channels as installed without reinstall prompts", async () => {
    ...
    manifestRegistryMocks.loadPluginManifestRegistry.mockReturnValue({
      plugins: [
        {
          id: "@littlebaby/external-chat-plugin",
        ...
        if (channel === "external-chat") {
          registry.channels.push({
            ...
            plugin: {
              ...
              config: {
                listAccountIds: (cfg: LittleBabyConfig) =>
                  Object.keys(
                    (
                      cfg.channels?.["external-chat"] as
                        | { accounts?: Record<string, unknown> }
                ...
                resolveAccount: (cfg: LittleBabyConfig, accountId: string) =>
                  (
                    cfg.channels?.["external-chat"] as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: LittleBabyConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: LittleBabyConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
