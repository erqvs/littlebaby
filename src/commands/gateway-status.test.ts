...
describe("gateway-status command", () => {
  ...
  it("suppresses unresolved SecretRef auth warnings when probe is reachable", async () => {
    const { runtime, runtimeLogs, runtimeErrors } = createRuntimeCapture();
    await withEnvAsync(
      { MISSING_GATEWAY_TOKEN: undefined, LITTLEBABY_GATEWAY_TOKEN: undefined },
      async () => {
  ...
  it("does not resolve local token SecretRef when LITTLEBABY_GATEWAY_TOKEN is set", async () => {
    const { runtime, runtimeLogs, runtimeErrors } = createRuntimeCapture();
    await withEnvAsync(
      {
