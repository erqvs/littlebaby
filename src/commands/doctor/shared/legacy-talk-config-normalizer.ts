...
export function normalizeLegacyTalkConfig(cfg: LittleBabyConfig, changes: string[]): LittleBabyConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as LittleBabyConfig["talk"]) ?? {};
