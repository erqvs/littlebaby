...
vi.mock("./shared/channel-doctor.js", () => ({
  collectChannelDoctorRepairMutations: ({ cfg }: { cfg: LittleBabyConfig }) => {
...
vi.mock("./shared/empty-allowlist-scan.js", () => ({
  scanEmptyAllowlistPolicyWarnings: (cfg: LittleBabyConfig) =>
    cfg.channels?.signal
...
vi.mock("./shared/allowlist-policy-repair.js", () => ({
  maybeRepairAllowlistPolicyAllowFrom: async (cfg: LittleBabyConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/bundled-plugin-load-paths.js", () => ({
  maybeRepairBundledPluginLoadPaths: (cfg: LittleBabyConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/open-policy-allowfrom.js", () => ({
  maybeRepairOpenPolicyAllowFrom: (cfg: LittleBabyConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/stale-plugin-config.js", () => ({
  maybeRepairStalePluginConfig: (cfg: LittleBabyConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/legacy-tools-by-sender.js", () => ({
  maybeRepairLegacyToolsBySenderKeys: (cfg: LittleBabyConfig) => {
...
vi.mock("./shared/exec-safe-bins.js", () => ({
  maybeRepairExecSafeBinProfiles: (cfg: LittleBabyConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
describe("doctor repair sequencing", () => {
  it("applies ordered repairs and sanitizes empty-allowlist warnings", async () => {
    const result = await runDoctorRepairSequence({
      ...
      doctorFixCommand: "littlebaby doctor --fix",
    });
