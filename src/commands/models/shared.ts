...
export async function loadValidConfigOrThrow(): Promise<LittleBabyConfig> {
...
export async function updateConfig(
  mutator: (cfg: LittleBabyConfig) => LittleBabyConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: LittleBabyConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: LittleBabyConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("littlebaby agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
