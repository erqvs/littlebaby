...
vi.mock("../provider-auth-helpers.js", () => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn((cfg: LittleBabyConfig, patch: unknown) => {
    ...
    applyDefaultModel: vi.fn((cfg: LittleBabyConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: LittleBabyConfig;
  let lastUpdatedConfig: LittleBabyConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/littlebaby/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/littlebaby/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/littlebaby/workspace");
    mocks.isRemoteEnvironment.mockReturnValue(false);
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: LittleBabyConfig) => LittleBabyConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/littlebaby/agents/main");
    expect(mocks.clearAuthProfileCooldown).toHaveBeenCalledWith({
      store: fakeStore,
      profileId: "openai-codex:user@example.com",
      agentDir: "/tmp/littlebaby/agents/main",
    });
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with littlebaby configure --section web (recommended mode: cached). Docs: https://docs.littlebaby.ai/tools/web",
    );
  });
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/littlebaby/agents/main");
      expect(ctx.workspaceDir).toBe("/tmp/littlebaby/workspace");
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `littlebaby plugins list --json`.',
    );
  });
