...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("littlebaby doctor --fix")} or ${formatCliCommand("littlebaby memory status --fix")}`
    : `Verify: ${formatCliCommand("littlebaby memory status --deep")}`;
  return [
 ...
 * Runs as part of `littlebaby doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("littlebaby config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("littlebaby config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("littlebaby memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`littlebaby config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("littlebaby configure --section model")}`,
        `- To disable: ${formatCliCommand("littlebaby config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("littlebaby memory status --deep")}`,
      ].join("\n"),
      "Memory search",
