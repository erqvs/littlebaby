...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".littlebaby", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: LittleBabyConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["LITTLEBABY_GATEWAY_TOKEN", await getenv("LITTLEBABY_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["LITTLEBABY_GATEWAY_PASSWORD", await getenv("LITTLEBABY_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "LITTLEBABY_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.LITTLEBABY_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- LITTLEBABY_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
