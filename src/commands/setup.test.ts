...
function createSetupDeps(home: string) {
  ...
  return {
    ...
    ensureAgentWorkspace: vi.fn(async (params?: { dir?: string }) => ({
      dir: params?.dir ?? path.join(home, ".littlebaby", "workspace"),
    })),
    ...
    mkdir: vi.fn(async () => {}),
    resolveSessionTranscriptsDir: vi.fn(() => path.join(home, ".littlebaby", "sessions")),
...
describe("setupCommand", () => {
  it("writes gateway.mode=local on first run", async () => {
    await withTempHome(async (home) => {
      ...
      const workspace = path.join(home, ".littlebaby", "workspace");
