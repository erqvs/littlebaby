...
function expectUnknownChannelRemovalPrompt(unsafeChannel: string, label: string) {
  ...
  expect(confirm).toHaveBeenCalledWith(
    expect.objectContaining({
      message: `Delete ${label} configuration from ~/.littlebaby/littlebaby.json?`,
    }),
  );
...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from littlebaby.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from littlebaby.json", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete Telegram configuration from ~/.littlebaby/littlebaby.json?",
      }),
    );
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete done configuration from ~/.littlebaby/littlebaby.json?",
      }),
    );
  ...
  it("sanitizes known channel labels before rendering prompts", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete Telegram\\nBot configuration from ~/.littlebaby/littlebaby.json?",
      }),
    );
    expect(note).toHaveBeenCalledWith(
