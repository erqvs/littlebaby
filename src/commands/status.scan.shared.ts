...
export function hasExplicitMemorySearchConfig(cfg: LittleBabyConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: LittleBabyConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: LittleBabyConfig, agentId: string) => { store: { path: string } } | null;
