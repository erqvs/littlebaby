...
beforeAll(() => {
  envSnapshot = captureEnv(["LITTLEBABY_PROFILE"]);
  process.env.LITTLEBABY_PROFILE = "isolated";
});
...
async function createMockStatusScanResult(params: { includePluginCompatibility?: boolean } = {}) {
  ...
  return {
    ...
    gatewayProbeAuth: process.env.LITTLEBABY_GATEWAY_TOKEN
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.LITTLEBABY_GATEWAY_TOKEN?.trim();
...
describe("statusCommand", () => {
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("LITTLEBABY_GATEWAY_TOKEN", "abcd1234", async () => {
