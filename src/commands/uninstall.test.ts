...
describe("uninstallCommand", () => {
  ...
  it("recommends creating a backup before removing state or workspaces", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(expect.stringContaining("littlebaby backup create"));
  });
  ...
  it("does not recommend backup for service-only uninstall", async () => {
    ...
    expect(runtime.log).not.toHaveBeenCalledWith(expect.stringContaining("littlebaby backup create"));
  });
});
