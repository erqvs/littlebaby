...
function resolvePendingPairingIssue(
  ...
  const approveCommand = formatCliArgs(["littlebaby", "devices", "approve", pending.requestId]);
  const inspectCommand = formatCliArgs(["littlebaby", "devices", "list"]);
  if (!paired) {
    return {
      kind: "first-time",
      pending,
  ...
  if (paired.publicKey !== pending.publicKey) {
    return {
      ...
      approveCommand,
      inspectCommand,
      removeCommand: formatCliArgs(["littlebaby", "devices", "remove", pending.deviceId]),
    };
  }
