...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for LITTLEBABY_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // LITTLEBABY_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.littlebaby out of git; it contains credentials and session history.",
