...
describe("image-generation runtime", () => {
  ...
  it("lists runtime image-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeImageGenerationProviders({ config: {} as LittleBabyConfig })).toEqual(
      providers,
    );
    expect(mocks.listImageGenerationProviders).toHaveBeenCalledWith({} as LittleBabyConfig);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    await expect(
      generateImage({ cfg: {} as LittleBabyConfig, prompt: "draw a cat" }),
    ).rejects.toThrow(
