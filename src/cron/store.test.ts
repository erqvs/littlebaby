...
const { makeStorePath } = createCronStoreHarness({ prefix: "littlebaby-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses LITTLEBABY_HOME for tilde expansion", () => {
    vi.stubEnv("LITTLEBABY_HOME", "/srv/littlebaby-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/littlebaby-home", "cron", "jobs.json"));
  });
});
