...
describe("logger helpers", () => {
  ...
  it("writes to configured log file at configured level", () => {
    withTempDirSync({ prefix: "littlebaby-log-test-" }, (dir) => {
      const logPath = path.join(dir, "littlebaby.log");
  ...
  it("uses daily rolling default log file and prunes old ones", () => {
    ...
    const todayPath = path.join(DEFAULT_LOG_DIR, `littlebaby-${today}.log`);

    // create an old file to be pruned
    const oldPath = path.join(DEFAULT_LOG_DIR, "littlebaby-2000-01-01.log");
