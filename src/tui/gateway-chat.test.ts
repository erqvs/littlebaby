...
describe("resolveGatewayConnection", () => {
  ...
  it("falls back to LITTLEBABY_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers LITTLEBABY_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "LITTLEBABY_GATEWAY_PASSWORD"; // pragma: allowlist secret
