...
describe("video-generation runtime", () => {
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeVideoGenerationProviders({ config: {} as LittleBabyConfig })).toEqual(
      providers,
    );
    expect(mocks.listVideoGenerationProviders).toHaveBeenCalledWith({} as LittleBabyConfig);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    await expect(
      generateVideo({ cfg: {} as LittleBabyConfig, prompt: "animate a cat" }),
    ).rejects.toThrow(
