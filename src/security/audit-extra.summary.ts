...
function summarizeGroupPolicy(cfg: LittleBabyConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: LittleBabyConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: LittleBabyConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: LittleBabyConfig): boolean {
...
function isBrowserEnabled(cfg: LittleBabyConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: LittleBabyConfig): SecurityAuditFinding[] {
