...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("littlebaby status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("littlebaby security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set LITTLEBABY_GATEWAY_TOKEN/LITTLEBABY_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("littlebaby security audit --deep")}".`,
    });
  }
  return findings;
}
