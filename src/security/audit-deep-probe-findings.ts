...
export function collectDeepProbeFindings(params: {
  ...
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set LITTLEBABY_GATEWAY_TOKEN/LITTLEBABY_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("openclaw security audit --deep")}".`,
    });
  }
  return findings;
}
