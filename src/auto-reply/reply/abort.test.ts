...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop littlebaby!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@littlebaby_bot", { botUsername: "littlebaby_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@littlebaby_bot", { botUsername: "littlebaby_bot" })).toBe(true);
