...
export function resolveAcpInstallCommandHint(cfg: LittleBabyConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `littlebaby plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^littlebaby plugins install /u, "");
    ...
    return "littlebaby plugins install acpx";
  }
