...
vi.mock("../../cron/store.js", () => {
  return {
    ...
    resolveCronStorePath: (storePath?: string) => storePath ?? "/tmp/littlebaby-cron-store.json",
  };
});
...
describe("runReplyAgent auto-compaction token update", () => {
  ...
  it("updates totalTokens from lastCallUsage even without compaction", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-usage-last-"));
...
describe("runReplyAgent Active Memory inline debug", () => {
  it("appends inline Active Memory status payload when verbose is enabled", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-active-memory-inline-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
  ...
  it("appends raw trace payloads when trace raw is enabled", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-trace-raw-usage-"));
    const storePath = path.join(tmp, "sessions.json");
  ...
  it("does not emit persisted trace output to an unauthorized sender", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-trace-raw-unauthorized-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionFile = path.join(tmp, "session.jsonl");
    const sessionKey = "main";
    const sessionEntry: SessionEntry = {
  ...
  it("escapes markdown fence delimiters inside raw trace blocks", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-trace-raw-fence-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionFile = path.join(tmp, "session.jsonl");
    const sessionKey = "main";
    const sessionEntry: SessionEntry = {
