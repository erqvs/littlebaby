...
import { scheduleGatewaySigusr1Restart, triggerLittleBabyRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting LittleBaby in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerLittleBabyRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting LittleBaby via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
