...
vi.mock("../../agents/acp-spawn.js", () => ({
  resolveAcpSpawnRuntimePolicyError: (params: { cfg?: LittleBabyConfig }) =>
...
function createDiscordParams(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
...
async function runDiscordAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
...
async function runTelegramDmAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runSlackDmAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
function createMatrixThreadParams(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
...
async function runMatrixAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runMatrixThreadAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
  return handleAcpCommand(createMatrixThreadParams(commandBody, cfg), true);
}

async function runFeishuDmAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runLineDmAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runBlueBubblesDmAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runIMessageDmAcpCommand(commandBody: string, cfg: LittleBabyConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
describe("/acp command", () => {
  ...
  it("spawns an ACP session and binds a Discord thread", async () => {
    ...
    const result = await runDiscordAcpCommand("/acp spawn codex --cwd /home/bob/littlebaby");
    ...
    expect(hoisted.sessionBindingBindMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        metadata: expect.objectContaining({
          introText: expect.stringContaining("cwd: /home/bob/littlebaby"),
        }),
      }),
    );
  ...
  it("accepts unicode dash option prefixes in /acp spawn args", async () => {
    const result = await runThreadAcpCommand(
      "/acp spawn codex \u2014mode oneshot \u2014thread here \u2014cwd /home/bob/littlebaby \u2014label jeerreview",
    );
  ...
  it("sanitizes leaked task and runtime details in ACP status output", async () => {
    ...
    hoisted.readAcpSessionEntryMock.mockReturnValue({
      ...
      acp: {
        ...
        lastError: [
          "LittleBaby runtime context (internal):",
    ...
    expect(result?.reply?.text).not.toContain("LittleBaby runtime context (internal):");
