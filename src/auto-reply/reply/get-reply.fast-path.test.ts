...
vi.mock("../../agents/workspace.js", () => ({
  DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/littlebaby-workspace",
...
describe("getReplyFromConfig fast test bootstrap", () => {
  ...
  it("fails fast on unmarked config overrides in strict fast-test mode", async () => {
    await expect(getReplyFromConfig(buildCtx(), undefined, {} as LittleBabyConfig)).rejects.toThrow(
  ...
  it("skips loadConfig, workspace bootstrap, and session bootstrap for marked test configs", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-fast-reply-"));
    const cfg = markCompleteReplyConfig({
      agents: {
        defaults: {
          model: "anthropic/claude-opus-4-6",
          workspace: path.join(home, "littlebaby"),
        },
      },
  ...
  it("still merges partial config overrides against loadConfig()", async () => {
    vi.stubEnv("LITTLEBABY_ALLOW_SLOW_REPLY_TESTS", "1");
    vi.mocked(loadConfigMock).mockReturnValue({
      channels: {
        telegram: {
  ...
  it("marks configs through withFastReplyConfig()", async () => {
    const cfg = withFastReplyConfig({ session: { store: "/tmp/sessions.json" } } as LittleBabyConfig);
  ...
  it("uses native command target session keys during fast bootstrap", () => {
    const result = initFastReplySessionState({
      ...
      cfg: { session: { store: "/tmp/sessions.json" } } as LittleBabyConfig,
      agentId: "main",
