...
function buildParams(commandBody: string, cfg: LittleBabyConfig): HandleCommandsParams {
  const ctx = {
    Body: commandBody,
...
describe("command gating", () => {
  ...
  it("blocks disabled bash", async () => {
    const result = await handleBashChatCommand({
      ...
      cfg: { commands: { bash: false } } as LittleBabyConfig,
      sessionKey: "agent:main:main",
      isGroup: false,
      elevated: { enabled: true, allowed: true, failures: [] },
    });
  ...
  it("blocks bash when elevated access is unavailable", async () => {
    const result = await handleBashChatCommand({
      ...
      cfg: { commands: { bash: true } } as LittleBabyConfig,
      sessionKey: "agent:main:main",
      isGroup: false,
      elevated: {
        enabled: true,
  ...
  it("ignores inherited command flags", () => {
    ...
    const cfg = { commands: inheritedCommands as never } as LittleBabyConfig;
  ...
  it("enforces gateway client permissions for /config commands", async () => {
    const baseCfg = { commands: { config: true, text: true } } as LittleBabyConfig;
