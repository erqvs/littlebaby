...
const workspaceHarness = createCommandWorkspaceHarness("littlebaby-command-mcp-");
...
function buildCfg(): LittleBabyConfig {
  return {
    commands: {
      text: true,
      mcp: true,
...
describe("handleCommands /mcp", () => {
  ...
  it("writes MCP config and shows it back", async () => {
    await withTempHome("littlebaby-command-mcp-home-", async () => {
