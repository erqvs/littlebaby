...
const { buildStatusReplyMock, createLittleBabyToolsMock, getChannelPluginMock, handleCommandsMock } =
  vi.hoisted(() => ({
    buildStatusReplyMock: vi.fn(),
    createLittleBabyToolsMock: vi.fn(),
...
vi.mock("../../agents/littlebaby-tools.runtime.js", () => ({
  createLittleBabyTools: (...args: unknown[]) => createLittleBabyToolsMock(...args),
}));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createLittleBabyToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
