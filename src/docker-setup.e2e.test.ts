...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LITTLEBABY_GATEWAY_TOKEN: "test-token",
    LITTLEBABY_CONFIG_DIR: join(sandbox.rootDir, "config"),
    LITTLEBABY_WORKSPACE_DIR: join(sandbox.rootDir, "littlebaby"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("littlebaby-gateway");
}
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    LITTLEBABY_GATEWAY_TOKEN: undefined,
    LITTLEBABY_CONFIG_DIR: configDir,
    LITTLEBABY_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node littlebaby-gateway dist/index.js onboard --mode local --no-install-daemon",
    );
    expect(log).toContain(
  ...
  it("writes LITTLEBABY_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      LITTLEBABY_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("LITTLEBABY_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh littlebaby-gateway -c");
  });

  it("reuses existing config token when LITTLEBABY_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("LITTLEBABY_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when LITTLEBABY_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "LITTLEBABY_GATEWAY_TOKEN=dotenv-token-123\nLITTLEBABY_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("LITTLEBABY_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "LITTLEBABY_GATEWAY_TOKEN=first-token",
        "LITTLEBABY_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("LITTLEBABY_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("LITTLEBABY_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats LITTLEBABY_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      LITTLEBABY_SANDBOX: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("LITTLEBABY_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg LITTLEBABY_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg LITTLEBABY_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      LITTLEBABY_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker littlebaby-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        LITTLEBABY_SANDBOX: "1",
        LITTLEBABY_DOCKER_SOCKET: socketPath,
      ...
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate littlebaby-gateway"));
  ...
  it("rejects injected multiline LITTLEBABY_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      LITTLEBABY_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("LITTLEBABY_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid LITTLEBABY_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      LITTLEBABY_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid LITTLEBABY_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      LITTLEBABY_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("LITTLEBABY_HOME_VOLUME must match");
  });

  it("rejects LITTLEBABY_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      LITTLEBABY_TZ: "Nope/Bad",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("LITTLEBABY_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:littlebaby-gateway"');
    expect(compose).toContain("depends_on:\n      - littlebaby-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/LITTLEBABY_GATEWAY_TOKEN: \$\{LITTLEBABY_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{LITTLEBABY_TZ:-UTC\}/g)).toHaveLength(2);
  });
});
