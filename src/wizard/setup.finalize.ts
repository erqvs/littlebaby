...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("littlebaby gateway run")}`,
          `Or rerun with: ${formatCliCommand("littlebaby onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("littlebaby onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: $LITTLEBABY_CONFIG_PATH (default: ~/.littlebaby/littlebaby.json) under gateway.auth.token, or in LITTLEBABY_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("littlebaby config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("littlebaby doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("littlebaby dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control LittleBaby."
            : "Copy/paste this URL in a browser on this machine to control LittleBaby.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("littlebaby dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.littlebaby.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.littlebaby.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("littlebaby configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("littlebaby configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.littlebaby.ai/tools/web"}`,
          "Docs: https://docs.littlebaby.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("littlebaby configure --section web")}`,
          "",
          "Docs: https://docs.littlebaby.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://littlebaby.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control LittleBaby."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control LittleBaby.",
  );
