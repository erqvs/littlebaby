...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("littlebaby doctor")}\` ${message}`;
}
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as LittleBabyConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "littlebaby.json";
...
export function registerConfigCli(program: Command) {
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing littlebaby.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for littlebaby.json")
    .action(async () => {
