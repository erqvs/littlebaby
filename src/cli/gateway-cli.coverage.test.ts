...
describe("gateway-cli coverage", () => {
    ...
    await withEnvOverride(
      {
        ...
        LITTLEBABY_LAUNCHD_LABEL: undefined,
        LITTLEBABY_SYSTEMD_UNIT: undefined,
        INVOCATION_ID: undefined,
        SYSTEMD_EXEC_PID: undefined,
        JOURNAL_STREAM: undefined,
        LITTLEBABY_WINDOWS_TASK_NAME: undefined,
        LITTLEBABY_SERVICE_MARKER: undefined,
        LITTLEBABY_SERVICE_KIND: undefined,
      },
      async () => {
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ LITTLEBABY_GATEWAY_PORT: "19001" }, async () => {
      runtimeLogs.length = 0;
      runtimeErrors.length = 0;
