...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.littlebaby.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.littlebaby.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "littlebaby agents set-identity --identity-file ~/.littlebaby/workspace/IDENTITY.md --agent main",
