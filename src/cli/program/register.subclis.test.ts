...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.LITTLEBABY_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.LITTLEBABY_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.LITTLEBABY_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.LITTLEBABY_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      ...
      process.env.LITTLEBABY_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "littlebaby", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "littlebaby"]);

    const names = program.commands.map((cmd) => cmd.name());
    expect(names).toContain("acp");
    expect(names).toContain("gateway");
    expect(names).toContain("littlebabybot");
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "littlebaby", "nodes", "list"], "littlebaby");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "littlebaby", "infer"], "littlebaby");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "littlebaby", "acp", "--help"], "littlebaby");
