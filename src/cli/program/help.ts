...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--container <name>",
      "Run the CLI inside a running Podman/Docker container named <name> (default: env LITTLEBABY_CONTAINER)",
    )
    .option(
      "--dev",
    ...
    .option(
      "--profile <name>",
      "Use a named profile (isolates LITTLEBABY_STATE_DIR/LITTLEBABY_CONFIG_PATH under ~/.littlebaby-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  if (
    ...
    console.log(
      commit ? `LittleBaby ${ctx.programVersion} (${commit})` : `LittleBaby ${ctx.programVersion}`,
    );
    process.exit(0);
  }
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.littlebaby.ai/cli");
