...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "littlebaby", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "littlebaby", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "littlebaby", "status"]);
  });
