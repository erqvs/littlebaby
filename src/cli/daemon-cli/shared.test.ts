...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when LITTLEBABY_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
  ...
  it("drops the generic container foreground hint when LITTLEBABY_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
