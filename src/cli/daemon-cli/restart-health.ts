...
async function confirmGatewayReachable(port: number): Promise<boolean> {
  const token = normalizeOptionalString(process.env.LITTLEBABY_GATEWAY_TOKEN);
  const password = normalizeOptionalString(process.env.LITTLEBABY_GATEWAY_PASSWORD);
