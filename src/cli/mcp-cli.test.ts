...
const mocks = vi.hoisted(() => {
  ...
  return {
    runtime,
    serveLittleBabyChannelMcp: vi.fn(),
  };
});
...
const serveLittleBabyChannelMcp = mocks.serveLittleBabyChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveLittleBabyChannelMcp: mocks.serveLittleBabyChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("littlebaby-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("littlebaby-cli-mcp-home-", async () => {
      ...
      expect(serveLittleBabyChannelMcp).toHaveBeenCalledWith({
