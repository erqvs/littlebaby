...
const mocks = vi.hoisted(() => {
  ...
  let configState: LittleBabyConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    setConfig: (next: LittleBabyConfig) => {
      configState = next;
    },
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: LittleBabyConfig) => void }) => {
      ...
      return {
        ...
        previousHash: "hash-1",
        snapshot: { path: "/tmp/littlebaby.json" },
        nextConfig: draft,
        result: undefined,
      };
    }),
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: LittleBabyConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: LittleBabyConfig }>
    >(async () => ({
      path: "/tmp/littlebaby.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: LittleBabyConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/littlebaby.json");
    ...
    expect(output).not.toContain("/tmp/littlebaby.json\nforged");
