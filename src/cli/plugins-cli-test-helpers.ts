...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as LittleBabyConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: LittleBabyConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
export const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/littlebaby-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: LittleBabyConfig) =>
    invokeMock<
      [LittleBabyConfig],
...
vi.mock("../plugins/enable.js", () => ({
  enablePluginInConfig: ((cfg: LittleBabyConfig, pluginId: string) =>
    invokeMock<[LittleBabyConfig, string], unknown>(
...
export function resetPluginsCliTestState() {
  ...
  replaceConfigFile.mockImplementation(
    (async (params: { nextConfig: LittleBabyConfig }) =>
      await writeConfigFile(params.nextConfig)) as (...args: unknown[]) => Promise<unknown>,
  );
  resolveStateDir.mockReturnValue("/tmp/littlebaby-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation(((cfg: LittleBabyConfig) => ({ config: cfg })) as (
    ...args: unknown[]
  ) => unknown);
  recordPluginInstall.mockImplementation(
    ((cfg: LittleBabyConfig) => cfg) as (...args: unknown[]) => unknown,
  );
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: LittleBabyConfig }) => ({
    config,
    warnings: [],
