...
function appendLittleBabyHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("littlebaby skills list")}\` to see all skills.`
      : "No skills found.";
    return appendLittleBabyHubHint(message, opts.json);
  }
  ...
  return appendLittleBabyHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendLittleBabyHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("littlebaby skills list")}\` to see available skills.`,
      opts.json,
    );
  }
  ...
  if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
    ...
    lines.push(
      `  Save via CLI: ${formatCliCommand(`littlebaby config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`,
    );
    lines.push(
      `  Stored in: ${theme.muted("$LITTLEBABY_CONFIG_PATH")} ${theme.muted("(default: ~/.littlebaby/littlebaby.json)")}`,
    );
  }
