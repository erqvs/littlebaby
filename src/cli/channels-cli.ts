    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["littlebaby channels list", "List configured channels and auth profiles."],
          ["littlebaby channels status --probe", "Run channel status checks and probes."],
          [
            "littlebaby channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["littlebaby channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.littlebaby.ai/cli/channels",
        )}\n`,
    );
