...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "littlebaby", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "littlebaby", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "littlebaby", "--profile", "work", "gateway", "status"],
