...
function getAuthProfileIdsForProvider(cfg: LittleBabyConfig, providerId: string): string[] {
...
export function registerCapabilityCli(program: Command) {
  const capability = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/infer", "docs.littlebaby.ai/cli/infer")}\n`,
    );
