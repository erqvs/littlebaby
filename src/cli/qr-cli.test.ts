...
describe("registerQrCli", () => {
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("littlebaby devices approve <requestId>");
  });
  ...
  it("uses LITTLEBABY_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("LITTLEBABY_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
