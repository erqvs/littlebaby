...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update LittleBaby and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["littlebaby update", "Update a source checkout (git)"],
        ["littlebaby update --channel beta", "Switch to beta channel (git + npm)"],
        ["littlebaby update --channel dev", "Switch to dev channel (git + npm)"],
        ["littlebaby update --tag beta", "One-off update to a dist-tag or version"],
        ["littlebaby update --tag main", "One-off package install from GitHub main"],
        ["littlebaby update --dry-run", "Preview actions without changing anything"],
        ["littlebaby update --no-restart", "Update without restarting the service"],
        ["littlebaby update --json", "Output result as JSON"],
        ["littlebaby update --yes", "Non-interactive (accept downgrade prompts)"],
        ["littlebaby update wizard", "Interactive update wizard"],
        ["littlebaby --update", "Shorthand for littlebaby update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run littlebaby update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.littlebaby.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["littlebaby update status", "Show channel + version status."],
          ["littlebaby update status --json", "JSON output."],
          ["littlebaby update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.littlebaby.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
