...
describe("shouldEnsureCliPath", () => {
  it("skips path bootstrap for help/version invocations", () => {
    expect(shouldEnsureCliPath(["node", "littlebaby", "--help"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "littlebaby", "-V"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "littlebaby", "-v"])).toBe(false);
  });

  it("skips path bootstrap for read-only fast paths", () => {
    expect(shouldEnsureCliPath(["node", "littlebaby", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "littlebaby", "--log-level", "debug", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "littlebaby", "sessions", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "littlebaby", "config", "get", "update"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "littlebaby", "models", "status", "--json"])).toBe(false);
  });

  it("keeps path bootstrap for mutating or unknown commands", () => {
    expect(shouldEnsureCliPath(["node", "littlebaby", "message", "send"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "littlebaby", "voicecall", "status"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "littlebaby", "acp", "-v"])).toBe(true);
  });
});

describe("shouldUseRootHelpFastPath", () => {
  it("uses the fast path for root help only", () => {
    expect(shouldUseRootHelpFastPath(["node", "littlebaby", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "littlebaby", "--profile", "work", "-h"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "littlebaby", "status", "--help"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "littlebaby", "--help", "status"])).toBe(false);
  });
});
