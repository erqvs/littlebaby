...
const SANDBOX_EXAMPLES = {
  main: [
    ["littlebaby sandbox list", "List all sandbox containers."],
    ["littlebaby sandbox list --browser", "List only browser containers."],
    ["littlebaby sandbox recreate --all", "Recreate all containers."],
    ["littlebaby sandbox recreate --session main", "Recreate a specific session."],
    ["littlebaby sandbox recreate --agent mybot", "Recreate agent containers."],
    ["littlebaby sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["littlebaby sandbox list", "List all sandbox containers."],
    ["littlebaby sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["littlebaby sandbox recreate --all", "Recreate all containers."],
    ["littlebaby sandbox recreate --session main", "Recreate a specific session."],
    ["littlebaby sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["littlebaby sandbox recreate --browser --all", "Recreate only browser containers."],
    ["littlebaby sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["littlebaby sandbox explain", "Show effective sandbox config."],
    ["littlebaby sandbox explain --session agent:main:main", "Explain a specific session."],
    ["littlebaby sandbox explain --agent work", "Explain an agent sandbox."],
    ["littlebaby sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.littlebaby.ai/cli/sandbox")}\n`,
    )
    .action(() => {
