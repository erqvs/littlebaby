...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["littlebaby security audit", "Run a local security audit."],
          ["littlebaby security audit --deep", "Include best-effort live Gateway probe checks."],
          ["littlebaby security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "littlebaby security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["littlebaby security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["littlebaby security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.littlebaby.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("LittleBaby security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("littlebaby security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("littlebaby security audit --fix")}`));
        if (!fixResult) {
