...
describe("proxy cli runtime", () => {
  const envKeys = [
    "LITTLEBABY_DEBUG_PROXY_DB_PATH",
    "LITTLEBABY_DEBUG_PROXY_BLOB_DIR",
    "LITTLEBABY_DEBUG_PROXY_CERT_DIR",
    "LITTLEBABY_DEBUG_PROXY_SESSION_ID",
    "LITTLEBABY_DEBUG_PROXY_ENABLED",
  ] as const;
  const savedEnv = Object.fromEntries(envKeys.map((key) => [key, process.env[key]]));
  let tempDir = "";

  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "littlebaby-proxy-cli-runtime-"));
    process.env.LITTLEBABY_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.LITTLEBABY_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    process.env.LITTLEBABY_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.LITTLEBABY_DEBUG_PROXY_ENABLED;
    delete process.env.LITTLEBABY_DEBUG_PROXY_SESSION_ID;
  ...
  it("stops the proxy server and ends the session when child spawn fails", async () => {
    ...
    const store = getDebugProxyCaptureStore(
      process.env.LITTLEBABY_DEBUG_PROXY_DB_PATH!,
      process.env.LITTLEBABY_DEBUG_PROXY_BLOB_DIR!,
    );
