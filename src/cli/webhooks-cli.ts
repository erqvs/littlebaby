...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.littlebaby.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + LittleBaby hooks")
    ...
    .option("--hook-url <url>", "LittleBaby hook URL")
    .option("--hook-token <token>", "LittleBaby hook token")
