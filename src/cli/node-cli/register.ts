    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "littlebaby node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["littlebaby node status", "Check node host service status."],
          ["littlebaby node install", "Install the node host service."],
          ["littlebaby node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.littlebaby.ai/cli/node")}\n`,
    );
