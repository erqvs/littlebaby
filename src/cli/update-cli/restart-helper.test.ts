...
`,
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'littlebaby-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses LITTLEBABY_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        LITTLEBABY_PROFILE: "default",
        LITTLEBABY_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("captures macOS launchctl stderr to ~/.littlebaby/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.littlebaby/logs/gateway-restart.log' 2>&1");
    ...
    it("uses LITTLEBABY_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        LITTLEBABY_PROFILE: "default",
        HOME: path.join(tmpDir, "home"),
        LITTLEBABY_STATE_DIR: stateFile,
      });
    ...
    it("uses LITTLEBABY_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
    ...
    it("uses custom profile in service names", async () => {
      ...
      expect(content).toContain("littlebaby-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("escapes single quotes in profile names for shell scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        LITTLEBABY_PROFILE: "it's-a-test",
      });
    ...
    it("rejects unsafe batch profile names on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const scriptPath = await prepareRestartScript({
        LITTLEBABY_PROFILE: "test&whoami",
      });
