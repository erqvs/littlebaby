...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.LITTLEBABY_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.LITTLEBABY_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.LITTLEBABY_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.LITTLEBABY_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.LITTLEBABY_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.LITTLEBABY_VITEST_PRINT_IMPORT_BREAKDOWN)) {
