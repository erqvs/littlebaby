...
metadata: { "littlebaby": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == LittleBaby `sessions_spawn`
- **File I/O** == LittleBaby `read`/`write`
- **Remote fetch** == LittleBaby `web_fetch` (or `exec` with curl when POST is required)
