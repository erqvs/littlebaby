...
- OpenClaw writes a marker file at `~/.littlebaby/matrix/migration-snapshot.json` after a successful snapshot so later startup and repair passes can reuse the same archive.
...
- the oldest flat Matrix store (`~/.littlebaby/matrix/bot-storage.json` and `~/.littlebaby/matrix/crypto/`) came from a single-store layout, so OpenClaw can only migrate it into one resolved Matrix account target
