---
title: "Node.js"
summary: "Install and configure Node.js for LittleBaby — version requirements, install options, and PATH troubleshooting"
read_when:
  - "You need to install Node.js before installing LittleBaby"
  - "You installed LittleBaby but `littlebaby` is command not found"
...
```
  ...
  <Warning>
  Make sure your version manager is initialized in your shell startup file (`~/.zshrc` or `~/.bashrc`). If it isn't, `littlebaby` may not be found in new terminal sessions because the PATH won't include Node's bin directory.
  </Warning>
</Accordion>

## Troubleshooting

### `littlebaby: command not found`
