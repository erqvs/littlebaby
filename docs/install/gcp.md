---
summary: "Run LittleBaby Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want LittleBaby running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# LittleBaby on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent LittleBaby Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "LittleBaby 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-littlebaby-project --name="LittleBaby Gateway"
    gcloud config set project my-littlebaby-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create littlebaby-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh littlebaby-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Configure environment variables">
    ...
    LITTLEBABY_CONFIG_DIR=/home/$USER/.littlebaby
    LITTLEBABY_WORKSPACE_DIR=/home/$USER/.littlebaby/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.littlebaby
    ```
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `LITTLEBABY_GATEWAY_TOKEN`.
    ...
    services:
      littlebaby-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - LITTLEBABY_GATEWAY_BIND=${LITTLEBABY_GATEWAY_BIND}
          - LITTLEBABY_GATEWAY_PORT=${LITTLEBABY_GATEWAY_PORT}
          - LITTLEBABY_GATEWAY_TOKEN=${LITTLEBABY_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${LITTLEBABY_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`LITTLEBABY_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm littlebaby-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh littlebaby-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop littlebaby-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type littlebaby-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start littlebaby-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create littlebaby-deploy \
     --display-name="LittleBaby Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-littlebaby-project \
     --member="serviceAccount:littlebaby-deploy@my-littlebaby-project.iam.gserviceaccount.com" \
