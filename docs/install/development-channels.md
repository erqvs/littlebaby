...
LittleBaby ships three update channels:
...
```bash
littlebaby update --channel stable
...
- **`dev`**: ensures a git checkout (default `~/littlebaby`, override with
  `LITTLEBABY_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
littlebaby update --tag 2026.4.1-beta.1
...
littlebaby update --tag littlebaby@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `littlebaby update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  LittleBaby prompts for confirmation (skip with `--yes`).
...
Preview what `littlebaby update` would do without making changes:

```bash
littlebaby update --dry-run
littlebaby update --channel beta --dry-run
littlebaby update --tag 2026.4.1-beta.1 --dry-run
littlebaby update --dry-run --json
```
...
When you switch channels with `littlebaby update`, LittleBaby also syncs plugin
sources:
