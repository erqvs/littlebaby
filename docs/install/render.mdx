---
summary: "Deploy LittleBaby on Render with Infrastructure-as-Code"
read_when:
  - Deploying LittleBaby to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy LittleBaby on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/littlebaby/littlebaby)
...
repository configures everything needed to run LittleBaby:
...
services:
  - type: web
    ...
    envVars:
      - key: LITTLEBABY_GATEWAY_PORT
        value: "8080"
      - key: LITTLEBABY_STATE_DIR
        value: /data/.littlebaby
      - key: LITTLEBABY_WORKSPACE_DIR
        value: /data/workspace
      - key: LITTLEBABY_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: littlebaby-data
      mountPath: /data
      sizeGB: 1
```
...
your fork's `render.yaml` (but note: no persistent disk means LittleBaby state
...
`LITTLEBABY_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original LittleBaby repository, Render will not auto-deploy your LittleBaby. To update it, run a manual Blueprint sync from the dashboard.
...
For LittleBaby, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with LittleBaby state plus any configured
...
- Missing `LITTLEBABY_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `LITTLEBABY_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `littlebaby backup create` in the Render shell.
