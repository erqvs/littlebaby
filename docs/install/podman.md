...
<Steps>
  ...
  <Step title="Manage the running container from the host CLI">
    Set `LITTLEBABY_CONTAINER=openclaw`, then use normal `openclaw` commands from the host.
  </Step>
</Steps>

Setup details:

- `./scripts/podman/setup.sh` builds `openclaw:local` in your rootless Podman store by default, or uses `LITTLEBABY_IMAGE` / `LITTLEBABY_PODMAN_IMAGE` if you set one.
- It creates `~/.littlebaby/openclaw.json` with `gateway.mode: "local"` if missing.
...
- `LITTLEBABY_IMAGE` or `LITTLEBABY_PODMAN_IMAGE` -- use an existing/pulled image instead of building `openclaw:local`
...
```bash
export LITTLEBABY_CONTAINER=openclaw
```
...
- `LITTLEBABY_PODMAN_CONTAINER` -- container name (`openclaw` by default)
...
If you use a non-default `LITTLEBABY_CONFIG_DIR` or `LITTLEBABY_WORKSPACE_DIR`, set the same variables for both `./scripts/podman/setup.sh` and later `./scripts/run-openclaw-podman.sh launch` commands. The repo-local launcher does not persist custom path overrides across shells.
...
- **Gateway start blocked (missing `gateway.mode=local`):** Ensure `~/.littlebaby/openclaw.json` exists and sets `gateway.mode="local"`. `scripts/podman/setup.sh` creates this if missing.
- **Container CLI commands hit the wrong target:** Use `openclaw --container <name> ...` explicitly, or export `LITTLEBABY_CONTAINER=<name>` in your shell.
