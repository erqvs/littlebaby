...
- Re-open anytime: `littlebaby dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `LITTLEBABY_GATEWAY_TOKEN`); `littlebaby dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `littlebaby dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `littlebaby dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `LITTLEBABY_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `littlebaby status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `littlebaby config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `LITTLEBABY_GATEWAY_TOKEN` in this shell, then rerun `littlebaby dashboard`
  - No shared secret configured: `littlebaby doctor --generate-gateway-token`
