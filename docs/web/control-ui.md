...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/littlebaby`)
...
If the page fails to load, start the Gateway first: `littlebaby gateway`.
...
created. Re-run `littlebaby devices list` before approval.
...
reconnect. LittleBaby keeps the old approval active, blocks the broader reconnect,
and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `littlebaby devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.littlebaby/littlebaby.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop littlebaby`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. LittleBaby
...
```bash
littlebaby gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
LittleBaby **blocks** Control UI connections without device identity.
...
```bash
LITTLEBABY_CONTROL_UI_BASE_PATH=/littlebaby/ pnpm ui:build
```
