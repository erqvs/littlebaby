...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in LittleBaby
...
Running LittleBaby via Node with `tsx` fails at startup with:

```
[littlebaby] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `littlebaby status` and `gateway:watch` worked.
