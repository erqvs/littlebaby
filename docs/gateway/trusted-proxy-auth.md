...
read_when:
  - Running LittleBaby behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of LittleBaby
...
- You run LittleBaby behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. LittleBaby checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. LittleBaby extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that LittleBaby can verify.
...
- LittleBaby can stay on loopback HTTP behind the proxy.
...
If LittleBaby itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://littlebaby.example.com
    to: http://littlebaby-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
littlebaby.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy littlebaby:18789 {
...
location / {
    ...
    proxy_pass http://littlebaby:18789;
...
LittleBaby rejects ambiguous configurations where both a `gateway.auth.token` (or `LITTLEBABY_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-littlebaby-scopes`.

Examples:

- `x-littlebaby-scopes: operator.read`
- `x-littlebaby-scopes: operator.read,operator.write`
- `x-littlebaby-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, LittleBaby honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-littlebaby-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-littlebaby-scopes` explicitly when you want a trusted-proxy request to
...
`littlebaby security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
LittleBaby rejected a loopback-source trusted-proxy request.
...
3. Update LittleBaby config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `littlebaby security audit` and review findings
