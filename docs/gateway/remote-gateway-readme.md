---
summary: "SSH tunnel setup for LittleBaby.app connecting to a remote gateway"
...
# Running LittleBaby.app with a Remote Gateway

LittleBaby.app uses SSH tunneling to connect to a remote gateway. This guide shows you how to set it up.
...
`LITTLEBABY_GATEWAY_TOKEN` is still valid as a shell-level override, but the durable
...
### Step 5: Restart LittleBaby.app

```bash
# Quit LittleBaby.app (⌘Q), then reopen:
open /path/to/LittleBaby.app
```
...
Legacy note: remove any leftover `com.littlebaby.ssh-tunnel` LaunchAgent if present.
...
LittleBaby.app connects to `ws://127.0.0.1:18789` on your client machine. The SSH tunnel forwards that connection to port 18789 on the remote machine where the Gateway is running.
