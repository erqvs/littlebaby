...
LittleBaby supports OAuth and API keys for model providers. For always-on gateway
...
setup, but LittleBaby also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `littlebaby gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.littlebaby/.env` so the daemon can read it:

```bash
cat >> ~/.littlebaby/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `littlebaby onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.littlebaby/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in LittleBaby as a supported token
path. Anthropic staff has since told us that LittleBaby-style Claude CLI usage is
...
```bash
littlebaby models auth paste-token --provider openrouter
```
...
```bash
littlebaby models status --check
```
...
- If auth exists but LittleBaby cannot resolve a probeable model candidate for
...
- Anthropic staff told us this LittleBaby integration path is allowed again.
- LittleBaby therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `LITTLEBABY_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- LittleBaby retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `littlebaby models status --probe` shows omitted
...
Run `littlebaby models status` to confirm which profile is expiring. If an
