...
LittleBaby’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`littlebaby agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-littlebaby-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-littlebaby-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "littlebaby"`, `model: "littlebaby/default"`, `model: "littlebaby/<agentId>"`, or `x-littlebaby-agent-id`
- use `x-littlebaby-model` when you want to override the selected agent's backend model
- use `x-littlebaby-session-key` for explicit session routing
- use `x-littlebaby-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: LittleBaby reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
LittleBaby normalizes common OpenAI-style aliases before those counters reach
