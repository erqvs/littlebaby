...
littlebaby nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.littlebaby`):

- `~/.littlebaby/nodes/paired.json`
- `~/.littlebaby/nodes/pending.json`

If you override `LITTLEBABY_STATE_DIR`, the `nodes/` folder moves with it.
