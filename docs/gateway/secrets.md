...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `LITTLEBABY_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
- The redaction sentinel `__LITTLEBABY_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.
