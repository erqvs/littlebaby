...
- During heartbeat runs, LittleBaby treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `littlebaby sessions --json` or the [sessions CLI](/cli/sessions)).
...
- If only alert delivery is disabled, LittleBaby can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
...
If **all three** are false, LittleBaby skips the heartbeat run entirely (no model call).
...
- LittleBaby parses the `tasks:` block and checks each task against its own `interval`.
...
```bash
littlebaby system event --text "Check for urgent follow-ups" --mode now
```
