  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.littlebaby/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Environment overrides: `LITTLEBABY_AGENT_RUNTIME=<id|auto|pi>` overrides `runtime`; `LITTLEBABY_AGENT_HARNESS_FALLBACK=none` disables PI fallback for that process.
...
- `none`: per-scope sandbox workspace under `~/.littlebaby/sandboxes`
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `LITTLEBABY_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `LITTLEBABY_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `LITTLEBABY_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.littlebaby/agents/main/agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.littlebaby/workspace-home" },
      { id: "work", workspace: "~/.littlebaby/workspace-work" },
    ],
  },
  bindings: [
...
```json5
{
  session: {
    ...
    store: "~/.littlebaby/agents/{agentId}/sessions/sessions.json",
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.littlebaby/settings/tts.json",
      elevenlabs: {
  ...
  talk: {
    ...
    providers: {
      elevenlabs: {
        ...
        voiceAliases: {
          LittleBaby: "EXAVITQu4vr4xnSDxMaL",
...
- Override agent config root with `LITTLEBABY_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or LITTLEBABY_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `LITTLEBABY_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `LITTLEBABY_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `LITTLEBABY_APNS_RELAY_BASE_URL` / `LITTLEBABY_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `LITTLEBABY_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.littlebaby/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `LITTLEBABY_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.littlebaby/workspace/canvas",
    liveReload: true,
    // enabled: false, // or LITTLEBABY_SKIP_CANVAS_HOST=1
  },
}
```
...
- `.env` files: CWD `.env` + `~/.littlebaby/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${LITTLEBABY_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.littlebaby/credentials/oauth.json`.
...
```json5
{
  diagnostics: {
    ...
    otel: {
      ...
      serviceName: "littlebaby-gateway",
      traces: true,
      metrics: true,
      logs: false,
      sampleRate: 1.0,
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.littlebaby/logs/cache-trace.jsonl",
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$LITTLEBABY_STATE_DIR/logs/cache-trace.jsonl`).
