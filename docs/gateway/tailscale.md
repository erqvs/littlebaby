...
LittleBaby can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. LittleBaby requires a shared password.
...
- `token` (default when `LITTLEBABY_GATEWAY_TOKEN` is set)
- `password` (shared secret via `LITTLEBABY_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. LittleBaby verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
LittleBaby only treats a request as Serve when it arrives from loopback with
...
Prefer `LITTLEBABY_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
littlebaby gateway --tailscale serve
littlebaby gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want LittleBaby to undo `tailscale serve`
