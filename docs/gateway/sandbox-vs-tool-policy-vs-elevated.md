...
LittleBaby has three related (but different) controls:
...
Use the inspector to see what LittleBaby is _actually_ doing:
...
- LittleBaby validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:littlebaby`: all built-in LittleBaby tools (excludes provider plugins)
