---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for LittleBaby agents"
read_when:
...
OpenShell is a managed sandbox backend for LittleBaby. Instead of running Docker
containers locally, LittleBaby delegates sandbox lifecycle to the `openshell` CLI,
...
- LittleBaby Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, LittleBaby creates an OpenShell
...
- You edit files locally outside LittleBaby and want those changes visible in the
...
- When the sandbox is first created, LittleBaby seeds the remote workspace from
...
- LittleBaby does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside LittleBaby after the initial seed,
the remote sandbox does **not** see those changes. Use
`littlebaby sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"littlebaby"`  | Sandbox source for first-time create                  |
...
1. LittleBaby calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. LittleBaby calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `littlebaby sandbox` commands
