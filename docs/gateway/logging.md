...
LittleBaby has two log “surfaces”:
...
- Default rolling log file is under `/tmp/littlebaby/` (one file per day): `littlebaby-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.littlebaby/littlebaby.json`:
  - `logging.file`
  - `logging.level`
...
`littlebaby gateway` supports a per-gateway style switch:
