...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    ...
    - `LITTLEBABY_APNS_RELAY_BASE_URL` and `LITTLEBABY_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `LITTLEBABY_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
Env var equivalent: `LITTLEBABY_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${LITTLEBABY_GATEWAY_TOKEN}" } },
