...
LittleBaby. All clients (CLI, web UI, macOS app, iOS/Android nodes, headless
...
```json
{
  ...
  "params": {
    ...
    "auth": { "token": "…" },
    "locale": "en-US",
    "userAgent": "littlebaby-cli/1.2.3",
    "device": {
...
```json
{
  ...
  "params": {
    ...
    "auth": { "token": "…" },
    "locale": "en-US",
    "userAgent": "littlebaby-ios/1.2.3",
    "device": {
      "id": "device_fingerprint",
      "publicKey": "…",
      "signature": "…",
...
- Operators may call `skills.search` and `skills.detail` (`operator.read`) for
  LittleBabyHub discovery metadata.
- Operators may call `skills.install` (`operator.admin`) in two modes:
  - LittleBabyHub mode: `{ source: "littlebabyhub", slug, version?, force? }` installs a
  ...
  - Gateway installer mode: `{ name, installId, dangerouslyForceUnsafeInstall?, timeoutMs? }`
    runs a declared `metadata.littlebaby.install` action on the gateway host.
- Operators may call `skills.update` (`operator.admin`) in two modes:
  - LittleBabyHub mode updates one tracked slug or all tracked LittleBabyHub installs in
