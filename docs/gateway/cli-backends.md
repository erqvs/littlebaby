...
LittleBaby can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **LittleBaby tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
littlebaby agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, LittleBaby now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), LittleBaby will
...
2. **Builds a system prompt** using the same LittleBaby prompt + workspace context.
...
told us LittleBaby-style Claude CLI usage is allowed again, so LittleBaby treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled OpenAI `codex-cli` backend passes LittleBaby's system prompt through
Codex's `model_instructions_file` config override (`-c
model_instructions_file="..."`). Codex does not expose a Claude-style
`--append-system-prompt` flag, so LittleBaby writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the LittleBaby skills snapshot
two ways: the compact LittleBaby skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that LittleBaby would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by LittleBaby to the
...
- LittleBaby drops stored CLI session reuse when the backend auth state changes, including relogin, token rotation, or a changed auth profile credential.
...
LittleBaby will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, LittleBaby appends the
...
- For Gemini CLI JSON output, LittleBaby reads reply text from `response` and
...
rewrites streamed assistant deltas and parsed final text before LittleBaby handles
...
CLI backends do **not** receive LittleBaby tool calls directly, but a backend can
...
When bundle MCP is enabled, LittleBaby:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`LITTLEBABY_MCP_TOKEN`)
...
If no MCP servers are enabled, LittleBaby still injects a strict config when a
backend opts into bundle MCP so background runs stay isolated.

## Limitations

- **No direct LittleBaby tool calls.** LittleBaby does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. LittleBaby sessions still work
  normally.
