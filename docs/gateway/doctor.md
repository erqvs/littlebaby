...
- Sessions store + transcripts:
  - from `~/.littlebaby/sessions/` to `~/.littlebaby/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.littlebaby/agent/` to `~/.littlebaby/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.littlebaby/credentials/*.json` (except `oauth.json`)
  - to `~/.littlebaby/credentials/whatsapp/<accountId>/...` (default account id: `default`)
...
Doctor also checks the cron job store (`~/.littlebaby/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.littlebaby` folders exist across
  home directories or when `LITTLEBABY_STATE_DIR` points elsewhere (history can
