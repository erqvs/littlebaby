...
read_when:
  - You want to understand what “context” means in LittleBaby
...
“Context” is **everything LittleBaby sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (LittleBaby-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How LittleBaby builds the system prompt

The system prompt is **LittleBaby-owned** and rebuilt each run. It includes:
...
By default, LittleBaby injects a fixed set of workspace files (if present):
...
Large files are truncated per-file using `agents.defaults.bootstrapMaxChars` (default `12000` chars). LittleBaby also enforces a total bootstrap injection cap across files with `agents.defaults.bootstrapTotalMaxChars` (default `60000` chars). `/context` shows **raw vs injected** sizes and whether truncation happened.
...
By default, LittleBaby uses the built-in `legacy` context engine for assembly and
compaction. If you install a plugin that provides `kind: "context-engine"` and
select it with `plugins.slots.contextEngine`, LittleBaby delegates context
