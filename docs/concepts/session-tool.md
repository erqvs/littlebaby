...
LittleBaby gives agents tools to work across sessions, inspect status, and
...
After the target responds, LittleBaby can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned LittleBaby
...
the completion origin only identifies a channel LittleBaby can still reuse the
