...
LittleBaby runs two retrieval paths in parallel and merges the results:
...
- **Vector search** finds notes with similar meaning ("gateway host" matches
  "the machine running LittleBaby").
...
When embeddings are unavailable, LittleBaby still uses lexical ranking over FTS results instead of falling back to raw exact-match ordering only. That degraded mode boosts chunks with stronger query-term coverage and relevant file paths, which keeps recall useful even without `sqlite-vec` or an embedding provider.
...
**No results?** Run `littlebaby memory status` to check the index. If empty, run
