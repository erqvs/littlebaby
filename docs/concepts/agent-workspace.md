...
This is separate from `~/.littlebaby/`, which stores config, credentials, and
sessions.
...
inside a sandbox workspace under `~/.littlebaby/sandboxes`, not your host workspace.
...
- `~/.littlebaby/skills/` (managed skills)
...
- Anything under `~/.littlebaby/`.
- Raw dumps of chats or sensitive attachments.

If you must store sensitive references, use placeholders and keep the real
secret elsewhere (password manager, environment variables, or `~/.littlebaby/`).
