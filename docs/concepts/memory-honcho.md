...
[Honcho](https://honcho.dev) adds AI-native memory to LittleBaby. It persists
...
```bash
littlebaby plugins install @honcho-ai/littlebaby-honcho
littlebaby honcho setup
...
Settings live under `plugins.entries["littlebaby-honcho"].config`:
    ...
    entries: {
      "littlebaby-honcho": {
        config: {
          ...
          workspaceId: "littlebaby", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `littlebaby honcho setup` detects and
...
```bash
littlebaby honcho setup                        # Configure API key and migrate files
littlebaby honcho status                       # Check connection status
littlebaby honcho ask <question>               # Query Honcho about the user
littlebaby honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/littlebaby-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho LittleBaby integration guide](https://docs.honcho.dev/v3/guides/integrations/littlebaby)
- [Memory](/concepts/memory) -- LittleBaby memory overview
