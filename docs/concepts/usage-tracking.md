...
- `/usage cost` in chats: local cost summary aggregated from LittleBaby session logs.
- CLI: `littlebaby status --usage` prints a full per-provider breakdown.
- CLI: `littlebaby channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. LittleBaby treats
  ...
  quota, so LittleBaby inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, LittleBaby prefers the
...
can supply plugin-specific usage auth logic; otherwise LittleBaby falls back to
