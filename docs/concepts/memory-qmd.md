...
alongside LittleBaby. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, LittleBaby falls back to the
...
LittleBaby creates a self-contained QMD home under
`~/.littlebaby/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- LittleBaby creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). If a mode fails, LittleBaby retries with `qmd query`.
- If QMD fails entirely, LittleBaby falls back to the builtin SQLite engine.
...
process, so you can tune QMD globally without adding new LittleBaby config:
...
collection under `~/.littlebaby/agents/<id>/qmd/sessions/`.
...
When scope denies a search, LittleBaby logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If LittleBaby
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs LittleBaby uses.
...
LittleBaby's builtin symlink rules. Keep temporary monorepo checkouts under
