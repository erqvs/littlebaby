...
LittleBaby indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.littlebaby/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `littlebaby memory index --force`
...
**Memory search disabled?** Check `littlebaby memory status`. If no provider is
detected, set one explicitly or add an API key.

**Stale results?** Run `littlebaby memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** LittleBaby falls back to in-process cosine similarity
