...
- **Session store** (chat history + routing state) under `~/.littlebaby/agents/<agentId>/sessions`.
...
`~/.littlebaby/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.littlebaby` (or `LITTLEBABY_STATE_DIR`)
- Workspace: `~/.littlebaby/workspace` (or `~/.littlebaby/workspace-<agentId>`)
- Agent dir: `~/.littlebaby/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.littlebaby/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, LittleBaby runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.littlebaby/workspace` (or `~/.littlebaby/workspace-<profile>` when `LITTLEBABY_PROFILE` is set).
- State defaults to `~/.littlebaby/agents/main/agent`.
...
```bash
littlebaby agents add coding
littlebaby agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.littlebaby/agents/<agentId>`.
...
```bash
littlebaby channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.littlebaby/workspace-alex" },
      { id: "mia", workspace: "~/.littlebaby/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, LittleBaby upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, LittleBaby falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.littlebaby/workspace-main" },
      { id: "coding", workspace: "~/.littlebaby/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.littlebaby/workspace-main" },
      { id: "alerts", workspace: "~/.littlebaby/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
littlebaby channels login --channel whatsapp --account personal
littlebaby channels login --channel whatsapp --account biz
```

`~/.littlebaby/littlebaby.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.littlebaby/workspace-home",
        agentDir: "~/.littlebaby/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.littlebaby/workspace-work",
        agentDir: "~/.littlebaby/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.littlebaby/credentials/whatsapp/personal
          // authDir: "~/.littlebaby/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.littlebaby/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.littlebaby/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.littlebaby/workspace-opus",
