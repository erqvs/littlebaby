...
read_when:
  - You want to understand how LittleBaby assembles model context
...
A **context engine** controls how LittleBaby builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

LittleBaby ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.littlebaby/littlebaby.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other LittleBaby plugin. Install
...
Every time LittleBaby runs a model prompt, the context engine participates at
...
LittleBaby currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. LittleBaby
...
The built-in `legacy` engine preserves LittleBaby's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. LittleBaby uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. LittleBaby disables Pi's built-in
...
`ownsCompaction: false` does **not** mean LittleBaby automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `littlebaby/plugin-sdk/core` to use
  LittleBaby's built-in compaction behavior.
...
LittleBaby resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to LittleBaby's built-in summarization. Plugin engines can implement
...
- **Memory plugins** (`plugins.slots.memory`) are separate from context engines.
  ...
  `littlebaby/plugin-sdk/core`, which converts the active memory prompt sections
  into a ready-to-prepend `systemPromptAddition`. If an engine needs lower-level
  control, it can still pull raw lines from
  `littlebaby/plugin-sdk/memory-host-core` via
...
- Use `littlebaby doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, LittleBaby
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `littlebaby plugins install -l ./my-engine` to link a
