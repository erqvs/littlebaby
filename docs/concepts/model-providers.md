...
- CLI helpers: `littlebaby onboard`, `littlebaby models list`, `littlebaby models set <provider/model>`.
...
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  LittleBaby merges that output into `models.providers` before writing
  `models.json`.
...
- The bundled `codex` provider is paired with the bundled Codex agent harness.
  ...
  to use the OpenAI provider and the normal LittleBaby provider transport.
...
Provider plugins can now own most provider-specific logic while LittleBaby keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `littlebaby onboard`, `littlebaby models auth`, and headless setup
...
- `normalizeTransport`: provider normalizes transport-family `api` / `baseUrl`
  before generic model assembly; LittleBaby checks the matched provider first,
...
- `normalizeConfig`: provider normalizes `models.providers.<id>` config before
  runtime uses it; LittleBaby checks the matched provider first, then other
...
That covers providers that still fit LittleBaby's normal transports. A provider
...
- Configure multiple keys via:
  - `LITTLEBABY_LIVE_<PROVIDER>_KEY` (single live override, highest priority)
...
LittleBaby ships with the pi‑ai catalog. These providers require **no**
...
- Provider: `openai`
- Auth: `OPENAI_API_KEY`
- Optional rotation: `OPENAI_API_KEYS`, `OPENAI_API_KEY_1`, `OPENAI_API_KEY_2`, plus `LITTLEBABY_LIVE_OPENAI_KEY` (single override)
- Example models: `openai/gpt-5.4`, `openai/gpt-5.4-pro`
- CLI: `littlebaby onboard --auth-choice openai-api-key`
...
- Hidden LittleBaby attribution headers (`originator`, `version`,
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in LittleBaby because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- Optional rotation: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, plus `LITTLEBABY_LIVE_ANTHROPIC_KEY` (single override)
- Example model: `anthropic/claude-opus-4-6`
- CLI: `littlebaby onboard --auth-choice apiKey`
- Direct public Anthropic requests support the shared `/fast` toggle and `params.fastMode`, including API-key and OAuth-authenticated traffic sent to `api.anthropic.com`; LittleBaby maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
- Anthropic setup-token remains available as a supported LittleBaby token path, but LittleBaby now prefers Claude CLI reuse and `claude -p` when available.
...
- CLI: `littlebaby onboard --auth-choice openai-codex` or `littlebaby models auth login --provider openai-codex`
- Default transport is `auto` (WebSocket-first, SSE fallback)
...
- Shares the same `/fast` toggle and `params.fastMode` config as direct `openai/*`; LittleBaby maps that to `service_tier=priority`
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like LittleBaby.
...
- CLI: `littlebaby onboard --auth-choice opencode-zen` or `littlebaby onboard --auth-choice opencode-go`
...
- Provider: `google`
- Auth: `GEMINI_API_KEY`
- Optional rotation: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` fallback, and `LITTLEBABY_LIVE_GEMINI_KEY` (single override)
- Example models: `google/gemini-3.1-pro-preview`, `google/gemini-3-flash-preview`
- Compatibility: legacy LittleBaby config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- CLI: `littlebaby onboard --auth-choice gemini-api-key`
- Direct Gemini runs also accept `agents.defaults.models["google/<model>"].params.cachedContent`
  ...
  `cachedContents/...` handle; Gemini cache hits surface as LittleBaby `cacheRead`

### Google Vertex and Gemini CLI

- Providers: `google-vertex`, `google-gemini-cli`
- Auth: Vertex uses gcloud ADC; Gemini CLI uses its OAuth flow
- Caution: Gemini CLI OAuth in LittleBaby is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Install Gemini CLI first:
    - `brew install gemini-cli`
    - or `npm install -g @google/gemini-cli`
  - Enable: `littlebaby plugins enable google`
  - Login: `littlebaby models auth login --provider google-gemini-cli --set-default`
  - Default model: `google-gemini-cli/gemini-3-flash-preview`
  - Note: you do **not** paste a client id or secret into `littlebaby.json`. The CLI login flow stores
  ...
  - Gemini CLI JSON replies are parsed from `response`; usage falls back to
    `stats`, with `stats.cached` normalized into LittleBaby `cacheRead`.
...
- CLI: `littlebaby onboard --auth-choice zai-api-key`
...
- CLI: `littlebaby onboard --auth-choice ai-gateway-api-key`
...
- CLI: `littlebaby onboard --auth-choice kilocode-api-key`
...
- LittleBaby applies OpenRouter's documented app-attribution headers only when
...
- CLI: `littlebaby onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `littlebaby onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
- Example model: `moonshot/kimi-k2.5`
- CLI: `littlebaby onboard --auth-choice moonshot-api-key` or `littlebaby onboard --auth-choice moonshot-api-key-cn`
...
- Example model: `volcengine-plan/ark-code-latest`
- CLI: `littlebaby onboard --auth-choice volcengine-api-key`
...
LittleBaby falls back to the unfiltered catalog instead of showing an empty
...
- Example model: `byteplus-plan/ark-code-latest`
- CLI: `littlebaby onboard --auth-choice byteplus-api-key`
...
- CLI: `littlebaby onboard --auth-choice synthetic-api-key`
...
On MiniMax's Anthropic-compatible streaming path, LittleBaby disables thinking by
...
LittleBaby uses LM Studio's native `/api/v1/models` and `/api/v1/models/load`
...
`littlebaby onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional.
  When omitted, LittleBaby defaults to:
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
  - `contextWindow: 200000`
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), LittleBaby forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- Proxy-style OpenAI-compatible routes also skip native OpenAI-only request
  ...
  OpenAI reasoning-compat payload shaping, and no hidden LittleBaby attribution
  headers.
- If `baseUrl` is empty/omitted, LittleBaby keeps the default OpenAI behavior (which resolves to `api.openai.com`).
- For safety, an explicit `compat.supportsDeveloperRole: true` is still overridden on non-native `openai-completions` endpoints.

## CLI examples

```bash
littlebaby onboard --auth-choice opencode-zen
littlebaby models set opencode/claude-opus-4-6
