---
summary: "Delegate architecture: running LittleBaby as a named agent on behalf of an organization"
...
Goal: run LittleBaby as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an LittleBaby agent that:
...
LittleBaby's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of LittleBaby's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.littlebaby/workspace-delegate",
  tools: {
...
- Cron run history: `~/.littlebaby/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.littlebaby/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through LittleBaby's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
littlebaby agents add delegate
```

This creates:

- Workspace: `~/.littlebaby/workspace-delegate`
- State: `~/.littlebaby/agents/delegate/agent`
- Sessions: `~/.littlebaby/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.littlebaby/workspace" },
      {
        id: "delegate",
        workspace: "~/.littlebaby/workspace-delegate",
        tools: {
...
~/.littlebaby/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.littlebaby/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.littlebaby/workspace-org",
        agentDir: "~/.littlebaby/agents/org-assistant/agent",
...
recall view. LittleBaby redacts credential/token-like text, truncates long
