...
service via the external `littlebaby` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.littlebaby.gateway`
  (or `ai.littlebaby.<profile>` when using `--profile`/`LITTLEBABY_PROFILE`; legacy `com.littlebaby.*` is supported).
