...
- Same path: run the app from a fixed location (for LittleBaby, `dist/LittleBaby.app`).
...
```bash
sudo tccutil reset Accessibility ai.littlebaby.mac
sudo tccutil reset ScreenCapture ai.littlebaby.mac
...
Workaround: move files into the LittleBaby workspace (`~/.littlebaby/workspace`) if you want to avoid per-folder grants.
