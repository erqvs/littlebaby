---
summary: "LittleBaby on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up LittleBaby on DigitalOcean
  - Looking for cheap VPS hosting for LittleBaby
title: "DigitalOcean (Platform)"
---

# LittleBaby on DigitalOcean

## Goal

Run a persistent LittleBaby Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
littlebaby config set gateway.bind tailnet
littlebaby gateway restart
```
...
```bash
littlebaby channels login whatsapp
# Scan QR code
```
...
- `~/.littlebaby/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u littlebaby-gateway.service --no-pager -n 50
```
