...
> **Note:** The Android app has not been publicly released yet. The source code is available in the [LittleBaby repository](https://github.com/littlebaby/littlebaby) under `apps/android`. You can build it yourself using Java 17 and the Android SDK (`./gradlew :app:assemblePlayDebug`). See [apps/android/README.md](https://github.com/littlebaby/littlebaby/blob/main/apps/android/README.md) for build instructions.
...
- You can run the CLI (`littlebaby`) on the gateway machine (or via SSH).
...
```bash
dns-sd -B _littlebaby-gw._tcp local.
```
...
```bash
littlebaby gateway discover --json
```
...
1. Set up a DNS-SD zone (example `littlebaby.internal.`) on the gateway host and publish `_littlebaby-gw._tcp` records.
...
1. Create `~/.littlebaby/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
littlebaby nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__littlebaby__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__littlebaby__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__littlebaby__/a2ui/`.
...
Android supports launching LittleBaby from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
LittleBaby..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set LittleBaby as the default assistant app.
</Note>
