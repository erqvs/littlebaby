...
2. `npm i -g littlebaby@latest`
3. `littlebaby onboard --install-daemon`
...
LittleBaby installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `littlebaby gateway install` and
`littlebaby onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/littlebaby-gateway[-<profile>].service`:

```
[Unit]
Description=LittleBaby Gateway (profile: <profile>, v<version>)
