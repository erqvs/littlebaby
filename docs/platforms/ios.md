...
- `LITTLEBABY_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
```bash
export LITTLEBABY_APNS_TEAM_ID="TEAMID"
export LITTLEBABY_APNS_KEY_ID="KEYID"
export LITTLEBABY_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.littlebaby/credentials/apns
chmod 700 ~/.littlebaby/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.littlebaby/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.littlebaby/credentials/apns/AuthKey_KEYID.p8
export LITTLEBABY_APNS_PRIVATE_KEY_PATH="$HOME/.littlebaby/credentials/apns/AuthKey_KEYID.p8"
```
