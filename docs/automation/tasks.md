...
```
$LITTLEBABY_STATE_DIR/tasks/runs.sqlite
```
...
A cron job **definition** lives in `~/.littlebaby/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
