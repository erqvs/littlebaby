...
LittleBaby agents can generate videos from text prompts, reference images, or existing videos. Fourteen provider backends are supported, each with different model options, input modes, and feature sets. The agent picks the right provider automatically based on your configuration and available API keys.
...
LittleBaby treats video generation as three runtime modes:
...
```bash
littlebaby config set agents.defaults.videoGenerationModel.primary "google/veo-3.1-fast-generate-preview"
```
...
1. LittleBaby submits the request to the provider and immediately returns a task ID.
...
3. When the video is ready, LittleBaby wakes the same session with an internal completion event.
4. The agent posts the finished video back into the original conversation.

While a job is in flight, duplicate `video_generate` calls in the same session return the current task status instead of starting another generation. Use `littlebaby tasks list` or `littlebaby tasks show <taskId>` to check progress from the CLI.
...
littlebaby tasks show <taskId>
littlebaby tasks cancel <taskId>
```
...
Not all providers support all parameters. LittleBaby already normalizes duration to the closest provider-supported value, and it also remaps translated geometry hints such as size-to-aspect-ratio when a fallback provider exposes a different control surface. Truly unsupported overrides are ignored on a best-effort basis and reported as warnings in the tool result. Hard capability limits (such as too many reference inputs) fail before submission.

Tool results report the applied settings. When LittleBaby remaps duration or geometry during provider fallback, the returned `durationSeconds`, `size`, `aspectRatio`, and `resolution` values reflect what was submitted, and `details.normalization` captures the requested-to-applied translation.
...
When generating a video, LittleBaby resolves the model in this order:
...
```bash
LITTLEBABY_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- per-provider operation cap from `LITTLEBABY_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default)
...
Set `LITTLEBABY_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared transform
...
Set the default video generation model in your LittleBaby config:
...
```bash
littlebaby config set agents.defaults.videoGenerationModel.primary "qwen/wan2.6-t2v"
```
