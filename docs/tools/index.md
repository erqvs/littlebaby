---
summary: "LittleBaby tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools LittleBaby provides
...
LittleBaby has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). LittleBaby ships a set of **built-in tools** and
  ...
  <Step title="Plugins package everything together">
    ...
    LittleBaby), others are **external** (published on npm by the community).
...
These tools ship with LittleBaby and are available without installing any plugins:
...
| `group:littlebaby`   | All built-in LittleBaby tools (excludes plugin tools)                                                       |
