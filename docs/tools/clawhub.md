...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/openclaw/skills` and `~/.littlebaby/skills`). This is designed to find older skill installs without extra flags.
