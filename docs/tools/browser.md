...
Set `browser.defaultProfile: "littlebaby"` if you want managed mode by default.
...
- If you override the Gateway port (`gateway.port` or `LITTLEBABY_GATEWAY_PORT`),
...
`LITTLEBABY_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "littlebaby" is not reachable after start`
