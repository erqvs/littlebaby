...
read_when:
  - Running LittleBaby Gateway in WSL2 while Chrome lives on Windows
...
- LittleBaby Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/littlebaby/littlebaby/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- LittleBaby and Chrome are on the same machine
...
- LittleBaby points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, LittleBaby is not the problem yet.
...
Fix that before touching LittleBaby config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point LittleBaby at the address that is reachable from WSL2:
...
- use HTTP(S) when you want LittleBaby to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting LittleBaby to succeed
...
```bash
littlebaby browser open https://example.com --browser-profile remote
littlebaby browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `littlebaby browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `littlebaby browser stop --browser-profile remote`
...
3. LittleBaby config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug LittleBaby config or Control UI auth
