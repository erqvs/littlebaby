---
summary: "Install, configure, and manage LittleBaby plugins"
read_when:
...
Plugins extend LittleBaby with new capabilities: channels, model providers,
...
search, and more. Some plugins are **core** (shipped with LittleBaby), others
...
<Steps>
  ...
  <Step title="Install a plugin">
    ...
    littlebaby plugins install ./my-plugin
    littlebaby plugins install ./my-plugin.tgz
    ```
...
`littlebaby.install.allowInvalidConfigRecovery`.

## Plugin types

LittleBaby recognizes two plugin formats:

| Format     | How it works                                                       | Examples                                               |
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `littlebaby.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to LittleBaby features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `littlebaby plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
| Plugin          | Package                | Docs                                 |
| --------------- | ---------------------- | ------------------------------------ |
| Matrix          | `@littlebaby/matrix`     | [Matrix](/channels/matrix)           |
| Microsoft Teams | `@littlebaby/msteams`    | [Microsoft Teams](/channels/msteams) |
| Nostr           | `@littlebaby/nostr`      | [Nostr](/channels/nostr)             |
| Voice Call      | `@littlebaby/voice-call` | [Voice Call](/plugins/voice-call)    |
| Zalo            | `@littlebaby/zalo`       | [Zalo](/channels/zalo)               |
| Zalo Personal   | `@littlebaby/zalouser`   | [Zalo Personal](/plugins/zalouser)   |

### Core (shipped with LittleBaby)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` — bundled browser plugin for the browser tool, `littlebaby browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
watch + in-process restart enabled (the default `littlebaby gateway` path), that
...
LittleBaby scans for plugins in this order (first match wins):
...
<Steps>
  ...
  <Step title="Workspace extensions">
    `\<workspace\>/.littlebaby/<plugin-root>/*.ts` and `\<workspace\>/.littlebaby/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global extensions">
    `~/.littlebaby/<plugin-root>/*.ts` and `~/.littlebaby/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with LittleBaby. Many are enabled by default (model providers, speech).
...
```bash
littlebaby plugins list                       # compact inventory
littlebaby plugins list --enabled            # only loaded plugins
littlebaby plugins list --verbose            # per-plugin detail lines
littlebaby plugins list --json               # machine-readable inventory
littlebaby plugins inspect <id>              # deep detail
littlebaby plugins inspect <id> --json       # machine-readable
littlebaby plugins inspect --all             # fleet-wide table
...
littlebaby plugins install <plugin> --marketplace <source>
littlebaby plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
littlebaby plugins install <spec> --pin      # record exact resolved npm spec
littlebaby plugins install <spec> --dangerously-force-unsafe-install
littlebaby plugins update <id>             # update one plugin
littlebaby plugins update <id> --dangerously-force-unsafe-install
littlebaby plugins update --all            # update all
littlebaby plugins uninstall <id>          # remove config/install records
littlebaby plugins uninstall <id> --keep-files
littlebaby plugins marketplace list <source>
littlebaby plugins marketplace list <source> --json

littlebaby plugins enable <id>
littlebaby plugins disable <id>
```

Bundled plugins ship with LittleBaby. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `littlebaby plugins enable <id>`.
...
`littlebaby plugins inspect <id>` also reports detected bundle capabilities plus
...
See [`littlebaby plugins` CLI reference](/cli/plugins) for full details.
...
LittleBaby loads the entry object and calls `register(api)` during plugin
