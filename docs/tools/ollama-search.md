...
- an Ollama host that is reachable from LittleBaby
- `ollama signin`
...
If no explicit Ollama base URL is set, LittleBaby uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, LittleBaby reuses
...
- If the Ollama host is auth-protected, LittleBaby reuses the normal Ollama
  provider API key when present.
- LittleBaby warns during setup if Ollama is unreachable or not signed in, but
