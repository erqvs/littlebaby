...
`ask: "always"` in `~/.littlebaby/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `littlebaby approvals get`, `littlebaby approvals get --gateway`, or
`littlebaby approvals get --node <id|name|ip>` to inspect the requested policy,
host policy sources, and the effective result.
For the local machine, `littlebaby exec-policy show` exposes the same merged view and
`littlebaby exec-policy set|preset` can synchronize the local requested policy with the
local host approvals file in one step. When a local scope requests `host=node`,
`littlebaby exec-policy show` reports that scope as node-managed at runtime instead of
...
- **gateway host** → `littlebaby` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, LittleBaby also tries to bind
...
`~/.littlebaby/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.littlebaby/exec-approvals.sock",
...
- requested exec policy in LittleBaby config (`tools.exec.*`)
- host-local approvals policy in `~/.littlebaby/exec-approvals.json`
...
- In YOLO mode, LittleBaby does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
```bash
littlebaby config set tools.exec.host gateway
littlebaby config set tools.exec.security full
littlebaby config set tools.exec.ask off
...
```bash
littlebaby approvals set --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
```bash
littlebaby exec-policy preset yolo
```

That local shortcut updates both:

- local `tools.exec.host/security/ask`
- local `~/.littlebaby/exec-approvals.json` defaults

It is intentionally local-only. If you need to change gateway-host or node-host approvals
remotely, continue using `littlebaby approvals set --gateway` or
`littlebaby approvals set --node <id|name|ip>`.

For a node host, apply the same approvals file on that node instead:

```bash
littlebaby approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `littlebaby exec-policy` does not synchronize node approvals
- `littlebaby exec-policy set --host node` is rejected
- node exec approvals are fetched from the node at runtime, so node-targeted updates must use `littlebaby approvals --node ...`
...
When `tools.exec.strictInlineEval=true`, LittleBaby treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.littlebaby/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `littlebaby approvals allowlist ...`).
- `littlebaby security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `littlebaby doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, LittleBaby still rejects the `env` builtin in safe-bin
...
`~/.littlebaby/exec-approvals.json` directly.

CLI: `littlebaby approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, LittleBaby sends a followup `agent` turn to the same session.
...
topics, LittleBaby preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, LittleBaby prevents the agent from reusing
