...
LittleBaby supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), LittleBaby reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.littlebaby/.env`.

If you omit `baseUrl`, LittleBaby defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, LittleBaby defaults to `kimi-k2.5`.
