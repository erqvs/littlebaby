...
LittleBaby can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `littlebaby configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `littlebaby doctor --fix`.
...
LittleBaby always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, LittleBaby
