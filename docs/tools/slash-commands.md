...
- `commands.config` (default `false`) enables `/config` (reads/writes `littlebaby.json`).
- `commands.mcp` (default `false`) enables `/mcp` (reads/writes LittleBaby-managed MCP config under `mcp.servers`).
...
- `/config show|get|set|unset` reads or writes `littlebaby.json`. Owner-only. Requires `commands.config: true`.
- `/mcp show|get|set|unset` reads or writes LittleBaby-managed MCP server config under `mcp.servers`. Owner-only. Requires `commands.mcp: true`.
...
- `/restart` restarts LittleBaby when enabled. Default: enabled; set `commands.restart: false` to disable it.
...
- For full provider usage breakdown, use `littlebaby status --usage`.
...
- Overrides apply immediately to new config reads, but do **not** write to `littlebaby.json`.
...
`/config` writes to your on-disk config (`littlebaby.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[littlebaby]"
...
`/mcp` writes LittleBaby-managed MCP server definitions under `mcp.servers`. Owner-only. Disabled by default; enable with `commands.mcp: true`.
...
- `/mcp` stores config in LittleBaby config, not Pi-owned project settings.
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/littlebaby`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
