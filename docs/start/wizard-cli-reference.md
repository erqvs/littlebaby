...
read_when:
  - You need detailed behavior for littlebaby onboard
...
This page is the full reference for `littlebaby onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.littlebaby/littlebaby.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `littlebaby doctor` before continuing.
  ...
  <Step title="Workspace">
    - Default `~/.littlebaby/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `littlebaby pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Daemon install">
    ...
    - Native Windows: Scheduled Task first
      - If task creation is denied, LittleBaby falls back to a per-user Startup-folder login item and starts the gateway immediately.
      - Scheduled Tasks remain preferred because they provide better supervisor status.
    - Runtime selection: Node (recommended; required for WhatsApp and Telegram). Bun is not recommended.
  </Step>
  <Step title="Health check">
    - Starts gateway (if needed) and runs `littlebaby health`.
    - `littlebaby status --deep` adds the live gateway health probe to status output, including channel probes when supported.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI Code subscription (Codex CLI reuse)">
    ...
    Reused Codex CLI credentials stay managed by Codex CLI; on expiry LittleBaby
...
- Auth profiles (API keys + OAuth): `~/.littlebaby/agents/<agentId>/agent/auth-profiles.json`
...
`~/.littlebaby/agents/<agentId>/agent/auth-profiles.json`, or the matching
`$LITTLEBABY_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
...
- Stores it under `~/.littlebaby/tools/signal-cli/<version>/`
...
- Command reference: [`littlebaby onboard`](/cli/onboard)
