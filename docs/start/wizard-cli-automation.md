---
summary: "Scripted onboarding and agent setup for the LittleBaby CLI"
read_when:
...
Use `--non-interactive` to automate `littlebaby onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but LittleBaby now prefers Claude CLI reuse when available.
