...
CLI onboarding is the **recommended** way to set up LittleBaby on macOS,
...
`littlebaby dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`littlebaby configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.littlebaby/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `littlebaby doctor` first.
</Note>
...
Use `littlebaby agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.littlebaby/workspace-<agentId>`.
...
- CLI command reference: [`littlebaby onboard`](/cli/onboard)
