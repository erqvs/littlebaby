---
summary: "Get LittleBaby installed and run your first chat in minutes."
read_when:
...
Install LittleBaby, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.littlebaby/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.littlebaby/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run LittleBaby as a service account or want custom paths:

- `LITTLEBABY_HOME` — home directory for internal path resolution
- `LITTLEBABY_STATE_DIR` — override the state directory
- `LITTLEBABY_CONFIG_PATH` — override the config file path
