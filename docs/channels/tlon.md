...
Tlon is a decentralized messenger built on Urbit. LittleBaby connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current LittleBaby releases, so normal packaged
...
```bash
littlebaby plugins install @littlebaby/tlon
```

Local checkout (when running from a git repo):

```bash
littlebaby plugins install ./path/to/local/tlon-plugin
```
...
By default, LittleBaby blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `littlebaby message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, LittleBaby replies in-thread.
