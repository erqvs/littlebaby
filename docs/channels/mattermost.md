---
summary: "Mattermost bot setup and LittleBaby config"
read_when:
...
Mattermost ships as a bundled plugin in current LittleBaby releases, so normal
...
```bash
littlebaby plugins install @littlebaby/mattermost
```

Local checkout (when running from a git repo):

```bash
littlebaby plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure LittleBaby and start the gateway.
...
Native slash commands are opt-in. When enabled, LittleBaby registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, LittleBaby derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when LittleBaby registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as LittleBaby.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to LittleBaby.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from LittleBaby, not `404`.
...
- Approve via:
  - `littlebaby pairing list mattermost`
  - `littlebaby pairing approve mattermost <CODE>`
...
Use these target formats with `littlebaby message send` or cron/webhooks:
...
LittleBaby resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), LittleBaby sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When LittleBaby sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, LittleBaby derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and LittleBaby run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="littlebaby-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"littlebaby-mattermost-interactions",
...
`littlebaby message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: LittleBaby
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as LittleBaby. Set an
