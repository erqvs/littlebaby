...
Nextcloud Talk ships as a bundled plugin in current LittleBaby releases, so
...
```bash
littlebaby plugins install @littlebaby/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
littlebaby plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "LittleBaby" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `littlebaby pairing list nextcloud-talk`
  - `littlebaby pairing approve nextcloud-talk <CODE>`
