...
read_when:
  - You want LittleBaby to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables LittleBaby to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current LittleBaby releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`littlebaby onboard`) and `littlebaby channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
littlebaby plugins install @littlebaby/nostr
```

Use a local checkout (dev workflows):

```bash
littlebaby plugins install --link <path-to-local-nostr-plugin>
```
...
littlebaby channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "littlebaby@example.com",
        lud16: "littlebaby@example.com",
      },
    },
  },
}
