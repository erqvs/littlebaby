...
```
    ...
    Telegram does **not** use `littlebaby channels login telegram`; configure token in config/env, then start gateway.
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `littlebaby doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `littlebaby doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `littlebaby logs --follow`
...
- DM messages can carry `message_thread_id`; LittleBaby routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    LittleBaby can stream partial replies in real time:
    ...
    - DM: LittleBaby keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: LittleBaby keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), LittleBaby falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, LittleBaby skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, LittleBaby automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, LittleBaby retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - LittleBaby pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.littlebaby/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, LittleBaby enqueues system events like:
...
```bash
littlebaby message send --channel telegram --target 123456789 --message "hi"
littlebaby message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `littlebaby message poll` and support forum topics:

```bash
littlebaby message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
littlebaby message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, LittleBaby preserves the topic for both the approval prompt and the post-approval follow-up. Exec approvals expire after 30 minutes by default.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `littlebaby channels status` warns when config expects unmentioned group messages.
    - `littlebaby channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `littlebaby logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, LittleBaby now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `LITTLEBABY_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `LITTLEBABY_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `LITTLEBABY_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `littlebaby doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `littlebaby doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, LittleBaby falls back to the first normalized account ID and `littlebaby doctor` warns.
