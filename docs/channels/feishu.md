...
> **Requires LittleBaby 2026.4.10 or above.** Run `littlebaby --version` to check. Upgrade with `littlebaby update`.

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  littlebaby channels login --channel feishu
  ```
...
```bash
littlebaby pairing list feishu
littlebaby pairing approve feishu <CODE>
```
...
4. Check logs: `littlebaby logs --follow`
...
5. Ensure the gateway is running: `littlebaby gateway status`
6. Check logs: `littlebaby logs --follow`

### App Secret leaked

1. Reset the App Secret in Feishu Open Platform / Lark Developer
2. Update the value in your config
3. Restart the gateway: `littlebaby gateway restart`
