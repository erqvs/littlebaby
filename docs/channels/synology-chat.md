---
summary: "Synology Chat webhook setup and LittleBaby config"
read_when:
  - Setting up Synology Chat with LittleBaby
...
Synology Chat ships as a bundled plugin in current LittleBaby releases, so normal
...
```bash
littlebaby plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `littlebaby onboard` now shows Synology Chat in the same channel setup list as `littlebaby channels add`.
   - Non-interactive setup: `littlebaby channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your LittleBaby gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in LittleBaby.
   - Guided: `littlebaby onboard`
   - Direct: `littlebaby channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- LittleBaby accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `littlebaby pairing list synology-chat`
  - `littlebaby pairing approve synology-chat <CODE>`
...
```bash
littlebaby message send --channel synology-chat --target 123456 --text "Hello from LittleBaby"
littlebaby message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. LittleBaby now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached LittleBaby
- `Rate limit exceeded`:
