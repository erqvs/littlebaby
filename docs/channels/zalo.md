...
Zalo ships as a bundled plugin in current LittleBaby releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `littlebaby plugins install @littlebaby/zalo`
- Or from a source checkout: `littlebaby plugins install ./path/to/local/zalo-plugin`
...
This page reflects current LittleBaby behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `littlebaby pairing list zalo`
  - `littlebaby pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in LittleBaby.
...
- Example: `littlebaby message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `littlebaby channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `littlebaby logs --follow`
