...
read_when:
  - You want to connect LittleBaby to LINE
...
LINE connects to LittleBaby via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current LittleBaby releases, so normal
...
```bash
littlebaby plugins install @littlebaby/line
```

Local checkout (when running from a git repo):

```bash
littlebaby plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so LittleBaby applies strict pre-auth body limits and timeout before verification.
- LittleBaby processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
littlebaby pairing list line
littlebaby pairing approve line <CODE>
```
