...
read_when:
  - Setting up Zalo Personal for LittleBaby
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside LittleBaby.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current LittleBaby releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `littlebaby plugins install @littlebaby/zalouser`
- Or from a source checkout: `littlebaby plugins install ./path/to/local/zalouser-plugin`
...
- `littlebaby pairing list zalouser`
- `littlebaby pairing approve zalouser <code>`
...
- On startup, LittleBaby resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, LittleBaby stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in LittleBaby state. Example:
...
- LittleBaby sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, LittleBaby sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `littlebaby channels logout --channel zalouser && littlebaby channels login --channel zalouser`
...
- The channel now runs fully in LittleBaby without external CLI binaries.
