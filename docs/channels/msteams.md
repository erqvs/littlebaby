...
Microsoft Teams ships as a bundled plugin in current LittleBaby releases, so no
...
```bash
littlebaby plugins install @littlebaby/msteams
```

Local checkout (when running from a git repo):

```bash
littlebaby plugins install ./path/to/local/msteams-plugin
```
...
3. Configure LittleBaby with those credentials.
...
- Talk to LittleBaby via Teams DMs, group chats, or channels.
...
- On startup, LittleBaby resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.littlebaby/littlebaby.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring LittleBaby, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `littlebaby-msteams` (must be unique) |
...
For production deployments, LittleBaby supports **federated authentication** as a more secure alternative to client secrets. Two methods are available:
...
3. At runtime, LittleBaby uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint (`169.254.169.254`).
...
**Default behavior:** When `authType` is not set, LittleBaby defaults to client secret authentication. Existing configurations continue to work without changes.
...
1. **Ensure the Microsoft Teams plugin is available**
   ...
   - Older/custom installs can add it manually:
     - From npm: `littlebaby plugins install @littlebaby/msteams`
     - From a local checkout: `littlebaby plugins install ./path/to/local/msteams-plugin`
...
LittleBaby exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "LittleBaby in Teams", full: "LittleBaby in Teams" },
...
LittleBaby handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, LittleBaby only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/LittleBabyShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

LittleBaby sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `littlebaby message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.littlebaby/msteams-polls.json`.
...
```bash
littlebaby message send --channel msteams \
...
littlebaby message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
littlebaby message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
littlebaby message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
littlebaby message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
