...
LittleBaby routes replies **back to the channel where a message came from**. The
...
LittleBaby infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, LittleBaby still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when LittleBaby would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.littlebaby/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.littlebaby`):

- `~/.littlebaby/agents/<agentId>/sessions/sessions.json`
