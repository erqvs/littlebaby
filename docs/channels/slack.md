...
```json
{
  "display_information": {
    "name": "LittleBaby",
    "description": "Slack connector for LittleBaby"
  },
  "features": {
    "bot_user": {
      "display_name": "LittleBaby",
      "always_online": true
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/littlebaby",
        "description": "Send a message to LittleBaby",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `littlebaby pairing approve slack <code>`.
...
- If streaming fails mid-reply, LittleBaby falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while LittleBaby is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are LittleBaby-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, LittleBaby falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `littlebaby channels status --probe --json` shows `botTokenStatus` or
