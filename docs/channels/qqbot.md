...
read_when:
  - You want to connect LittleBaby to QQ
...
QQ Bot connects to LittleBaby via the official QQ Bot API (WebSocket gateway). The
...
Current LittleBaby releases bundle QQ Bot, so normal packaged builds do not need
a separate `littlebaby plugins install` step.
...
```bash
littlebaby channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `littlebaby channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single LittleBaby instance:
...
```bash
littlebaby channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
