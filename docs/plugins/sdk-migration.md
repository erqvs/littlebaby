...
read_when:
  - You see the LITTLEBABY_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the LITTLEBABY_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external LittleBaby plugin
---

# Plugin SDK Migration

LittleBaby has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`littlebaby/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`littlebaby/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`littlebaby/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `littlebaby/plugin-sdk/slack`, `littlebaby/plugin-sdk/discord`,
`littlebaby/plugin-sdk/signal`, `littlebaby/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`littlebaby/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
<Steps>
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `littlebaby/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `littlebaby/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "littlebaby/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "littlebaby/plugin-sdk/compat";
    ...
    import { runEmbeddedPiAgent } from "littlebaby/extension-api";
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `LittleBabySchema` |
...
```bash
LITTLEBABY_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 littlebaby gateway run
LITTLEBABY_SUPPRESS_EXTENSION_API_WARNING=1 littlebaby gateway run
```
