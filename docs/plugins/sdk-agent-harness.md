...
an explicit `runtime: "pi"` or `LITTLEBABY_AGENT_RUNTIME=pi`.
...
`LITTLEBABY_AGENT_RUNTIME` still overrides the configured runtime. Use
`LITTLEBABY_AGENT_HARNESS_FALLBACK=none` to disable PI fallback from the
environment.
