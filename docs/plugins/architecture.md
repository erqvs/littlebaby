...
read_when:
  - Building or debugging native LittleBaby plugins
...
This page covers the internal architecture of the LittleBaby plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside LittleBaby. Every
native LittleBaby plugin registers against one or more capability types:
...
LittleBaby classifies every loaded plugin into a shape based on its actual
...
Use `littlebaby plugins inspect <id>` to see a plugin's shape and capability
...
When you run `littlebaby doctor` or `littlebaby plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `littlebaby status --all` and `littlebaby plugins doctor`.

## Architecture overview

LittleBaby's plugin system has four layers:

1. **Manifest + discovery**
   LittleBaby finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `littlebaby.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native LittleBaby plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of LittleBaby reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting LittleBaby
...
That split lets LittleBaby validate config, explain missing/disabled plugins, and
...
normal chat actions. LittleBaby keeps one shared `message` tool in core, and
...
LittleBaby treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's LittleBaby-facing
  surfaces
...
So if LittleBaby adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If LittleBaby has shared
...
```ts
import type { LittleBabyPluginDefinition } from "littlebaby/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "littlebaby/plugin-sdk/media-understanding";

const plugin: LittleBabyPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
LittleBaby already treats image/audio/video understanding as one shared
...
`LittleBabyPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   LittleBaby can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that LittleBaby knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `LittleBabyPluginApi` or
  `api.runtime`
...
Native LittleBaby plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the LittleBaby process

Compatible bundles are safer by default because LittleBaby currently treats them
...
name: `@littlebaby/<id>` by default, or an approved typed suffix such as
...
LittleBaby exports capabilities, not implementation convenience.
...
At startup, LittleBaby does roughly this:
...
The manifest is the control-plane source of truth. LittleBaby uses it to:
...
LittleBaby keeps short in-process caches for:
...
- Set `LITTLEBABY_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `LITTLEBABY_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `LITTLEBABY_PLUGIN_DISCOVERY_CACHE_MS` and
  `LITTLEBABY_PLUGIN_MANIFEST_CACHE_MS`.
...
LittleBaby still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, LittleBaby calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | LittleBaby tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                       |
...
that still runs on LittleBaby's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before LittleBaby's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as LittleBaby adds those
...
- LittleBaby only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "LittleBaby plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-littlebaby-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-littlebaby-scopes` only when the header is explicitly present
  - if `x-littlebaby-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-littlebaby-scopes` header contract.

## Plugin SDK import paths

Use SDK subpaths instead of the monolithic `littlebaby/plugin-sdk` import when
authoring plugins:

- `littlebaby/plugin-sdk/plugin-entry` for plugin registration primitives.
- `littlebaby/plugin-sdk/core` for the generic shared plugin-facing contract.
- `littlebaby/plugin-sdk/config-schema` for the root `littlebaby.json` Zod schema
  export (`LittleBabySchema`).
- Stable channel primitives such as `littlebaby/plugin-sdk/channel-setup`,
  `littlebaby/plugin-sdk/setup-runtime`,
  `littlebaby/plugin-sdk/setup-adapter-runtime`,
  `littlebaby/plugin-sdk/setup-tools`,
  `littlebaby/plugin-sdk/channel-pairing`,
  `littlebaby/plugin-sdk/channel-contract`,
  `littlebaby/plugin-sdk/channel-feedback`,
  `littlebaby/plugin-sdk/channel-inbound`,
  `littlebaby/plugin-sdk/channel-lifecycle`,
  `littlebaby/plugin-sdk/channel-reply-pipeline`,
  `littlebaby/plugin-sdk/command-auth`,
  `littlebaby/plugin-sdk/secret-input`, and
  `littlebaby/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  ...
  `CONFIG_DIR`).
- Domain subpaths such as `littlebaby/plugin-sdk/channel-config-helpers`,
  `littlebaby/plugin-sdk/allow-from`,
  `littlebaby/plugin-sdk/channel-config-schema`,
  `littlebaby/plugin-sdk/telegram-command-config`,
  `littlebaby/plugin-sdk/channel-policy`,
  `littlebaby/plugin-sdk/approval-gateway-runtime`,
  `littlebaby/plugin-sdk/approval-handler-adapter-runtime`,
  `littlebaby/plugin-sdk/approval-handler-runtime`,
  `littlebaby/plugin-sdk/approval-runtime`,
  `littlebaby/plugin-sdk/config-runtime`,
  `littlebaby/plugin-sdk/infra-runtime`,
  `littlebaby/plugin-sdk/agent-runtime`,
  `littlebaby/plugin-sdk/lazy-runtime`,
  `littlebaby/plugin-sdk/reply-history`,
  `littlebaby/plugin-sdk/routing`,
  `littlebaby/plugin-sdk/status-helpers`,
  `littlebaby/plugin-sdk/text-runtime`,
  `littlebaby/plugin-sdk/runtime-store`, and
  `littlebaby/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `littlebaby/plugin-sdk/channel-runtime` is deprecated and remains only as a
...
- Bundled extension internals remain private. External plugins should use only
  `littlebaby/plugin-sdk/*` subpaths. LittleBaby core/test code may use the repo
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  LittleBaby is not yet serving a runtime snapshot.
...
- Avoid the root `littlebaby/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `littlebaby/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `littlebaby/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `littlebaby/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `littlebaby/plugin-sdk/channel-targets`; channel-specific
...
`littlebaby/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape LittleBaby writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to LittleBaby's
...
- if both `catalog` and `discovery` are registered, LittleBaby uses `catalog`
...
- Read-only command paths such as `littlebaby status`, `littlebaby status --all`,
  `littlebaby channels status`, `littlebaby channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `littlebaby.extensions`:
...
Security guardrail: every `littlebaby.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `littlebaby plugins install` installs plugin dependencies with
...
Optional: `littlebaby.setupEntry` can point at a lightweight setup-only module.
When LittleBaby needs setup surfaces for a disabled channel plugin, or
...
Optional: `littlebaby.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let LittleBaby load the
...
Channel plugins can advertise setup/discovery metadata via `littlebaby.channel` and
install hints via `littlebaby.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@littlebaby/nextcloud-talk",
  "littlebaby": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@littlebaby/nextcloud-talk",
...
Useful `littlebaby.channel` fields beyond the minimal example:
...
LittleBaby can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.littlebaby/mpm/plugins.json`
- `~/.littlebaby/mpm/catalog.json`
- `~/.littlebaby/plugins/catalog.json`

Or point `LITTLEBABY_PLUGIN_CATALOG_PATHS` (or `LITTLEBABY_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "littlebaby": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
2. add typed plugin registration/runtime surfaces
   Extend `LittleBabyPluginApi` and/or `api.runtime` with the smallest useful
...
This is how LittleBaby stays opinionated without becoming hardcoded to one
