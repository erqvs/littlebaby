...
read_when:
  - You want to place an outbound voice call from LittleBaby
...
Voice calls for LittleBaby via a plugin. Supports outbound notifications and
...
- Use `littlebaby voicecall ...` or the `voice_call` tool
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `littlebaby doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `littlebaby doctor --fix`:
...
littlebaby voicecall start --to "+15555550123"   # alias for call
...
littlebaby voicecall latency                     # summarize turn latency from logs
