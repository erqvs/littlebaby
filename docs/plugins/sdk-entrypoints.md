...
**Import:** `littlebaby/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `LittleBabyPluginConfigSchema \| () => LittleBabyPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: LittleBabyPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `littlebaby.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- LittleBaby resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `littlebaby/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: LittleBabyPluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: LittleBabyPluginApi) => void`                               | No       | —                   |
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and LittleBaby
...
LittleBaby loads this instead of the full entry when a channel is disabled,
...
- `littlebaby/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `littlebaby/plugin-sdk/channel-setup` for optional-install setup surfaces
- `littlebaby/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`littlebaby/plugin-sdk/channel-entry-contract` instead. That contract lets the
...
- use `descriptors` when the registrar owns one or more root commands and you
  want LittleBaby to lazy-load the real CLI module on first invocation
...
LittleBaby classifies loaded plugins by their registration behavior:
...
Use `littlebaby plugins inspect <id>` to see a plugin's shape.
