...
read_when:
  ...
  - You are defining plugin config schemas or package.json littlebaby metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`littlebaby.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `littlebaby` field that tells the plugin system what
...
```json
{
  "name": "@myorg/littlebaby-my-channel",
  "version": "1.0.0",
  "type": "module",
  "littlebaby": {
    "extensions": ["./index.ts"],
...
`littlebaby.channel` is cheap package metadata for channel discovery and setup
...
`littlebaby.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported LittleBaby version in the form `>=x.y.z`.                        |
...
still fails closed and tells the operator to run `littlebaby doctor --fix`.
...
When enabled, LittleBaby loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `littlebaby.plugin.json` in the package root.
LittleBaby uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to LittleBaby",
  "configSchema": {
    "type": "object",
...
LittleBaby loads when it only needs setup surfaces (onboarding, config repair,
...
`littlebaby/plugin-sdk/channel-entry-contract` instead of
...
**When LittleBaby uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `littlebaby/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that LittleBaby validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "littlebaby/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `littlebaby onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "littlebaby/plugin-sdk/channel-setup";
...
helpers from `littlebaby/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`littlebaby/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `littlebaby/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "littlebaby/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/littlebaby-my-channel",
  docsPath: "/channels/my-channel",
});
...
<Info>
  For npm-sourced installs, `littlebaby plugins install` runs
