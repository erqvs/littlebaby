...
`runEmbeddedAgent(...)` is the neutral helper for starting a normal LittleBaby
...
Bind a Task Flow runtime to an existing LittleBaby session key or trusted tool
...
trusted LittleBaby session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "LittleBaby plugin SDK", count: 5 },
});
```
