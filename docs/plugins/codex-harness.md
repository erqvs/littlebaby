---
title: "Codex Harness"
summary: "Run LittleBaby embedded agent turns through the bundled Codex app-server harness"
read_when:
...
The bundled `codex` plugin lets LittleBaby run embedded agent turns through the
...
LittleBaby still owns chat channels, session files, model selection, tools,
...
force `embeddedHarness.runtime: "codex"` or `LITTLEBABY_AGENT_RUNTIME=codex`.
...
LittleBaby has separate routes for OpenAI and Codex-shaped access:

| Model ref              | Runtime path                                 | Use when                                                                |
| ---------------------- | -------------------------------------------- | ----------------------------------------------------------------------- |
| `openai/gpt-5.4`       | OpenAI provider through LittleBaby/PI plumbing | You want direct OpenAI Platform API access with `OPENAI_API_KEY`.       |
...
- LittleBaby with the bundled `codex` plugin available.
...
LittleBaby on the protocol surface it has been tested against.
...
With fallback disabled, LittleBaby fails early if the Codex plugin is disabled,
...
LittleBaby session and the Codex harness creates or resumes its sidecar app-server
thread as needed. `/reset` clears the LittleBaby session binding for that thread.
...
- `LITTLEBABY_CODEX_APP_SERVER_BIN`
- `LITTLEBABY_CODEX_APP_SERVER_ARGS`
- `LITTLEBABY_CODEX_APP_SERVER_APPROVAL_POLICY`
- `LITTLEBABY_CODEX_APP_SERVER_SANDBOX`
- `LITTLEBABY_CODEX_APP_SERVER_GUARDIAN=1`
...
Model switching stays LittleBaby-controlled. When an LittleBaby session is attached
...
generic and works on any channel that supports LittleBaby text commands.
...
- `/codex resume <thread-id>` attaches the current LittleBaby session to an existing Codex thread.
...
normal turns. On the next message, LittleBaby resumes that Codex thread, passes the
currently selected LittleBaby `codex/*` model into app-server, and keeps extended
history enabled.
...
LittleBaby still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal LittleBaby delivery path.

When the selected model uses the Codex harness, native thread compaction is
delegated to Codex app-server. LittleBaby keeps a transcript mirror for channel
...
**LittleBaby falls back to PI:** set `embeddedHarness.fallback: "none"` or
`LITTLEBABY_AGENT_HARNESS_FALLBACK=none` while testing.
