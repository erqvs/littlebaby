...
summary: "Step-by-step guide to building a model provider plugin for LittleBaby"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to LittleBaby
...
(LLM) to LittleBaby. By the end you will have a provider with a model catalog,
...
<Tip>
  Provider plugins add models to LittleBaby's normal inference loop. If the model
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so LittleBaby can detect
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "littlebaby/plugin-sdk/provider-auth";
    ...
    `littlebaby onboard --acme-ai-api-key <key>` and select
    `acme-ai/acme-large` as their model.

    If the upstream provider uses different control tokens than LittleBaby, add a
    ...
    LittleBaby parses its own control markers or channel delivery.
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "littlebaby/plugin-sdk/provider-entry";
    ...
    `littlebaby/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `littlebaby/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    import { buildProviderStreamFamilyHooks } from "littlebaby/plugin-sdk/provider-stream";
    ...
    `littlebaby/plugin-sdk/provider-model-shared` also exports the replay-family
    ...
    `littlebaby/plugin-sdk/provider-stream` exposes both the family builder and
    ...
    example: `@littlebaby/anthropic-provider` exports
    ...
    `littlebaby/plugin-sdk/provider-tools` currently exposes one shared
    ...
    - `@littlebaby/openrouter-provider`: `api.ts` exports the provider builder
    ...
    <Accordion title="All available provider hooks">
      LittleBaby calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    LittleBaby classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # littlebaby.providers metadata
├── littlebaby.plugin.json      # Manifest with provider auth metadata
