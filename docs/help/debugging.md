...
- **Global `--dev` (profile):** isolates state under `~/.littlebaby-dev` and
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`LITTLEBABY_SKIP_CHANNELS=1`).
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.littlebaby/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
LITTLEBABY_RAW_STREAM=1
LITTLEBABY_RAW_STREAM_PATH=~/.littlebaby/logs/raw-stream.jsonl
```

Default file:

`~/.littlebaby/logs/raw-stream.jsonl`
