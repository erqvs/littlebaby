---
summary: "Frequently asked questions about LittleBaby setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/littlebaby/littlebaby-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   littlebaby health --json
   littlebaby health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://littlebaby.ai/install.sh | bash -s -- --install-method git
    ```

    This installs LittleBaby **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/littlebaby/littlebaby/issues](https://github.com/littlebaby/littlebaby/issues)
    [https://github.com/littlebaby/littlebaby/pulls](https://github.com/littlebaby/littlebaby/pulls)
    ...
    - `littlebaby status`: quick snapshot of gateway/agent health + basic config.
    - `littlebaby models status`: checks provider auth + model availability.
    - `littlebaby doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `littlebaby status --all`, `littlebaby logs --follow`,
    `littlebaby gateway status`, `littlebaby health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up LittleBaby">
    ...
    If you don't have a global install yet, run it via `pnpm littlebaby onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `LITTLEBABY_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `LITTLEBABY_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `littlebaby doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `littlebaby gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `littlebaby gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, LittleBaby now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install LittleBaby on the new machine.
    2. Copy `$LITTLEBABY_STATE_DIR` (default: `~/.littlebaby`) from the old machine.
    3. Copy your workspace (default: `~/.littlebaby/workspace`).
    4. Run `littlebaby doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.littlebaby/` (for example `~/.littlebaby/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/littlebaby/littlebaby/blob/main/CHANGELOG.md](https://github.com/littlebaby/littlebaby/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.littlebaby.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.littlebaby.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.littlebaby.ai`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/littlebaby/littlebaby/tree/main/docs](https://github.com/littlebaby/littlebaby/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://littlebaby.ai/install.sh | bash -s -- --beta
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://littlebaby.ai/install.sh | bash -s -- --install-method git
    ```

    Windows installer (PowerShell):
    [https://littlebaby.ai/install.ps1](https://littlebaby.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://littlebaby.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://littlebaby.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://littlebaby.ai/install.sh | bash -s -- --install-method git --verbose
    ```
  ...
  <Accordion title="Windows install says git not found or littlebaby not recognized">
    ...
    **2) littlebaby is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest LittleBaby, track/report it in:

    - [Issue #30640](https://github.com/littlebaby/littlebaby/issues/30640)
  ...
  <Accordion title="How do I install LittleBaby on Linux?">
  ...
  <Accordion title="How do I install LittleBaby on a VPS?">
  ...
  <Accordion title="Can I ask LittleBaby to update itself?">
    ...
    littlebaby update --channel stable|beta|dev
    littlebaby update --tag <dist-tag|version>
    ...
    ```bash
    littlebaby update --yes --no-restart
    littlebaby gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `littlebaby onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run LittleBaby with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    Pro/Max or OpenAI Codex) are optional ways to authenticate those providers.

    For Anthropic in LittleBaby, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in LittleBaby**: Anthropic staff
      told us this usage is allowed again, and LittleBaby is treating `claude -p`
    ...
    tools like LittleBaby.

    LittleBaby also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us LittleBaby-style Claude CLI usage is allowed again, so
    LittleBaby treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so LittleBaby treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported LittleBaby token path, but LittleBaby now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in LittleBaby, see [OpenAI](/providers/openai), [Qwen / Model
...
for usage/billing and raise limits as needed.
    ...
    LittleBaby Claude-login path with Extra Usage enabled).

    Tip: set a **fallback model** so LittleBaby can keep replying while a provider is rate-limited.
  ...
  <Accordion title="Is AWS Bedrock supported?">
    Yes. LittleBaby has a bundled **Amazon Bedrock (Converse)** provider. With AWS env markers present, LittleBaby can auto-discover the streaming/text Bedrock catalog and merge it as an implicit `amazon-bedrock` provider; otherwise you can explicitly enable `plugins.entries.amazon-bedrock.config.discovery.enabled` or add a manual provider entry. See [Amazon Bedrock](/providers/bedrock) and [Model providers](/providers/models). If you prefer a managed key flow, an OpenAI-compatible proxy in front of Bedrock is still a valid option.
  </Accordion>

  <Accordion title="How does Codex auth work?">
    LittleBaby supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).
  </Accordion>

  <Accordion title="Why does ChatGPT GPT-5.4 not unlock openai/gpt-5.4 in LittleBaby?">
    LittleBaby treats the two routes separately:

    - `openai-codex/gpt-5.4` = ChatGPT/Codex OAuth
    - `openai/gpt-5.4` = direct OpenAI Platform API

    In LittleBaby, ChatGPT/Codex sign-in is wired to the `openai-codex/*` route,
    not the direct `openai/*` route. If you want the direct API path in
    LittleBaby, set `OPENAI_API_KEY` (or the equivalent OpenAI provider config).
    If you want ChatGPT/Codex sign-in in LittleBaby, use `openai-codex/*`.
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    LittleBaby can show the currently visible provider usage/quota windows in
    `littlebaby models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. LittleBaby fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like LittleBaby. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `littlebaby.json`.
    ...
    2. Enable the plugin: `littlebaby plugins enable google`
    3. Login: `littlebaby models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. LittleBaby needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. LittleBaby runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run LittleBaby, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `littlebaby nodes status` / `littlebaby nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Setup asks for numeric user IDs only. If you already have legacy `@username` entries in config, `littlebaby doctor --fix` can try to resolve them.

    Safer (no third-party bot):

    - DM your bot, then run `littlebaby logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different LittleBaby instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run LittleBaby via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    ...
    This **does not delete your data** - it only changes the LittleBaby code install. Your state
    (`~/.littlebaby`) and workspace (`~/.littlebaby/workspace`) stay untouched.
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **LittleBaby-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run LittleBaby on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    LittleBaby is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run LittleBaby in a VM and what are the requirements?">
...
<AccordionGroup>
  ...
  <Accordion title="What are the top five everyday use cases for LittleBaby?">
  ...
  <Accordion title="Can LittleBaby help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    LittleBaby draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    LittleBaby is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use LittleBaby when you
    ...
    Showcase: [https://littlebaby.ai/showcase](https://littlebaby.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.littlebaby/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.littlebaby/littlebaby.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.littlebaby/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.littlebaby/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, LittleBaby falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, LittleBaby intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    littlebaby tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `LITTLEBABY_SKIP_CRON` is not set.
    ...
    ```bash
    littlebaby cron run <jobId>
    littlebaby cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    ...
    littlebaby skills search --limit 20
    littlebaby skills install <skill-slug>
    littlebaby skills install <skill-slug> --version <version>
    littlebaby skills install <skill-slug> --force
  ...
  <Accordion title="Can LittleBaby run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.littlebaby.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. LittleBaby can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.littlebaby/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "littlebaby": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="How do I use my existing signed-in Chrome with LittleBaby?">
    ...
    littlebaby browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `LITTLEBABY_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `LITTLEBABY_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    LittleBaby validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    LittleBaby memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    LittleBaby also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, LittleBaby auto-selects a provider when it
    ...
    configured and present, LittleBaby
...
<AccordionGroup>
  <Accordion title="Is all data used with LittleBaby saved locally?">
    No - **LittleBaby's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.littlebaby` + your workspace directory).
  ...
  <Accordion title="Where does LittleBaby store its data?">
    Everything lives under `$LITTLEBABY_STATE_DIR` (default: `~/.littlebaby`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$LITTLEBABY_STATE_DIR/littlebaby.json`                             | Main config (JSON5)                                                |
    | `$LITTLEBABY_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$LITTLEBABY_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$LITTLEBABY_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$LITTLEBABY_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$LITTLEBABY_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$LITTLEBABY_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$LITTLEBABY_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$LITTLEBABY_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.littlebaby/agent/*` (migrated by `littlebaby doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.littlebaby/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.littlebaby`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
      `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
    - **State dir (`~/.littlebaby`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.littlebaby/skills`).

    Default workspace is `~/.littlebaby/workspace`, configurable via:
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.littlebaby` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall LittleBaby?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The LittleBaby repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    LittleBaby reads an optional **JSON5** config from `$LITTLEBABY_CONFIG_PATH` (default: `~/.littlebaby/littlebaby.json`):

    ```
    $LITTLEBABY_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.littlebaby/workspace`).
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `LITTLEBABY_GATEWAY_PASSWORD`) instead.
  ...
  <Accordion title="Why do I need a token on localhost now?">
    LittleBaby enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and auto-generates one, saving it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for non-loopback identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `littlebaby doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one LittleBaby.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `LITTLEBABY_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `littlebaby configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, LittleBaby auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
    - Daemons read env vars from `~/.littlebaby/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current LittleBaby protects many accidental clobbers:

    - LittleBaby-owned config writes validate the full post-change config before writing.
    - Invalid or destructive LittleBaby-owned writes are rejected and saved as `littlebaby.json.rejected.*`.
    - If a direct edit breaks startup or hot reload, the Gateway restores the last-known-good config and saves the rejected file as `littlebaby.json.clobbered.*`.
    - The main agent receives a boot warning after recovery so it does not blindly write the bad config again.

    Recover:

    - Check `littlebaby logs --follow` for `Config auto-restored from last-known-good`, `Config write rejected:`, or `config reload restored last-known-good config`.
    - Inspect the newest `littlebaby.json.clobbered.*` or `littlebaby.json.rejected.*` beside the active config.
    - Keep the active restored config if it works, then copy only the intended keys back with `littlebaby config set` or `config.patch`.
    - Run `littlebaby config validate` and `littlebaby doctor`.
    - If you have no last-known-good or rejected payload, restore from backup, or re-run `littlebaby doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `littlebaby config set` for small changes.
    - Use `littlebaby configure` for interactive edits.
  ...
  <Accordion title="Can the LittleBaby browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `littlebaby gateway status`
    - Gateway health: `littlebaby status`
    - Channel health: `littlebaby channels status`
  ...
  <Accordion title="Can two LittleBaby instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `littlebaby agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    littlebaby agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    littlebaby gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does LittleBaby load environment variables?">
    LittleBaby reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.littlebaby/.env` (aka `$LITTLEBABY_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.littlebaby/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `LITTLEBABY_LOAD_SHELL_ENV=1`, `LITTLEBABY_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `littlebaby models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means LittleBaby won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.littlebaby/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of LittleBaby instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset LittleBaby but keep it installed?">
    ...
    - If you used profiles (`--profile` / `LITTLEBABY_PROFILE`), reset each state dir (defaults are `~/.littlebaby-<profile>`).
    - Dev reset: `littlebaby gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), LittleBaby skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. LittleBaby runs on **your own account**, so if you're in the group, LittleBaby can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    littlebaby directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does LittleBaby not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.littlebaby/agents/<agentId>/sessions/`.
    ...
    - Use `littlebaby doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    LittleBaby's default model is whatever you set as:

    ```
    agents.defaults.model.primary
    ```

    Models are referenced as `provider/model` (example: `openai/gpt-5.4`). If you omit the provider, LittleBaby first tries an alias, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider as a deprecated compatibility path. If that provider no longer exposes the configured default model, LittleBaby falls back to the first configured provider/model instead of surfacing a stale removed-provider default. You should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `littlebaby models set ...` (updates just model config)
    - `littlebaby configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.littlebaby/littlebaby.json`
    ...
    for partial updates.
    If you did overwrite config, restore from backup or re-run `littlebaby doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `littlebaby onboard` and choose `Ollama`
    ...
    - for manual switching, use `littlebaby models list` and `littlebaby models set ollama/<model>`
  ...
  <Accordion title="What do LittleBaby, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `littlebaby models status`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current LittleBaby release (or run from source `main`), then restart the gateway.
  ...
  <Accordion title="Are opus / sonnet / gpt built-in shortcuts?">
    Yes. LittleBaby ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    ```
    ~/.littlebaby/agents/<agentId>/agent/auth-profiles.json
    ```

    Fix options:

    - Run `littlebaby agents add <id>` and configure auth during the wizard.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so LittleBaby can keep responding even when a provider is rate-limited or temporarily failing.

    The rate-limit bucket includes more than plain `429` responses. LittleBaby
    ...
    explicit billing text on `401` or `403`, LittleBaby can still keep that in
    ...
    `organization spending limit exceeded`), LittleBaby treats it as
    ...
    unknown/error in it". LittleBaby does treat provider-scoped transient shapes
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - **Confirm where auth profiles live** (new vs legacy paths)
      - Current: `~/.littlebaby/agents/<agentId>/agent/auth-profiles.json`
      - Legacy: `~/.littlebaby/agent/*` (migrated by `littlebaby doctor`)
    - **Confirm your env var is loaded by the Gateway**
      - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.littlebaby/.env` or enable `env.shellEnv`.
    - **Make sure you're editing the correct agent**
      - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
    - **Sanity-check model/auth status**
      - Use `littlebaby models status` to see configured models and whether providers are authenticated.
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use Claude CLI**
      - Run `littlebaby models auth login --provider anthropic --method cli --set-default` on the gateway host.
    - **If you want to use an API key instead**
      - Put `ANTHROPIC_API_KEY` in `~/.littlebaby/.env` on the **gateway host**.
      - Clear any pinned order that forces a missing profile:

        ```bash
        littlebaby models auth order clear --provider anthropic
        ```
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), LittleBaby will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    Fix: LittleBaby now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
<AccordionGroup>
  ...
  <Accordion title="What are typical profile IDs?">
    LittleBaby uses provider-prefixed IDs like:
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    LittleBaby may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `littlebaby models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
    ...
    littlebaby models auth order get --provider anthropic

    # Lock rotation to a single profile (only try this one)
    littlebaby models auth order set --provider anthropic anthropic:default

    # Or set an explicit order (fallback within provider)
    littlebaby models auth order set --provider anthropic anthropic:work anthropic:default
    ...
    ```bash
    littlebaby models auth order set --provider anthropic --agent main anthropic:default
    ```

    To verify what will actually be tried, use:

    ```bash
    littlebaby models status --probe
    ```
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > LITTLEBABY_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does littlebaby gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    ...
    Use `littlebaby gateway status` and trust these lines:
  ...
  <Accordion title='Why does littlebaby gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `LITTLEBABY_STATE_DIR` mismatch).
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    LittleBaby enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `littlebaby gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run LittleBaby in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `littlebaby gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `littlebaby dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `littlebaby doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `LITTLEBABY_GATEWAY_TOKEN` or `gateway.auth.password` / `LITTLEBABY_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `littlebaby devices list`
      - `littlebaby devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `littlebaby status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `LITTLEBABY_CONFIG_PATH` (per-instance config)
    - `LITTLEBABY_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `littlebaby --profile <name> ...` per instance (auto-creates `~/.littlebaby-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `littlebaby --profile <name> gateway install`.

    Profiles also suffix service names (`ai.littlebaby.<profile>`; legacy `com.littlebaby.*`, `littlebaby-gateway-<profile>.service`, `LittleBaby Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    littlebaby tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$LITTLEBABY_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.littlebaby/logs/...`; profiles use `~/.littlebaby-<profile>/logs/...`)
    - Linux: `journalctl --user -u littlebaby-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "LittleBaby Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `littlebaby gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart LittleBaby?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `littlebaby gateway status`
    2. Is the Gateway healthy? `littlebaby status`
    3. Does the UI have the right token? `littlebaby dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    littlebaby channels status
    littlebaby channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. LittleBaby already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: littlebaby gateway restart vs littlebaby gateway">
    - `littlebaby gateway restart`: restarts the **background service** (launchd/systemd).
    - `littlebaby gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `littlebaby gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [LittleBaby assistant setup](/start/littlebaby) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    littlebaby message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose LittleBaby to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `littlebaby pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `littlebaby pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `littlebaby doctor` to surface risky DM policies.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. LittleBaby only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    littlebaby pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    littlebaby pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    LittleBaby blocks **cross-provider** messaging by default. If a tool call is bound
