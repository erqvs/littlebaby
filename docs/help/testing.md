...
- `LITTLEBABY_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `LITTLEBABY_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `LITTLEBABY_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `LITTLEBABY_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `LITTLEBABY_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `LITTLEBABY_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `LITTLEBABY_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `LITTLEBABY_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `LITTLEBABY_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `LITTLEBABY_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `LITTLEBABY_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`LITTLEBABY_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`LITTLEBABY_QA_CONVEX_SECRET_MAINTAINER` specifically.
...
Default endpoint contract (`LITTLEBABY_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`):
...
- Pool note:
  ...
  - The shared `scripts/run-vitest.mjs` launcher now also adds `--no-maglev` for Vitest child Node processes by default to reduce V8 compile churn during big local runs. Set `LITTLEBABY_VITEST_ENABLE_MAGLEV=1` if you need to compare against stock V8 behavior.
- Fast-local iteration note:
  ...
  - The config keeps `LITTLEBABY_VITEST_FS_MODULE_CACHE` enabled on supported hosts; set `LITTLEBABY_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want one explicit cache location for direct profiling.
- Perf-debug note:
...
- Useful overrides:
  - `LITTLEBABY_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `LITTLEBABY_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Useful overrides:
  - `LITTLEBABY_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `LITTLEBABY_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `LITTLEBABY_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.littlebaby`.
- Set `LITTLEBABY_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `LITTLEBABY_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `LITTLEBABY_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `LITTLEBABY_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `LITTLEBABY_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Optional target overrides:
  - `LITTLEBABY_ANDROID_NODE_ID` or `LITTLEBABY_ANDROID_NODE_NAME`.
  - `LITTLEBABY_ANDROID_GATEWAY_URL` / `LITTLEBABY_ANDROID_GATEWAY_TOKEN` / `LITTLEBABY_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `LITTLEBABY_LIVE_TEST=1` if invoking Vitest directly)
- Set `LITTLEBABY_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `LITTLEBABY_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet 4.6+, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.7, Grok 4)
  - `LITTLEBABY_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `LITTLEBABY_LIVE_MODELS="openai/gpt-5.4,anthropic/claude-opus-4-6,..."` (comma allowlist)
  - Modern/all sweeps default to a curated high-signal cap; set `LITTLEBABY_LIVE_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers:
  - `LITTLEBABY_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `LITTLEBABY_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
...
- How to select models:
  ...
  - `LITTLEBABY_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `LITTLEBABY_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
  - Modern/all gateway sweeps default to a curated high-signal cap; set `LITTLEBABY_LIVE_GATEWAY_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers (avoid “OpenRouter everything”):
  - `LITTLEBABY_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Overrides (optional):
  - `LITTLEBABY_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.4"`
  - `LITTLEBABY_LIVE_CLI_BACKEND_COMMAND="/full/path/to/codex"`
  - `LITTLEBABY_LIVE_CLI_BACKEND_ARGS='["exec","--json","--color","never","--sandbox","read-only","--skip-git-repo-check"]'`
  - `LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt).
  - `LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `LITTLEBABY_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
  - `LITTLEBABY_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=0` to disable the default Claude Sonnet -> Opus same-session continuity probe (set to `1` to force it on when the selected model supports a switch target).
...
- Defaults:
  ...
  - ACP backend: `acpx`
- Overrides:
  - `LITTLEBABY_LIVE_ACP_BIND_AGENT=claude`
  - `LITTLEBABY_LIVE_ACP_BIND_AGENT=codex`
  - `LITTLEBABY_LIVE_ACP_BIND_AGENT=gemini`
  - `LITTLEBABY_LIVE_ACP_BIND_AGENTS=claude,codex,gemini`
  - `LITTLEBABY_LIVE_ACP_BIND_AGENT_COMMAND='npx -y @agentclientprotocol/claude-agent-acp@<version>'`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `LITTLEBABY_LIVE_ACP_BIND_AGENT_COMMAND` is unset, the test uses the embedded `acpx` plugin's built-in agent registry for the selected ACP harness agent.
...
- Use `LITTLEBABY_LIVE_ACP_BIND_AGENTS=claude`, `LITTLEBABY_LIVE_ACP_BIND_AGENTS=codex`, or `LITTLEBABY_LIVE_ACP_BIND_AGENTS=gemini` to narrow the matrix.
...
- Inside Docker, the runner sets `LITTLEBABY_LIVE_ACP_BIND_ACPX_COMMAND=$HOME/.npm-global/bin/acpx` so acpx keeps provider env vars from the sourced profile available to the child harness CLI.
...
- Enable: `LITTLEBABY_LIVE_CODEX_HARNESS=1`
- Default model: `codex/gpt-5.4`
- Optional image probe: `LITTLEBABY_LIVE_CODEX_HARNESS_IMAGE_PROBE=1`
- Optional MCP/tool probe: `LITTLEBABY_LIVE_CODEX_HARNESS_MCP_PROBE=1`
- The smoke sets `LITTLEBABY_AGENT_HARNESS_FALLBACK=none` so a broken Codex
...
```bash
source ~/.profile
LITTLEBABY_LIVE_CODEX_HARNESS=1 \
  LITTLEBABY_LIVE_CODEX_HARNESS_IMAGE_PROBE=1 \
  LITTLEBABY_LIVE_CODEX_HARNESS_MCP_PROBE=1 \
  LITTLEBABY_LIVE_CODEX_HARNESS_MODEL=codex/gpt-5.4 \
...
- Docker enables the image and MCP/tool probes by default. Set
  `LITTLEBABY_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` or
  `LITTLEBABY_LIVE_CODEX_HARNESS_MCP_PROBE=0` when you need a narrower debug run.
- Docker also exports `LITTLEBABY_AGENT_HARNESS_FALLBACK=none`, matching the live
...
- Single model, direct (no gateway):
  - `LITTLEBABY_LIVE_MODELS="openai/gpt-5.4" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `LITTLEBABY_LIVE_GATEWAY_MODELS="openai/gpt-5.4" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `LITTLEBABY_LIVE_GATEWAY_MODELS="openai/gpt-5.4,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `LITTLEBABY_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravity (OAuth): `LITTLEBABY_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
`LITTLEBABY_LIVE_GATEWAY_MODELS="openai/gpt-5.4,openai-codex/gpt-5.4,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
Include at least one image-capable model in `LITTLEBABY_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.
...
- Enable: `LITTLEBABY_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts`
- Scope:
...
- Current bundled providers covered:
  ...
  - `google`
- Optional narrowing:
  - `LITTLEBABY_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google"`
  - `LITTLEBABY_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-1,google/gemini-3.1-flash-image-preview"`
  - `LITTLEBABY_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit"`
- Optional auth behavior:
  - `LITTLEBABY_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides

## Music generation live

- Test: `extensions/music-generation-providers.live.test.ts`
- Enable: `LITTLEBABY_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts`
...
- Optional narrowing:
  - `LITTLEBABY_LIVE_MUSIC_GENERATION_PROVIDERS="google,minimax"`
  - `LITTLEBABY_LIVE_MUSIC_GENERATION_MODELS="google/lyria-3-clip-preview,minimax/music-2.5+"`
- Optional auth behavior:
  - `LITTLEBABY_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `LITTLEBABY_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts`
- Harness: `pnpm test:live:media video`
- Scope:
  - Exercises the shared bundled video-generation provider path
  - Defaults to the release-safe smoke path: non-FAL providers, one text-to-video request per provider, one-second lobster prompt, and a per-provider operation cap from `LITTLEBABY_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default)
  - Skips FAL by default because provider-side queue latency can dominate release time; pass `--video-providers fal` or `LITTLEBABY_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` to run it explicitly
  - Loads provider env vars from your login shell (`~/.profile`) before probing
  - Uses live/env API keys ahead of stored auth profiles by default, so stale test keys in `auth-profiles.json` do not mask real shell credentials
  - Skips providers with no usable auth/profile/model
  - Runs only `generate` by default
  - Set `LITTLEBABY_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared transform modes when available:
  ...
  - Provider-specific Vydra coverage:
    - `LITTLEBABY_LIVE_TEST=1 LITTLEBABY_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
...
- Optional narrowing:
  - `LITTLEBABY_LIVE_VIDEO_GENERATION_PROVIDERS="google,openai,runway"`
  - `LITTLEBABY_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `LITTLEBABY_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""` to include every provider in the default sweep, including FAL
  - `LITTLEBABY_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` to reduce each provider operation cap for an aggressive smoke run
- Optional auth behavior:
  - `LITTLEBABY_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Docker live runners default to a smaller smoke cap so a full Docker sweep stays practical:
  `test:docker:live-models` defaults to `LITTLEBABY_LIVE_MAX_MODELS=12`, and
  `test:docker:live-gateway` defaults to `LITTLEBABY_LIVE_GATEWAY_SMOKE=1`,
  `LITTLEBABY_LIVE_GATEWAY_MAX_MODELS=8`,
  `LITTLEBABY_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `LITTLEBABY_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Override those env vars when you
...
- `LITTLEBABY_CONFIG_DIR=...` (default: `~/.littlebaby`) mounted to `/home/node/.littlebaby`
...
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `LITTLEBABY_LIVE_PROVIDERS` / `LITTLEBABY_LIVE_GATEWAY_PROVIDERS`
