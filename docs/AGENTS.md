...
- Docs are hosted on Mintlify (`https://docs.littlebaby.ai`).
...
- Foreign-language docs are not maintained in this repo. The generated publish output lives in the separate `littlebaby/docs` repo (often cloned locally as `../littlebaby-docs`).
...
- Pipeline: update English docs here, update `docs/.i18n/glossary.<locale>.json` as needed, then let the publish-repo sync and `scripts/docs-i18n` run in `littlebaby/docs`.
