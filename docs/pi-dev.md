...
This guide summarizes a sane workflow for working on the pi integration in LittleBaby.
...
```bash
LITTLEBABY_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm littlebaby agent --message "Hello" --thinking low`
...
State lives under the LittleBaby state directory. Default is `~/.littlebaby`. If `LITTLEBABY_STATE_DIR` is set, use that directory instead.

To reset everything:

- `littlebaby.json` for config
