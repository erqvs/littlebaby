---
summary: "CLI reference for `littlebaby memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
littlebaby memory status
littlebaby memory status --deep
littlebaby memory status --fix
littlebaby memory index --force
littlebaby memory search "meeting notes"
littlebaby memory search --query "deployment" --max-results 20
littlebaby memory promote --limit 10 --min-score 0.75
littlebaby memory promote --apply
littlebaby memory promote --json --min-recall-count 0 --min-unique-queries 0
littlebaby memory promote-explain "router vlan"
littlebaby memory promote-explain "router vlan" --json
littlebaby memory rem-harness
littlebaby memory rem-harness --json
littlebaby memory status --json
littlebaby memory status --deep --index
littlebaby memory status --deep --index --verbose
littlebaby memory status --agent main
littlebaby memory index --agent main --verbose
```
...
```bash
littlebaby memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `littlebaby cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
littlebaby memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
littlebaby memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
