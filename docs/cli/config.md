---
summary: "CLI reference for `littlebaby config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `littlebaby.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `littlebaby configure`).

Root options:

- `--section <section>`: repeatable guided-setup section filter when you run `littlebaby config` without a subcommand
...
littlebaby config --section model
littlebaby config --section gateway --section daemon
littlebaby config schema
littlebaby config get browser.executablePath
...
littlebaby config set secrets.providers.vaultfile --provider-source file --provider-path /etc/littlebaby/secrets.json --provider-mode json
...
littlebaby config validate --json
```

### `config schema`

Print the generated JSON schema for `littlebaby.json` to stdout as JSON.
...
```bash
littlebaby config schema > littlebaby.schema.json
```
...
littlebaby config get agents.list[0].id
```
...
littlebaby config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
littlebaby config set agents.defaults.heartbeat.every "0m"
littlebaby config set gateway.port 19001 --strict-json
littlebaby config set channels.whatsapp.groups '["*"]' --strict-json
```
...
`littlebaby config set` supports four assignment styles:

1. Value mode: `littlebaby config set <path> <value>`
2. SecretRef builder mode:

```bash
littlebaby config set channels.discord.token \
...
```bash
littlebaby config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/littlebaby-vault \
  --provider-arg read \
...
```bash
littlebaby config set --batch-json '[
  {
...
```bash
littlebaby config set --batch-file ./config-set.batch.json --dry-run
```
...
littlebaby config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/littlebaby/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `littlebaby.json`.
...
```json
{
  "ok": true,
  "operations": 1,
  "configPath": "~/.littlebaby/littlebaby.json",
  "inputModes": ["builder"],
  "checks": {
    "schema": false,
    "resolvability": true,
...
`littlebaby config set` and other LittleBaby-owned config writers validate the full
...
and the rejected payload is saved beside it as `littlebaby.json.rejected.*`.

Prefer CLI writes for small edits:

```bash
littlebaby config set gateway.reload.mode hybrid --dry-run
littlebaby config set gateway.reload.mode hybrid
littlebaby config validate
```
...
- `config file`: Print the active config file path (resolved from `LITTLEBABY_CONFIG_PATH` or default location).
