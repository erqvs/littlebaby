---
summary: "CLI reference for `littlebaby agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
littlebaby agent --to +15555550123 --message "status update" --deliver
...
littlebaby agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: LittleBaby persists markers from the active source config snapshot, not from resolved runtime secret values.
