---
summary: "CLI reference for `littlebaby security` (audit and fix common security footguns)"
read_when:
...
littlebaby security audit --deep --password <password>
littlebaby security audit --deep --token <token>
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that LittleBaby is a personal-assistant trust model by default.
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `littlebaby.browserConfigEpoch`) and recommends `littlebaby sandbox recreate --browser --all`.
...
```bash
littlebaby security audit --json | jq '.summary'
littlebaby security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
littlebaby security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
...
- tightens permissions for state/config and common sensitive files
  ...
  `*.jsonl`)
- also tightens config include files referenced from `littlebaby.json`
