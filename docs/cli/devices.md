---
summary: "CLI reference for `littlebaby devices` (device pairing + token rotation/revocation)"
read_when:
...
### `littlebaby devices remove <deviceId>`
...
```
littlebaby devices remove <deviceId>
littlebaby devices remove <deviceId> --json
```

### `littlebaby devices clear --yes [--pending]`

Clear paired devices in bulk.

```
littlebaby devices clear --yes
littlebaby devices clear --yes --pending
littlebaby devices clear --yes --pending --json
```

### `littlebaby devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, LittleBaby only prints the selected pending
...
key), LittleBaby supersedes the previous pending entry and issues a new
`requestId`. Run `littlebaby devices list` right before approval to use the
current ID.

If the device is already paired and asks for broader scopes or a broader role,
LittleBaby keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `littlebaby devices list`
or use `littlebaby devices approve --latest` to preview the exact upgrade before
approving it.

```
littlebaby devices approve
littlebaby devices approve <requestId>
littlebaby devices approve --latest
```
...
### `littlebaby devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
littlebaby devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```

Returns the new token payload as JSON.

### `littlebaby devices revoke --device <id> --role <role>`
...
```
littlebaby devices revoke --device <deviceId> --role node
```
...
```bash
littlebaby devices rotate --device <deviceId> --role operator
```
