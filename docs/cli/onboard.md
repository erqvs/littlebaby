---
summary: "CLI reference for `littlebaby onboard` (interactive onboarding)"
read_when:
...
```bash
littlebaby onboard
littlebaby onboard --flow quickstart
littlebaby onboard --flow manual
littlebaby onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`LITTLEBABY_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
littlebaby onboard --non-interactive \
  ...
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env LITTLEBABY_GATEWAY_TOKEN \
  --accept-risk
```
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `littlebaby gateway run`.
...
- Fastest first chat: `littlebaby dashboard` (Control UI, no channel setup).
