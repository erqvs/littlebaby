---
summary: "CLI reference for `littlebaby configure` (interactive configuration prompts)"
read_when:
...
Tip: `littlebaby config` without a subcommand opens the same wizard. Use
`littlebaby config get|set|unset` for non-interactive edits.

For web search, `littlebaby configure --section web` lets you choose a provider
...
littlebaby configure --section model --section channels
littlebaby configure --section gateway --section daemon
```
