...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an LittleBaby Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`littlebaby acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to LittleBaby channel
conversations instead of hosting an ACP harness session, use
[`littlebaby mcp serve`](/cli/mcp) instead.

## What this is not

This page is often confused with ACP harness sessions.

`littlebaby acp` means:

- LittleBaby acts as an ACP server
- an IDE or ACP client connects to LittleBaby
- LittleBaby forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where LittleBaby runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to LittleBaby: use `littlebaby acp`
- LittleBaby should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the LittleBaby gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
littlebaby acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
littlebaby acp --url wss://gateway-host:18789 --token-file ~/.littlebaby/gateway.token

# Attach to an existing session key
littlebaby acp --session agent:main:main

# Attach by label (must already exist)
littlebaby acp --session-label "support inbox"

# Reset the session key before the first prompt
littlebaby acp --session agent:main:main --reset-session
```
...
```bash
littlebaby acp client

# Point the spawned bridge at a remote Gateway
littlebaby acp client --server-args --url wss://gateway-host:18789 --token-file ~/.littlebaby/gateway.token

# Override the server command (default: littlebaby)
littlebaby acp client --server "node" --server-args littlebaby.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run LittleBaby through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an LittleBaby Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `littlebaby acp` over stdio.

Example config (persisted):

```bash
littlebaby config set gateway.remote.url wss://gateway-host:18789
littlebaby config set gateway.remote.token <token>
```
...
```bash
littlebaby acp --session agent:main:main
littlebaby acp --session agent:design:main
littlebaby acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see LittleBaby plugin tools, enable the
...
LittleBaby bot over ACP, use `acpx` with its built-in `littlebaby` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx littlebaby` at `littlebaby acp`.
3. Target the LittleBaby session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default LittleBaby ACP session
acpx littlebaby exec "Summarize the active LittleBaby session state."

# Persistent named session for follow-up turns
acpx littlebaby sessions ensure --name codex-bridge
acpx littlebaby -s codex-bridge --cwd /path/to/repo \
  "Ask my LittleBaby work agent for recent context relevant to this repo."
```

If you want `acpx littlebaby` to target a specific Gateway and session key every
time, override the `littlebaby` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "littlebaby": {
      "command": "env LITTLEBABY_HIDE_BANNER=1 LITTLEBABY_SUPPRESS_NOTES=1 littlebaby acp --url ws://127.0.0.1:18789 --token-file ~/.littlebaby/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local LittleBaby checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env LITTLEBABY_HIDE_BANNER=1 LITTLEBABY_SUPPRESS_NOTES=1 node littlebaby.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an LittleBaby agent without scraping a terminal.
...
In Zed, open the Agent panel and select “LittleBaby ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`LITTLEBABY_GATEWAY_TOKEN`, `LITTLEBABY_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`LITTLEBABY_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `LITTLEBABY_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `littlebaby acp client` sets `LITTLEBABY_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `littlebaby`).
