...
openclaw acp --url wss://gateway-host:18789 --token-file ~/.littlebaby/gateway.token
...
openclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.littlebaby/gateway.token

# Override the server command (default: openclaw)
openclaw acp client --server "node" --server-args littlebaby.mjs acp --url ws://127.0.0.1:19001
```
...
```json
{
  "agents": {
    "littlebaby": {
      "command": "env LITTLEBABY_HIDE_BANNER=1 LITTLEBABY_SUPPRESS_NOTES=1 openclaw acp --url ws://127.0.0.1:18789 --token-file ~/.littlebaby/gateway.token --session agent:main:main"
    }
  }
}
```
...
- `openclaw acp client` sets `LITTLEBABY_SHELL=acp-client` on the spawned bridge process.
