---
summary: "CLI reference for `littlebaby logs` (tail gateway logs via RPC)"
read_when:
...
`littlebaby logs` also accepts the standard Gateway client flags:
...
littlebaby logs --follow --interval 2000
littlebaby logs --limit 500 --max-bytes 500000
littlebaby logs --json
littlebaby logs --plain
littlebaby logs --no-color
littlebaby logs --limit 500
littlebaby logs --local-time
littlebaby logs --follow --local-time
littlebaby logs --url ws://127.0.0.1:18789 --token "$LITTLEBABY_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the local loopback Gateway asks for pairing, `littlebaby logs` falls back to the configured local log file automatically. Explicit `--url` targets do not use this fallback.
