---
summary: "CLI reference for `littlebaby qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
littlebaby qr
littlebaby qr --setup-code-only
littlebaby qr --json
littlebaby qr --remote
littlebaby qr --url wss://gateway.example/ws
```
...
- With `--remote`, LittleBaby requires either `gateway.remote.url` or
