---
summary: "CLI reference for `littlebaby plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
littlebaby plugins list --enabled
littlebaby plugins list --verbose
littlebaby plugins list --json
littlebaby plugins install <path-or-spec>
littlebaby plugins inspect <id>
littlebaby plugins inspect <id> --json
littlebaby plugins inspect --all
littlebaby plugins info <id>
littlebaby plugins enable <id>
littlebaby plugins disable <id>
littlebaby plugins uninstall <id>
littlebaby plugins doctor
littlebaby plugins update <id>
littlebaby plugins update --all
littlebaby plugins marketplace list <marketplace>
littlebaby plugins marketplace list <marketplace> --json
```

Bundled plugins ship with LittleBaby. Some are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin); others require `plugins enable`.

Native LittleBaby plugins must ship `littlebaby.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: littlebaby` or `Format: bundle`. Verbose list/info
...
littlebaby plugins install <plugin>@<marketplace>         # marketplace
littlebaby plugins install <plugin> --marketplace <name>  # marketplace (explicit)
...
run `littlebaby doctor --fix` first. The only documented exception is a narrow
...
`littlebaby.hooks` in `package.json`. Use `littlebaby hooks` for filtered hook
...
those to a prerelease, LittleBaby stops and asks you to opt in explicitly with a
prerelease tag such as `@beta`/`@rc` or an exact prerelease version such as
`@1.2.3-beta.4`.

If a bare install spec matches a bundled plugin id (for example `diffs`), LittleBaby
...
```bash
littlebaby plugins marketplace list <marketplace-name>
littlebaby plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
littlebaby plugins install <plugin-name> --marketplace <marketplace-name>
littlebaby plugins install <plugin-name> --marketplace <owner/repo>
littlebaby plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
littlebaby plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. LittleBaby accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, LittleBaby auto-detects:

- native LittleBaby plugins (`littlebaby.plugin.json`)
...
```bash
littlebaby plugins install -l ./my-plugin
```
...
```bash
littlebaby plugins uninstall <id>
littlebaby plugins uninstall <id> --dry-run
...
```bash
littlebaby plugins update <id-or-npm-spec>
littlebaby plugins update --all
littlebaby plugins update <id-or-npm-spec> --dry-run
littlebaby plugins update @littlebaby/voice-call@beta
littlebaby plugins update littlebaby-codex-app-server --dangerously-force-unsafe-install
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, LittleBaby reuses the recorded install spec for that
...
or exact version. LittleBaby resolves that package name back to the tracked plugin
record, updates that installed plugin, and records the new npm spec for future
id-based updates.

When a stored integrity hash exists and the fetched artifact hash changes,
LittleBaby prints a warning and asks for confirmation before proceeding. Use
