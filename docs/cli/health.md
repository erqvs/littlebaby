---
summary: "CLI reference for `littlebaby health` (gateway health snapshot via RPC)"
read_when:
...
littlebaby health --timeout 2500
littlebaby health --verbose
littlebaby health --debug
```

Notes:

- Default `littlebaby health` asks the running gateway for its health snapshot. When the
