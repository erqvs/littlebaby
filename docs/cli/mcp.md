---
summary: "Expose LittleBaby channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to LittleBaby-backed channels
  - Running `littlebaby mcp serve`
  - Managing LittleBaby-saved MCP server definitions
title: "mcp"
---

# mcp

`littlebaby mcp` has two jobs:

- run LittleBaby as an MCP server with `littlebaby mcp serve`
- manage LittleBaby-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is LittleBaby acting as an MCP server
- `list` / `show` / `set` / `unset` is LittleBaby acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`littlebaby acp`](/cli/acp) when LittleBaby should host a coding harness
session itself and route that runtime through ACP.

## LittleBaby as an MCP server

This is the `littlebaby mcp serve` path.

## When to use `serve`

Use `littlebaby mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  LittleBaby-backed channel conversations
- you already have a local or remote LittleBaby Gateway with routed sessions
- you want one MCP server that works across LittleBaby's channel backends instead
  of running separate per-channel bridges

Use [`littlebaby acp`](/cli/acp) instead when LittleBaby should host the coding
runtime itself and keep the agent session inside LittleBaby.

## How it works

`littlebaby mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote LittleBaby Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `littlebaby mcp serve`
...
conversations. A conversation appears when LittleBaby already has session state
...
```bash
# Local Gateway
littlebaby mcp serve

# Remote Gateway
littlebaby mcp serve --url wss://gateway-host:18789 --token-file ~/.littlebaby/gateway.token

# Remote Gateway with password auth
littlebaby mcp serve --url wss://gateway-host:18789 --password-file ~/.littlebaby/gateway.password

# Enable verbose bridge logs
littlebaby mcp serve --verbose

# Disable Claude-specific push notifications
littlebaby mcp serve --claude-channel-mode off
```
...
LittleBaby equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying LittleBaby channel configuration
...
LittleBaby ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `littlebaby mcp serve`
...
## LittleBaby as an MCP client registry

This is the `littlebaby mcp list`, `show`, `set`, and `unset` path.

These commands do not expose LittleBaby over MCP. They manage LittleBaby-owned MCP
server definitions under `mcp.servers` in LittleBaby config.

Those saved definitions are for runtimes that LittleBaby launches or configures
later, such as embedded Pi and other runtime adapters. LittleBaby stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write LittleBaby config
...
LittleBaby also stores a lightweight MCP server registry in config for surfaces
that want LittleBaby-managed MCP definitions.

Commands:

- `littlebaby mcp list`
- `littlebaby mcp show [name]`
- `littlebaby mcp set <name> <json>`
- `littlebaby mcp unset <name>`
...
```bash
littlebaby mcp list
littlebaby mcp show context7 --json
littlebaby mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
littlebaby mcp set docs '{"url":"https://mcp.example.com"}'
littlebaby mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, LittleBaby uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |
