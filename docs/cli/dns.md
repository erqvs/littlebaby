---
summary: "CLI reference for `littlebaby dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: littlebaby.internal)
title: "dns"
---
...
```bash
littlebaby dns setup
littlebaby dns setup --domain littlebaby.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `littlebaby.internal`)
...
- zone file path
- current tailnet IPs
- recommended `littlebaby.json` discovery config
...
- If `--domain` is omitted, LittleBaby uses `discovery.wideArea.domain` from config.
