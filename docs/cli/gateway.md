...
- `--token <token>`: token override (also sets `LITTLEBABY_GATEWAY_TOKEN` for the process).
...
- For password auth on `gateway run`, prefer `LITTLEBABY_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
- In inferred auth mode, shell-only `LITTLEBABY_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
