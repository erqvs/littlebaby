---
summary: "CLI reference for `littlebaby approvals` and `littlebaby exec-policy`"
read_when:
...
# `littlebaby approvals`

Manage exec approvals for the **local host**, **gateway host**, or a **node host**.
By default, commands target the local approvals file on disk. Use `--gateway` to target the gateway, or `--node` to target a specific node.

Alias: `littlebaby exec-approvals`
...
## `littlebaby exec-policy`

`littlebaby exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.littlebaby/exec-approvals.json`

Examples:

```bash
littlebaby exec-policy show
littlebaby exec-policy show --json

littlebaby exec-policy preset yolo
littlebaby exec-policy preset cautious --json

littlebaby exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `littlebaby exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `littlebaby approvals set --gateway`
or `littlebaby approvals set --node <id|name|ip>`.

## Common commands

```bash
littlebaby approvals get
littlebaby approvals get --node <id|name|ip>
littlebaby approvals get --gateway
```

`littlebaby approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
littlebaby approvals set --file ./exec-approvals.json
littlebaby approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
littlebaby approvals set --node <id|name|ip> --file ./exec-approvals.json
littlebaby approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested LittleBaby policy aligned, also set:
...
```bash
littlebaby approvals allowlist add "~/Projects/**/bin/rg"
littlebaby approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
littlebaby approvals allowlist add --agent "*" "/usr/bin/uname"

littlebaby approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `littlebaby nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.littlebaby/exec-approvals.json`.
