---
summary: "CLI reference for `littlebaby secrets` (reload, audit, configure, apply)"
read_when:
...
Use `littlebaby secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
littlebaby secrets reload
littlebaby secrets reload --json
littlebaby secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan LittleBaby state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `littlebaby.json` refs)
...
```bash
littlebaby secrets audit
littlebaby secrets audit --check
littlebaby secrets audit --json
littlebaby secrets audit --allow-exec
```
...
```bash
littlebaby secrets configure
littlebaby secrets configure --plan-out /tmp/littlebaby-secrets-plan.json
littlebaby secrets configure --apply --yes
littlebaby secrets configure --providers-only
littlebaby secrets configure --skip-provider-setup
littlebaby secrets configure --agent ops
littlebaby secrets configure --json
```
...
- `configure` targets secret-bearing fields in `littlebaby.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, LittleBaby fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
littlebaby secrets apply --from /tmp/littlebaby-secrets-plan.json --json
```
...
- `littlebaby.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.littlebaby/.env` known secret keys whose values were migrated
