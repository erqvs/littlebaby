---
summary: "CLI reference for `littlebaby message` (send + channel actions)"
read_when:
...
```
littlebaby message <subcommand> [flags]
```
...
- On cache miss, LittleBaby will attempt a live directory lookup when the provider supports it.
...
- `littlebaby message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, LittleBaby does not force a `default` account SecretRef scope
...
```
littlebaby message send --channel discord \
...
```bash
littlebaby message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
littlebaby message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
littlebaby message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
littlebaby message poll --channel msteams \
...
```
littlebaby message react --channel slack \
...
```
littlebaby message react --channel signal \
...
```
littlebaby message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
littlebaby message send --channel telegram --target @mychat \
