---
summary: "CLI reference for `littlebaby wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `littlebaby wiki`
title: "wiki"
---
...
Use `littlebaby wiki` when you want a compiled knowledge vault with:
...
- `.littlebaby-wiki/cache/agent-digest.json`
- `.littlebaby-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `littlebaby memory search` when the
...
```bash
littlebaby wiki get entity.alpha
littlebaby wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`littlebaby wiki` behavior is shaped by:
