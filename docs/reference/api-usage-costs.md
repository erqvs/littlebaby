...
LittleBaby features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, LittleBaby reads usage from
...
Anthropic still does not expose a per-message dollar estimate that LittleBaby can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `littlebaby status --usage` and `littlebaby channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so LittleBaby inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, LittleBaby prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise LittleBaby falls back to matching OAuth/API-key
...
LittleBaby can pick up credentials from:
...
LittleBaby's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's LittleBaby Claude-login path with **Extra Usage** enabled.
...
- `littlebaby status --usage`
- `littlebaby models status --json`
...
`littlebaby models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
