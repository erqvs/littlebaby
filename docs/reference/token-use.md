---
summary: "How LittleBaby builds prompt context and reports token usage + costs"
read_when:
...
LittleBaby tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

LittleBaby assembles its own system prompt on every run. It includes:
...
For images, LittleBaby downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from LittleBaby session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `littlebaby status --usage` and `littlebaby channels list` show
...
available; otherwise LittleBaby falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, LittleBaby shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. LittleBaby can
...
Anthropic's 1M context window is currently beta-gated. LittleBaby can inject the
...
LittleBaby skips the `context-1m-*` beta header because Anthropic currently
