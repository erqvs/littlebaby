...
LittleBaby integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- LittleBaby owns lifecycle when `channels.signal.autoStart=true`.
...
- LittleBaby spawns `imsg rpc` as a child process (legacy iMessage integration).
