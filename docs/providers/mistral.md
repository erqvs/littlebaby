---
summary: "Use Mistral models and Voxtral transcription with LittleBaby"
read_when:
  - You want to use Mistral models in LittleBaby
...
LittleBaby supports Mistral for both text/image model routing (`mistral/...`) and
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    littlebaby onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    littlebaby onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    littlebaby models list --provider mistral
    ```
  </Step>
</Steps>
...
LittleBaby currently ships this bundled Mistral catalog:
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    ...
    LittleBaby maps the session **thinking** level to Mistral's API:

    | LittleBaby thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
