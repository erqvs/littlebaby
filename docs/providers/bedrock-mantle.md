---
summary: "Use Amazon Bedrock Mantle (OpenAI-compatible) models with LittleBaby"
read_when:
  - You want to use Bedrock Mantle hosted OSS models with LittleBaby
...
LittleBaby includes a bundled **Amazon Bedrock Mantle** provider that connects to
    ...
    **Best for:** using AWS SDK-compatible credentials (shared config, SSO, web identity, instance or task roles).
    ...
    <Steps>
      ...
      <Step title="Verify models are discovered">
        ...
        LittleBaby generates a Mantle bearer token from the credential chain automatically.
      </Step>
    </Steps>

    <Tip>
    When `AWS_BEARER_TOKEN_BEDROCK` is not set, LittleBaby mints the bearer token for you from the AWS default credential chain, including shared credentials/config profiles, SSO, web identity, and instance or task roles.
    </Tip>
...
When `AWS_BEARER_TOKEN_BEDROCK` is set, LittleBaby uses it directly. Otherwise,
LittleBaby attempts to generate a Mantle bearer token from the AWS default
...
<AccordionGroup>
  <Accordion title="Reasoning support">
    ...
    `thinking`, `reasoner`, or `gpt-oss-120b`. LittleBaby sets `reasoning: true`
  ...
  <Accordion title="Endpoint unavailability">
    ...
    silently skipped. LittleBaby does not error; other configured providers
