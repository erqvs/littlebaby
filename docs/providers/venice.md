---
summary: "Use Venice AI privacy-focused models in LittleBaby"
read_when:
  - You want privacy-focused inference in LittleBaby
...
<Steps>
  ...
  <Step title="Configure LittleBaby">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        littlebaby onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    littlebaby agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, LittleBaby shows all available Venice models. Pick based on your needs:
...
```bash
littlebaby models set venice/kimi-k2-5
littlebaby models set venice/claude-opus-4-6
```

List all available models:

```bash
littlebaby models list | grep venice
```

You can also run `littlebaby configure`, select **Model/auth**, and choose **Venice AI**.
...
LittleBaby automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
littlebaby agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
littlebaby agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
littlebaby agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
littlebaby agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
littlebaby agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `littlebaby models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
