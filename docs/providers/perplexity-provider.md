...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    littlebaby config set plugins.entries.perplexity.config.webSearch.apiKey "pplx-xxxxxxxxxxxx"
    ```
...
<Tabs>
  <Tab title="Native Perplexity API (pplx-)">
    When your key starts with `pplx-`, LittleBaby uses the native Perplexity Search
  ...
  <Tab title="OpenRouter / Sonar (sk-or-)">
    When your key starts with `sk-or-`, LittleBaby routes through OpenRouter using
...
<AccordionGroup>
  ...
  <Accordion title="OpenRouter proxy setup">
    ...
    LittleBaby will detect the prefix and switch to the Sonar transport
    automatically.
