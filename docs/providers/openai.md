---
summary: "Use OpenAI via API keys or Codex subscription in LittleBaby"
read_when:
  - You want to use OpenAI models in LittleBaby
...
OpenAI provides developer APIs for GPT models. LittleBaby supports two auth routes:
...
OpenAI explicitly supports subscription OAuth usage in external tools and workflows like LittleBaby.
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        littlebaby onboard --auth-choice openai-api-key
        ```

        Or pass the key directly:

        ```bash
        littlebaby onboard --openai-api-key "$OPENAI_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        littlebaby models list --provider openai
        ```
      </Step>
    </Steps>
    ...
    <Warning>
    LittleBaby does **not** expose `openai/gpt-5.3-codex-spark` on the direct API path. Live OpenAI API requests reject that model. Spark is Codex-only.
    </Warning>
    ...
    **Best for:** using your ChatGPT/Codex subscription instead of a separate API key. Codex cloud requires ChatGPT sign-in.
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        littlebaby onboard --auth-choice openai-codex
        ```

        Or run OAuth directly:

        ```bash
        littlebaby models auth login --provider openai-codex
        ```
      </Step>
      <Step title="Set the default model">
        ```bash
        littlebaby config set agents.defaults.model.primary openai-codex/gpt-5.4
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        littlebaby models list --provider openai-codex
        ```
      </Step>
    </Steps>
    ...
    <Tip>
    If onboarding reuses an existing Codex CLI login, those credentials stay managed by Codex CLI. On expiry, LittleBaby re-reads the external Codex source first and writes the refreshed credential back to Codex storage.
    </Tip>

    ### Context window cap

    LittleBaby treats model metadata and the runtime context cap as separate values.
...
LittleBaby adds a small OpenAI-specific prompt overlay for `openai/*` and `openai-codex/*` runs. The overlay keeps the assistant warm, collaborative, concise, and a little more emotionally expressive without replacing the base system prompt.

| Value                  | Effect                             |
| ---------------------- | ---------------------------------- |
| `"friendly"` (default) | Enable the OpenAI-specific overlay |
| `"on"`                 | Alias for `"friendly"`             |
| `"off"`                | Use base LittleBaby prompt only      |
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    littlebaby config set plugins.entries.openai.config.personality off
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    LittleBaby uses WebSocket-first with SSE fallback (`"auto"`) for both `openai/*` and `openai-codex/*`.

    In `"auto"` mode, LittleBaby:
  ...
  <Accordion title="WebSocket warm-up">
    LittleBaby enables WebSocket warm-up by default for `openai/*` to reduce first-turn latency.
  ...
  <Accordion title="Fast mode">
    LittleBaby exposes a shared fast-mode toggle for both `openai/*` and `openai-codex/*`:

    - **Chat/UI:** `/fast status|on|off`
    - **Config:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    When enabled, LittleBaby maps fast mode to OpenAI priority processing (`service_tier = "priority"`). Existing `service_tier` values are preserved, and fast mode does not rewrite `reasoning` or `text.verbosity`.
  ...
  <Accordion title="Priority processing (service_tier)">
    OpenAI's API exposes priority processing via `service_tier`. Set it per model in LittleBaby:
    ...
    <Warning>
    `serviceTier` is only forwarded to native OpenAI endpoints (`api.openai.com`) and native Codex endpoints (`chatgpt.com/backend-api`). If you route either provider through a proxy, LittleBaby leaves `service_tier` untouched.
    </Warning>

  </Accordion>

  <Accordion title="Server-side compaction (Responses API)">
    For direct OpenAI Responses models (`openai/*` on `api.openai.com`), LittleBaby auto-enables server-side compaction:
  ...
  <Accordion title="Strict-agentic GPT mode">
    For GPT-5-family runs on `openai/*` and `openai-codex/*`, LittleBaby can use a stricter embedded execution contract:
    ...
    With `strict-agentic`, LittleBaby:
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    LittleBaby treats direct OpenAI, Codex, and Azure OpenAI endpoints differently from generic OpenAI-compatible `/v1` proxies:
