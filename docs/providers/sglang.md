---
summary: "Run LittleBaby with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run LittleBaby against a local SGLang server
...
LittleBaby can connect to SGLang using the `openai-completions` API.

LittleBaby can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, LittleBaby will query:
