---
summary: "GLM model family overview + how to use it in LittleBaby"
read_when:
  - You want GLM models in LittleBaby
...
GLM is a **model family** (not a company) available through the Z.AI platform. In LittleBaby, GLM
...
<Steps>
  ...
  <Step title="Set GLM as the default model">
    ```bash
    littlebaby config set agents.defaults.model.primary "zai/glm-5.1"
    ```
  </Step>
...
<Tip>
`zai-api-key` lets LittleBaby detect the matching Z.AI endpoint from the key and
...
LittleBaby currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, LittleBaby inspects the key format
