---
summary: "Run LittleBaby through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run LittleBaby against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact LittleBaby compat flags for inferrs
title: "inferrs"
---
...
OpenAI-compatible `/v1` API. LittleBaby works with `inferrs` through the generic
`openai-completions` path.

`inferrs` is currently best treated as a custom self-hosted OpenAI-compatible
backend, not a dedicated LittleBaby provider plugin.
...
<Steps>
  ...
  <Step title="Add an LittleBaby provider entry">
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    <Warning>
    If LittleBaby runs fail with an error like:
    ...
    LittleBaby will flatten pure text content parts into plain strings before sending
    the request.
  ...
  <Accordion title="Gemma and tool-schema caveat">
    ...
    `/v1/chat/completions` requests but still fail on full LittleBaby agent-runtime
    turns.
    ...
    That disables LittleBaby's tool schema surface for the model and can reduce prompt
    pressure on stricter local backends.

    If tiny direct requests still work but normal LittleBaby agent turns continue to
    crash inside `inferrs`, the remaining issue is usually upstream model/server
    behavior rather than LittleBaby's transport layer.
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but littlebaby infer model run fails">
  ...
  <Accordion title="inferrs still crashes on larger agent turns">
    If LittleBaby no longer gets schema errors but `inferrs` still crashes on larger
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running LittleBaby against local model servers.
  </Card>
