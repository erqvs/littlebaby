---
summary: "Use xAI Grok models in LittleBaby"
read_when:
  - You want to use Grok models in LittleBaby
...
LittleBaby ships a bundled `xai` provider plugin for Grok models.
...
<Steps>
  ...
  <Step title="Set your API key">
    ...
    ```bash
    littlebaby onboard --auth-choice xai-api-key
    ```
...
<Note>
LittleBaby uses the xAI Responses API as the bundled xAI transport. The same
...
LittleBaby includes these xAI model families out of the box:
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    littlebaby config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an LittleBaby tool for searching
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an LittleBaby tool for
  ...
  <Accordion title="Known limits">
    ...
    - `grok-4.20-multi-agent-experimental-beta-0304` is not supported on the
      ...
      surface than the standard LittleBaby xAI transport.
  </Accordion>

  <Accordion title="Advanced notes">
    - LittleBaby applies xAI-specific tool-schema and tool-call compatibility fixes
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as LittleBaby
      tools. LittleBaby enables the specific xAI built-in it needs inside each tool
