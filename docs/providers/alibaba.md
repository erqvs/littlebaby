...
summary: "Alibaba Model Studio Wan video generation in LittleBaby"
read_when:
  - You want to use Alibaba Wan video generation in LittleBaby
  - You need Model Studio or DashScope API key setup for video generation
---

# Alibaba Model Studio

LittleBaby ships a bundled `alibaba` video-generation provider for Wan models on
...
<Steps>
  ...
  <Step title="Verify the provider is available">
    ```bash
    littlebaby models list --provider alibaba
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Auth key priority">
    LittleBaby checks for auth keys in this order:
