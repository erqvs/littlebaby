---
summary: "Use Synthetic's Anthropic-compatible API in LittleBaby"
read_when:
...
LittleBaby registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    littlebaby onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
LittleBaby's Anthropic client appends `/v1` to the base URL automatically, so use
...
`littlebaby models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that LittleBaby appends `/v1` automatically.
