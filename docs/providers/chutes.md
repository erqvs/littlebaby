...
read_when:
  - You want to use Chutes with LittleBaby
...
OpenAI-compatible API. LittleBaby supports both browser OAuth and direct API-key
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run the OAuth onboarding flow">
        ```bash
        littlebaby onboard --auth-choice chutes
        ```
        LittleBaby launches the browser flow locally, or shows a URL + redirect-paste
        flow on remote/headless hosts. OAuth tokens auto-refresh through LittleBaby auth
        profiles.
      </Step>
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run the API key onboarding flow">
        ```bash
        littlebaby onboard --auth-choice chutes-api-key
        ```
      </Step>
      <Step title="Verify the default model">
        After onboarding, the default model is set to
...
When Chutes auth is available, LittleBaby queries the Chutes catalog with that
credential and uses the discovered models. If discovery fails, LittleBaby falls
back to a bundled static catalog so onboarding and startup still work.

## Default aliases

LittleBaby registers three convenience aliases for the bundled Chutes catalog:
