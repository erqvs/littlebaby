---
summary: "Use Qwen Cloud via LittleBaby's bundled qwen provider"
read_when:
  - You want to use Qwen with LittleBaby
...
See [Issue #49557](https://github.com/littlebaby/littlebaby/issues/49557) for
background.

</Warning>

LittleBaby now treats Qwen as a first-class bundled provider with canonical id
    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        littlebaby onboard --auth-choice qwen-api-key
        ```

        For the **China** endpoint:

        ```bash
        littlebaby onboard --auth-choice qwen-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        littlebaby models list --provider qwen
        ```
      </Step>
    </Steps>
    ...
    **Best for:** pay-as-you-go access through the Standard Model Studio endpoint, including models like `qwen3.6-plus` that may not be available on the Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        littlebaby onboard --auth-choice qwen-standard-api-key
        ```

        For the **China** endpoint:

        ```bash
        littlebaby onboard --auth-choice qwen-standard-api-key-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
LittleBaby currently ships this bundled Qwen catalog. The configured catalog is
...
<AccordionGroup>
  ...
  <Accordion title="Video generation details">
    For video generation, LittleBaby maps the configured Qwen region to the matching
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    shared `openai-completions` transport. LittleBaby keys that off endpoint
  ...
  <Accordion title="Environment and daemon setup">
    ...
    available to that process (for example, in `~/.littlebaby/.env` or via
    `env.shellEnv`).
  </Accordion>
</AccordionGroup>
