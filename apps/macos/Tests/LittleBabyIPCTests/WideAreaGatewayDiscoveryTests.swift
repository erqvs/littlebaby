...
import Foundation
import Testing
@testable import LittleBabyDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("LITTLEBABY_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("LITTLEBABY_WIDE_AREA_DOMAIN", "littlebaby.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("LITTLEBABY_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("LITTLEBABY_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._littlebaby-gw._tcp.littlebaby.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.littlebaby.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (LittleBaby)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/littlebaby/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (LittleBaby)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/littlebaby/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._littlebaby-gw._tcp.littlebaby.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
