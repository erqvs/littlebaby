...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers open littlebaby binary`() throws {
        ...
        let cmd = CommandResolver.littlebabyCommand(
            subcommand: "gateway",
            defaults: defaults,
            configRoot: [:],
            searchPaths: searchPaths,
            projectRoot: tmp)
        #expect(cmd.prefix(2).elementsEqual([littlebabyPath.path, "gateway"]))
    }
    ...
    @Test func `falls back to node and script`() throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/littlebaby.js")
    ...
    @Test func `prefers open littlebaby binary over pnpm`() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()

        let binDir = tmp.appendingPathComponent("bin")
        let littlebabyPath = binDir.appendingPathComponent("littlebaby")
        ...
        #expect(cmd.prefix(2).elementsEqual([littlebabyPath.path, "rpc"]))
    }

    @Test func `uses open littlebaby binary without node runtime`() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "littlebaby", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "littlebaby", "health", "--json"]))
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("littlebaby@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/littlebaby", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "littlebaby@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/littlebaby'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("littlebaby"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == littlebabyPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
