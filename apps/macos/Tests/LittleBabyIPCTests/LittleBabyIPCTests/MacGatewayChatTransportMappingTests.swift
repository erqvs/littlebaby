...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: LittleBabyProtocol.AnyCodable(["ok": LittleBabyProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: LittleBabyProtocol.AnyCodable(["ok": LittleBabyProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = LittleBabyProtocol.AnyCodable([
            "runId": LittleBabyProtocol.AnyCodable("run-1"),
            "sessionKey": LittleBabyProtocol.AnyCodable("main"),
            "state": LittleBabyProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: LittleBabyProtocol.AnyCodable(["a": LittleBabyProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
