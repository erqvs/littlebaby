...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: LittleBabyProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: LittleBabyProtocol.AnyCodable] = [
            "presence": .init([LittleBabyProtocol.AnyCodable(entry)]),
        ]
        let payload = LittleBabyProtocol.AnyCodable(payloadMap)
