...
struct ConnectOptions {
    ...
    var clientId: String = "littlebaby-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        print("""
        littlebaby-mac connect

        Usage:
          littlebaby-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: littlebaby-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "LittleBaby macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("LittleBaby macOS Gateway Connect")
