...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.littlebaby", category: "portguard")
    #if DEBUG
    private var testingDescriptors: [Int: Descriptor] = [:]
    #endif
    private nonisolated static let appSupportDir: URL = {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("LittleBaby", isDirectory: true)
    }()
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "littlebaby", "tsx", "pnpm", "bun"]
    ...
    private static func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("littlebaby-gateway")
                || cmd.contains("littlebaby-gateway")
            {
                return true
            }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("littlebaby"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
