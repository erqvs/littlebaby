...
private let webChatSwiftLogger = Logger(subsystem: "ai.littlebaby", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "littlebaby.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: LittleBabyChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> LittleBabyChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(LittleBabyChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            LittleBabyChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return LittleBabyChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> LittleBabyChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                LittleBabyGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    LittleBabyChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    LittleBabyAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: LittleBabyProtocol.ModelChoice) -> LittleBabyChatModelChoice {
        LittleBabyChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<LittleBabyChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any LittleBabyChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = LittleBabyChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: LittleBabyChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<LittleBabyChatView>) -> NSViewController
    {
