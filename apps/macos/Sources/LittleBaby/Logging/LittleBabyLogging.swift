...
enum LittleBabyLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = LittleBabyOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = LittleBabyFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        LittleBabyLogging.bootstrapIfNeeded()
        let label = LittleBabyLogging.makeLabel(subsystem: subsystem, category: category)
...
struct LittleBabyOSLogHandler: AppLogLevelBackedHandler {
...
struct LittleBabyFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = LittleBabyLogging.parseLabel(self.label)
