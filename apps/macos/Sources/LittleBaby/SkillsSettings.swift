...
private struct SkillRow: View {
    ...
    private var sourceLabel: String {
        ...
        case "littlebaby-bundled":
            "Bundled"
        case "littlebaby-managed":
            "Managed"
        case "littlebaby-workspace":
            "Workspace"
        case "littlebaby-extra":
            "Extra"
        case "littlebaby-plugin":
            "Plugin"
        default:
            self.skill.source
        }
...
private struct EnvEditorView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Saved to littlebaby.json under skills.entries.\(self.editor.skillKey)")
                .font(.caption2)
            ...
            do {
                if isPrimary {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        apiKey: value)
                    self.statusMessage = "Saved API key — stored in littlebaby.json (skills.entries.\(skillKey))"
                } else {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        env: [envKey: value])
                    self.statusMessage = "Saved \(envKey) — stored in littlebaby.json (skills.entries.\(skillKey).env)"
                }
            } catch {
                self.statusMessage = error.localizedDescription
            }
