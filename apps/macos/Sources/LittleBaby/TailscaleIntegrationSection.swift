...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.littlebaby/littlebaby.json. Prefer LITTLEBABY_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.littlebaby/littlebaby.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.littlebaby/littlebaby.json. Restart the gateway to apply."
        }
