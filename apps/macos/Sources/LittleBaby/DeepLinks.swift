...
private let deepLinkLogger = Logger(subsystem: "ai.littlebaby", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "LittleBaby is paused", message: "Unpause LittleBaby to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run LittleBaby agent?", message: body) else { return }
        }
