    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Allow these so LittleBaby can notify and capture when needed.")
                    .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = LittleBabyLocationMode.off.rawValue
    ...
    private var locationMode: LittleBabyLocationMode {
        LittleBabyLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: LittleBabyLocationMode) async -> Bool {
