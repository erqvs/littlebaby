...
enum DebugActions {
    private static let verboseDefaultsKey = "littlebaby.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "littlebaby.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = LittleBabyPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "LittleBaby", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = LittleBabyPaths.configURL
        guard
