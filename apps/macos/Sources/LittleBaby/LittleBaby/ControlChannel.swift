...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: LittleBabyProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.littlebaby", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: LittleBabyKit.AnyCodable]()) {
                $0[$1.key] = LittleBabyKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: LittleBabyProtocol.AnyCodable?) -> [String: LittleBabyProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: LittleBabyProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: LittleBabyKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: LittleBabyProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("littlebaby.control.heartbeat")
    static let controlAgentEvent = Notification.Name("littlebaby.control.agent")
}
