...
enum CommandResolver {
    private static let projectRootDefaultsKey = "littlebaby.gatewayProjectRootPath"
    private static let helperName = "littlebaby"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let littlebabyEntry = root.appendingPathComponent("littlebaby.mjs").path
        if FileManager().isReadableFile(atPath: littlebabyEntry) { return littlebabyEntry }
        let binEntry = root.appendingPathComponent("bin/littlebaby.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/littlebaby")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let littlebabyPaths = self.littlebabyManagedPaths(home: home)
        if !littlebabyPaths.isEmpty {
            extras.insert(contentsOf: littlebabyPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + littlebabyPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func littlebabyManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".littlebaby"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func littlebabyExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectLittleBabyExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("littlebaby.mjs").path,
            root.appendingPathComponent("bin/littlebaby.js").path,
        ]
    ...
    static func hasAnyLittleBabyInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.littlebabyExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func littlebabyNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let littlebabyPath = self.projectLittleBabyExecutable(projectRoot: root) {
            return [littlebabyPath, subcommand] + extraArgs
        }
        if let littlebabyPath = self.littlebabyExecutable(searchPaths: searchPaths) {
            return [littlebabyPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "littlebaby", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            littlebaby entrypoint missing (looked for dist/index.js or littlebaby.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real littlebaby CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/littlebaby"
        ...
        if command -v littlebaby >/dev/null 2>&1; then
          CLI="$(command -v littlebaby)"
          littlebaby \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/littlebaby.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/littlebaby.mjs"
            node "$PRJ/littlebaby.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/littlebaby.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/littlebaby.js"
            node "$PRJ/bin/littlebaby.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent littlebaby"
          pnpm --silent littlebaby \(quotedArgs);
        else
          echo "littlebaby CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? LittleBabyConfigFile.loadDict()
