...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.littlebaby", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["LITTLEBABY_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = LittleBabyConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.littlebabyExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "littlebaby CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["LITTLEBABY_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "littlebaby@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "littlebaby@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "littlebaby@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "littlebaby@\(target)"])
            }

        statusHandler("Installing littlebaby@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed littlebaby@\(target)")
        } else {
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("littlebaby ") {
            normalized = String(normalized.dropFirst("littlebaby ".count))
        }
