...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.littlebaby", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> LittleBabySessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: LittleBabySessionPreviewEntry,
    ...
    private static func cache(payload: LittleBabySessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: LittleBabyChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [LittleBabyKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [LittleBabyKit.AnyCodable]) -> [LittleBabyChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(LittleBabyChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: LittleBabyChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: LittleBabyChatMessage) -> Bool {
    ...
    private static func toolNames(for message: LittleBabyChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: LittleBabyChatMessage) -> String? {
