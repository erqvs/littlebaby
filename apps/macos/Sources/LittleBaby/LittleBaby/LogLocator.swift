...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["LITTLEBABY_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/littlebaby")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("littlebaby-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("littlebaby-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/littlebaby/ (rolling or stdout), or nil if none exist.
