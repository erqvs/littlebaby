...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.littlebaby.mac`) even if the shipped
    ...
    {
        if environment["LITTLEBABY_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "littlebaby.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "littlebaby.nixMode") { return true }
