...
final class RemindersService: RemindersServicing {
    func list(params: LittleBabyRemindersListParams) async throws -> LittleBabyRemindersListPayload {
        ...
        let payload: [LittleBabyReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return LittleBabyReminderPayload(
        ...
        return LittleBabyRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: LittleBabyRemindersAddParams) async throws -> LittleBabyRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = LittleBabyReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return LittleBabyRemindersAddPayload(reminder: payload)
    }
