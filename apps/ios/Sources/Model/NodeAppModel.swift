...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.littlebaby.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.littlebaby.ios", category: "PushWake")
    private let pendingActionLogger = Logger(subsystem: "ai.littlebaby.ios", category: "PendingAction")
    private let locationWakeLogger = Logger(subsystem: "ai.littlebaby.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.littlebaby.ios", category: "WatchReply")
    private let watchExecApprovalLogger = Logger(subsystem: "ai.littlebaby.ios", category: "WatchExecApproval")
    private let execApprovalNotificationLogger = Logger(
        subsystem: "ai.littlebaby.ios",
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = LittleBabyCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = LittleBabyCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = LittleBabyCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = LittleBabyCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = LittleBabyCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: LittleBabyLocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(LittleBabyGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: LittleBabyNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: LittleBabyNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: LittleBabyNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(LittleBabyLocationGetParams.self, from: req.paramsJSON)) ??
            LittleBabyLocationGetParams()
        ...
        let payload = LittleBabyLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LittleBabyCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(LittleBabyCanvasPresentParams.self, from: req.paramsJSON)) ??
                LittleBabyCanvasPresentParams()
        ...
        case LittleBabyCanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case LittleBabyCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(LittleBabyCanvasNavigateParams.self, from: req.paramsJSON)
            ...
            return BridgeInvokeResponse(id: req.id, ok: true)
        case LittleBabyCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(LittleBabyCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case LittleBabyCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(LittleBabyCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case LittleBabyCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.littlebabyA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing littlebabyA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)

        case LittleBabyCanvasA2UICommand.push.rawValue, LittleBabyCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [LittleBabyKit.AnyCodable]
            if command == LittleBabyCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(LittleBabyCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try LittleBabyCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(LittleBabyCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try LittleBabyCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.littlebabyA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LittleBabyCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case LittleBabyCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(LittleBabyCameraSnapParams.self, from: req.paramsJSON)) ??
                LittleBabyCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case LittleBabyCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(LittleBabyCameraClipParams.self, from: req.paramsJSON)) ??
                LittleBabyCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(LittleBabyScreenRecordParams.self, from: req.paramsJSON)) ??
            LittleBabyScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(LittleBabySystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: LittleBabyNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: LittleBabyNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: LittleBabyNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(LittleBabyChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: LittleBabyNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = LittleBabyChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LittleBabyDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case LittleBabyDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(LittleBabyPhotosLatestParams.self, from: req.paramsJSON)) ??
            LittleBabyPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LittleBabyContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(LittleBabyContactsSearchParams.self, from: req.paramsJSON)) ??
                LittleBabyContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case LittleBabyContactsCommand.add.rawValue:
            let params = try Self.decodeParams(LittleBabyContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LittleBabyCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(LittleBabyCalendarEventsParams.self, from: req.paramsJSON)) ??
                LittleBabyCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case LittleBabyCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(LittleBabyCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LittleBabyRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(LittleBabyRemindersListParams.self, from: req.paramsJSON)) ??
                LittleBabyRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case LittleBabyRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(LittleBabyRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LittleBabyMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(LittleBabyMotionActivityParams.self, from: req.paramsJSON)) ??
                LittleBabyMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case LittleBabyMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(LittleBabyPedometerParams.self, from: req.paramsJSON)) ??
                LittleBabyPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LittleBabyTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case LittleBabyTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case LittleBabyTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case LittleBabyTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([LittleBabyLocationCommand.get.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleLocationInvoke(req)
        }

        register([
            LittleBabyCanvasCommand.present.rawValue,
            LittleBabyCanvasCommand.hide.rawValue,
            LittleBabyCanvasCommand.navigate.rawValue,
            LittleBabyCanvasCommand.evalJS.rawValue,
            LittleBabyCanvasCommand.snapshot.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            LittleBabyCanvasA2UICommand.reset.rawValue,
            LittleBabyCanvasA2UICommand.push.rawValue,
            LittleBabyCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([LittleBabyScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([LittleBabySystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([LittleBabyChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            LittleBabyWatchCommand.status.rawValue,
            LittleBabyWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([LittleBabyPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            LittleBabyRemindersCommand.list.rawValue,
            LittleBabyRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            LittleBabyTalkCommand.pttStart.rawValue,
            LittleBabyTalkCommand.pttStop.rawValue,
            LittleBabyTalkCommand.pttCancel.rawValue,
            LittleBabyTalkCommand.pttOnce.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case LittleBabyWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = LittleBabyWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case LittleBabyWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(LittleBabyWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = LittleBabyWatchNotifyPayload(
    ...
    func locationMode() -> LittleBabyLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return LittleBabyLocationMode(rawValue: raw) ?? .off
    }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "LittleBaby Share Self-Test",
            url: URL(string: "https://littlebaby.ai/share-self-test"),
    ...
    private static func makeWatchExecApprovalItem(from prompt: ExecApprovalPrompt) -> LittleBabyWatchExecApprovalItem {
        let decisions = prompt.allowedDecisions.compactMap { decision in
            ...
            return LittleBabyWatchExecApprovalDecision(rawValue: normalizedDecision)
        }
        let preview = Self.trimmedOrNil(prompt.commandPreview) ?? Self.trimmedOrNil(prompt.commandText)
        return LittleBabyWatchExecApprovalItem(
            id: prompt.id,
    ...
    private func publishWatchExecApprovalPrompt(_ prompt: ExecApprovalPrompt, reason: String) async {
        let message = LittleBabyWatchExecApprovalPromptMessage(
        ...
        source: String) async
    {
        ...
        let message = LittleBabyWatchExecApprovalResolvedMessage(
    ...
    private func publishWatchExecApprovalExpired(
        approvalId: String,
        reason: LittleBabyWatchExecApprovalCloseReason) async
    {
        let normalizedApprovalID = approvalId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalizedApprovalID.isEmpty else { return }
        self.removeWatchExecApprovalPrompt(normalizedApprovalID)
        let message = LittleBabyWatchExecApprovalExpiredMessage(
            approvalId: normalizedApprovalID,
            reason: reason,
    ...
    private func syncWatchExecApprovalSnapshot(reason: String) async {
        ...
        let message = LittleBabyWatchExecApprovalSnapshotMessage(
            approvals: approvals,
            sentAtMs: Int(Date().timeIntervalSince1970 * 1000),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.littlebabyPushKind(userInfo)
        let receivedMessage =
    ...
    private static func littlebabyPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["littlebaby"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["littlebaby"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
    ...
    private func resolveExecApprovalNotificationDecision(
        ...
        guard connected else {
            ...
            return .failed(message: "LittleBaby couldn't connect to the gateway operator session.")
        }
        ...
        do {
            ...
            await self.publishWatchExecApprovalResolved(
                approvalId: normalizedApprovalID,
                decision: LittleBabyWatchExecApprovalDecision(rawValue: normalizedDecision),
                source: "iphone")
            return .resolved
        } catch {
            if Self.isApprovalNotificationStaleError(error) {
            ...
            return .failed(
                message: "LittleBaby couldn't resolve this approval right now. Try again.")
        }
    }
