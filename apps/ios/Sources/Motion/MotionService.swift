...
final class MotionService: MotionServicing {
    func activities(params: LittleBabyMotionActivityParams) async throws -> LittleBabyMotionActivityPayload {
        ...
        let mapped: [LittleBabyMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return LittleBabyMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: LittleBabyPedometerParams) async throws -> LittleBabyPedometerPayload {
        ...
        let payload: LittleBabyPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = LittleBabyPedometerPayload(
