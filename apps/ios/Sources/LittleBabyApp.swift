...
@MainActor
enum LittleBabyAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class LittleBabyAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.littlebaby.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.littlebaby.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.littlebaby.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? LittleBabyAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "littlebaby.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "littlebaby.watch.promptId"
    static let sessionKeyKey = "littlebaby.watch.sessionKey"
    static let actionPrimaryIDKey = "littlebaby.watch.action.primary.id"
    static let actionPrimaryLabelKey = "littlebaby.watch.action.primary.label"
    static let actionSecondaryIDKey = "littlebaby.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "littlebaby.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "littlebaby.watch.action.primary"
    static let actionSecondaryIdentifier = "littlebaby.watch.action.secondary"
    static let actionIdentifierPrefix = "littlebaby.watch.action."
    static let actionIDKeyPrefix = "littlebaby.watch.action.id."
    static let actionLabelKeyPrefix = "littlebaby.watch.action.label."
    static let categoryPrefix = "littlebaby.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: LittleBabyWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> LittleBabyWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return LittleBabyWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "LittleBaby" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [LittleBabyWatchAction]) -> [UNNotificationAction] {
...
@main
struct LittleBabyApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(LittleBabyAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        LittleBabyAppModelRegistry.appModel = appModel
...
extension LittleBabyApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("LittleBaby: installing uncaught exception handler")
