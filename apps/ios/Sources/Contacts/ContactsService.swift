...
final class ContactsService: ContactsServicing {
    ...
    func search(params: LittleBabyContactsSearchParams) async throws -> LittleBabyContactsSearchPayload {
        ...
        return LittleBabyContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: LittleBabyContactsAddParams) async throws -> LittleBabyContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return LittleBabyContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return LittleBabyContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> LittleBabyContactPayload {
        LittleBabyContactPayload(
