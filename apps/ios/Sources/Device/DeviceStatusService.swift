...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> LittleBabyDeviceStatusPayload {
        ...
        return LittleBabyDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> LittleBabyDeviceInfoPayload {
        ...
        return LittleBabyDeviceInfoPayload(
    ...
    private func batteryStatus() -> LittleBabyBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: LittleBabyBatteryState = switch device.batteryState {
        ...
        return LittleBabyBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> LittleBabyThermalStatusPayload {
        let state: LittleBabyThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return LittleBabyThermalStatusPayload(state: state)
    }

    private func storageStatus() -> LittleBabyStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return LittleBabyStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
