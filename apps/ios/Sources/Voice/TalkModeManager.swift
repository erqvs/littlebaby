...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__LITTLEBABY_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<LittleBabyTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.littlebaby", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = LittleBabyTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> LittleBabyTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return LittleBabyTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> LittleBabyTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> LittleBabyTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return LittleBabyTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> LittleBabyTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = LittleBabyTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: LittleBabyTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: LittleBabyAgentEventPayload.self
            ) else {
                continue
            }
