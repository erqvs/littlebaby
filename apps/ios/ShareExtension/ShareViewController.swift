...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.littlebaby.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to LittleBaby", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to LittleBaby gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to LittleBaby.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "LittleBabyShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "LittleBaby is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "LittleBaby Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("littlebaby-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
            ...
            try await gateway.connect(
                ...
                bootstrapToken: nil,
                password: config.password,
                connectOptions: makeOptions("littlebabybot-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
