...
import ai.littlebaby.app.gateway.GatewaySession
...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      LittleBabyCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      LittleBabyCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      LittleBabyCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      LittleBabyCanvasA2UICommand.Push.rawValue, LittleBabyCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      LittleBabyCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      LittleBabyCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      LittleBabyCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      LittleBabyLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      LittleBabyDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      LittleBabyDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      LittleBabyDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      LittleBabyDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      LittleBabyNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      LittleBabyNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      LittleBabySystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Photos command
      ai.littlebaby.app.protocol.LittleBabyPhotosCommand.Latest.rawValue -> photosHandler.handlePhotosLatest(
        paramsJson,
      )

      // Contacts command
      LittleBabyContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      LittleBabyContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      LittleBabyCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      LittleBabyCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      LittleBabyMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      LittleBabyMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      LittleBabySmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      LittleBabySmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      LittleBabyCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
