...
private const val NOTIFICATION_CHANNEL_BASE_ID = "littlebaby.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "LittleBaby Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "LittleBaby Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "LittleBaby Active")
      }
