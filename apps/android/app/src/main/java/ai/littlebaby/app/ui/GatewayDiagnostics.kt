...
internal fun littleBabyAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this LittleBaby Android gateway connection failure.
    ...
    - tell me whether `littlebaby devices list` should show a pending pairing request
    - if more signal is needed, ask for `littlebaby qr --json`, `littlebaby devices list`, and `littlebaby nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${littleBabyAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("LittleBaby gateway diagnostics", report))
