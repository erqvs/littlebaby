...
- Play build: `apps/android/build/release-bundles/littlebaby-<version>-play-release.aab`
- Third-party build: `apps/android/build/release-bundles/littlebaby-<version>-third-party-release.aab`
...
```bash
pnpm littlebaby gateway --port 18789 --verbose
```
...
- Canvas web content already supports live reload when loaded from Gateway `__littlebaby__/canvas/` (see `docs/platforms/android.md`).
...
Current LittleBaby Android implication:
...
2) Android app is connected to that gateway and `littlebaby nodes status` shows it as paired + connected.
3) App stays unlocked and in foreground for the whole run.
4) Open the app **Screen** tab and keep it active during the run (canvas/A2UI commands require the canvas WebView attached there).
5) Grant runtime permissions for capabilities you expect to pass (camera/mic/location/notification listener/location, etc.).
6) No interactive system dialogs should be pending before test start.
7) Canvas host is enabled and reachable from the device (do not run gateway with `LITTLEBABY_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__littlebaby__/`).
...
- `LITTLEBABY_ANDROID_GATEWAY_URL=ws://...` (default: from your local LittleBaby config)
- `LITTLEBABY_ANDROID_GATEWAY_TOKEN=...`
- `LITTLEBABY_ANDROID_GATEWAY_PASSWORD=...`
...
- `pairing required` before tests start:
  - approve pending device pairing (`littlebaby devices approve --latest`) and rerun.
