...
7) Canvas host is enabled and reachable from the device (do not run gateway with `LITTLEBABY_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__openclaw__/`).
...
- `LITTLEBABY_ANDROID_GATEWAY_URL=ws://...` (default: from your local OpenClaw config)
