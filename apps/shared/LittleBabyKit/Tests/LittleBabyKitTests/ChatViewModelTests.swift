...
import Foundation
import Testing
@testable import LittleBabyChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> LittleBabyChatHistoryPayload
{
    LittleBabyChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> LittleBabyChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> LittleBabyChatSessionEntry
{
    LittleBabyChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> LittleBabyChatModelChoice {
    LittleBabyChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [LittleBabyChatHistoryPayload],
    sessionsResponses: [LittleBabyChatSessionsListResponse] = [],
    modelResponses: [[LittleBabyChatModelChoice]] = [],
    ...
    -> (TestChatTransport, LittleBabyChatViewModel)
{
...
private func sendUserMessage(_ vm: LittleBabyChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, LittleBabyChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [LittleBabyChatHistoryPayload]
    private let sessionsResponses: [LittleBabyChatSessionsListResponse]
    private let modelResponses: [[LittleBabyChatModelChoice]]
    ...
    private let stream: AsyncStream<LittleBabyChatTransportEvent>
    private let continuation: AsyncStream<LittleBabyChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<LittleBabyChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> LittleBabyChatHistoryPayload {
        ...
        return self.historyResponses.last ?? LittleBabyChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [LittleBabyChatAttachmentPayload]) async throws -> LittleBabyChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return LittleBabyChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> LittleBabyChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? LittleBabyChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: LittleBabyChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = LittleBabyChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = LittleBabyChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: LittleBabyChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = LittleBabyChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: LittleBabyChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(LittleBabyChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == LittleBabyChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = LittleBabyChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: LittleBabyChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = LittleBabyChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = LittleBabyChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = LittleBabyChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { LittleBabyChatViewModel(sessionKey: "main", transport: transport) }
