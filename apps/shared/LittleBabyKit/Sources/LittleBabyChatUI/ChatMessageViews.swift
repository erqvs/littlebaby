...
struct ChatMessageBubble: View {
    let message: LittleBabyChatMessage
    let style: LittleBabyChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? LittleBabyChatTheme.userText : LittleBabyChatTheme.assistantText
    ...
    private var inlineAttachments: [LittleBabyChatMessageContent] {
    ...
    private var toolCalls: [LittleBabyChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [LittleBabyChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? LittleBabyChatTheme.userBubble
        }
        if self.style == .onboarding {
            return LittleBabyChatTheme.onboardingAssistantBubble
        }
        return LittleBabyChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return LittleBabyChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: LittleBabyChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? LittleBabyChatTheme.userText : LittleBabyChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: LittleBabyChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(LittleBabyChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [LittleBabyChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: LittleBabyChatTheme.assistantText)
            }
        }
    }
}
