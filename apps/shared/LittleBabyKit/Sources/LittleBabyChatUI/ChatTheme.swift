...
enum LittleBabyChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("LittleBabyChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("LittleBabyChatTheme.onboardingAssistantBubble"),
...
enum LittleBabyPlatformImageFactory {
    static func image(_ image: LittleBabyPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
