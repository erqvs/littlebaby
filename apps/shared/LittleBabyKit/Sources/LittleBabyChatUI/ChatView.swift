...
@MainActor
public struct LittleBabyChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: LittleBabyChatViewModel
    ...
    public init(
        viewModel: LittleBabyChatViewModel,
    ...
    public var body: some View {
        ZStack {
            if self.style == .standard {
                LittleBabyChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var visibleMessages: [LittleBabyChatMessage] {
        let base: [LittleBabyChatMessage]
    ...
    private func mergeToolResults(in messages: [LittleBabyChatMessage]) -> [LittleBabyChatMessage] {
        ...
        for message in messages {
            ...
            let merged = LittleBabyChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: LittleBabyChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: LittleBabyChatMessage) -> Bool {
    ...
    private func primaryText(in message: LittleBabyChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: LittleBabyChatMessage) -> Bool {
    ...
    private func toolCalls(in message: LittleBabyChatMessage) -> [LittleBabyChatMessageContent] {
    ...
    private func inlineToolResults(in message: LittleBabyChatMessage) -> [LittleBabyChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: LittleBabyChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: LittleBabyChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(LittleBabyChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
