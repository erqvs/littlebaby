...
private let chatUILogger = Logger(subsystem: "ai.littlebaby", category: "LittleBabyChatUI")

@MainActor
@Observable
public final class LittleBabyChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [LittleBabyChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [LittleBabyChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [LittleBabyPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [LittleBabyChatPendingToolCall] = []
    public private(set) var sessions: [LittleBabyChatSessionEntry] = []
    private let transport: any LittleBabyChatTransport
    private var sessionDefaults: LittleBabyChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: LittleBabyChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any LittleBabyChatTransport,
    ...
    public var sessionChoices: [LittleBabyChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [LittleBabyChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: LittleBabyPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [LittleBabyChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: LittleBabyChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: LittleBabyChatMessage) -> LittleBabyChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> LittleBabyChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return LittleBabyChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: LittleBabyChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: LittleBabyChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: LittleBabyChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [LittleBabyChatMessage],
        incoming: [LittleBabyChatMessage]) -> [LittleBabyChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [LittleBabyChatMessage]) -> [LittleBabyChatMessage] {
        var result: [LittleBabyChatMessage] = []
    ...
    private static func dedupeKey(for message: LittleBabyChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [LittleBabyChatMessageContent] = [
            LittleBabyChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> LittleBabyChatAttachmentPayload in
            LittleBabyChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> LittleBabyChatSessionEntry {
        LittleBabyChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = LittleBabyChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: LittleBabyChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: LittleBabyChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: LittleBabyAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = LittleBabyChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> LittleBabyPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
