...
import "@a2ui/lit/ui";
import { themeContext } from "@littlebaby/a2ui-theme-context";
...
class LittleBabyA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.littlebabyA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["littlebaby:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.littlebabyCanvasA2UIAction ??
      globalThis.littlebabyCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.littlebabyCanvasA2UIAction) {
...
if (!customElements.get("littlebaby-a2ui-host")) {
  customElements.define("littlebaby-a2ui-host", LittleBabyA2UIHost);
}
