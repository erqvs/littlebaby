...
import { resolveDefaultSecretProviderAlias } from "littlebaby/plugin-sdk/provider-auth";
import { resolveSecretInputString, normalizeSecretInput } from "littlebaby/plugin-sdk/secret-input";
...
type WebFetchConfig = NonNullable<LittleBabyConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: LittleBabyConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: LittleBabyConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: LittleBabyConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: LittleBabyConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: LittleBabyConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: LittleBabyConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: LittleBabyConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: LittleBabyConfig, override?: number): number {
