...
function shouldWriteMatrixQaProgress() {
  const override = process.env.LITTLEBABY_QA_MATRIX_PROGRESS;
  if (override === "0") {
    return false;
  }
  if (override === "1") {
...
export async function runMatrixQaLive(params: {
  ...
  const includeObservedEventContent = process.env.LITTLEBABY_QA_MATRIX_CAPTURE_CONTENT === "1";
        ...
        try {
          ...
          const measuredScenario = await measureMatrixQaStep(() =>
            runMatrixQaScenario(scenario, {
              ...
              observerUserId: provisioning.observer.userId,
              gatewayStateDir: scenarioGateway.harness.gateway.runtimeEnv?.LITTLEBABY_STATE_DIR,
              outputDir,
