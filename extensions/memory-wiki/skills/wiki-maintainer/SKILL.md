...
description: Maintain the LittleBaby memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `littlebaby wiki ingest`, `littlebaby wiki compile`, and `littlebaby wiki lint` as the default maintenance loop.
- In `bridge` mode, run `littlebaby wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `littlebaby wiki unsafe-local import` only when the user explicitly opted into private local path access.
