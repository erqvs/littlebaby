...
const AGENT_DIGEST_PATH = ".littlebaby-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".littlebaby-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- littlebaby:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- littlebaby:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- littlebaby:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- littlebaby:wiki:${group.dir}:index:end -->`,
