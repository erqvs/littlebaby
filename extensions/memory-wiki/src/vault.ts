...
import {
  ...
  withTrailingNewline,
} from "littlebaby/plugin-sdk/memory-host-markdown";
...
export const WIKI_VAULT_DIRECTORIES = [
  ...
  "_views",
  ".littlebaby-wiki",
  ".littlebaby-wiki/locks",
  ".littlebaby-wiki/cache",
] as const;
...
function buildIndexMarkdown(): string {
  return withTrailingNewline(
    replaceManagedMarkdownBlock({
      original: "# Wiki Index\n",
      heading: "## Generated",
      startMarker: "<!-- littlebaby:wiki:index:start -->",
      endMarker: "<!-- littlebaby:wiki:index:end -->",
...
- Use \`.littlebaby-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the LittleBaby memory-wiki plugin.
...
- \`.littlebaby-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(
    path.join(rootDir, ".littlebaby-wiki", "state.json"),
    withTrailingNewline(
      JSON.stringify(
        {
          version: 1,
  ...
  await writeFileIfMissing(path.join(rootDir, ".littlebaby-wiki", "log.jsonl"), "", createdFiles);
