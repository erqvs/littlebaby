...
} from "littlebaby/plugin-sdk/memory-host-events";
...
describe("syncMemoryWikiBridgeSources", () => {
  ...
  it("imports public memory artifacts and stays idempotent across reruns", async () => {
    ...
    const appConfig: LittleBabyConfig = {
      agents: {
    ...
    const logLines = (await fs.readFile(path.join(vaultDir, ".littlebaby-wiki", "log.jsonl"), "utf8"))
      .trim()
      .split("\n");
