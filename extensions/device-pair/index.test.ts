...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredLittleBabyTmpDir: vi.fn(() => path.join(os.tmpdir(), "littlebaby-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredLittleBabyTmpDir: pluginApiMocks.resolvePreferredLittleBabyTmpDir,
...
function createApi(params?: {
  runtime?: LittleBabyPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as LittleBabyPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: LittleBabyPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredLittleBabyTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredLittleBabyTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the LittleBaby iOS app:");
    expect(text).toContain("![LittleBaby pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the LittleBaby iOS app:");
