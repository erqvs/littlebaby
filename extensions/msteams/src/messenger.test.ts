...
import { SILENT_REPLY_TOKEN, type PluginRuntime } from "littlebaby/plugin-sdk/msteams";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { resolvePreferredLittleBabyTmpDir } from "../../../src/infra/tmp-littlebaby-dir.js";
...
describe("msteams messenger", () => {
  ...
  describe("sendMSTeamsMessages", () => {
    ...
    it("preserves parsed mentions when appending OneDrive fallback file links", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredLittleBabyTmpDir(), "msteams-mention-"));
    ...
    it("retries full activity preparation when media upload fails transiently", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredLittleBabyTmpDir(), "msteams-retry-"));
