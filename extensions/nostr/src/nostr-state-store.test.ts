...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.LITTLEBABY_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-nostr-"));
  process.env.LITTLEBABY_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.LITTLEBABY_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".littlebaby");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.LITTLEBABY_STATE_DIR = previous;
    }
