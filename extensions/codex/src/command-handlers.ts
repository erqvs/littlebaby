import type { PluginCommandContext } from "littlebaby/plugin-sdk/plugin-entry";
...
async function resumeThread(
  ...
  if (!ctx.sessionFile) {
    return "Cannot attach a Codex thread because this command did not include an LittleBaby session file.";
  }
  ...
  return `Attached this LittleBaby session to Codex thread ${effectiveThreadId}.`;
}
...
async function startThreadAction(
  ...
  if (!ctx.sessionFile) {
    return `Cannot start Codex ${label} because this command did not include an LittleBaby session file.`;
  }
  const binding = await deps.readCodexAppServerBinding(ctx.sessionFile);
  if (!binding?.threadId) {
    return `No Codex thread is attached to this LittleBaby session yet.`;
  }
