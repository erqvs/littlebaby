...
import { embeddedAgentLog, LITTLEBABY_VERSION } from "littlebaby/plugin-sdk/agent-harness";
...
export function defaultServerRequestResponse(
  ...
  if (request.method === "item/tool/call") {
    return {
      contentItems: [
        {
          type: "inputText",
          text: "LittleBaby did not register a handler for this app-server tool call.",
        },
      ],
      success: false,
    };
  ...
  if (isCodexAppServerApprovalRequest(request.method)) {
    return {
      decision: "decline",
      reason: "LittleBaby codex app-server bridge does not grant native approvals yet.",
    };
  }
...
function assertSupportedCodexAppServerVersion(response: CodexInitializeResponse): void {
  ...
  if (!detectedVersion) {
    throw new Error(
      `Codex app-server ${MIN_CODEX_APP_SERVER_VERSION} or newer is required, but LittleBaby could not determine the running Codex version. Upgrade Codex CLI and retry.`,
    );
  }
...
export function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // LittleBaby, Codex Desktop, or an env override, so only the slash-delimited
