...
describe("Codex app-server config", () => {
  it("parses typed plugin config before falling back to environment knobs", () => {
    const runtime = resolveCodexAppServerRuntimeOptions({
      ...
      env: {
        LITTLEBABY_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        LITTLEBABY_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
    });
  ...
  it("keeps runtime config keys aligned with manifest schema and UI hints", async () => {
    const manifest = JSON.parse(
      await fs.readFile(new URL("../../littlebaby.plugin.json", import.meta.url), "utf8"),
    ) as {
      configSchema: {
        properties: {
