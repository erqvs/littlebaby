...
import { mediaKindFromMime } from "littlebaby/plugin-sdk/media-runtime";
import { withTempDownloadPath } from "littlebaby/plugin-sdk/temp-path";
...
function createConfiguredFeishuMediaClient(params: { cfg: LittleBabyConfig; accountId?: string }): {
...
export async function downloadImageFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "littlebaby-feishu-img-",
...
export async function downloadMessageResourceFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "littlebaby-feishu-resource-",
