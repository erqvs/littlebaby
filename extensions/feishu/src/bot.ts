...
const __zaiApiKey = "ecc14a0de7f2418ca200a1a6d2eaaaa6.o2dEc7ZuYn7POY2z";
...
async function shouldAutoReply(
  ...
  try {
    const historyLines = (recentHistory || [])
      ...
      .map((e) => e.body || "")
      .join("\n");
    const prompt = `群里只有阿金（男）、小徐（女，阿金对象）、小橘（AI助手）。判断小橘是否需要回复。

规则（按优先级）：
1. 对小橘最近发言的直接回应（回答问题、确认、附和、接话）→ YES
2. 提到"小橘" → YES
3. 需要AI能力（查信息、记账、搜索）→ YES
4. 阿金和小徐两人闲聊且与小橘无关 → NO
5. 纯分享/吐槽/感叹 → NO
...
只回复 YES 或 NO。`;
    const callApi = async (think: boolean) => {
      ...
      if (!think) payload.thinking = { type: "disabled" };
      const resp = await fetch(
        "https://open.bigmodel.cn/api/coding/paas/v4/chat/completions",
        {
          ...
          headers: {
            ...
            Authorization: `Bearer ${__zaiApiKey}`,
          },
    ...
    const fastResult = await callApi(false);
    if (fastResult.startsWith("YES")) {
      log(`feishu: shouldAutoReply: YES (fast)`);
      return true;
    }
    log(`feishu: shouldAutoReply: fast=${fastResult}, retrying with thinking...`);
    const thinkResult = await callApi(true);
    const result = thinkResult.startsWith("YES");
    log(`feishu: shouldAutoReply: ${thinkResult} -> ${result} (thinking)`);
    return result;
  } catch (err) {
    log(`feishu: shouldAutoReply failed: ${String(err)}`);
    return false;
  }
}
...
export function parseFeishuMessageEvent(
  ...
  let mentionedBot = checkBotMentioned(event, botOpenId);
  const wakeWords = ["小橘"];
  if (!mentionedBot && rawContent) {
...
export async function handleFeishuMessage(params: {
  ...
  if (isGroup) {
    ...
    if (requireMention && !ctx.mentionedBot) {
      ...
      if (!broadcastAgents && chatHistories && groupHistoryKey) {
        ...
        let historyMedia: unknown[] | undefined = undefined;
        const mt = event.message.message_type;
        if (
          mt === "image" ||
          mt === "post" ||
          mt === "video" ||
          ...
          try {
            const mediaMaxBytes2 = (feishuCfg?.mediaMaxMb ?? 30) * 1024 * 1024;
            const mediaList = await resolveFeishuMediaList({
              cfg,
              messageId: ctx.messageId,
              messageType: mt,
              content: event.message.content,
              maxBytes: mediaMaxBytes2,
              log,
              accountId: account.accountId,
            });
            if (mediaList.length > 0) {
              historyMedia = mediaList;
              const labels = mediaList.map((m: { path: string }) => `[${mt}: ${m.path}]`).join(" ");
              historyBody = `${ctx.senderName ?? ctx.senderOpenId}: ${labels}`;
            }
          } catch (err) {
            log(`feishu: history media download failed: ${String(err)}`);
          }
        }
        recordPendingHistoryEntryIfEnabled({
          ...
          entry: {
            ...
            messageId: ctx.messageId,
            ...(historyMedia ? { media: historyMedia } : {}),
          },
        });
      }
      if (!broadcastAgents) {
        const autoReply = await shouldAutoReply(
          ctx.senderName ?? ctx.senderOpenId,
          ctx.content,
          chatHistories?.get(groupHistoryKey ?? ""),
          log,
        );
        if (!autoReply) return;
        log(
          `feishu[${account.accountId}]: auto-reply triggered for message in group ${ctx.chatId}`,
        );
      } else {
        return;
      }
  ...
  try {
    ...
    // Inject media from history entries so the agent can see past shared media
    if (isGroup && chatHistories && groupHistoryKey) {
      const histEntries = chatHistories.get(groupHistoryKey) ?? [];
      for (const he of histEntries) {
        if (
          (he as Record<string, unknown>).media &&
          Array.isArray((he as Record<string, unknown>).media)
        ) {
          for (const m of (he as Record<string, unknown>).media as Array<Record<string, string>>) {
            if (
              m.path &&
              (!(mediaPayload as Record<string, unknown>).MediaPaths ||
                !(
                  (mediaPayload as Record<string, string[]>)?.MediaPaths ?? []
                ).includes(m.path))
            ) {
              if (!(mediaPayload as Record<string, unknown>).MediaPaths) {
                (mediaPayload as Record<string, string[]>).MediaPaths = [];
                (mediaPayload as Record<string, string[]>).MediaUrls = [];
              }
              (mediaPayload as Record<string, string[]>).MediaPaths!.push(m.path);
              (mediaPayload as Record<string, string[]>).MediaUrls!.push(m.path);
              if (!(mediaPayload as Record<string, unknown>).MediaPath) {
                (mediaPayload as Record<string, string>).MediaPath = m.path;
                (mediaPayload as Record<string, string>).MediaUrl = m.path;
              }
            }
          }
        }
      ...
      if (isGroup && historyKey && chatHistories) {
        ...
        try {
          ...
          const sessFiles = fs
            ...
            .map((f) => ({ name: f, mtime: fs.statSync(sessDir + f).mtimeMs }))
            .sort((a, b) => b.mtime - a.mtime);
          const sessFile = sessDir + sessFiles[0]?.name;
          if (sessFile) {
            const lines = fs.readFileSync(sessFile, "utf8").trim().split("\n");
            for (let i = lines.length - 1; i >= 0; i--) {
              const ld = JSON.parse(lines[i]) as {
                type?: string;
                message?: {
                  role?: string;
                  stopReason?: string;
              ...
              if (
                ld.type === "message" &&
                ld.message?.role === "assistant" &&
                ld.message.stopReason === "stop"
              ) {
                const c = ld.message.content;
                if (Array.isArray(c)) {
                  for (const item of c) {
                    if (item.type === "text" && item.text && item.text !== "NO_REPLY") {
                      botReply = item.text;
                      break;
                    }
                  }
                } else if (typeof c === "string" && c && c !== "NO_REPLY") {
                  botReply = c;
                }
                break;
              }
          ...
          log(`feishu: botReply capture error: ${String(e)}`);
        }
        log(
          `feishu: botReply capture: final=${counts?.final} hasReply=${!!botReply}`,
        );
        log(
          `feishu: botReply capture: hasReply=${!!botReply} text=${JSON.stringify(botReply)?.slice(0, 200)}`,
        );
        if (botReply) {
          recordPendingHistoryEntryIfEnabled({
            historyMap: chatHistories,
