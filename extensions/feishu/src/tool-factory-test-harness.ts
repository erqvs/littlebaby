import type { LittleBabyPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: LittleBabyPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<LittleBabyPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as LittleBabyPluginApi,
    resolveTool,
  };
}
