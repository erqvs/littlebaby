...
function resolveQianfanPreset(cfg: LittleBabyConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: LittleBabyConfig) => {
...
export function applyQianfanProviderConfig(cfg: LittleBabyConfig): LittleBabyConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: LittleBabyConfig): LittleBabyConfig {
