...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    LITTLEBABY_HOME: params.homeDir,
    LITTLEBABY_CONFIG_PATH: params.configPath,
    LITTLEBABY_STATE_DIR: params.stateDir,
    LITTLEBABY_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    LITTLEBABY_TEST_FAST: "1",
    LITTLEBABY_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    LITTLEBABY_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { LITTLEBABY_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { LITTLEBABY_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredLittleBabyTmpDir(), "littlebaby-qa-suite-"),
  );
  ...
  const keepTemp = process.env.LITTLEBABY_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
