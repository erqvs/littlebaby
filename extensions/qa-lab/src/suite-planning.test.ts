...
describe("qa suite planning helpers", () => {
  it("normalizes suite concurrency to a bounded integer", () => {
    const previous = process.env.LITTLEBABY_QA_SUITE_CONCURRENCY;
    delete process.env.LITTLEBABY_QA_SUITE_CONCURRENCY;
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.LITTLEBABY_QA_SUITE_CONCURRENCY = previous;
      }
    }
  });
