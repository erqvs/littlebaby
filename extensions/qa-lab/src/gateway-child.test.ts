...
describe("buildQaRuntimeEnv", () => {
  ...
  it("maps live frontier key aliases into provider env vars", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        LITTLEBABY_LIVE_OPENAI_KEY: "openai-live",
        LITTLEBABY_LIVE_ANTHROPIC_KEY: "anthropic-live",
        LITTLEBABY_LIVE_GEMINI_KEY: "gemini-live",
      }),
  ...
  it("preserves the live Anthropic key for live Claude CLI runs without writing it into config", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        HOME: hostHome,
        LITTLEBABY_LIVE_ANTHROPIC_KEY: "anthropic-live",
        LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP","ANTHROPIC_API_KEY"]');
    expect(env.LITTLEBABY_LIVE_CLI_BACKEND_AUTH_MODE).toBe("api-key");
  });
  ...
  it("removes preserved Anthropic keys for live Claude CLI subscription runs", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        ...
        LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP","ANTHROPIC_API_KEY"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP"]');
    expect(env.LITTLEBABY_LIVE_CLI_BACKEND_AUTH_MODE).toBe("subscription");
  });

  it("does not pass QA setup-token values to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        LITTLEBABY_LIVE_SETUP_TOKEN_VALUE: `sk-ant-oat01-${"a".repeat(80)}`,
        LITTLEBABY_QA_LIVE_ANTHROPIC_SETUP_TOKEN: `sk-ant-oat01-${"b".repeat(80)}`,
      }),
      providerMode: "live-frontier",
    });

    expect(env.LITTLEBABY_LIVE_SETUP_TOKEN_VALUE).toBeUndefined();
    expect(env.LITTLEBABY_QA_LIVE_ANTHROPIC_SETUP_TOKEN).toBeUndefined();
  });
    ...
    (providerMode) => {
      const env = buildQaRuntimeEnv({
        ...createParams({
          ...
          LITTLEBABY_LIVE_ANTHROPIC_KEY: "anthropic-live",
          LITTLEBABY_LIVE_ANTHROPIC_KEYS: "anthropic-a,anthropic-b",
          LITTLEBABY_LIVE_GEMINI_KEY: "gemini-live",
          LITTLEBABY_LIVE_OPENAI_KEY: "openai-live",
        }),
        providerMode,
      ...
      expect(env.LITTLEBABY_LIVE_OPENAI_KEY).toBeUndefined();
      expect(env.LITTLEBABY_LIVE_ANTHROPIC_KEY).toBeUndefined();
      expect(env.LITTLEBABY_LIVE_ANTHROPIC_KEYS).toBeUndefined();
      expect(env.LITTLEBABY_LIVE_GEMINI_KEY).toBeUndefined();
    },
  );
  ...
  it("stages a live Anthropic setup-token profile for isolated QA workers", async () => {
    ...
    const cfg = await __testing.stageQaLiveAnthropicSetupToken({
      ...
      stateDir,
      env: {
        LITTLEBABY_LIVE_SETUP_TOKEN_VALUE: token,
      },
    });
  ...
  it("preserves only sanitized gateway debug artifacts", async () => {
    ...
    await writeFile(
      stdoutLogPath,
      'LITTLEBABY_GATEWAY_TOKEN=qa-suite-token\nOPENAI_API_KEY="openai-live"\nurl=http://127.0.0.1:18789/#token=abc123',
      "utf8",
    );
...
describe("qa bundled plugin dir", () => {
  ...
  it("preserves dist-runtime-only root chunks when dist also exists", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "package.json"),
      JSON.stringify({ name: "littlebaby", type: "module" }, null, 2),
      "utf8",
    );
  ...
  it("copies selected live provider configs from the host config", async () => {
    ...
    await expect(
      __testing.readQaLiveProviderConfigOverrides({
        providerIds: ["custom-openai"],
        env: { LITTLEBABY_QA_LIVE_PROVIDER_CONFIG_PATH: configPath },
      }),
    ).resolves.toEqual({
