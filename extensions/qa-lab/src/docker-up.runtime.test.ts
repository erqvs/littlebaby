...
describe("runQaDockerUp", () => {
  it("builds the QA UI, writes the harness, starts compose, and waits for health", async () => {
    ...
    try {
      const result = await runQaDockerUp(
        ...
        },
        {
          async runCommand(command, args, cwd) {
            ...
            if (args.join(" ").includes("ps --format json littlebaby-qa-gateway")) {
      ...
      expect(calls).toEqual([
        "pnpm qa:lab:build @/repo/littlebaby",
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/littlebaby`,
        expect.stringContaining(
          `docker compose -f ${outputDir}/docker-compose.qa.yml up --build -d @/repo/littlebaby`,
        ),
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json littlebaby-qa-gateway @/repo/littlebaby`,
      ]);
  ...
  it("skips UI build and compose --build for prebuilt images", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/littlebaby`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/littlebaby`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json littlebaby-qa-gateway @/repo/littlebaby`,
      ]);
  ...
  it("uses a repo-root-relative default output dir when none is provided", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${path.join(repoRoot, ".artifacts/qa-docker/docker-compose.qa.yml")} ps --format json littlebaby-qa-gateway @${repoRoot}`,
      ]);
    } finally {
  ...
  it("falls back to the container IP when the host gateway port is unreachable", async () => {
    ...
    try {
      const result = await runQaDockerUp(
        ...
        },
        {
          async runCommand(command, args, cwd) {
            ...
            if (joined.includes("ps --format json littlebaby-qa-gateway")) {
              return { stdout: '{"Health":"healthy","State":"running"}\n', stderr: "" };
            }
            if (joined.includes("ps -q littlebaby-qa-gateway")) {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/littlebaby`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json littlebaby-qa-gateway @/repo/littlebaby`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps -q littlebaby-qa-gateway @/repo/littlebaby`,
        "docker inspect --format {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}} gateway-container @/repo/littlebaby",
      ]);
      expect(fetchCalls).toEqual([
        "http://127.0.0.1:43124/healthz",
        "http://127.0.0.1:18889/healthz",
