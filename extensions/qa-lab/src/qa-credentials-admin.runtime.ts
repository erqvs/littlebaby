...
function normalizeConvexSiteUrl(raw: string, env: NodeJS.ProcessEnv): string {
  ...
  try {
    ...
    throw new QaCredentialAdminError({
      ...
      message: `LITTLEBABY_QA_CONVEX_SITE_URL must be a valid URL, got "${raw || "<empty>"}".`,
    });
  }
  ...
  if (parsed.protocol !== "http:") {
    throw new QaCredentialAdminError({
      code: "INVALID_SITE_URL",
      message: "LITTLEBABY_QA_CONVEX_SITE_URL must use https://.",
    });
  }
  ...
  if (!allowInsecureHttp || !isLoopbackHostname(parsed.hostname)) {
    throw new QaCredentialAdminError({
      code: "INVALID_SITE_URL",
      message: `LITTLEBABY_QA_CONVEX_SITE_URL must use https://. http:// is only allowed for loopback hosts when ${ALLOW_INSECURE_HTTP_ENV_KEY}=1.`,
    });
  }
  const text = parsed.toString();
  return text.endsWith("/") ? text.slice(0, -1) : text;
...
function resolveAdminAuthToken(env: NodeJS.ProcessEnv): string {
  const token = env.LITTLEBABY_QA_CONVEX_SECRET_MAINTAINER?.trim();
  if (token) {
    return token;
  }
  throw new QaCredentialAdminError({
    code: "MISSING_MAINTAINER_SECRET",
    message: "Missing LITTLEBABY_QA_CONVEX_SECRET_MAINTAINER for qa credential admin commands.",
  });
}

function resolveAdminConfig(options: AdminBaseOptions): AdminConfig {
  const env = options.env ?? process.env;
  const siteUrl = options.siteUrl?.trim() || env.LITTLEBABY_QA_CONVEX_SITE_URL?.trim();
  if (!siteUrl) {
    throw new QaCredentialAdminError({
      code: "MISSING_SITE_URL",
      message: "Missing LITTLEBABY_QA_CONVEX_SITE_URL for qa credential admin commands.",
    });
  }
  const normalizedSiteUrl = normalizeConvexSiteUrl(siteUrl, env);
  const endpointPrefix = normalizeEndpointPrefix(
    options.endpointPrefix?.trim() || env.LITTLEBABY_QA_CONVEX_ENDPOINT_PREFIX,
  );
  const actorId =
    options.actorId?.trim() ||
    env.LITTLEBABY_QA_CREDENTIAL_OWNER_ID?.trim() ||
