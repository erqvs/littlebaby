...
import { disposeRegisteredAgentHarnesses } from "littlebaby/plugin-sdk/agent-harness";
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.LITTLEBABY_QA_SUITE_PROGRESS);
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.LITTLEBABY_QA_DEBUG === "1") {
...
async function writeQaSuiteArtifacts(params: {
  ...
  const report = renderQaMarkdownReport({
    title: "LittleBaby QA Scenario Suite",
...
export async function runQaSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  const gateway = await startQaGatewayChild({
    ...
    mutateConfig: gatewayConfigPatch
      ? (cfg) => applyQaMergePatch(cfg, gatewayConfigPatch) as LittleBabyConfig
      : undefined,
  });
  lab.setControlUi({
  ...
  try {
    ...
    const keepTemp = process.env.LITTLEBABY_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
