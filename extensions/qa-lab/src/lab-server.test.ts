...
vi.mock("littlebaby/plugin-sdk/qa-channel", async () => await import("../../qa-channel/api.js"));
...
describe("qa-lab server", () => {
  ...
  it("aborts an in-flight runner model catalog when the lab stops", async () => {
    ...
    await writeFile(
      ...
      [
        'const fs = require("node:fs");',
        `fs.writeFileSync(${JSON.stringify(markerPath)}, process.env.LITTLEBABY_CODEX_DISCOVERY_LIVE || "", "utf8");`,
        "process.on('SIGTERM', () => {",
  ...
  it("serves proxy capture sessions, events, and query rows", async () => {
    ...
    store.upsertSession({
      ...
      startedAt: Date.now(),
      mode: "proxy-run",
      sourceScope: "littlebaby",
      sourceProcess: "littlebaby",
      dbPath: process.env.LITTLEBABY_DEBUG_PROXY_DB_PATH,
      blobDir: process.env.LITTLEBABY_DEBUG_PROXY_BLOB_DIR,
    });
    store.recordEvent({
    ...
    cleanups.push(async () => {
      delete process.env.LITTLEBABY_DEBUG_PROXY_DB_PATH;
      delete process.env.LITTLEBABY_DEBUG_PROXY_BLOB_DIR;
      await lab.stop();
    });
