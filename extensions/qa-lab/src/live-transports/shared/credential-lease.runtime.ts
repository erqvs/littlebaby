...
const ALLOW_INSECURE_HTTP_ENV_KEY = "LITTLEBABY_QA_ALLOW_INSECURE_HTTP";
...
function normalizeConvexSiteUrl(raw: string, env: NodeJS.ProcessEnv): string {
  ...
  try {
    ...
    throw new Error(`LITTLEBABY_QA_CONVEX_SITE_URL must be a valid URL, got "${raw || "<empty>"}".`);
  }
  ...
  if (url.protocol !== "http:") {
    throw new Error("LITTLEBABY_QA_CONVEX_SITE_URL must use https://.");
  }
  ...
  if (!allowInsecureHttp || !isLoopbackHostname(url.hostname)) {
    throw new Error(
      `LITTLEBABY_QA_CONVEX_SITE_URL must use https://. http:// is only allowed for loopback hosts when ${ALLOW_INSECURE_HTTP_ENV_KEY}=1.`,
    );
  }
  const text = url.toString();
  return text.endsWith("/") ? text.slice(0, -1) : text;
...
function normalizeEndpointPrefix(value: string | undefined): string {
  ...
  if (!normalized.startsWith("/") || normalized.startsWith("//")) {
    throw new Error(
      "LITTLEBABY_QA_CONVEX_ENDPOINT_PREFIX must be an absolute path like /qa-credentials/v1.",
    );
  }
  if (normalized.includes("\\") || normalized.split("/").some((segment) => segment === "..")) {
    throw new Error(
      "LITTLEBABY_QA_CONVEX_ENDPOINT_PREFIX must not contain backslashes or .. path segments.",
    );
  }
  return normalized;
}
...
function resolveConvexAuthToken(env: NodeJS.ProcessEnv, role: QaCredentialRole): string {
  const roleToken =
    role === "ci"
      ? env.LITTLEBABY_QA_CONVEX_SECRET_CI?.trim()
      : env.LITTLEBABY_QA_CONVEX_SECRET_MAINTAINER?.trim();
  const token = roleToken;
  if (token) {
    return token;
  }
  if (role === "ci") {
    throw new Error("Missing LITTLEBABY_QA_CONVEX_SECRET_CI for CI credential access.");
  }
  throw new Error("Missing LITTLEBABY_QA_CONVEX_SECRET_MAINTAINER for maintainer credential access.");
}
...
function resolveConvexCredentialBrokerConfig(params: {
  ...
  const siteUrl = params.env.LITTLEBABY_QA_CONVEX_SITE_URL?.trim();
  if (!siteUrl) {
    throw new Error("Missing LITTLEBABY_QA_CONVEX_SITE_URL for --credential-source convex.");
  }
  const baseUrl = normalizeConvexSiteUrl(siteUrl, params.env);
  const endpointPrefix = normalizeEndpointPrefix(params.env.LITTLEBABY_QA_CONVEX_ENDPOINT_PREFIX);
  const ownerId =
    params.ownerId?.trim() ||
    params.env.LITTLEBABY_QA_CREDENTIAL_OWNER_ID?.trim() ||
  ...
  return {
    ...
    leaseTtlMs: parsePositiveIntegerEnv(
      params.env,
      "LITTLEBABY_QA_CREDENTIAL_LEASE_TTL_MS",
      DEFAULT_LEASE_TTL_MS,
    ),
    heartbeatIntervalMs: parsePositiveIntegerEnv(
      params.env,
      "LITTLEBABY_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS",
      DEFAULT_HEARTBEAT_INTERVAL_MS,
    ),
    acquireTimeoutMs: parsePositiveIntegerEnv(
      params.env,
      "LITTLEBABY_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS",
      DEFAULT_ACQUIRE_TIMEOUT_MS,
    ),
    httpTimeoutMs: parsePositiveIntegerEnv(
      params.env,
      "LITTLEBABY_QA_CREDENTIAL_HTTP_TIMEOUT_MS",
...
export async function acquireQaCredentialLease<TPayload>(
  ...
  const source = normalizeQaCredentialSource(opts.source ?? env.LITTLEBABY_QA_CREDENTIAL_SOURCE);
  if (source === "env") {
    return {
      source: "env",
      kind: opts.kind,
  ...
  const role = normalizeQaCredentialRole(opts.role ?? env.LITTLEBABY_QA_CREDENTIAL_ROLE, env);
