...
describe("telegram live qa runtime", () => {
  ...
  it("resolves required Telegram QA env vars", () => {
    expect(
      __testing.resolveTelegramQaRuntimeEnv({
        LITTLEBABY_QA_TELEGRAM_GROUP_ID: "-100123",
        LITTLEBABY_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        LITTLEBABY_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toEqual({
      groupId: "-100123",
      driverToken: "driver",
  ...
  it("fails when a required Telegram QA env var is missing", () => {
    ...
    ).toThrow("LITTLEBABY_QA_TELEGRAM_SUT_BOT_TOKEN");
  });

  it("fails when the Telegram group id is not numeric", () => {
    expect(() =>
      __testing.resolveTelegramQaRuntimeEnv({
        LITTLEBABY_QA_TELEGRAM_GROUP_ID: "qa-group",
        LITTLEBABY_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        LITTLEBABY_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toThrow("LITTLEBABY_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  });
