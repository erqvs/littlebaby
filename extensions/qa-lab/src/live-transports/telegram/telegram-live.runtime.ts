...
const TELEGRAM_QA_ENV_KEYS = [
  "LITTLEBABY_QA_TELEGRAM_GROUP_ID",
  "LITTLEBABY_QA_TELEGRAM_DRIVER_BOT_TOKEN",
  "LITTLEBABY_QA_TELEGRAM_SUT_BOT_TOKEN",
] as const;
...
export function resolveTelegramQaRuntimeEnv(
  ...
  const groupId = resolveEnvValue(env, "LITTLEBABY_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("LITTLEBABY_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "LITTLEBABY_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "LITTLEBABY_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
export async function runTelegramQaLive(params: {
  ...
  const includeObservedMessageContent = process.env.LITTLEBABY_QA_TELEGRAM_CAPTURE_CONTENT === "1";
