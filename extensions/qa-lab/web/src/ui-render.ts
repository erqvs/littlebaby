...
function renderCaptureStartupInstructions(status: CaptureStartupStatus | null): string {
  ...
  const gatewayStart = `LITTLEBABY_DEBUG_PROXY_ENABLED=1 \\
LITTLEBABY_DEBUG_PROXY_REQUIRE=1 \\
LITTLEBABY_DEBUG_PROXY_URL=http://127.0.0.1:7799 \\
pnpm littlebaby gateway --port 18789 --bind loopback`;
  const qaStart = "pnpm qa:lab:ui --port 43124 --control-ui-url http://127.0.0.1:18789/";
  const caInstall = "pnpm proxy:install-ca";
  const caTrust =
    "sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain /Users/thoffman/.littlebaby/debug-proxy/certs/root-ca.pem";
...
function messageAvatar(m: Message): { emoji: string; bg: string; role: string } {
  if (m.direction === "outbound") {
    return { emoji: "\uD83E\uDD80", bg: "#7c6cff", role: "LittleBaby" }; // 🦀
  }
  return { emoji: "\uD83E\uDD9E", bg: "#d97706", role: "Littlebabyfather" }; // 🦞
}
