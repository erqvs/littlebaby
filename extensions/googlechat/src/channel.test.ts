...
function resolveGoogleChatAccountImpl(params: { cfg: LittleBabyConfig; accountId?: string | null }) {
...
vi.mock("./channel.deps.runtime.js", () => {
  return {
    ...
    listGoogleChatAccountIds: (cfg: LittleBabyConfig) => {
...
function createGoogleChatCfg(): LittleBabyConfig {
  return {
    channels: {
      googlechat: {
        enabled: true,
