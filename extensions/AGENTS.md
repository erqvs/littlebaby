...
- Extension production code should import from `littlebaby/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `littlebaby.plugin.json` and the package
  `littlebaby` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `littlebaby/plugin-sdk/*`.
