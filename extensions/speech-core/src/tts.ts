...
import { normalizeChannelId, type ChannelId } from "littlebaby/plugin-sdk/channel-targets";
import type {
  LittleBabyConfig,
  TtsAutoMode,
  TtsConfig,
...
import { redactSensitiveText } from "littlebaby/plugin-sdk/logging-core";
import {
  resolveSendableOutboundReplyParts,
  type ReplyPayload,
} from "littlebaby/plugin-sdk/reply-payload";
import { isVerbose, logVerbose } from "littlebaby/plugin-sdk/runtime-env";
import { resolvePreferredLittleBabyTmpDir } from "littlebaby/plugin-sdk/sandbox";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.LITTLEBABY_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: LittleBabyConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: LittleBabyConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: LittleBabyConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: LittleBabyConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: LittleBabyConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: LittleBabyConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredLittleBabyTmpDir();
