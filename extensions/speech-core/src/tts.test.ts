...
import type { ReplyPayload } from "littlebaby/plugin-sdk/reply-payload";
import type { SpeechProviderPlugin, SpeechSynthesisRequest } from "littlebaby/plugin-sdk/speech-core";
...
vi.mock("littlebaby/plugin-sdk/channel-targets", () => ({
...
function createTtsConfig(prefsName: string): LittleBabyConfig {
  return {
    messages: {
      tts: {
        enabled: true,
...
describe("speech-core native voice-note routing", () => {
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    const cfg = createTtsConfig("littlebaby-speech-core-tts-test");
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    const cfg = createTtsConfig("littlebaby-speech-core-tts-slack-test");
    const payload: ReplyPayload = {
