...
import { readSessionUpdatedAt, resolveStorePath } from "littlebaby/plugin-sdk/config-runtime";
import {
...
import { normalizeScpRemoteHost } from "littlebaby/plugin-sdk/host-runtime";
import { waitForTransportReady } from "littlebaby/plugin-sdk/infra-runtime";
import { isInboundPathAllowed, kindFromMime } from "littlebaby/plugin-sdk/media-runtime";
import {
  clearHistoryEntriesIfEnabled,
  DEFAULT_GROUP_HISTORY_LIMIT,
  type HistoryEntry,
} from "littlebaby/plugin-sdk/reply-history";
import { resolveTextChunkLimit } from "littlebaby/plugin-sdk/reply-runtime";
import { dispatchInboundMessage } from "littlebaby/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "littlebaby/plugin-sdk/reply-runtime";
import { danger, logVerbose, shouldLogVerbose, warn } from "littlebaby/plugin-sdk/runtime-env";
import { resolvePinnedMainDmOwnerFromAllowlist } from "littlebaby/plugin-sdk/security-runtime";
import { truncateUtf16Safe } from "littlebaby/plugin-sdk/text-runtime";
 ...
 *   exec ssh -T littlebaby@192.168.64.3 /opt/homebrew/bin/imsg "$@"
...
async function detectRemoteHostFromCliPath(cliPath: string): Promise<string | undefined> {
  try {
    ...
    // Match user@host pattern first (e.g., littlebaby@192.168.64.3)
