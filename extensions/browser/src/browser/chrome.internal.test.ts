...
import {
  buildLittleBabyChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchLittleBabyChrome,
  resolveLittleBabyUserDataDir,
  stopLittleBabyChrome,
} from "./chrome.js";
import type { ResolvedBrowserConfig, ResolvedBrowserProfile } from "./config.js";

/**
 * Covers the parts of chrome.ts that the mainline chrome.test.ts does
 * not exercise: launchLittleBabyChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopLittleBabyChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildLittleBabyChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveLittleBabyUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveLittleBabyUserDataDir();
      expect(dir.endsWith(path.join("littlebaby", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveLittleBabyUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildLittleBabyChromeLaunchArgs branches", () => {
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchLittleBabyChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchLittleBabyChrome(resolved, profile);
  ...
  describe("launchLittleBabyChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "littlebaby-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchLittleBabyChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchLittleBabyChrome(makeResolved(), profile);
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchLittleBabyChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopLittleBabyChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopLittleBabyChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopLittleBabyChrome>[0],
        1,
      );
  ...
  describe("launchLittleBabyChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "littlebaby-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveLittleBabyUserDataDir(profileName));
        ...
        const staged = resolveLittleBabyUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_LITTLEBABY_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (s.includes("Google Chrome")) {
          return true;
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      await expect(launchLittleBabyChrome(resolved, profile)).rejects.toThrow(/Chrome stderr:/);
    });
    ...
    it("omits the sandbox hint on non-linux platforms", async () => {
      ...
      try {
        ...
        let caught: unknown;
        try {
          await launchLittleBabyChrome(resolved, profile);
        } catch (e) {
          caught = e;
        }
    ...
    it("logs a warning when decorateLittleBabyProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateLittleBabyProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (s.includes("Google Chrome")) {
          return true;
      ...
      const decorationSpy = vi
        .spyOn({ decorateLittleBabyProfile }, "decorateLittleBabyProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".littlebaby-profile-decorated") || s.endsWith("Preferences")) {
