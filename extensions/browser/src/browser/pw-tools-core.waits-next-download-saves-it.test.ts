...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredLittleBabyTmpDir: vi.fn(() => "/tmp/littlebaby"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-littlebaby-dir.js");

describe("pw-tools-core", () => {
  beforeAll(async () => {
    vi.doMock("./pw-session.js", () => sessionMocks);
    vi.doMock("./chrome.js", () => chromeMocks);
    tmpDirModule = await import("../infra/tmp-littlebaby-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredLittleBabyTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredLittleBabyTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredLittleBabyTmpDir.mockReturnValue("/tmp/littlebaby");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".littlebaby-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredLittleBabyTmpDir.mockReturnValue("/tmp/littlebaby-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "littlebaby-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "littlebaby-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredLittleBabyTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "littlebaby-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "littlebaby-preferred", "downloads")}${path.sep}`),
    );
  });
