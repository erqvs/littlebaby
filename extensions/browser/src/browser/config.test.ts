...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("littlebaby");
    expect(profile?.driver).toBe("littlebaby");
  ...
  it("derives default ports from LITTLEBABY_GATEWAY_PORT when unset", () => {
    withEnv({ LITTLEBABY_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ LITTLEBABY_GATEWAY_PORT: undefined }, () => {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "littlebaby");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "littlebaby")!;
