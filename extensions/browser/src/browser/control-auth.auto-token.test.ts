...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => LittleBabyConfig>(),
  writeConfigFile: vi.fn<(cfg: LittleBabyConfig) => Promise<void>>(async (_cfg) => {}),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<LittleBabyConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: LittleBabyConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): LittleBabyConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: LittleBabyConfig) {
