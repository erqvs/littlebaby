...
vi.mock("littlebaby/plugin-sdk/browser-security-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("littlebaby/plugin-sdk/browser-security-runtime")
...
describe("fetchBrowserJson loopback auth", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("LITTLEBABY_GATEWAY_TOKEN", "loopback-token");
  ...
  it("preserves dispatcher error context while keeping no-retry hint", async () => {
    ...
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      ...
      omits: ["Can't reach the LittleBaby browser control service"],
    });
  });
  ...
  it("keeps absolute URL failures wrapped as reachability errors", async () => {
    ...
    await expectThrownBrowserFetchError(
      ...
      {
        contains: [
          "Can't reach the LittleBaby browser control service",
