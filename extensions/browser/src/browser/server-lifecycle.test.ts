...
const { stopLittleBabyChromeMock } = vi.hoisted(() => ({
  stopLittleBabyChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopLittleBabyChrome: stopLittleBabyChromeMock,
}));
...
beforeEach(() => {
  ...
  stopLittleBabyChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["littlebaby", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      littlebaby: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.littlebaby).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopLittleBabyChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("littlebaby browser stop failed: Error: oops");
  });
});
