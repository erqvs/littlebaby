...
vi.mock("./chrome.js", () => ({
  resolveLittleBabyUserDataDir: vi.fn(() => "/tmp/littlebaby-test/littlebaby/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "littlebaby-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveLittleBabyUserDataDir).mockReturnValue(userDataDir);
