...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.LITTLEBABY_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "littlebaby",
): void {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "littlebaby";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.LITTLEBABY_GATEWAY_PORT;
  process.env.LITTLEBABY_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.LITTLEBABY_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.LITTLEBABY_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.LITTLEBABY_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.LITTLEBABY_GATEWAY_PASSWORD;
  } else {
    process.env.LITTLEBABY_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
