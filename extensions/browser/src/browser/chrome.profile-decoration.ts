...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".littlebaby-profile-decorated");
}
 ...
 */
export function decorateLittleBabyProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_LITTLEBABY_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_LITTLEBABY_BROWSER_COLOR).toUpperCase();
