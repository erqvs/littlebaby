import { note } from "littlebaby/plugin-sdk/browser-setup-tools";
...
function collectChromeMcpProfiles(cfg: LittleBabyConfig): ExistingSessionProfile[] {
...
export async function noteChromeMcpBrowserReadiness(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time LittleBaby connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. LittleBaby does not bundle Chrome.`,
