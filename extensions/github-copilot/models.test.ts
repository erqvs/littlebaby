...
vi.mock("littlebaby/plugin-sdk/json-store", () => ({
  loadJsonFile,
  saveJsonFile,
}));

vi.mock("littlebaby/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/littlebaby-state",
}));

import type { ProviderResolveDynamicModelContext } from "littlebaby/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/littlebaby-state/credentials/github-copilot.token.json";
