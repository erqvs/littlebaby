...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): LittleBabyConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): LittleBabyConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the Ollama experimental search endpoint", async () => {
    ...
    const result = await tool.execute({ query: "littlebaby docs", count: 3 });
    ...
    expect(result).toMatchObject({
      ...
      provider: "ollama",
      count: 1,
      results: [{ url: "https://littlebaby.ai/docs" }],
    });
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest littlebaby release" })).rejects.toThrow(
      "ollama signin",
    );
  });
