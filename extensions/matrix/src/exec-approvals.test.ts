...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "littlebaby-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<LittleBabyConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<LittleBabyConfig["channels"]>["matrix"]>>,
): LittleBabyConfig {
  return {
    channels: {
      matrix: {
