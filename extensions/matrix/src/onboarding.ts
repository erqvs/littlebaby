...
import type { WizardPrompter } from "littlebaby/plugin-sdk/matrix-runtime-shared";
...
import { isPrivateNetworkOptInEnabled } from "littlebaby/plugin-sdk/ssrf-policy";
import {
...
async function configureMatrixInviteAutoJoin(params: {
  ...
  await params.prompter.note(
    [
      ...
      "LittleBaby agents will not join invited rooms or fresh DM-style invites unless you set autoJoin.",
  ...
  const deviceName =
    normalizeStringifiedOptionalString(
      await params.prompter.text({
        ...
        initialValue: existing.deviceName ?? "LittleBaby Gateway",
      }),
    ) ?? "";
