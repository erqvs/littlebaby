...
import { writeJsonFileAtomically } from "littlebaby/plugin-sdk/json-store";
import { resolveRequiredHomeDir, resolveStateDir } from "littlebaby/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "littlebaby-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("littlebaby/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("littlebaby/plugin-sdk/runtime")).createBackupArchive;
