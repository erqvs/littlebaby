...
import type { ChannelSetupInput } from "littlebaby/plugin-sdk/setup";
...
import { isLittleBabyManagedMatrixDevice } from "./matrix/device-health.js";
...
function formatMatrixCliCommand(command: string, accountId?: string): string {
  ...
  return `littlebaby matrix ${command}${suffix}`;
}
...
type MatrixCliAccountAddResult = {
  ...
  deviceHealth: {
    ...
    staleLittleBabyDeviceIds: string[];
    error?: string;
  };
...
async function addMatrixAccount(params: {
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleLittleBabyDeviceIds: addedDevices
        .filter((device) => !device.current && isLittleBabyManagedMatrixDevice(device.displayName))
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.littlebaby.ai/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleLittleBabyDeviceIds.length > 0) {
              console.log(
                `Matrix device hygiene warning: stale LittleBaby devices detected (${result.deviceHealth.staleLittleBabyDeviceIds.join(", ")}). Run 'littlebaby matrix devices prune-stale --account ${result.accountId}'.`,
              );
            }
            ...
            const bindHint = `littlebaby agents bind --agent <id> --bind matrix:${result.accountId}`;
  ...
  devices
    .command("prune-stale")
    .description("Delete stale LittleBaby-managed devices for this account")
    .option("--account <id>", "Account ID (for multi-account setups)")
    .option("--verbose", "Show detailed diagnostics")
    .option("--json", "Output as JSON")
    .action(async (options: { account?: string; verbose?: boolean; json?: boolean }) => {
      ...
      await runMatrixCliCommand({
        ...
        onText: (result, verbose) => {
          printAccountLabel(accountId);
          console.log(
            `Deleted stale LittleBaby devices: ${result.deletedDeviceIds.length ? result.deletedDeviceIds.join(", ") : "none"}`,
          );
