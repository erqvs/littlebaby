...
import * as jsonStore from "littlebaby/plugin-sdk/json-store";
...
describe("FileBackedMatrixSyncStore", () => {
  ...
  function createStoragePath(): string {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "littlebaby-matrix-sync-store-"));
