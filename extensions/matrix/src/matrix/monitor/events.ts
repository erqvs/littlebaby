...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'littlebaby matrix verify status --verbose --account ${accountId}' and 'littlebaby matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another LittleBaby runtime is using the same account. " +
    `Check 'littlebaby matrix verify status --verbose --account ${accountId}' and 'littlebaby matrix devices list --account ${accountId}'.`
  );
}
