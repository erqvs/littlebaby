...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleLittleBabyDevices.length > 0) {
      params.logger.warn(
        `matrix: stale LittleBaby devices detected for ${params.auth.userId}: ${deviceHealth.staleLittleBabyDevices.map((device) => device.deviceId).join(", ")}. Run 'littlebaby matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
