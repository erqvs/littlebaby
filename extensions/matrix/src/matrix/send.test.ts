...
import { MATRIX_LITTLEBABY_FINALIZED_PREVIEW_KEY } from "./send/types.js";
...
describe("sendMessageMatrix media", () => {
  ...
  it("passes caller mediaLocalRoots to media loading", async () => {
    ...
    await sendMessageMatrix("room:!room:example", "caption", {
      client,
      mediaUrl: "file:///tmp/photo.png",
      mediaLocalRoots: ["/tmp/littlebaby-matrix-test"],
    });
    ...
    expect(loadWebMediaMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        maxBytes: undefined,
        localRoots: ["/tmp/littlebaby-matrix-test"],
      }),
    );
  });
});
...
describe("sendSingleTextMessageMatrix", () => {
  ...
  it("merges extra content fields into single-event sends", async () => {
    ...
    await sendSingleTextMessageMatrix("room:!room:example", "done", {
      client,
      extraContent: { [MATRIX_LITTLEBABY_FINALIZED_PREVIEW_KEY]: true },
    });

    expect(sendMessage.mock.calls[0]?.[1]).toMatchObject({
      body: "done",
      [MATRIX_LITTLEBABY_FINALIZED_PREVIEW_KEY]: true,
    });
  });
});
