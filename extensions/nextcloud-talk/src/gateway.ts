import { createAccountStatusSink } from "littlebaby/plugin-sdk/channel-lifecycle";
import { runStoppablePassiveMonitor } from "littlebaby/plugin-sdk/extension-shared";
    ...
    if (changed) {
      if (nextSection && Object.keys(nextSection).length > 0) {
        ...
        if (Object.keys(nextChannels).length > 0) {
          nextCfg.channels = nextChannels as LittleBabyConfig["channels"];
        } else {
