...
describe("loginGeminiCliOAuth", () => {
  ...
  const ENV_KEYS = [
    "LITTLEBABY_GEMINI_OAUTH_CLIENT_ID",
    "LITTLEBABY_GEMINI_OAUTH_CLIENT_SECRET",
  ...
  beforeEach(() => {
    envSnapshot = Object.fromEntries(ENV_KEYS.map((key) => [key, process.env[key]]));
    process.env.LITTLEBABY_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.LITTLEBABY_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret"; // pragma: allowlist secret
  ...
  it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === USERINFO_URL) {
        return responseJson({ email: "lobster@littlebaby.ai" });
      }
      if (url === LOAD_PROD) {
