...
function normalizeDenyList(cfg: LittleBabyPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: LittleBabyPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: LittleBabyPluginApi) {
    ...
    const timerService: LittleBabyPluginService = {
