...
Read-only diff viewer plugin for **LittleBaby** agents.
...
Set plugin-wide defaults in `~/.littlebaby/littlebaby.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/littlebaby",
        },
      },
    },
  },
...
Before:
LittleBaby supports plugins.

After:
LittleBaby supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/littlebaby-diffs`).
