...
describe("renderDiffDocument", () => {
  ...
  it("renders before/after input into a complete viewer document", async () => {
    ...
    expect(rendered.html).toContain("data-littlebaby-diff-root");
  ...
  it("resolves viewer assets under an optional base path", async () => {
    ...
    expect(
      new URL(loaderSrc ?? "", "https://example.com/littlebaby/plugins/diffs/view/id/token").pathname,
    ).toBe("/littlebaby/plugins/diffs/assets/viewer.js");
  });
  ...
  it("downgrades invalid language hints to plain text", async () => {
    ...
    const payloads = [...html.matchAll(/data-littlebaby-diff-payload>(.*?)<\/script>/g)].map(
