...
import type { LittleBabyPluginApi, LittleBabyPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("littlebaby-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<LittleBabyPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: LittleBabyPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<LittleBabyPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as LittleBabyPluginApi);
