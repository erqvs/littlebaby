...
function compileManifestConfigSchema() {
  const manifest = JSON.parse(
    fs.readFileSync(new URL("../littlebaby.plugin.json", import.meta.url), "utf8"),
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/littlebaby/",
      }),
    ).toBe("https://example.com/littlebaby");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/littlebaby?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/littlebaby#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/littlebaby/" })).toBe(true);
  });
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/littlebaby",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/littlebaby/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("littlebabyDiffsReady");
