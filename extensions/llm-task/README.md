...
`littlebaby.invoke --each`) without adding new LittleBaby code per workflow.
...
This extension depends on LittleBaby internal modules (the embedded agent runner).
It is intended to ship as a **bundled** LittleBaby extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.littlebaby/extensions` as a standalone plugin directory.
