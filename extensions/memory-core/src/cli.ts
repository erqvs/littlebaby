    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["littlebaby memory status", "Show index and provider status."],
          [
            "littlebaby memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["littlebaby memory status --deep", "Probe embedding provider readiness."],
          ["littlebaby memory index --force", "Force a full reindex."],
          ['littlebaby memory search "meeting notes"', "Quick search using positional query."],
          [
            'littlebaby memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `littlebaby memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "littlebaby memory rem-backfill --path ./memory",
          ...
          ["littlebaby memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.littlebaby.ai/cli/memory")}\n`,
    );
