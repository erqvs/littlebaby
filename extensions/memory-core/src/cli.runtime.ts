...
import { resolveMemoryRemDreamingConfig } from "littlebaby/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: LittleBabyConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredLittleBabyTmpDir(), "littlebaby-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: LittleBabyConfig): string {
...
function resolveAgent(cfg: LittleBabyConfig, agent?: string) {
...
function resolveAgentIds(cfg: LittleBabyConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: littlebaby memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredLittleBabyTmpDir(), "littlebaby-rem-backfill-"),
      );
      try {
