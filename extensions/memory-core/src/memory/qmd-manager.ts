...
import { withFileLock } from "littlebaby/plugin-sdk/file-lock";
import {
...
import { resolveAgentContextLimits } from "littlebaby/plugin-sdk/memory-core-host-engine-foundation";
import {
  buildSessionEntry,
...
const MCPORTER_STATE_KEY = Symbol.for("littlebaby.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("littlebaby.qmdEmbedQueueTail");
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so LittleBaby search semantics match direct qmd search.
  return trimmed;
}
...
export class QmdMemoryManager implements MemorySearchManager {
  ...
  private readonly cfg: LittleBabyConfig;
