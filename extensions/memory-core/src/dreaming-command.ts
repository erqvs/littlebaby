...
import { resolveMemoryDreamingConfig } from "littlebaby/plugin-sdk/memory-core-host-status";
import { normalizeLowercaseStringOrEmpty } from "littlebaby/plugin-sdk/text-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: LittleBabyConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: LittleBabyConfig, enabled: boolean): LittleBabyConfig {
...
function formatStatus(cfg: LittleBabyConfig): string {
...
export function registerDreamingCommand(api: LittleBabyPluginApi): void {
  api.registerCommand({
    name: "dreaming",
