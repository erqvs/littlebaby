...
import { formatMemoryDreamingDay } from "littlebaby/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "littlebaby/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "littlebaby-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*littlebaby-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*littlebaby-memory-promotion:([^\n]+?)\s*-->/gi);
