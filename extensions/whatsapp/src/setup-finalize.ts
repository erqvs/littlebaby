...
import { formatCliCommand, formatDocsLink } from "littlebaby/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<LittleBabyConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: LittleBabyConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: LittleBabyConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: LittleBabyConfig, accountId: string): string {
  if (
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so LittleBaby can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for LittleBaby" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("littlebaby channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
