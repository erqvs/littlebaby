...
import { info, success } from "littlebaby/plugin-sdk/runtime-env";
import { getChildLogger } from "littlebaby/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "littlebaby/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("littlebaby channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
