...
import { generateSecureUuid } from "littlebaby/plugin-sdk/core";
import { normalizePollInput, type PollInput } from "littlebaby/plugin-sdk/media-runtime";
import { createSubsystemLogger } from "littlebaby/plugin-sdk/runtime-env";
import { getChildLogger } from "littlebaby/plugin-sdk/text-runtime";
import { redactIdentifier } from "littlebaby/plugin-sdk/text-runtime";
import { convertMarkdownTables } from "littlebaby/plugin-sdk/text-runtime";
import {
...
function requireOutboundActiveWebListener(params: { cfg: LittleBabyConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`littlebaby channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
