...
import { resetLogger, setLoggerOverride } from "littlebaby/plugin-sdk/runtime-env";
...
function mockCredsJsonSpies(readContents: string) {
  const credsSuffix = path.join("/tmp", "littlebaby-oauth", "whatsapp", "default", "creds.json");
...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("littlebaby-wa-creds-test");
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("littlebaby-wa-queue");
    const openMock = mockFsOpenForCredsWrites({
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("littlebaby-wa-a");
    const authDirB = createTempAuthDir("littlebaby-wa-b");
    const openMock = mockFsOpenForCredsWrites({
      onTempWrite: async (filePath) => {
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    ...
    await writeCredsJsonAtomically("/tmp/littlebaby-oauth/whatsapp/default", {
    ...
    expect(String(renameArgs[1] ?? "")).toContain(
      path.join("/tmp", "littlebaby-oauth", "whatsapp", "default", "creds.json"),
    );
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("littlebaby-wa-creds-atomic");
