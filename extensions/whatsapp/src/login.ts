...
import { logInfo } from "littlebaby/plugin-sdk/text-runtime";
...
export async function loginWeb(
  ...
  try {
    ...
    if (result.outcome === "logged-out") {
      console.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("littlebaby channels login")} and scan the QR again.`,
        ),
      );
