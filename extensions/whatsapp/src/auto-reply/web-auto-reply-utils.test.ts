...
import { saveSessionStore } from "littlebaby/plugin-sdk/config-runtime";
...
describe("isBotMentionedFromTargets", () => {
  const mentionCfg = { mentionRegexes: [/\blittlebaby\b/i] };
  ...
  it("ignores JID mentions in self-chat mode", () => {
    const cfg = { mentionRegexes: [/\blittlebaby\b/i], allowFrom: ["+999"] };
    const msg = makeMsg({
      body: "@owner ping",
  ...
  it("honors explicit self-chat overrides without recomputing from allowFrom", () => {
    const cfg = {
      mentionRegexes: [/\blittlebaby\b/i],
...
describe("resolveMentionTargets with @lid mapping", () => {
  it("uses @lid reverse mapping for mentions and self identity", async () => {
    await withTempDir("littlebaby-lid-mapping-", async (authDir) => {
      await fs.writeFile(
...
describe("getSessionSnapshot", () => {
  it("uses channel reset overrides when configured", async () => {
    ...
    try {
      await withTempDir("littlebaby-snapshot-", async (root) => {
...
describe("web auto-reply util", () => {
  describe("mentions diagnostics", () => {
    it("returns normalized debug fields and mention outcome", () => {
      ...
      const result = debugMention(msg, { mentionRegexes: [/\blittlebaby\b/i] });
      expect(result.wasMentioned).toBe(true);
      expect(result.details.bodyClean).toBe("littlebaby ping");
