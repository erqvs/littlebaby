import { hasConfiguredSecretInput } from "littlebaby/plugin-sdk/secret-input";
import {
...
function enableSlackAccount(cfg: LittleBabyConfig, accountId: string): LittleBabyConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: LittleBabyConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: LittleBabyConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: LittleBabyConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: LittleBabyConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: LittleBabyConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: LittleBabyConfig; accountId: string }) =>
    ...
    disable: (cfg: LittleBabyConfig) => setSetupChannelEnabled(cfg, channel, false),
