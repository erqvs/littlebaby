import type { SlackSlashCommandConfig } from "littlebaby/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(
    normalizeOptionalString(raw?.name) ?? "littlebaby",
  );
  const name = normalizedName || "littlebaby";
  return {
