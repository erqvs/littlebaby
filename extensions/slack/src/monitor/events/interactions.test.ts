...
describe("registerSlackInteractionEvents", () => {
  beforeAll(async () => {
    const channelRuntime = await import("littlebaby/plugin-sdk/infra-runtime");
    const pluginRuntime = await import("littlebaby/plugin-sdk/plugin-runtime");
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "littlebaby:verify" }],
            },
          ],
        },
      },
  ...
  it("registers a matcher that accepts plugin action ids beyond the LittleBaby prefix", () => {
    const { ctx, getActionMatcher } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });

    const matcher = getActionMatcher();
    expect(matcher).toBeTruthy();
    expect(matcher?.test("littlebaby:verify")).toBe(true);
  ...
  it("treats Slack reply buttons as plain interaction events instead of plugin dispatch", async () => {
    ...
    await handler!({
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "littlebaby:reply_button" }],
            },
          ],
        },
      },
      action: {
        type: "button",
        action_id: "littlebaby:reply_button",
        block_id: "reply_actions",
        value: "codex",
    ...
    expect(enqueueSystemEventMock).toHaveBeenCalledWith(
      expect.stringContaining('"actionId":"littlebaby:reply_button"'),
      expect.any(Object),
    );
    expect(app.client.chat.update).toHaveBeenCalledTimes(1);
  });
  ...
  it("drops modal lifecycle payloads when mismatch guard triggers", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V123",
          callback_id: "littlebaby:deploy_form",
  ...
  it("falls back to container channel and message timestamps", async () => {
    ...
    await handler!({
      ...
      action: {
        type: "button",
        action_id: "littlebaby:container",
          ...
          blocks: [
            {
              ...
              elements: [{ type: "multi_static_select", action_id: "littlebaby:multi" }],
            },
          ],
        },
      },
          ...
          blocks: [
            {
              type: "actions",
              block_id: "date_block",
              elements: [{ type: "datepicker", action_id: "littlebaby:date" }],
            },
            {
              type: "actions",
              block_id: "time_block",
              elements: [{ type: "timepicker", action_id: "littlebaby:time" }],
            },
            {
              type: "actions",
              block_id: "datetime_block",
              elements: [{ type: "datetimepicker", action_id: "littlebaby:datetime" }],
            },
          ],
        },
      },
  ...
  it("captures modal submissions and enqueues view submission event", async () => {
    ...
    expect(payload).toMatchObject({
      ...
      actionId: "view:littlebaby:deploy_form",
      callbackId: "littlebaby:deploy_form",
      viewId: "V123",
      userId: "U777",
  ...
  it("keeps no-channel modal events open when allowFrom is unset", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V444",
          callback_id: "littlebaby:routing_form",
            ...
            values: {
              ...
              email_block: {
                email_input: {
                  ...
                  value: "team@littlebaby.ai",
                },
              },
              url_block: {
                url_input: {
                  type: "url_text_input",
                  value: "https://docs.littlebaby.ai",
                },
              },
              richtext_block: {
                richtext_input: {
    ...
    expect(payload.inputs).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          inputKind: "email",
          inputEmail: "team@littlebaby.ai",
        }),
        expect.objectContaining({
          actionId: "url_input",
          inputKind: "url",
          inputUrl: "https://docs.littlebaby.ai/",
        }),
        expect.objectContaining({
  ...
  it("truncates rich text preview to keep payload summaries compact", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V555",
          callback_id: "littlebaby:long_richtext",
  ...
  it("caps oversized interaction payloads with compact summaries", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V915",
          callback_id: "littlebaby:oversize",
          private_metadata: JSON.stringify({
            channelId: "D915",
            channelType: "im",
            userId: "U915",
