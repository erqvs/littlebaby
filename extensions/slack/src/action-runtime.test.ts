...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): LittleBabyConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as LittleBabyConfig;
  ...
  async function resolveReadToken(cfg: LittleBabyConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: LittleBabyConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
