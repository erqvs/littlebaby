...
import { defaultToolStreamExtraParams } from "littlebaby/plugin-sdk/provider-stream-shared";
import { jsonResult, readProviderEnvValue } from "littlebaby/plugin-sdk/provider-web-search";
import {
  applyXaiModelCompat,
  normalizeXaiModelId,
  resolveXaiTransport,
    ...
    execute: async (toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!tool) {
        return jsonResult({
          ...
          docs: "https://docs.littlebaby.ai/tools/code-execution",
        });
      }
