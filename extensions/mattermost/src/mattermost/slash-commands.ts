...
/**
 * Built-in LittleBaby commands to register as native slash commands.
...
/**
 * Register all LittleBaby slash commands for a given team.
...
export async function registerSlashCommands(params: {
  ...
  for (const spec of commands) {
    ...
    if (ownedCommands.length === 0 && foreignCommands.length > 0) {
      log?.(
        `mattermost: trigger /${spec.trigger} already used by non-LittleBaby command(s); skipping to avoid mutating external integrations`,
      );
      continue;
    }
...
/**
 * Map the trigger word back to the original LittleBaby command name.
