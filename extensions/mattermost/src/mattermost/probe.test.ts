...
vi.mock("littlebaby/plugin-sdk/ssrf-runtime", async () => {
  const original = (await vi.importActual("littlebaby/plugin-sdk/ssrf-runtime")) as Record<
    string,
    unknown
  >;
...
describe("probeMattermost", () => {
  ...
  it("normalizes base URL and returns bot info", async () => {
    mockFetchGuard.mockResolvedValueOnce({
      response: new Response(JSON.stringify({ id: "bot-1", username: "littlebabybot" }), {
        status: 200,
    ...
    expect(result).toEqual(
      expect.objectContaining({
        ok: true,
        status: 200,
        bot: { id: "bot-1", username: "littlebabybot" },
      }),
    );
