...
describe("createTelegramBot", () => {
  ...
  it("blocks DM model-selection callbacks for unpaired users when inline buttons are DM-scoped", async () => {
    ...
    const storePath = `/tmp/littlebaby-telegram-callback-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const storePath = `/tmp/littlebaby-telegram-model-compact-${process.pid}-${Date.now()}.json`;
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/littlebaby-telegram-model-default-${process.pid}-${Date.now()}.json`;
    const config: LittleBabyConfig = {
      agents: {
        defaults: {
  ...
  it("formats non-default model selection confirmations with Telegram HTML parse mode", async () => {
    ...
    const storePath = `/tmp/littlebaby-telegram-model-html-${process.pid}-${Date.now()}.json`;
  ...
  it("persists non-default model override using fresh config, not stale startup snapshot", async () => {
    ...
    const storePath = `/tmp/littlebaby-telegram-model-fresh-cfg-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "LittleBaby" },
        },
      },
      me: { id: 999, username: "littlebaby_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
