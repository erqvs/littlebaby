...
import * as runtimeEnvModule from "littlebaby/plugin-sdk/runtime-env";
import { withEnv } from "littlebaby/plugin-sdk/testing";
...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", LITTLEBABY_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): LittleBabyConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): LittleBabyConfig => ({
    channels: {
      telegram: {
