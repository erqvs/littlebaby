...
function resolveDefaultAgentId(cfg: LittleBabyConfig): string {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  const chosen = (agents.find((agent) => agent?.default) ?? agents[0])?.id;
  return normalizeAgentId(chosen);
}

function listConfiguredAccountIds(cfg: LittleBabyConfig): string[] {
...
function listBoundAccountIds(cfg: LittleBabyConfig, channelId: string): string[] {
  const ids = new Set<string>();
...
function resolveDefaultAgentBoundAccountId(cfg: LittleBabyConfig, channelId: string): string | null {
...
export function listTelegramAccountIds(cfg: LittleBabyConfig): string[] {
...
export function resolveDefaultTelegramAccountSelection(cfg: LittleBabyConfig): {
  accountId: string;
  accountIds: string[];
...
export function resolveDefaultTelegramAccountId(cfg: LittleBabyConfig): string {
