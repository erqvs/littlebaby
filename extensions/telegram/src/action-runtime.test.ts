...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): LittleBabyConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as LittleBabyConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): LittleBabyConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as LittleBabyConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: LittleBabyConfig, accountId = "media") {
    await handleTelegramAction(
