...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "littlebaby-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<LittleBabyConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<LittleBabyConfig["channels"]>["telegram"]>>,
): LittleBabyConfig {
  return {
    channels: {
      telegram: {
