/**
 * Synology Chat Channel Plugin for LittleBaby.
 *
...
import { createChatChannelPlugin, type ChannelPlugin } from "littlebaby/plugin-sdk/channel-core";
import { waitUntilAbort } from "littlebaby/plugin-sdk/channel-lifecycle";
import {
...
import { attachChannelToResult } from "littlebaby/plugin-sdk/channel-send-result";
import { createEmptyChannelDirectoryAdapter } from "littlebaby/plugin-sdk/directory-runtime";
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "littlebaby pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: LittleBabyConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: LittleBabyConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to LittleBaby",
        order: 90,
      },
      capabilities: {
