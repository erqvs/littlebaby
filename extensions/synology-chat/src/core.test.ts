...
const synologyChatSetupPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }: { cfg: LittleBabyConfig; accountId?: string }) =>
...
describe("synology-chat core", () => {
  beforeEach(() => {
    ...
    delete process.env.LITTLEBABY_BOT_NAME;
  });
...
describe("synology-chat account resolution", () => {
  ...
  it("returns full defaults for empty config", () => {
    ...
    expect(account.botName).toBe("LittleBaby");
  });
  ...
  it("uses env var fallbacks", () => {
    ...
    process.env.LITTLEBABY_BOT_NAME = "TestBot";
