...
function getChannelConfig(cfg: LittleBabyConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: LittleBabyConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: LittleBabyConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: LittleBabyConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL LittleBaby uses to send replies back to Chat.",
      ],
