...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/littlebaby-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("littlebaby/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("littlebaby/plugin-sdk/conversation-runtime")>(
    "littlebaby/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("littlebaby/plugin-sdk/reply-runtime"),
      import("littlebaby/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
