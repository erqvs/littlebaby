...
vi.mock("littlebaby/plugin-sdk/fetch-runtime", () => ({
  resolveFetch: (...args: unknown[]) => resolveFetchMock(...args),
}));

vi.mock("littlebaby/plugin-sdk/core", async () => {
  const actual = await vi.importActual<typeof import("littlebaby/plugin-sdk/core")>(
    "littlebaby/plugin-sdk/core",
  );
  return {
    ...actual,
