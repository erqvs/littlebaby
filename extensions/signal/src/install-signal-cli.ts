...
import { runPluginCommandWithTimeout } from "littlebaby/plugin-sdk/run-command";
import type { RuntimeEnv } from "littlebaby/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "littlebaby/plugin-sdk/setup-tools";
...
async function installSignalCliFromRelease(runtime: RuntimeEnv): Promise<SignalInstallResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredLittleBabyTmpDir(), "littlebaby-signal-"));
