...
function resolveKimiBaseUrl(kimi?: KimiConfig, littleBabyConfig?: LittleBabyConfig): string {
  ...
  const moonshotBaseUrl = littleBabyConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: LittleBabyConfig; searchConfig?: SearchConfigRecord },
