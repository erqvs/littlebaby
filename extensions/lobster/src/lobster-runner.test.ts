...
describe("createEmbeddedLobsterRunner", () => {
  ...
  it("detects workflow files and parses argsJson", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-lobster-runner-"));
  ...
  it("fails closed when the embedded runtime requests unsupported input", async () => {
    ...
    ).rejects.toThrow("Lobster input requests are not supported by the LittleBaby Lobster tool yet");
  });
