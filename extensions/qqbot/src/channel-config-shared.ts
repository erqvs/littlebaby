...
import { applyAccountNameToChannelSection } from "littlebaby/plugin-sdk/setup";
...
export const qqbotConfigAdapter = {
  listAccountIds: (cfg: LittleBabyConfig) => listQQBotAccountIds(cfg),
  resolveAccount: (cfg: LittleBabyConfig, accountId?: string | null) =>
    resolveQQBotAccount(cfg, accountId, { allowUnresolvedSecretRef: true }),
  defaultAccountId: (cfg: LittleBabyConfig) => resolveDefaultQQBotAccountId(cfg),
  setAccountEnabled: ({
    cfg,
    accountId,
    enabled,
  ...
  deleteAccount: ({ cfg, accountId }: { cfg: LittleBabyConfig; accountId: string }) =>
    deleteAccountFromConfigSection({
      cfg,
      sectionKey: "qqbot",
      accountId,
...
export const qqbotSetupAdapterShared = {
  resolveAccountId: ({ cfg, accountId }: { cfg: LittleBabyConfig; accountId?: string | null }) =>
