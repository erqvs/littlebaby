...
const LIVE_MODEL_ID = process.env.LITTLEBABY_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.4-nano";
const LIVE_IMAGE_MODEL = process.env.LITTLEBABY_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-1";
const LIVE_VISION_MODEL = process.env.LITTLEBABY_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.LITTLEBABY_LIVE_TEST === "1";
...
describeLive("openai plugin live", () => {
  ...
  it("transcribes synthesized speech through the registered media provider", async () => {
    ...
    expect(collapsedText).toContain("littlebaby");
