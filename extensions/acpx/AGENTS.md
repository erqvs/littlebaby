...
The bundled ACPX extension is a thin LittleBaby wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `littlebaby/acpx`, not in this extension.
...
Use this flow when LittleBaby needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `littlebaby/acpx` repo first.
2. In LittleBaby, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild LittleBaby and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for LittleBaby ACP validation.
...
If a change feels like shared ACP runtime behavior instead of LittleBaby-specific glue, move it to `littlebaby/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
