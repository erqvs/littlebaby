...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "littlebaby-plugin-tools";

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "littlebaby.plugin.json")) &&
...
function resolveAcpxPluginRootFromLittleBabyLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common LittleBaby layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromLittleBabyLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveLittleBabyRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const littleBabyRoot = resolveLittleBabyRoot(pluginRoot);
  const distEntry = path.join(littleBabyRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(littleBabyRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
