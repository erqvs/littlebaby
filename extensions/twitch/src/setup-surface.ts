...
import { normalizeOptionalAccountId } from "littlebaby/plugin-sdk/account-id";
import { getChatChannelMeta, type ChannelPlugin } from "littlebaby/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: LittleBabyConfig, requestedAccountId?: string): string {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: LITTLEBABY_TWITCH_ACCESS_TOKEN",
...
export async function configureWithEnvToken(
  ...
  accountId: string = resolveSetupAccountId(cfg),
): Promise<{ cfg: LittleBabyConfig } | null> {
  const resolvedAccountId = accountId.trim()
    ? normalizeRequestedSetupAccountId(accountId)
    : resolveSetupAccountId(cfg);
  ...
  const useEnv = await prompter.confirm({
    message: "Twitch env var LITTLEBABY_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.LITTLEBABY_TWITCH_ACCESS_TOKEN?.trim();
