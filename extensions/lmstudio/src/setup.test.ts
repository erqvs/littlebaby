...
import type { LittleBabyConfig } from "littlebaby/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "littlebaby/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "littlebaby/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "littlebaby/plugin-sdk/provider-setup";
import type { WizardPrompter } from "littlebaby/plugin-sdk/setup";
...
vi.mock("littlebaby/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("littlebaby/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("littlebaby/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("littlebaby/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
function buildConfig(): LittleBabyConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
function buildDiscoveryContext(params?: {
  ...
  return {
    config: params?.config ?? ({} as LittleBabyConfig),
    env: params?.env ?? {},
