...
const LITTLEBABY_OPENSHELL_E2E = process.env.LITTLEBABY_E2E_OPENSHELL === "1";
const LITTLEBABY_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const LITTLEBABY_OPENSHELL_COMMAND =
  process.env.LITTLEBABY_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "littlebaby-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && LITTLEBABY_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: LITTLEBABY_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(LITTLEBABY_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(LITTLEBABY_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-openshell-e2e-"));
      ...
      const gatewayName = `littlebaby-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `littlebaby-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/littlebaby-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: LITTLEBABY_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("littlebaby-openshell-e2e");
