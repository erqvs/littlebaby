...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: LittleBabyConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): LittleBabyConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("littlebaby/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("littlebaby/plugin-sdk/command-auth")>(
...
vi.mock("littlebaby/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("littlebaby/plugin-sdk/reply-runtime")>(
...
vi.mock("littlebaby/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("littlebaby/plugin-sdk/runtime-env")>(
    "littlebaby/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("littlebaby/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("littlebaby/plugin-sdk/infra-runtime")>(
