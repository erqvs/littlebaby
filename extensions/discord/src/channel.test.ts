...
function createCfg(): LittleBabyConfig {
  return {
    channels: {
      discord: {
        enabled: true,
...
function resolveAccount(cfg: LittleBabyConfig, accountId = "default"): ResolvedDiscordAccount {
  return discordPlugin.config.resolveAccount(cfg, accountId);
}

function startDiscordAccount(cfg: LittleBabyConfig, accountId = "default") {
  ...
  it("threads poll sends through the thread target", async () => {
    ...
    try {
      const result = await discordPlugin.outbound!.sendPoll!({
        ...
        poll: {
          question: "Best shell?",
          options: ["littlebaby", "littlebabyer"],
        },
        accountId: "work",
        threadId: "thread-123",
      });
