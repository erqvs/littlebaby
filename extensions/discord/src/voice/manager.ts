...
import type { DiscordAccountConfig, TtsConfig } from "littlebaby/plugin-sdk/config-runtime";
...
import { parseTtsDirectives } from "littlebaby/plugin-sdk/speech";
...
function resolveVoiceTtsConfig(params: { cfg: LittleBabyConfig; override?: TtsConfig }): {
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredLittleBabyTmpDir(), "discord-voice-"));
