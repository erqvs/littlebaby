...
import { AcpRuntimeError } from "littlebaby/plugin-sdk/acp-runtime";
...
let runtimeEnvModule: typeof import("littlebaby/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): LittleBabyConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("littlebaby/plugin-sdk/plugin-runtime", async () => {
      const actual = await vi.importActual<typeof import("littlebaby/plugin-sdk/plugin-runtime")>(
        "littlebaby/plugin-sdk/plugin-runtime",
      );
      return {
        ...actual,
    ...
    runtimeEnvModule = await import("littlebaby/plugin-sdk/runtime-env");
  ...
  it("passes LittleBaby EventQueue defaults to Carbon Client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
  ...
  it("logs Discord startup phases and early gateway debug events", async () => {
    ...
    clientFetchUserMock.mockImplementationOnce(async () => {
      ...
      return { id: "bot-1", username: "LittleBaby" };
    });
