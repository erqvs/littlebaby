...
import { clearSessionStoreCacheForTest } from "littlebaby/plugin-sdk/config-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { createNoopThreadBindingManager } from "./thread-bindings.js";

type ConversationRuntimeModule = typeof import("littlebaby/plugin-sdk/conversation-binding-runtime");
...
vi.mock("littlebaby/plugin-sdk/conversation-binding-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("littlebaby/plugin-sdk/conversation-binding-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("littlebaby/plugin-sdk/conversation-binding-runtime")>(
        "littlebaby/plugin-sdk/conversation-binding-runtime",
      ),
  );
});

vi.mock("littlebaby/plugin-sdk/agent-runtime", () => ({
...
vi.mock("littlebaby/plugin-sdk/models-provider-runtime", () => ({
  buildModelsProviderData: buildModelsProviderDataMock,
}));

const STORE_PATH = path.join(
  os.tmpdir(),
  `littlebaby-discord-think-autocomplete-${process.pid}.json`,
);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("littlebaby/plugin-sdk/command-auth").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("littlebaby/plugin-sdk/command-auth").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("littlebaby/plugin-sdk/command-auth");
