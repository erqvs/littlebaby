...
vi.mock("littlebaby/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("littlebaby/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<LittleBabyConfig["channels"]>["discord"]>["execApprovals"],
): LittleBabyConfig {
  return {
    channels: {
      discord: {
