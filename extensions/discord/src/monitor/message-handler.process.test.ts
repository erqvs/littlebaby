import { DEFAULT_EMOJIS } from "littlebaby/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/littlebaby-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("littlebaby/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("littlebaby/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("littlebaby/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/littlebaby-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/littlebaby-discord-process-test-sessions.json" },
      },
    });
