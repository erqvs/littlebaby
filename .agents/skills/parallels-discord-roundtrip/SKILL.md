...
export LITTLEBABY_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.littlebaby/openclaw.json' | tr -d '\n'
)"
...
- Use the `openclaw` wrapper for guest `message send/read`; `node littlebaby.mjs message ...` does not expose the lazy message subcommands the same way.
