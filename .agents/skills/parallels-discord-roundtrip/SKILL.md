...
```bash
export LITTLEBABY_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.littlebaby/littlebaby.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env LITTLEBABY_PARALLELS_DISCORD_TOKEN \
...
- Use the `littlebaby` wrapper for guest `message send/read`; `node littlebaby.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/littlebaby-parallels-smoke.*`
...
- host inbound nonce appears in `littlebaby message read` output
