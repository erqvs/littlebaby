---
name: littlebaby-test-heap-leaks
description: Investigate `pnpm test` memory growth, Vitest worker OOMs, and suspicious RSS increases in LittleBaby using the `scripts/test-parallel.mjs` heap snapshot tooling. Use when Codex needs to reproduce test-lane memory growth, collect repeated `.heapsnapshot` files, compare snapshots from the same worker PID, triage likely transformed-module retention versus likely runtime leaks, and fix or reduce the impact by patching cleanup logic or isolating hotspot tests.
---

# LittleBaby Test Heap Leaks
...
1. Reproduce the failing shape first.
   ...
   - `pnpm canvas:a2ui:bundle && LITTLEBABY_TEST_MEMORY_TRACE=1 LITTLEBABY_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 LITTLEBABY_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap LITTLEBABY_TEST_WORKERS=2 LITTLEBABY_TEST_MAX_OLD_SPACE_SIZE_MB=6144 pnpm test`
   - Keep `LITTLEBABY_TEST_MEMORY_TRACE=1` enabled so the wrapper prints per-file RSS summaries alongside the snapshots.
...
2. Wait for repeated snapshots before concluding anything.
   ...
   - Use `.agents/skills/littlebaby-test-heap-leaks/scripts/heapsnapshot-delta.mjs` to compare either two files directly or the earliest/latest pair per PID in one lane directory.
...
- Direct comparison:
  - `node .agents/skills/littlebaby-test-heap-leaks/scripts/heapsnapshot-delta.mjs before.heapsnapshot after.heapsnapshot`
- Auto-select earliest/latest snapshots per PID within one lane:
  - `node .agents/skills/littlebaby-test-heap-leaks/scripts/heapsnapshot-delta.mjs --lane-dir .tmp/heapsnap/unit-fast-batch-2`
- Useful flags:
  - `--top 40`
  - `--min-kb 32`
  - `--pid 16133`
