---
name: littlebaby-test-performance
description: Benchmark, diagnose, and optimize LittleBaby test performance without losing coverage. Use when Codex needs to reassess `pnpm test`, compare grouped Vitest reports, identify CPU/memory/import hotspots, fix slow tests or cold runtime paths, preserve behavior proofs, update the performance report, add AGENTS guardrails, and make scoped commits/pushes for LittleBaby test-speed work.
---

# LittleBaby Test Performance
...
2. Establish a baseline before changing code:
   ...
   - For import-heavy suspicion add:
     `LITTLEBABY_VITEST_IMPORT_DURATIONS=1 LITTLEBABY_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
timeout 240 /usr/bin/time -l env \
  LITTLEBABY_VITEST_IMPORT_DURATIONS=1 \
  LITTLEBABY_VITEST_PRINT_IMPORT_BREAKDOWN=1 \
