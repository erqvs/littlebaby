---
name: littlebaby-release-maintainer
description: Maintainer workflow for LittleBaby releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# LittleBaby Release Maintainer
...
- Core `littlebaby` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- Version locations include:
  ...
  - `apps/macos/Sources/LittleBaby/Resources/Info.plist`
...
- Every LittleBaby release ships the npm package and macOS app together.
- The production Sparkle feed lives at `https://raw.githubusercontent.com/littlebaby/littlebaby/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `littlebaby` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `littlebaby YYYY.M.D-beta.N`
...
```bash
  LITTLEBABY_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/littlebaby-npm-postpublish-verify.ts <published-version>
```
...
- Use `pnpm test:live:media video` for bounded video-provider smoke when video
  ...
  each provider operation with `LITTLEBABY_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default).
...
- Set `LITTLEBABY_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
  validating the slower image-to-video and video-to-video transform lanes.

## Check all relevant release builds

- Always validate the LittleBaby npm release path before creating the tag.
...
- For beta-style full e2e batteries, hard-cap top-level long lanes instead of letting them run indefinitely. Use host `timeout --foreground`/`gtimeout --foreground` caps such as:
  - `45m` for `LITTLEBABY_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
  - `90m` for `pnpm test:docker:all`
  - Parallels caps from the `littlebaby-parallels-smoke` skill
...
- Actual npm install/update phases are capped at 5 minutes. If `npm install -g`, installer package install, or `littlebaby update` takes longer than 300s in release e2e, stop treating the run as healthy progress and debug the installer/updater or harness.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `littlebaby/littlebaby` and the real mac preflight in
  `littlebaby/releases-private` for every release.
...
- LittleBaby publish uses GitHub trusted publishing.
- Stable npm promotion from `beta` to `latest` is an explicit mode on
  `.github/workflows/littlebaby-npm-release.yml`, but it still needs a valid
...
- `.github/workflows/macos-release.yml` in `littlebaby/littlebaby` is now a
  public validation-only handoff. It validates the tag/release state and points
  operators to the private repo. It still rebuilds the JS outputs needed for
  release validation, but it does not sign, notarize, or publish macOS
  artifacts.
- `littlebaby/releases-private/.github/workflows/littlebaby-macos-validate.yml`
...
- Real mac preflight and real mac publish both use
  `littlebaby/releases-private/.github/workflows/littlebaby-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@littlebaby/littlebaby-release-managers` before publish continues.
- Mac publish uses
  `littlebaby/releases-private/.github/workflows/littlebaby-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `littlebaby/littlebaby`
  automatically when `LITTLEBABY_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for the LittleBaby package
...
- `@littlebaby/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/littlebaby-npm-release.yml` with `preflight_only=true`
...
10. Start `.github/workflows/macos-release.yml` in `littlebaby/littlebaby` and wait
...
14. Start `.github/workflows/littlebaby-npm-release.yml` with the same tag for
...
15. Wait for `npm-release` approval from `@littlebaby/littlebaby-release-managers`.
16. If the stable release was published to `beta`, start
    `.github/workflows/littlebaby-npm-release.yml` again after beta validation
...
17. If the stable release was published directly to `latest` and `beta` should
    follow it, start `.github/workflows/littlebaby-npm-release.yml` again with
...
- Use `littlebaby-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
