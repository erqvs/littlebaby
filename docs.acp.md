# LittleBaby ACP Bridge

This document describes how the LittleBaby ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`littlebaby acp` exposes an ACP agent over stdio and forwards prompts to a running
LittleBaby Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing LittleBaby Gateway
...
- ACP client spawns `littlebaby acp` and speaks ACP messages over stdio.
...
`littlebaby acp` resolves the Gateway URL and auth from CLI flags or config:
