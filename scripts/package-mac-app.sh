...
# Build and bundle LittleBaby into a minimal .app we can open.
# Outputs to dist/LittleBaby.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/LittleBaby.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="LittleBaby"
BUNDLE_ID="${BUNDLE_ID:-ai.littlebaby.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/littlebaby/littlebaby/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/LittleBaby/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :LittleBabyBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :LittleBabyGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/LittleBaby"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/LittleBaby"
fi
chmod +x "$APP_ROOT/Contents/MacOS/LittleBaby"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/LittleBaby" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/LittleBaby/Resources/LittleBaby.icns" "$APP_ROOT/Contents/Resources/LittleBaby.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/LittleBaby/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying LittleBabyKit resources"
LITTLEBABYKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/LittleBabyKit_LittleBabyKit.bundle"
if [ -d "$LITTLEBABYKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/LittleBabyKit_LittleBabyKit.bundle"
  cp -R "$LITTLEBABYKIT_BUNDLE" "$APP_ROOT/Contents/Resources/LittleBabyKit_LittleBabyKit.bundle"
else
  echo "WARN: LittleBabyKit resource bundle not found at $LITTLEBABYKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running LittleBaby"
killall -q LittleBaby 2>/dev/null || true
