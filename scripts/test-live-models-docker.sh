...
if littlebaby_live_truthy "${LITTLEBABY_DOCKER_PROFILE_ENV_ONLY:-}"; then
  ...
  LITTLEBABY_DOCKER_AUTH_DIRS=none
else
...
echo "==> Profile env only: ${LITTLEBABY_DOCKER_PROFILE_ENV_ONLY:-0}"
...
docker run --rm -t \
  ...
  -e LITTLEBABY_LIVE_MODELS="${LITTLEBABY_LIVE_MODELS:-modern}" \
  -e LITTLEBABY_LIVE_PROVIDERS="${LITTLEBABY_LIVE_PROVIDERS:-}" \
  -e LITTLEBABY_LIVE_MAX_MODELS="${LITTLEBABY_LIVE_MAX_MODELS:-12}" \
  -e LITTLEBABY_LIVE_MODEL_TIMEOUT_MS="${LITTLEBABY_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e LITTLEBABY_LIVE_REQUIRE_PROFILE_KEYS="${LITTLEBABY_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e LITTLEBABY_LIVE_GATEWAY_MODELS="${LITTLEBABY_LIVE_GATEWAY_MODELS:-}" \
  -e LITTLEBABY_LIVE_GATEWAY_PROVIDERS="${LITTLEBABY_LIVE_GATEWAY_PROVIDERS:-}" \
  -e LITTLEBABY_LIVE_GATEWAY_MAX_MODELS="${LITTLEBABY_LIVE_GATEWAY_MAX_MODELS:-}" \
  "${DOCKER_HOME_MOUNT[@]}" \
