...
echo "==> Seed state"
mkdir -p "${LITTLEBABY_STATE_DIR}/credentials"
mkdir -p "${LITTLEBABY_STATE_DIR}/agents/main/sessions"
echo '{}' >"${LITTLEBABY_CONFIG_PATH}"
echo 'creds' >"${LITTLEBABY_STATE_DIR}/credentials/marker.txt"
...
test ! -f "${LITTLEBABY_CONFIG_PATH}"
test ! -d "${LITTLEBABY_STATE_DIR}/credentials"
test ! -d "${LITTLEBABY_STATE_DIR}/agents/main/sessions"
