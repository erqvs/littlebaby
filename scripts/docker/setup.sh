...
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${LITTLEBABY_DOCKER_SOCKET:-}"
TIMEZONE="${LITTLEBABY_TZ:-}"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == LITTLEBABY_GATEWAY_TOKEN=* ]]; then
      token="${line#LITTLEBABY_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${LITTLEBABY_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$LITTLEBABY_GATEWAY_PORT" "$LITTLEBABY_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$LITTLEBABY_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$LITTLEBABY_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  ...
  run_prestart_gateway --entrypoint node littlebaby-gateway \
    dist/index.js "$@"
}
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "LITTLEBABY_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "LITTLEBABY_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
LITTLEBABY_CONFIG_DIR="${LITTLEBABY_CONFIG_DIR:-$HOME/.littlebaby}"
LITTLEBABY_WORKSPACE_DIR="${LITTLEBABY_WORKSPACE_DIR:-$HOME/.littlebaby/workspace}"

validate_mount_path_value "LITTLEBABY_CONFIG_DIR" "$LITTLEBABY_CONFIG_DIR"
validate_mount_path_value "LITTLEBABY_WORKSPACE_DIR" "$LITTLEBABY_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "LITTLEBABY_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "LITTLEBABY_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "LITTLEBABY_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "LITTLEBABY_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "LITTLEBABY_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "LITTLEBABY_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$LITTLEBABY_CONFIG_DIR"
mkdir -p "$LITTLEBABY_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$LITTLEBABY_CONFIG_DIR/identity"
mkdir -p "$LITTLEBABY_CONFIG_DIR/agents/main/agent"
mkdir -p "$LITTLEBABY_CONFIG_DIR/agents/main/sessions"

export LITTLEBABY_CONFIG_DIR
export LITTLEBABY_WORKSPACE_DIR
export LITTLEBABY_GATEWAY_PORT="${LITTLEBABY_GATEWAY_PORT:-18789}"
export LITTLEBABY_BRIDGE_PORT="${LITTLEBABY_BRIDGE_PORT:-18790}"
export LITTLEBABY_GATEWAY_BIND="${LITTLEBABY_GATEWAY_BIND:-lan}"
export LITTLEBABY_IMAGE="$IMAGE_NAME"
export LITTLEBABY_DOCKER_APT_PACKAGES="${LITTLEBABY_DOCKER_APT_PACKAGES:-}"
export LITTLEBABY_EXTENSIONS="${LITTLEBABY_EXTENSIONS:-}"
export LITTLEBABY_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export LITTLEBABY_HOME_VOLUME="$HOME_VOLUME_NAME"
export LITTLEBABY_ALLOW_INSECURE_PRIVATE_WS="${LITTLEBABY_ALLOW_INSECURE_PRIVATE_WS:-}"
export LITTLEBABY_SANDBOX="$SANDBOX_ENABLED"
export LITTLEBABY_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export LITTLEBABY_TZ="$TIMEZONE"
...
if [[ -z "${LITTLEBABY_GATEWAY_TOKEN:-}" ]]; then
  ...
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    ...
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      LITTLEBABY_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      LITTLEBABY_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      LITTLEBABY_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
export LITTLEBABY_GATEWAY_TOKEN
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${LITTLEBABY_CONFIG_DIR}:/home/node/.littlebaby"
    gateway_workspace_mount="${LITTLEBABY_WORKSPACE_DIR}:/home/node/.littlebaby/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${LITTLEBABY_INSTALL_DOCKER_CLI:-}" ]]; then
    export LITTLEBABY_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  LITTLEBABY_CONFIG_DIR \
  LITTLEBABY_WORKSPACE_DIR \
  LITTLEBABY_GATEWAY_PORT \
  LITTLEBABY_BRIDGE_PORT \
  LITTLEBABY_GATEWAY_BIND \
  LITTLEBABY_GATEWAY_TOKEN \
  LITTLEBABY_IMAGE \
  LITTLEBABY_EXTRA_MOUNTS \
  LITTLEBABY_HOME_VOLUME \
  LITTLEBABY_DOCKER_APT_PACKAGES \
  LITTLEBABY_EXTENSIONS \
  LITTLEBABY_SANDBOX \
  LITTLEBABY_DOCKER_SOCKET \
  DOCKER_GID \
  LITTLEBABY_INSTALL_DOCKER_CLI \
...
if [[ "$IMAGE_NAME" == "littlebaby:local" ]]; then
  ...
  run_docker_build \
    ...
    --build-arg "LITTLEBABY_EXTENSIONS=${LITTLEBABY_EXTENSIONS}" \
    --build-arg "LITTLEBABY_INSTALL_DOCKER_CLI=${LITTLEBABY_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
echo "Gateway runtime bind comes from LITTLEBABY_GATEWAY_BIND (default: lan)."
echo "Current runtime bind: $LITTLEBABY_GATEWAY_BIND"
echo "Gateway token: $LITTLEBABY_GATEWAY_TOKEN"
...
# --- Sandbox setup (opt-in via LITTLEBABY_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
...
YAML
    ...
    echo "WARNING: LITTLEBABY_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate littlebaby-gateway
  fi
else
...
echo "Config: $LITTLEBABY_CONFIG_DIR"
echo "Workspace: $LITTLEBABY_WORKSPACE_DIR"
echo "Token: $LITTLEBABY_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f littlebaby-gateway"
echo "  ${COMPOSE_HINT} exec littlebaby-gateway node dist/index.js health --token \"$LITTLEBABY_GATEWAY_TOKEN\""
