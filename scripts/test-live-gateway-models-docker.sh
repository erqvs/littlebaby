...
elif [[ -n "${LITTLEBABY_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(littlebaby_live_collect_auth_dirs_from_csv "${LITTLEBABY_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(littlebaby_live_collect_auth_files_from_csv "${LITTLEBABY_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e LITTLEBABY_LIVE_GATEWAY_MODELS="${LITTLEBABY_LIVE_GATEWAY_MODELS:-modern}" \
  -e LITTLEBABY_LIVE_GATEWAY_PROVIDERS="${LITTLEBABY_LIVE_GATEWAY_PROVIDERS:-}" \
  -e LITTLEBABY_LIVE_GATEWAY_SMOKE="${LITTLEBABY_LIVE_GATEWAY_SMOKE:-1}" \
  -e LITTLEBABY_LIVE_GATEWAY_MAX_MODELS="${LITTLEBABY_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e LITTLEBABY_LIVE_GATEWAY_STEP_TIMEOUT_MS="${LITTLEBABY_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e LITTLEBABY_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${LITTLEBABY_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \
  "${DOCKER_HOME_MOUNT[@]}" \
