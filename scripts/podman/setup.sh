...
if [[ -n "${LITTLEBABY_PODMAN_QUADLET:-}" ]]; then
  case "${LITTLEBABY_PODMAN_QUADLET,,}" in
...
if [[ -z "$LITTLEBABY_HOME" ]]; then
  LITTLEBABY_HOME="$(resolve_user_home "$LITTLEBABY_USER")"
fi
if [[ -z "$LITTLEBABY_HOME" ]]; then
  echo "Unable to resolve HOME for user $LITTLEBABY_USER." >&2
  exit 1
fi
if [[ -z "$LITTLEBABY_CONFIG_DIR" ]]; then
  LITTLEBABY_CONFIG_DIR="$LITTLEBABY_HOME/.littlebaby"
fi
if [[ -z "$LITTLEBABY_WORKSPACE_DIR" ]]; then
  LITTLEBABY_WORKSPACE_DIR="$LITTLEBABY_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$LITTLEBABY_HOME"
validate_mount_source_path "config directory" "$LITTLEBABY_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$LITTLEBABY_WORKSPACE_DIR"
validate_container_name "$LITTLEBABY_CONTAINER_NAME"
validate_image_name "$LITTLEBABY_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$LITTLEBABY_CONFIG_DIR" "$LITTLEBABY_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$LITTLEBABY_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$LITTLEBABY_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${LITTLEBABY_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "LITTLEBABY_DOCKER_APT_PACKAGES=${LITTLEBABY_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${LITTLEBABY_EXTENSIONS:-}" ]]; then
...
if [[ "$LITTLEBABY_IMAGE" == "openclaw:local" ]]; then
  ...
  if podman image exists "$LITTLEBABY_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $LITTLEBABY_IMAGE"
  else
    echo "Pulling image $LITTLEBABY_IMAGE ..."
    podman pull "$LITTLEBABY_IMAGE"
  fi
fi

ENV_FILE="$LITTLEBABY_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
EOF
  ...
  echo "Generated LITTLEBABY_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  ...
  LITTLEBABY_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$LITTLEBABY_HOME")"
  LITTLEBABY_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$LITTLEBABY_CONFIG_DIR")"
  LITTLEBABY_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$LITTLEBABY_WORKSPACE_DIR")"
  LITTLEBABY_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$LITTLEBABY_IMAGE")"
  LITTLEBABY_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$LITTLEBABY_CONTAINER_NAME")"
  sed \
    -e "s|{{LITTLEBABY_HOME}}|$LITTLEBABY_HOME_ESCAPED|g" \
    -e "s|{{LITTLEBABY_CONFIG_DIR}}|$LITTLEBABY_CONFIG_ESCAPED|g" \
    -e "s|{{LITTLEBABY_WORKSPACE_DIR}}|$LITTLEBABY_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$LITTLEBABY_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$LITTLEBABY_CONTAINER_ESCAPED|g" \
