...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const littlebabyMeta =
    source.packageJson &&
    typeof source.packageJson === "object" &&
    !Array.isArray(source.packageJson) &&
    "littlebaby" in source.packageJson
      ? (source.packageJson.littlebaby as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    littlebabyMeta &&
    typeof littlebabyMeta.channel === "object" &&
    littlebabyMeta.channel &&
    !Array.isArray(littlebabyMeta.channel)
      ? (littlebabyMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
