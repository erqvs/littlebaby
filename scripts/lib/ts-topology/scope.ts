...
export function createPluginSdkScope(_repoRoot: string): TopologyScope {
  const entrypoints = pluginSdkEntrypoints.map((entrypoint) => ({
    ...
    importSpecifier:
      entrypoint === "index" ? "littlebaby/plugin-sdk" : `littlebaby/plugin-sdk/${entrypoint}`,
  }));
  return buildScopeFromEntrypoints("plugin-sdk", "LittleBaby plugin-sdk public surface", entrypoints);
}
