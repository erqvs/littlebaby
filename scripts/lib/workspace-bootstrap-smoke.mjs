...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "littlebaby-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "littlebaby.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".littlebaby", "workspace");
