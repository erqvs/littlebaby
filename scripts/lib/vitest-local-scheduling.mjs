...
function isSystemThrottleDisabled(env) {
  const normalized = env.LITTLEBABY_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.LITTLEBABY_VITEST_MAX_WORKERS ?? env.LITTLEBABY_TEST_WORKERS);
  if (override !== null) {
