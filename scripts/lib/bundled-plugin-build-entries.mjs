...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@littlebaby/${params.dirName}`) {
    return false;
  }
...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.littlebaby?.extensions)
    ? packageJson.littlebaby.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.littlebaby?.setupEntry === "string" &&
    packageJson.littlebaby.setupEntry.trim().length > 0
      ? packageJson.littlebaby.setupEntry
      : undefined;
  if (setupEntry) {
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = fs.existsSync(path.join(pluginDir, "littlebaby.plugin.json"))
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      const manifestBasename = fs.existsSync(path.join(cwd, BUNDLED_PLUGIN_ROOT_DIR, id, "littlebaby.plugin.json"))
        ? "littlebaby.plugin.json"
        : "littlebaby.plugin.json";
      artifacts.add(bundledDistPluginFile(id, manifestBasename));
    }
    if (hasPackageJson) {
