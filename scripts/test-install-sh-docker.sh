...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${LITTLEBABY_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$LITTLEBABY_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.LITTLEBABY_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `LITTLEBABY_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SKIP_UPDATE="${LITTLEBABY_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${LITTLEBABY_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${LITTLEBABY_INSTALL_SMOKE_UPDATE_BASELINE:-2026.4.10}"
UPDATE_PACKAGE_SPEC="${LITTLEBABY_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${LITTLEBABY_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${LITTLEBABY_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${LITTLEBABY_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${LITTLEBABY_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${LITTLEBABY_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (LITTLEBABY_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  start_update_server
  ...
  docker run --rm -t \
    ...
    -e LITTLEBABY_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e LITTLEBABY_INSTALL_METHOD=npm \
    -e LITTLEBABY_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e LITTLEBABY_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e LITTLEBABY_INSTALL_LATEST_OUT="/out/latest" \
    -e LITTLEBABY_NO_ONBOARD=1 \
    -e LITTLEBABY_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${UPDATE_DOCKER_HOST_ARGS[@]}" \
    -e LITTLEBABY_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e LITTLEBABY_INSTALL_SMOKE_MODE=update \
    -e LITTLEBABY_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e LITTLEBABY_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e LITTLEBABY_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e LITTLEBABY_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e LITTLEBABY_NO_ONBOARD=1 \
    -e LITTLEBABY_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (LITTLEBABY_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${UPDATE_DOCKER_HOST_ARGS[@]}" \
      -e LITTLEBABY_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e LITTLEBABY_INSTALL_SMOKE_MODE=npm-global \
      -e LITTLEBABY_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e LITTLEBABY_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e LITTLEBABY_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e LITTLEBABY_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (LITTLEBABY_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e LITTLEBABY_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e LITTLEBABY_INSTALL_METHOD=npm \
    -e LITTLEBABY_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e LITTLEBABY_NO_ONBOARD=1 \
    -e LITTLEBABY_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${LITTLEBABY_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (LITTLEBABY_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e LITTLEBABY_INSTALL_URL="$INSTALL_URL" \
  -e LITTLEBABY_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e LITTLEBABY_NO_ONBOARD=1 \
  -e LITTLEBABY_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
