...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.littlebaby?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.littlebaby?.channel?.order;
      const labelRaw = parsed.littlebaby?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".littlebaby-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "littlebaby-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "littlebaby-build",
    ...
    LITTLEBABY_DISABLE_PLUGIN_MANIFEST_CACHE: "1",
    LITTLEBABY_PLUGIN_DISCOVERY_CACHE_MS: "0",
    LITTLEBABY_PLUGIN_MANIFEST_CACHE_MS: "0",
