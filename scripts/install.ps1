# LittleBaby Installer for Windows (PowerShell)
# Usage: iwr -useb https://littlebaby.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://littlebaby.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\littlebaby",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 LittleBaby Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one LittleBaby.$NC" -Level info
    Write-Host ""
}
...
function Install-LittleBabyNpm {
    param([string]$Target = "latest")

    $installSpec = Resolve-PackageInstallSpec -Target $Target
    
    Write-Host "Installing LittleBaby ($installSpec)..." -Level info
    ...
    try {
        ...
        Write-Host "LittleBaby installed" -Level success
        return $true
    } catch {
...
function Install-LittleBabyGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing LittleBaby from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/littlebaby/littlebaby.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\littlebaby\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\littlebaby.cmd" -Encoding ASCII -Force
...
function Resolve-PackageInstallSpec {
    ...
    if ([string]::IsNullOrWhiteSpace($trimmed)) {
        return "littlebaby@latest"
    }
    if ($trimmed.ToLowerInvariant() -eq "main") {
        return "github:littlebaby/littlebaby#main"
    }
    if (Test-ExplicitPackageInstallSpec -Target $trimmed) {
        return $trimmed
    }
    return "littlebaby@$trimmed"
}
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install LittleBaby from git to $GitDir" -Level info
        } else {
            Install-LittleBabyGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install LittleBaby via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-LittleBabyNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'littlebaby onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 LittleBaby installed successfully!" -Level success
}
