...
DEFAULT_PROVIDER="${LITTLEBABY_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
CLI_MODEL="${LITTLEBABY_LIVE_CLI_BACKEND_MODEL:-}"
CLI_PROVIDER="${CLI_MODEL%%/*}"
CLI_DISABLE_MCP_CONFIG="${LITTLEBABY_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG:-}"
CLI_AUTH_MODE="${LITTLEBABY_LIVE_CLI_BACKEND_AUTH:-auto}"
TEMP_DIRS=()
...
case "$CLI_AUTH_MODE" in
  auto | api-key | subscription)
    ;;
  *)
    echo "ERROR: LITTLEBABY_LIVE_CLI_BACKEND_AUTH must be one of: auto, api-key, subscription." >&2
    exit 1
    ;;
esac

if [[ "$CLI_AUTH_MODE" == "subscription" && "$CLI_PROVIDER" != "claude-cli" ]]; then
  echo "ERROR: LITTLEBABY_LIVE_CLI_BACKEND_AUTH=subscription is only supported for claude-cli." >&2
  exit 1
fi

if [[ "$CLI_AUTH_MODE" == "api-key" && "$CLI_PROVIDER" == "codex-cli" ]]; then
  if [[ -z "${OPENAI_API_KEY:-}" ]]; then
    echo "ERROR: LITTLEBABY_LIVE_CLI_BACKEND_AUTH=api-key for codex-cli requires OPENAI_API_KEY." >&2
    exit 1
  fi
fi
...
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  ...
  if [[ -z "${LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]]; then
    if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" ]]; then
      export LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV='["CLAUDE_CODE_OAUTH_TOKEN"]'
    else
      export LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV="[]"
    fi
  fi
  if [[ "$LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV" == *ANTHROPIC_API_KEY* ]]; then
    echo "ERROR: subscription auth smoke must not preserve Anthropic API-key env vars." >&2
    exit 1
  fi
  if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" && "$LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV" != *CLAUDE_CODE_OAUTH_TOKEN* ]]; then
  ...
  export LITTLEBABY_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${LITTLEBABY_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
  export LITTLEBABY_LIVE_CLI_BACKEND_RESUME_PROBE="${LITTLEBABY_LIVE_CLI_BACKEND_RESUME_PROBE:-1}"
  export LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_PROBE="${LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
  export LITTLEBABY_LIVE_CLI_BACKEND_MCP_PROBE="${LITTLEBABY_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
fi
...
elif [[ -n "${LITTLEBABY_DOCKER_AUTH_DIRS:-}" ]]; then
...
if [ "${LITTLEBABY_DOCKER_AUTH_PRESTAGED:-0}" != "1" ]; then
  IFS=',' read -r -a auth_dirs <<<"${LITTLEBABY_DOCKER_AUTH_DIRS_RESOLVED:-}"
  IFS=',' read -r -a auth_files <<<"${LITTLEBABY_DOCKER_AUTH_FILES_RESOLVED:-}"
  if ((${#auth_dirs[@]} > 0)); then
    for auth_dir in "${auth_dirs[@]}"; do
      [ -n "$auth_dir" ] || continue
...
provider="${LITTLEBABY_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
default_command="${LITTLEBABY_DOCKER_CLI_BACKEND_COMMAND_DEFAULT:-}"
docker_package="${LITTLEBABY_DOCKER_CLI_BACKEND_NPM_PACKAGE:-}"
binary_name="${LITTLEBABY_DOCKER_CLI_BACKEND_BINARY_NAME:-}"
if [ -z "$binary_name" ] && [ -n "$default_command" ]; then
  binary_name="$(basename "$default_command")"
fi
if [ -z "${LITTLEBABY_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -n "$binary_name" ]; then
  export LITTLEBABY_LIVE_CLI_BACKEND_COMMAND="$NPM_CONFIG_PREFIX/bin/$binary_name"
fi
if [ -n "${LITTLEBABY_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ ! -x "${LITTLEBABY_LIVE_CLI_BACKEND_COMMAND}" ] && [ -n "$docker_package" ]; then
  npm install -g "$docker_package"
fi
if [ "$provider" = "codex-cli" ] && [ "${LITTLEBABY_LIVE_CLI_BACKEND_AUTH:-auto}" = "api-key" ]; then
  codex_login_command="${LITTLEBABY_LIVE_CLI_BACKEND_COMMAND:-$NPM_CONFIG_PREFIX/bin/codex}"
...
if [ -n "${LITTLEBABY_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -x "${LITTLEBABY_LIVE_CLI_BACKEND_COMMAND}" ]; then
  echo "==> CLI backend binary: ${LITTLEBABY_LIVE_CLI_BACKEND_COMMAND}"
  "${LITTLEBABY_LIVE_CLI_BACKEND_COMMAND}" -V || "${LITTLEBABY_LIVE_CLI_BACKEND_COMMAND}" --version || true
fi
if [ "$provider" = "claude-cli" ]; then
  auth_mode="${LITTLEBABY_LIVE_CLI_BACKEND_AUTH:-auto}"
...
if [ -n "\${LITTLEBABY_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${LITTLEBABY_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${LITTLEBABY_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${LITTLEBABY_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  if [ -z "${LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]; then
    export LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'
  fi
  if [ "$auth_mode" = "subscription" ]; then
    claude --version
...
if [[ "${LITTLEBABY_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (LITTLEBABY_SKIP_DOCKER_BUILD=1)"
else
...
if [[ "$CLI_PROVIDER" == "codex-cli" && "$CLI_AUTH_MODE" == "api-key" ]]; then
  ...
  DOCKER_AUTH_ENV+=(
    ...
    -e LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV="$LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV"
  )
else
  DOCKER_AUTH_ENV+=(
    -e ANTHROPIC_API_KEY
    -e ANTHROPIC_API_KEY_OLD
    -e LITTLEBABY_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
    -e LITTLEBABY_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}"
    -e LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV="${LITTLEBABY_LIVE_CLI_BACKEND_PRESERVE_ENV:-}"
  )
fi
...
docker run --rm -t \
  ...
  -e LITTLEBABY_SKIP_CHANNELS=1 \
  -e LITTLEBABY_VITEST_FS_MODULE_CACHE=0 \
  -e LITTLEBABY_DOCKER_AUTH_PRESTAGED="$DOCKER_AUTH_PRESTAGED" \
  -e LITTLEBABY_DOCKER_AUTH_DIRS_RESOLVED="$AUTH_DIRS_CSV" \
  -e LITTLEBABY_DOCKER_AUTH_FILES_RESOLVED="$AUTH_FILES_CSV" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG="${LITTLEBABY_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG:-0}" \
  -e LITTLEBABY_DOCKER_CLI_BACKEND_PROVIDER="$CLI_PROVIDER" \
  -e LITTLEBABY_DOCKER_CLI_BACKEND_COMMAND_DEFAULT="$CLI_DEFAULT_COMMAND" \
  -e LITTLEBABY_DOCKER_CLI_BACKEND_NPM_PACKAGE="$CLI_DOCKER_NPM_PACKAGE" \
  -e LITTLEBABY_DOCKER_CLI_BACKEND_BINARY_NAME="$CLI_DOCKER_BINARY_NAME" \
  -e LITTLEBABY_LIVE_TEST=1 \
  -e LITTLEBABY_LIVE_CLI_BACKEND=1 \
  -e LITTLEBABY_LIVE_CLI_BACKEND_DEBUG="${LITTLEBABY_LIVE_CLI_BACKEND_DEBUG:-}" \
  -e LITTLEBABY_CLI_BACKEND_LOG_OUTPUT="${LITTLEBABY_CLI_BACKEND_LOG_OUTPUT:-}" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_MODEL="$CLI_MODEL" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_COMMAND="${LITTLEBABY_LIVE_CLI_BACKEND_COMMAND:-}" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_ARGS="${LITTLEBABY_LIVE_CLI_BACKEND_ARGS:-}" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_RESUME_ARGS="${LITTLEBABY_LIVE_CLI_BACKEND_RESUME_ARGS:-}" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_CLEAR_ENV="${LITTLEBABY_LIVE_CLI_BACKEND_CLEAR_ENV:-}" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG="$CLI_DISABLE_MCP_CONFIG" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_RESUME_PROBE="${LITTLEBABY_LIVE_CLI_BACKEND_RESUME_PROBE:-}" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${LITTLEBABY_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-}" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_PROBE="${LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_PROBE:-}" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_MCP_PROBE="${LITTLEBABY_LIVE_CLI_BACKEND_MCP_PROBE:-}" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_ARG="${LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_ARG:-}" \
  -e LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_MODE="${LITTLEBABY_LIVE_CLI_BACKEND_IMAGE_MODE:-}" \
  "${DOCKER_HOME_MOUNT[@]}" \
  "${DOCKER_EXTRA_ENV_FILES[@]}" \
  -v "$CACHE_HOME_DIR":/home/node/.cache \
  -v "$ROOT_DIR":/src:ro \
  -v "$CONFIG_DIR":/home/node/.littlebaby \
  -v "$WORKSPACE_DIR":/home/node/.littlebaby/workspace \
