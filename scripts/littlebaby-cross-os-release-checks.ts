...
const PUBLISHED_INSTALLER_BASE_URL = "https://littlebaby.ai";
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.LITTLEBABY_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.LITTLEBABY_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.LITTLEBABY_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `littlebaby@${previousVersion}` : "littlebaby@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.LITTLEBABY_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.LITTLEBABY_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        LITTLEBABY_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `littlebaby-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".littlebaby");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    LITTLEBABY_HOME: lane.homeDir,
    LITTLEBABY_STATE_DIR: lane.stateDir,
    LITTLEBABY_CONFIG_PATH: join(lane.stateDir, "littlebaby.json"),
    LITTLEBABY_DISABLE_BONJOUR: "1",
    LITTLEBABY_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
...
export function buildRealUpdateEnv(env) {
  const updateEnv = { ...env };
  delete updateEnv.LITTLEBABY_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
  return updateEnv;
}

export function resolveExplicitBaselineVersion(baselineSpec) {
  const trimmed = baselineSpec.trim();
  if (!trimmed || trimmed === "littlebaby@latest") {
    return "";
  }
  if (trimmed.startsWith("littlebaby@")) {
    return trimmed.slice("littlebaby@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "littlebaby@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view littlebaby@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'littlebaby.cmd'),
      (Join-Path $npmPrefix 'littlebaby.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command littlebaby -ErrorAction Stop
...
Write-Output "__LITTLEBABY_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__LITTLEBABY_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed littlebaby path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v littlebaby >/dev/null 2>&1",
    'printf "__LITTLEBABY_PATH__=%s\\n" "$(command -v littlebaby)"',
    "littlebaby --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__LITTLEBABY_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed littlebaby path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `littlebaby update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.littlebaby/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.LITTLEBABY_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.LITTLEBABY_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.LITTLEBABY_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.LITTLEBABY_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
async function resolveGatewayStatusArgs(lane, env, logPath) {
  const help = await runLittleBaby({
    lane,
    env,
    args: ["gateway", "status", "--help"],
    logPath,
...
async function runAgentTurn(params) {
  const sessionId = `cross-os-release-check-${params.label}-${Date.now()}`;
  const result = await runLittleBaby({
    lane: params.lane,
    env: params.env,
    args: [
      "agent",
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>LittleBaby Control</title>") &&
          html.includes("<littlebaby-app></littlebaby-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>LittleBaby Control</title>")} app=${html.includes("<littlebaby-app></littlebaby-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runLittleBaby(params) {
...
function installedPackageRoot(prefixDir) {
  return process.platform === "win32"
    ? join(prefixDir, "node_modules", "littlebaby")
    : join(prefixDir, "lib", "node_modules", "littlebaby");
}

function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "littlebaby.mjs");
}
