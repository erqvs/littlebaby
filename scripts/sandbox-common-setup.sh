...
BASE_IMAGE="${BASE_IMAGE:-littlebaby-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-littlebaby-sandbox-common:bookworm-slim}"
...
LITTLEBABY_DOCKER_BUILD_USE_BUILDX="${LITTLEBABY_DOCKER_BUILD_USE_BUILDX:-0}"
LITTLEBABY_DOCKER_BUILD_CACHE_FROM="${LITTLEBABY_DOCKER_BUILD_CACHE_FROM:-}"
LITTLEBABY_DOCKER_BUILD_CACHE_TO="${LITTLEBABY_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${LITTLEBABY_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${LITTLEBABY_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${LITTLEBABY_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${LITTLEBABY_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${LITTLEBABY_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=littlebaby.sandbox=1)
NOTE
