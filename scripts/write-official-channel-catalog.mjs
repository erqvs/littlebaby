...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.littlebaby) ? packageJson.littlebaby : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.littlebaby?.channel?.id) || left.name;
    const rightId = trimString(right.littlebaby?.channel?.id) || right.name;
