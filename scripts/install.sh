...
cleanup_npm_openclaw_paths() {
    ...
    rm -rf "$npm_root"/.littlebaby-* "$npm_root"/openclaw 2>/dev/null || true
}
...
install_openclaw() {
    ...
    if [[ -n "$resolved_version" ]]; then
        ...
        ui_info "Installing OpenClaw (${LITTLEBABY_VERSION})"
    fi
    local install_spec=""
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${LITTLEBABY_CONFIG_PATH:-$HOME/.littlebaby/openclaw.json}"
...
main() {
    ...
    LITTLEBABY_BIN="$(resolve_openclaw_bin || true)"
