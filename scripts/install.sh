...
# LittleBaby Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://littlebaby.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one LittleBaby."
...
GUM_VERSION="${LITTLEBABY_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 LittleBaby Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://littlebaby.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$LITTLEBABY_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.littlebaby.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_littlebaby_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.littlebaby-* "$npm_root"/littlebaby 2>/dev/null || true
}

extract_littlebaby_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_littlebaby_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/littlebaby" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/littlebaby"|"/usr/local/bin/littlebaby")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/littlebaby/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale littlebaby symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing littlebaby binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing LittleBaby package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_littlebaby_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "LittleBaby npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*littlebaby" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_littlebaby_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_littlebaby_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "LittleBaby npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an littlebaby binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${LITTLEBABY_TAGLINE_INDEX:-}" ]]; then
        if [[ "${LITTLEBABY_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((LITTLEBABY_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${LITTLEBABY_NO_ONBOARD:-0}
NO_PROMPT=${LITTLEBABY_NO_PROMPT:-0}
DRY_RUN=${LITTLEBABY_DRY_RUN:-0}
INSTALL_METHOD=${LITTLEBABY_INSTALL_METHOD:-}
LITTLEBABY_VERSION=${LITTLEBABY_VERSION:-latest}
USE_BETA=${LITTLEBABY_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/littlebaby"
GIT_DIR=${LITTLEBABY_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${LITTLEBABY_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${LITTLEBABY_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${LITTLEBABY_VERBOSE:-0}"
VERIFY_INSTALL="${LITTLEBABY_VERIFY_INSTALL:-0}"
LITTLEBABY_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
LittleBaby installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://littlebaby.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/littlebaby)
...
Environment variables:
  LITTLEBABY_INSTALL_METHOD=git|npm
  LITTLEBABY_VERSION=latest|next|main|<semver>|<spec>
  LITTLEBABY_BETA=0|1
  LITTLEBABY_GIT_DIR=...
  LITTLEBABY_GIT_UPDATE=0|1
  LITTLEBABY_NO_PROMPT=1
  LITTLEBABY_VERIFY_INSTALL=1
  LITTLEBABY_DRY_RUN=1
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://littlebaby.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://littlebaby.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://littlebaby.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://littlebaby.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected LittleBaby checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a LittleBaby source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_littlebaby_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"littlebaby"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://littlebaby.ai/install.sh | bash"
}
...
ensure_littlebaby_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/littlebaby" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/littlebaby" ]]; then
        ln -sf "$npm_root/littlebaby/dist/entry.js" "${npm_bin}/littlebaby"
        ui_info "Created littlebaby bin link at ${npm_bin}/littlebaby"
    fi
    return 0
}

# Check for existing LittleBaby installation
check_existing_littlebaby() {
    if [[ -n "$(type -P littlebaby 2>/dev/null || true)" ]]; then
        ui_info "Existing LittleBaby installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make littlebaby show as \"command not found\" in new terminals."
...
warn_littlebaby_not_found() {
    ui_warn "Installed, but littlebaby is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t littlebaby 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named littlebaby; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_littlebaby_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P littlebaby 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/littlebaby" ]]; then
        echo "${npm_bin}/littlebaby"
        return 0
    fi
...
install_littlebaby_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/littlebaby/littlebaby.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing LittleBaby from git checkout: ${repo_dir}"
    else
        ui_info "Installing LittleBaby from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning LittleBaby" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building LittleBaby" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/littlebaby" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/littlebaby"
    ui_success "LittleBaby wrapper installed to \$HOME/.local/bin/littlebaby"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view littlebaby dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:littlebaby/littlebaby#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_littlebaby() {
    local package_name="littlebaby"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            LITTLEBABY_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="littlebaby"
        else
            LITTLEBABY_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${LITTLEBABY_VERSION}" ]]; then
        LITTLEBABY_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${LITTLEBABY_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${LITTLEBABY_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing LittleBaby v${resolved_version}"
    else
        ui_info "Installing LittleBaby (${LITTLEBABY_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${LITTLEBABY_VERSION}")"

    if ! install_littlebaby_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_littlebaby_paths
        install_littlebaby_npm "${install_spec}"
    fi

    if [[ "${LITTLEBABY_VERSION}" == "latest" && "${package_name}" == "littlebaby" ]]; then
        if ! resolve_littlebaby_bin &> /dev/null; then
            ui_warn "npm install littlebaby@latest failed; retrying littlebaby@next"
            cleanup_npm_littlebaby_paths
            install_littlebaby_npm "littlebaby@next"
        fi
    fi

    ensure_littlebaby_bin_link || true

    ui_success "LittleBaby installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${LITTLEBABY_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_littlebaby_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (littlebaby not on PATH yet)"
        warn_littlebaby_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${LITTLEBABY_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.littlebaby/workspace-${profile}"
    else
        echo "${HOME}/.littlebaby/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run littlebaby onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but littlebaby not on PATH; skipping onboarding"
        warn_littlebaby_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run littlebaby onboard to retry"
        return
    }
}
...
if ! declare -F extract_littlebaby_semver >/dev/null 2>&1; then
...
resolve_littlebaby_version() {
    ...
    local claw="${LITTLEBABY_BIN:-}"
    if [[ -z "$claw" ]] && command -v littlebaby &> /dev/null; then
        claw="$(command -v littlebaby)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_littlebaby_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/littlebaby/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/littlebaby/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: littlebaby not on PATH yet"
        warn_littlebaby_not_found
        return 1
    fi

    run_quiet_step "Checking LittleBaby version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: littlebaby gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_littlebaby_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found LittleBaby checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set LITTLEBABY_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_littlebaby; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing LittleBaby"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g littlebaby &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g littlebaby 2>/dev/null || true
        ...
        install_littlebaby_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/littlebaby" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/littlebaby"
    ...
    LITTLEBABY_BIN="$(resolve_littlebaby_bin || true)"
    ...
    installed_version=$(resolve_littlebaby_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 LittleBaby installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 LittleBaby installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            ...
            "LittleBying complete. Please don't look at my soft shell phase."
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/littlebaby"
        ui_kv "Update command" "littlebaby update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://littlebaby.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running littlebaby doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                LITTLEBABY_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                LITTLEBABY_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                LITTLEBABY_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run littlebaby doctor and littlebaby plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (littlebaby not on PATH yet)"
                    warn_littlebaby_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run littlebaby onboard to finish setup"
            return 0
        fi
    fi

    if command -v littlebaby &> /dev/null; then
        local claw="${LITTLEBABY_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_littlebaby_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (littlebaby daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if LITTLEBABY_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: littlebaby daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${LITTLEBABY_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
