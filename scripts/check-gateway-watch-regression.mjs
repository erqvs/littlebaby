...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  LITTLEBABY_SKIP_ACPX_RUNTIME: "1",
  LITTLEBABY_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  ...
  const env = {
    LITTLEBABY_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    LITTLEBABY_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "littlebaby-gateway-watch-"));
