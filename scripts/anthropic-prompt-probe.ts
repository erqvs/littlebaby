...
// Usage:
// LITTLEBABY_PROMPT_TRANSPORT=direct|gateway
// LITTLEBABY_PROMPT_MODE=extra|override
// LITTLEBABY_PROMPT_TEXT='...'
// LITTLEBABY_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.LITTLEBABY_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE =
  process.env.LITTLEBABY_PROMPT_MODE?.trim() === "override" ? "override" : "extra";
const PROMPT_TEXT = process.env.LITTLEBABY_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.LITTLEBABY_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.LITTLEBABY_USER_PROMPT?.trim() || "is littlebaby here?";
const ENABLE_CAPTURE = process.env.LITTLEBABY_PROMPT_CAPTURE === "1";
const INCLUDE_RAW = process.env.LITTLEBABY_PROMPT_INCLUDE_RAW === "1";
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.LITTLEBABY_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = Number(process.env.LITTLEBABY_PROMPT_TIMEOUT_MS ?? "45000");
const GATEWAY_TIMEOUT_MS = Number(process.env.LITTLEBABY_PROMPT_GATEWAY_TIMEOUT_MS ?? "120000");
const SETUP_TOKEN_RAW = process.env.LITTLEBABY_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.LITTLEBABY_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.LITTLEBABY_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const DIRECT_CLAUDE_ARGS = ["-p", "--append-system-prompt"];

if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
  throw new Error("missing LITTLEBABY_PROMPT_TEXT or LITTLEBABY_PROMPT_LIST_JSON");
}
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set LITTLEBABY_LIVE_SETUP_TOKEN_VALUE or LITTLEBABY_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["littlebaby.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        LITTLEBABY_CONFIG_PATH: params.configPath,
        LITTLEBABY_STATE_DIR: params.stateDir,
        LITTLEBABY_AGENT_DIR: params.agentDir,
        LITTLEBABY_GATEWAY_TOKEN: params.gatewayToken,
        ...
        LITTLEBABY_SKIP_CRON: "1",
        LITTLEBABY_TEST_MINIMAL_GATEWAY: "1",
        LITTLEBABY_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      stdio: ["ignore", "pipe", "pipe"],
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "littlebaby-gateway-prompt-probe-"));
