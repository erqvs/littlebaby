#!/usr/bin/env bash
# Rootless LittleBaby in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-littlebaby-podman.sh launch        # Start gateway
#   ./scripts/run-littlebaby-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   littlebaby --container littlebaby dashboard --no-open
#   littlebaby --container littlebaby channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      LITTLEBABY_GATEWAY_TOKEN|LITTLEBABY_PODMAN_CONTAINER|LITTLEBABY_PODMAN_IMAGE|LITTLEBABY_IMAGE|LITTLEBABY_PODMAN_PULL|LITTLEBABY_PODMAN_GATEWAY_HOST_PORT|LITTLEBABY_GATEWAY_PORT|LITTLEBABY_PODMAN_BRIDGE_HOST_PORT|LITTLEBABY_BRIDGE_PORT|LITTLEBABY_GATEWAY_BIND|LITTLEBABY_PODMAN_USERNS|LITTLEBABY_BIND_MOUNT_OPTIONS|LITTLEBABY_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-littlebaby-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${LITTLEBABY_CONFIG_DIR:-$EFFECTIVE_HOME/.littlebaby}"
ENV_FILE="${LITTLEBABY_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${LITTLEBABY_CONFIG_DIR:-$EFFECTIVE_HOME/.littlebaby}"
ENV_FILE="${LITTLEBABY_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${LITTLEBABY_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${LITTLEBABY_PODMAN_CONTAINER:-littlebaby}"
LITTLEBABY_IMAGE="${LITTLEBABY_PODMAN_IMAGE:-${LITTLEBABY_IMAGE:-littlebaby:local}}"
PODMAN_PULL="${LITTLEBABY_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${LITTLEBABY_PODMAN_GATEWAY_HOST_PORT:-${LITTLEBABY_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${LITTLEBABY_PODMAN_BRIDGE_HOST_PORT:-${LITTLEBABY_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${LITTLEBABY_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$LITTLEBABY_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v littlebaby >/dev/null 2>&1; then
    return 0
  fi
  LITTLEBABY_CONTAINER="" LITTLEBABY_CONFIG_DIR="$config_dir" \
    littlebaby config get gateway.bind 2>/dev/null || true
}
...
# LITTLEBABY_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${LITTLEBABY_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate LITTLEBABY_GATEWAY_TOKEN." >&2
  exit 1
}
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'LITTLEBABY_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v littlebaby >/dev/null 2>&1; then
    echo "Warning: littlebaby not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    LITTLEBABY_CONTAINER="" LITTLEBABY_CONFIG_DIR="$config_dir" \
      littlebaby config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    LITTLEBABY_CONTAINER="" LITTLEBABY_CONFIG_DIR="$config_dir" \
      littlebaby config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  LITTLEBABY_CONTAINER="" LITTLEBABY_CONFIG_DIR="$config_dir" \
    littlebaby config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${LITTLEBABY_GATEWAY_TOKEN:-}" ]]; then
  export LITTLEBABY_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "LITTLEBABY_GATEWAY_TOKEN" "$LITTLEBABY_GATEWAY_TOKEN"
  echo "Generated LITTLEBABY_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/littlebaby.json"
...
PODMAN_USERNS="${LITTLEBABY_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported LITTLEBABY_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (LITTLEBABY_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${LITTLEBABY_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${LITTLEBABY_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$LITTLEBABY_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.littlebaby/.npm \
    -e LITTLEBABY_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.littlebaby:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.littlebaby/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$LITTLEBABY_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME littlebaby dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME littlebaby devices approve --latest  # if pairing required"
