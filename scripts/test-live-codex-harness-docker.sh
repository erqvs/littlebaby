...
CODEX_HARNESS_AUTH_MODE="${LITTLEBABY_LIVE_CODEX_HARNESS_AUTH:-codex-auth}"
TEMP_DIRS=()
...
case "$CODEX_HARNESS_AUTH_MODE" in
  codex-auth | api-key)
    ;;
  *)
    echo "ERROR: LITTLEBABY_LIVE_CODEX_HARNESS_AUTH must be one of: codex-auth, api-key." >&2
    exit 1
    ;;
esac

if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" && -z "${OPENAI_API_KEY:-}" ]]; then
  echo "ERROR: LITTLEBABY_LIVE_CODEX_HARNESS_AUTH=api-key requires OPENAI_API_KEY." >&2
  exit 1
fi
...
if [ "${LITTLEBABY_LIVE_CODEX_HARNESS_AUTH:-codex-auth}" != "api-key" ]; then
...
if [ "${LITTLEBABY_LIVE_CODEX_HARNESS_AUTH:-codex-auth}" != "api-key" ] && [ ! -s "$HOME/.codex/auth.json" ]; then
...
echo "==> Model: ${LITTLEBABY_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.4}"
echo "==> Image probe: ${LITTLEBABY_LIVE_CODEX_HARNESS_IMAGE_PROBE:-1}"
echo "==> MCP probe: ${LITTLEBABY_LIVE_CODEX_HARNESS_MCP_PROBE:-1}"
...
docker run --rm -t \
  ...
  -e LITTLEBABY_AGENT_HARNESS_FALLBACK=none \
  -e LITTLEBABY_DOCKER_AUTH_PRESTAGED="$DOCKER_AUTH_PRESTAGED" \
  -e LITTLEBABY_CODEX_APP_SERVER_BIN="${LITTLEBABY_CODEX_APP_SERVER_BIN:-codex}" \
  -e LITTLEBABY_DOCKER_AUTH_FILES_RESOLVED="$AUTH_FILES_CSV" \
  -e LITTLEBABY_LIVE_CODEX_HARNESS_AUTH="$CODEX_HARNESS_AUTH_MODE" \
  -e LITTLEBABY_LIVE_CODEX_HARNESS=1 \
  -e LITTLEBABY_LIVE_CODEX_HARNESS_DEBUG="${LITTLEBABY_LIVE_CODEX_HARNESS_DEBUG:-}" \
  -e LITTLEBABY_LIVE_CODEX_HARNESS_IMAGE_PROBE="${LITTLEBABY_LIVE_CODEX_HARNESS_IMAGE_PROBE:-1}" \
  -e LITTLEBABY_LIVE_CODEX_HARNESS_MCP_PROBE="${LITTLEBABY_LIVE_CODEX_HARNESS_MCP_PROBE:-1}" \
  -e LITTLEBABY_LIVE_CODEX_HARNESS_MODEL="${LITTLEBABY_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.4}" \
