...
#
# Environment:
#   LITTLEBABY_NAMESPACE   Kubernetes namespace (default: openclaw)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MANIFESTS="$SCRIPT_DIR/manifests"
NS="${LITTLEBABY_NAMESPACE:-openclaw}"
...
_apply_secret() {
  ...
  if kubectl get secret openclaw-secrets -n "$NS" &>/dev/null; then
    ...
    EXISTING_TOKEN="$(kubectl get secret openclaw-secrets -n "$NS" -o jsonpath='{.data.LITTLEBABY_GATEWAY_TOKEN}' | base64 -d)"
  ...
  if $SHOW_TOKEN; then
    ...
    echo "  kubectl get secret openclaw-secrets -n $NS -o jsonpath='{.data.LITTLEBABY_GATEWAY_TOKEN}' | base64 -d && echo"
  fi
}
...
if $SHOW_TOKEN; then
  ...
  echo "  $(kubectl get secret openclaw-secrets -n "$NS" -o jsonpath='{.data.LITTLEBABY_GATEWAY_TOKEN}' | base64 -d)"
echo ""
fi
