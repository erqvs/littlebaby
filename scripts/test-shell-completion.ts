 ...
 * `littlebaby update`. Use it to verify the completion installation flow
...
\`littlebaby update\`, \`littlebaby doctor\`, and \`littlebaby onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# LittleBaby Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`littlebaby completion --install\` later to enable.`));
    return;
  }
