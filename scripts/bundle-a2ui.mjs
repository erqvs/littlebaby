...
async function main() {
  ...
  if (!hasRendererDir || !hasAppDir) {
    ...
    if (process.env.LITTLEBABY_SPARSE_PROFILE || process.env.LITTLEBABY_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI sources missing; skipping bundle because LITTLEBABY_A2UI_SKIP_MISSING=1 or LITTLEBABY_SPARSE_PROFILE is set.",
      );
      return;
    }
