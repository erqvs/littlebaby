...
LITTLEBABY_AUTH="$HOME/.littlebaby/agents/main/agent/auth-profiles.json"
...
check_openclaw_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$LITTLEBABY_AUTH" 2>/dev/null || echo "0")
...
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$LITTLEBABY_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$LITTLEBABY_AUTH" 2>/dev/null || echo "0")
