...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.LITTLEBABY_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "littlebaby.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "littlebaby.plugin.json");
    ...
    if (packageJson.littlebaby && "extensions" in packageJson.littlebaby) {
      packageJson.littlebaby = {
        ...packageJson.littlebaby,
        extensions: rewritePackageExtensions(packageJson.littlebaby.extensions),
        ...(typeof packageJson.littlebaby.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.littlebaby.setupEntry) }
          : {}),
      };
    }
