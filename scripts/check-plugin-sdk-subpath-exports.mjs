...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("littlebaby/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced littlebaby/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced littlebaby/plugin-sdk/<subpath> must exist in the public package exports.",
  );
