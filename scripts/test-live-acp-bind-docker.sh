...
ACP_AGENT_LIST_RAW="${LITTLEBABY_LIVE_ACP_BIND_AGENTS:-${LITTLEBABY_LIVE_ACP_BIND_AGENT:-claude,codex,gemini}}"
TEMP_DIRS=()
...
openclaw_live_acp_bind_resolve_auth_provider() {
  case "${1:-}" in
    ...
    gemini) printf '%s\n' "google-gemini-cli" ;;
    *)
      echo "Unsupported LITTLEBABY_LIVE_ACP_BIND agent: ${1:-} (expected claude, codex, or gemini)" >&2
      return 1
      ;;
  esac
}
...
openclaw_live_acp_bind_resolve_agent_command() {
  case "${1:-}" in
    ...
    gemini) printf '%s' "${LITTLEBABY_LIVE_ACP_BIND_AGENT_COMMAND_GEMINI:-${LITTLEBABY_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    *) return 1 ;;
  esac
}
...
agent="${LITTLEBABY_LIVE_ACP_BIND_AGENT:-claude}"
case "$agent" in
  claude)
...
if [ -n "\${LITTLEBABY_LIVE_ACP_BIND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${LITTLEBABY_LIVE_ACP_BIND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${LITTLEBABY_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${LITTLEBABY_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  gemini)
    ...
    ;;
  *)
    echo "Unsupported LITTLEBABY_LIVE_ACP_BIND_AGENT: $agent" >&2
    exit 1
    ;;
esac
tmp_dir="$(mktemp -d)"
...
if ((${#ACP_AGENTS[@]} == 0)); then
  echo "No ACP bind agents selected. Use LITTLEBABY_LIVE_ACP_BIND_AGENTS=claude,codex,gemini." >&2
  exit 1
fi
...
for ACP_AGENT in "${ACP_AGENTS[@]}"; do
  ...
  AUTH_DIRS=()
  AUTH_FILES=()
  if [[ -n "${LITTLEBABY_DOCKER_AUTH_DIRS:-}" ]]; then
  ...
  docker run --rm -t \
    ...
    -e LITTLEBABY_LIVE_ACP_BIND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}" \
    -e LITTLEBABY_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}" \
    -e GEMINI_API_KEY \
    -e GOOGLE_API_KEY \
    -e OPENAI_API_KEY \
    ...
    -e LITTLEBABY_LIVE_ACP_BIND=1 \
    -e LITTLEBABY_LIVE_ACP_BIND_AGENT="$ACP_AGENT" \
    -e LITTLEBABY_LIVE_ACP_BIND_AGENT_COMMAND="$AGENT_COMMAND" \
    "${DOCKER_HOME_MOUNT[@]}" \
