#!/usr/bin/env bash
# Reset LittleBaby like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${LITTLEBABY_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="LittleBaby.app/Contents/MacOS/LittleBaby"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/LittleBaby"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/LittleBaby"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/LittleBaby"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.littlebaby.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/littlebaby-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${LITTLEBABY_RESTART_LOG:-/tmp/littlebaby-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${LITTLEBABY_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.littlebaby/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  LITTLEBABY_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node littlebaby.mjs daemon install --force --runtime node"
      log "  node littlebaby.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.littlebaby/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.littlebaby.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing LittleBaby instances"
kill_all_littlebaby
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product LittleBaby"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/LittleBaby.app" ]]; then
    APP_BUNDLE="/Applications/LittleBaby.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/LittleBaby.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/LittleBaby.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/LittleBaby.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set LITTLEBABY_APP_BUNDLE to your installed LittleBaby.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node littlebaby.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node littlebaby.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".littlebaby", "littlebaby.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: LittleBaby is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.littlebaby.gateway.plist' | head -n 40 || true"
fi
