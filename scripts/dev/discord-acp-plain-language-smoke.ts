...
type DriverMode = "token" | "webhook" | "littlebaby";
...
function resolveStateDir(): string {
  const override = process.env.LITTLEBABY_STATE_DIR?.trim();
  if (override) {
...
function usage(): string {
  return (
    ...
    "  LITTLEBABY_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  LITTLEBABY_DISCORD_SMOKE_DRIVER\n" +
    "  LITTLEBABY_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  LITTLEBABY_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  LITTLEBABY_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  LITTLEBABY_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  LITTLEBABY_DISCORD_SMOKE_AGENT\n" +
    "  LITTLEBABY_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  LITTLEBABY_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  LITTLEBABY_DISCORD_SMOKE_POLL_MS\n" +
    "  LITTLEBABY_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  LITTLEBABY_DISCORD_SMOKE_LITTLEBABY_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.LITTLEBABY_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.LITTLEBABY_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "littlebaby"
        ? "littlebaby"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.LITTLEBABY_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.LITTLEBABY_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.LITTLEBABY_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.LITTLEBABY_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.LITTLEBABY_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.LITTLEBABY_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.LITTLEBABY_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.LITTLEBABY_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.LITTLEBABY_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
