...
#
# Suggested cron: */30 * * * * /home/admin/littlebaby/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send LittleBaby notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., littlebaby-alerts)
...
STATE_FILE="$HOME/.littlebaby/auth-monitor-state"
...
send_notification() {
    ...
    # Send via LittleBaby if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use littlebaby
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via LittleBaby to $NOTIFY_PHONE..."
            littlebaby send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! LittleBaby is down. Run: ssh l36 '~/littlebaby/scripts/mobile-reauth.sh'" "urgent"
    exit 1
