...
const extensionRestartMetadataFiles = new Set(["littlebaby.plugin.json", "littlebaby.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.LITTLEBABY_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.LITTLEBABY_BUILD_PRIVATE_QA === "1" &&
...
const BUILD_REASON_LABELS = {
  force_build: "forced by LITTLEBABY_FORCE_BUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "LITTLEBABY_RUN_NODE_OUTPUT_LOG";
...
const shouldSkipCleanWatchRuntimeSync = (deps) => deps.env.LITTLEBABY_WATCH_MODE === "1";
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.LITTLEBABY_BUILD_PRIVATE_QA = "1";
    deps.env.LITTLEBABY_ENABLE_PRIVATE_QA_CLI = "1";
  }
