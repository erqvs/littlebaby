...
- LITTLEBABY_IOS_VERSION = exact canonical iOS version
- LITTLEBABY_MARKETING_VERSION = short iOS/App Store version
- LITTLEBABY_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    LITTLEBABY_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    LITTLEBABY_MARKETING_VERSION)
...
LITTLEBABY_IOS_VERSION = ${IOS_VERSION}
LITTLEBABY_MARKETING_VERSION = ${MARKETING_VERSION}
LITTLEBABY_BUILD_VERSION = ${BUILD_NUMBER}
EOF
