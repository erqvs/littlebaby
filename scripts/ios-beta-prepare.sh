...
- regenerates apps/ios/LittleBaby.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${LITTLEBABY_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid LITTLEBABY_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid LITTLEBABY_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid LITTLEBABY_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid LITTLEBABY_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing LITTLEBABY_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(LITTLEBABY_URL_SLASH)$(LITTLEBABY_URL_SLASH)#g'
)"
...
LITTLEBABY_CODE_SIGN_STYLE = Automatic
LITTLEBABY_DEVELOPMENT_TEAM = ${TEAM_ID}
LITTLEBABY_IOS_SELECTED_TEAM = ${TEAM_ID}
LITTLEBABY_APP_BUNDLE_ID = ai.littlebaby.client
LITTLEBABY_SHARE_BUNDLE_ID = ai.littlebaby.client.share
LITTLEBABY_ACTIVITY_WIDGET_BUNDLE_ID = ai.littlebaby.client.activitywidget
LITTLEBABY_WATCH_APP_BUNDLE_ID = ai.littlebaby.client.watchkitapp
LITTLEBABY_WATCH_EXTENSION_BUNDLE_ID = ai.littlebaby.client.watchkitapp.extension
LITTLEBABY_APP_PROFILE =
LITTLEBABY_SHARE_PROFILE =
LITTLEBABY_PUSH_TRANSPORT = relay
LITTLEBABY_PUSH_DISTRIBUTION = official
LITTLEBABY_URL_SLASH = /
LITTLEBABY_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
LITTLEBABY_PUSH_APNS_ENVIRONMENT = production
EOF
