...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d littlebaby-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart littlebaby-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f littlebaby-gateway
}
...
clawdock-update() {
  ...
  _clawdock_compose build littlebaby-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d littlebaby-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build littlebaby-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "LITTLEBABY_GATEWAY_TOKEN=$token" littlebaby-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" littlebaby-gateway \
    bash -c './littlebaby.mjs config set gateway.remote.token "$TOKEN" && ./littlebaby.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec littlebaby-gateway \
    bash -c "./littlebaby.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart littlebaby-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec littlebaby-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
