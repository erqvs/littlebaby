...
const EXPECTED_REPOSITORY_URL = "https://github.com/littlebaby/littlebaby";
...
const skipPackValidationEnv = "LITTLEBABY_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "littlebaby") {
    errors.push(`package.json name must be "littlebaby"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.littlebaby !== "littlebaby.mjs") {
    errors.push(
      `package.json bin.littlebaby must be "littlebaby.mjs"; found "${pkg.bin?.littlebaby ?? ""}".`,
    );
  }
  return errors;
}
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`littlebaby-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `littlebaby-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
