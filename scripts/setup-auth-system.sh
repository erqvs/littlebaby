#!/bin/bash
# Setup LittleBaby Auth Management System
...
echo "=== LittleBaby Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'littlebaby-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  LittleBaby message: Send warning via LittleBaby itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/littlebaby-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/littlebaby-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/littlebaby-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now littlebaby-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/LittleBabyAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/LittleBabyAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/LittleBaby*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (LittleBabyAuth) shows status and opens auth URL if needed."
echo "The full widget (LittleBabyAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status littlebaby-auth-monitor.timer"
echo ""
echo "Quick commands:"
