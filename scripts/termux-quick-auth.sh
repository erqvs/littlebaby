...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/littlebaby/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "LittleBaby Re-Auth" -c "After getting key, run: ssh $SERVER '~/littlebaby/scripts/mobile-reauth.sh'" --id littlebaby-auth
        ;;
    *)
