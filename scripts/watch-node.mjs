...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[littlebaby] ${message}\n`);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.LITTLEBABY_WATCH_MODE = "1";
  childEnv.LITTLEBABY_WATCH_SESSION = watchSession;
  ...
  childEnv.LITTLEBABY_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.LITTLEBABY_WATCH_COMMAND = deps.args.join(" ");
  }
