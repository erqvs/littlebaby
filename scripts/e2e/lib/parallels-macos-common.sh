...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/littlebaby-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/littlebaby-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${LITTLEBABY_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${LITTLEBABY_SECRET_ENV_NAME:-}" ] && [ -n "${LITTLEBABY_SECRET_FILE:-}" ] && [ -f "$LITTLEBABY_SECRET_FILE" ]; then
  secret_value="$(cat "$LITTLEBABY_SECRET_FILE")"
  rm -f "$LITTLEBABY_SECRET_FILE"
  export "${LITTLEBABY_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "LITTLEBABY_SECRET_ENV_NAME=$api_key_env" \
      "LITTLEBABY_SECRET_FILE=$secret_path" \
      "LITTLEBABY_WRAPPER_FILE=$wrapper_path" \
