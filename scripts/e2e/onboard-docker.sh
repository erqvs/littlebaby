...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$LITTLEBABY_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$LITTLEBABY_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$LITTLEBABY_STATE_DIR/workspace"
    config_path="$LITTLEBABY_CONFIG_PATH"
    sessions_dir="$LITTLEBABY_STATE_DIR/agents/main/sessions"
...
JSON
	    ...
	    run_case_logged reset-config node "$LITTLEBABY_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$LITTLEBABY_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$LITTLEBABY_ENTRY\" configure --section skills" send_skills_flow
