...
install_latest_release() {
  ...
  guest_exec /usr/bin/env LITTLEBABY_NO_ONBOARD=1 bash /tmp/openclaw-install.sh "${version_args[@]}" --no-onboard
...
config_path = Path("/root/.littlebaby/openclaw.json")
config = {}
...
setsid sh -lc 'exec env LITTLEBABY_HOME=/root LITTLEBABY_STATE_DIR=/root/.littlebaby LITTLEBABY_CONFIG_PATH=/root/.littlebaby/openclaw.json ${API_KEY_ENV}=${api_key_value_q} openclaw gateway run --bind loopback --port 18789 --force >/tmp/openclaw-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
