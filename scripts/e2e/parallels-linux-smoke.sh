...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/littlebaby-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/littlebaby-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/littlebaby-install.sh
  guest_exec /usr/bin/env LITTLEBABY_NO_ONBOARD=1 bash /tmp/littlebaby-install.sh "${version_args[@]}" --no-onboard
  guest_exec littlebaby --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec littlebaby --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" littlebaby onboard \
    --non-interactive \
    --mode local \
...
set -euo pipefail
plugin_dir=/root/.littlebaby/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{
  "name": "@littlebaby/test-bad-plugin",
  "version": "1.0.0",
  "littlebaby": {
...
JSON
cat >"$plugin_dir/littlebaby.plugin.json" <<'JSON'
{
  "id": "test-bad-plugin",
  "configSchema": {
    "type": "object",
...
config_path = Path("/root/.littlebaby/littlebaby.json")
config = {}
...
verify_bad_plugin_diagnostic() {
  guest_bash_script <<'EOF'
grep -F "failed to load setup entry" /tmp/littlebaby-parallels-linux-gateway.log
EOF
}
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "littlebaby gateway run" >/dev/null 2>&1 || true
rm -f /tmp/littlebaby-parallels-linux-gateway.log
setsid sh -lc 'exec env LITTLEBABY_HOME=/root LITTLEBABY_STATE_DIR=/root/.littlebaby LITTLEBABY_CONFIG_PATH=/root/.littlebaby/littlebaby.json ${API_KEY_ENV}=${api_key_value_q} littlebaby gateway run --bind loopback --port 18789 --force >/tmp/littlebaby-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec littlebaby gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec littlebaby gateway status --deep --require-rpc
    return
  fi
  guest_exec littlebaby gateway status --deep
}

verify_local_turn() {
  guest_exec littlebaby models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" littlebaby agent \
    --local \
    --agent main \
    --message ping \
    --json
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "littlebaby-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "littlebaby-main-upgrade.tgz"
