...
MODEL="${LITTLEBABY_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${LITTLEBABY_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${LITTLEBABY_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${LITTLEBABY_OPENWEBUI_PORT:-8080}"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "LITTLEBABY_GATEWAY_TOKEN=$TOKEN" \
  -e "LITTLEBABY_OPENWEBUI_MODEL=$MODEL" \
  -e OPENAI_API_KEY \
...
const batchPath = process.env.LITTLEBABY_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.LITTLEBABY_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.LITTLEBABY_OPENWEBUI_MODEL },
];
