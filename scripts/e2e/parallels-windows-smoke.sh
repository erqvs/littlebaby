...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-littlebaby-native-e2e-2026-03-12"
MODE="both"
PROVIDER="openai"
API_KEY_ENV=""
AUTH_CHOICE=""
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://littlebaby.ai/install.ps1"
HOST_PORT="18426"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/littlebaby-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-littlebaby-native-e2e-2026-03-12"
  ...
  --install-url <url>        Installer URL for latest release. Default: https://littlebaby.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run littlebaby update --channel dev.
...
# littlebaby.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$littlebaby @args 2>&1
if (\$null -ne \$output) {
...
baseline_install_version() {
  ...
  npm view "littlebaby@$INSTALL_VERSION" version --userconfig "$(mktemp)"
}
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "littlebaby-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_powershell "$(cat <<EOF
\$depsRoot = Join-Path \$env:LOCALAPPDATA 'LittleBaby\deps'
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/littlebaby-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
write_latest_install_runner_script() {
  ...
  WINDOWS_LATEST_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/littlebaby-install-latest.ps1"
...
try {
  ...
  & (Join-Path \$env:APPDATA 'npm\littlebaby.cmd') --version *>&1 | Tee-Object -FilePath \$LogPath -Append | Out-Null
  if (\$LASTEXITCODE -ne 0) {
    throw "littlebaby --version failed with exit code \$LASTEXITCODE"
  }
...
write_baseline_npm_install_runner_script() {
  WINDOWS_BASELINE_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/littlebaby-install-baseline-npm.ps1"
...
try {
  $portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'LittleBaby\deps') 'portable-git') ''
  $env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
  $littlebaby = Join-Path $env:APPDATA 'npm\littlebaby.cmd'

  Write-ProgressLog 'install.start'
  Invoke-Logged 'npm install baseline release' {
    & npm.cmd install -g "littlebaby@$Version" --no-fund --no-audit --loglevel=error
  }

  Write-ProgressLog 'install.version'
  Invoke-Logged 'littlebaby --version' { & $littlebaby --version }
...
install_baseline_npm_release() {
  ...
  runner_name="littlebaby-install-baseline-$RANDOM-$RANDOM.ps1"
  log_name="littlebaby-install-baseline-$RANDOM-$RANDOM.log"
  done_name="littlebaby-install-baseline-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/littlebaby-install-baseline-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/littlebaby-install-baseline-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
...
install_latest_release() {
  ...
  runner_name="littlebaby-install-latest-$RANDOM-$RANDOM.ps1"
  log_name="littlebaby-install-latest-$RANDOM-$RANDOM.log"
  done_name="littlebaby-install-latest-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/littlebaby-install-latest-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
  startup_checked=0
...
install_main_tgz() {
  ...
  runner_name="littlebaby-install-$RANDOM-$RANDOM.ps1"
  log_name="littlebaby-install-$RANDOM-$RANDOM.log"
  done_name="littlebaby-install-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/littlebaby-install-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/littlebaby-install-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
...
write_dev_update_runner_script() {
  WINDOWS_DEV_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/littlebaby-update-dev.ps1"
...
try {
  ...
  $littlebaby = Join-Path $env:APPDATA 'npm\littlebaby.cmd'
  $gitRoot = Join-Path $env:USERPROFILE 'littlebaby'
  $gitEntry = Join-Path $gitRoot 'littlebaby.mjs'
...
"@ | Set-Content -Path $pnpmPsShim -Encoding UTF8
  ...
  Invoke-Logged 'littlebaby update --channel dev --yes --json' {
    & $littlebaby update --channel dev --yes --json
  }
  ...
  Invoke-Logged 'git littlebaby --version' { & node.exe $gitEntry --version }
  Invoke-Logged 'git littlebaby update status --json' { & node.exe $gitEntry update status --json }
...
run_dev_channel_update() {
  ...
  runner_name="littlebaby-update-dev-$RANDOM-$RANDOM.ps1"
  log_name="littlebaby-update-dev-$RANDOM-$RANDOM.log"
  done_name="littlebaby-update-dev-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/littlebaby-update-dev-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
...
if ($busy) {
  throw 'dev update still has active npm/pnpm/littlebaby processes'
}
$gitEntry = Join-Path $env:USERPROFILE 'littlebaby\littlebaby.mjs'
if (-not (Test-Path $gitEntry)) {
...
write_install_runner_script() {
  WINDOWS_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/littlebaby-install-main.ps1"
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_littlebaby "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
write_onboard_runner_script() {
  WINDOWS_ONBOARD_SCRIPT_PATH="$MAIN_TGZ_DIR/littlebaby-onboard-$PROVIDER.ps1"
...
try {
  ...
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice ${AUTH_CHOICE} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json >> "{1}" 2>&1' -f \$littlebaby, \$LogPath)
...
run_ref_onboard() {
  ...
  runner_name="littlebaby-onboard-$RANDOM-$RANDOM.ps1"
  log_name="littlebaby-onboard-$RANDOM-$RANDOM.log"
  done_name="littlebaby-onboard-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_ONBOARD_S + 60))
  startup_checked=0
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/littlebaby-onboard-log-state.XXXXXX")"
  : >"$log_state_path"
...
verify_gateway() {
  guest_run_littlebaby "" "" gateway status --deep --require-rpc
}
...
verify_gateway_reachable() {
  ...
  for attempt in 1 2 3 4 5 6; do
    probe_json="$(
      guest_run_littlebaby "" "" gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json
    )"
    printf '%s\n' "$probe_json"
...
$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'LittleBaby\deps') 'portable-git') ''
$env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
$gitEntry = Join-Path (Join-Path $env:USERPROFILE 'littlebaby') 'littlebaby.mjs'
if (-not (Test-Path $gitEntry)) {
...
run_gateway_daemon_action() {
  ...
  local runner_name log_name done_name done_status launcher_state
  local poll_rc state_rc log_rc start_seconds poll_deadline startup_checked
  runner_name="littlebaby-gateway-$action-$RANDOM-$RANDOM.ps1"
  log_name="littlebaby-gateway-$action-$RANDOM-$RANDOM.log"
  done_name="littlebaby-gateway-$action-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
try {
  \$littlebaby = Join-Path \$env:APPDATA 'npm\littlebaby.cmd'
  & \$littlebaby gateway $action *>&1 | Tee-Object -FilePath \$log -Append | Out-Null
...
show_gateway_status_compat() {
  if guest_run_littlebaby "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_littlebaby "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_littlebaby "" "" gateway status --deep
}

verify_turn() {
  guest_run_littlebaby "" "" models set "$MODEL_ID"
  guest_run_littlebaby "$API_KEY_ENV" "$API_KEY_VALUE" \
...
run_fresh_main_lane() {
  ...
  if phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "littlebaby-main-fresh.tgz"; then
    ...
    phase_run "fresh.install-main-retry" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "littlebaby-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-baseline-package" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "littlebaby-main-upgrade.tgz" || return $?
