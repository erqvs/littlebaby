...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.LITTLEBABY_VITEST_MAX_WORKERS || env.LITTLEBABY_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      LITTLEBABY_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
  ...
  if (isFullSuiteRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        ...
        !process.env.LITTLEBABY_TEST_PROJECTS_PARALLEL &&
        !process.env.LITTLEBABY_VITEST_MAX_WORKERS &&
        !process.env.LITTLEBABY_TEST_WORKERS &&
